...
> was a promise nothing could keep. What IS reproducible, and was re-run again on 2026-08-19, is the
...
> `UNPOLISHED` · 1 `COULD_NOT_TEST` · 138 suspect files · 161 lessons.** Ninth reader, ninth identical
> tally. The instruction one block down still stands: **stop re-deriving the corpus.**
>
> **COVERAGE: 89 → 110 opened · 41 → 20 never opened · 8 unopenable as filed.** `110 + 20 + 8 = 138` ·
> `950 + 25 + 37 = 1,012` · `304 + 7 + 7 = 318`. Twenty-one new files, 34 findings — **and 0 criticals.**
> The never-opened critical row is **7 → 7**, unchanged, and those seven are the same seven §2.2 named
> by name last round. Rounds 8 and 9 took that row 25 → 11 → 7 by aiming at it; round 10 was aimed at
> the major/minor bulk and moved it not at all. Both numbers are given because *21 new files* is the
> one a summary will quote and *0 criticals* is the one that matters.
>
> **STANDING `BROKEN` IS UNCHANGED AT 1,012 AND §2.1 STILL HAS SEVEN ROWS.** Round 10 shipped thirteen
> new test files and 155 new assertions, all green, but nothing in it was put through the refutation
> standard §2.1 requires, so nothing was added there. **Work landed; the closed count did not move.**
>
> **OWNED IS NOT OPENED, AND THIS ROUND IS THE CLEANEST PROOF THE PROGRAMME HAS PRODUCED.** The seven
> round-10 lanes were handed 34 corpus files between them. **Thirteen were never edited** —
> `lesson-ui/GlanceEdgePings.tsx`, `components/sim/HeroCarBody.tsx`, `cockpit/index.ts`,
> `world/components/cityModels.ts`, `scene/obstacleSpec.ts`, `scene/ribbonStrip.ts`,
> `traces/scMergeLaneEnd.ts`, `traces/scMergeMotorwayExit.ts`, `traffic/index.ts`,
> `lessons/progression.ts`, `lesson-ui/briefingOverflow.test.tsx`, `public/world/ov-crossing-v1.json`
> and `public/world/mw-v1.json`. They are **exactly** the thirteen this gate's reader still lists as
> never opened. Had coverage been computed from the lane manifests instead of from `git diff`, the
> never-opened row would read **7 files, not 20**, and 13 files / 34 findings would have been recorded
> as covered with not one byte changed. **A file a lane was handed and never edited reads exactly like
> one that was worked on, in every artefact except the diff.** Compute coverage from edits. Only edits.
>
> **THE FOURTH GATE IS GREEN AND WAS RUN AS A GATE.** `tools-tests.mjs` exit 0, read from `$?`, not
> from a wrapper. `tsc --noEmit` exit 0. `validate-content.mjs` exit 0. `vitest run` exit 1 on **three**
> files — the two known-red plus one cross-lane collision, all three dissected in §4.

> **Re-run an EIGHTH time on 2026-08-19 for the round-9 gate, by an eighth independent reader**: all
> 24 JSONL files parsed, the re-drive supersession applied to 22 scenarios / 26 records, and the result
> is again **1,712 · 26 · 1,686 · 161 `SUMMARY` · 1,012 `BROKEN` (318 critical · 613 major · 81 minor) ·
> 512 `UNPOLISHED` · 1 `COULD_NOT_TEST` · 138 suspect files · 161 lessons.** Eight readers, eight
> identical tallies. **Stop re-deriving the corpus.** It has not moved in eight attempts and further
> re-derivation of it is not evidence of anything; what keeps being wrong is everything computed
> *against* it.
>
> **§1's coverage union moved for the third round running: 85 → 89 opened, 45 → 41 never opened, 8
> unopenable. The never-opened CRITICAL row fell 11 → 7** — 25 → 11 → 7 across rounds 8 and 9, after
> seven rounds frozen at 25. The three rows still sum to 138 / 1,012 / 318.
>
> **THE COVERAGE JOIN WAS INDEPENDENTLY CONFIRMED FOR THE FIRST TIME.** For four consecutive rounds
> the frozen tally reproduced and a hand-summed row did not; this round's reader rebuilt the union from
> the thirteen programme commits plus the working tree and landed on **exactly round 8's published
> figures plus round 9's four new files** (85 + 4, 902 + 14, 300 + 4). Two methods, one answer. The
> round-5/6/7 `git status` parsing bug was guarded against rather than avoided by luck: the reader
> asserts a known path is present before it counts.
>
> **ONE CORRECTION, AND IT IS DEFINITIONAL RATHER THAN ARITHMETIC.** The *unopenable as filed* row is
> 8 files / 37 / 7c only under the rule **„does not resolve to a FILE"**. Six of its entries are
> directory-shaped (`…/rules`, `…/collision`, `…/vehicle`, `…/world/builders`, `…/environment`,
> `…/traffic`) and those directories exist, so a reader that asks `existsSync` instead of
> `statSync().isFile()` silently moves 6 files / 14 findings / 4 criticals into *never opened* and
> implies six files exist to be opened when what exists is six directories. The published 8 / 37 / 7 is
> right; the obvious way to recompute it is wrong. Round 9's O48 is the first lane to go hunting for
> findings parked on those directory strings — it found three.
...
> again **1,712 · 26 · 1,686 · 161 `SUMMARY` · 1,012 `BROKEN` (318 critical · 613 major · 81 minor) ·
> 512 `UNPOLISHED` · 1 `COULD_NOT_TEST` · 138 suspect files · 161 lessons.** Six readers, six identical
> tallies. **The frozen corpus is settled and further re-derivation of it is no longer evidence of
> anything** — what keeps being wrong is everything computed *against* it, not the tally itself.
>
> **§1's coverage union moved for the first time in three rounds: 63 → 72 opened, 68 → 59 never opened,
> 7 unopenable.** The three rows still sum to 138 / 1,012 / 318. **The 25 never-opened criticals did
> not move, and have not moved in seven rounds.**
>
> **What did NOT reproduce this time was a bug in this round's own reader, caught only because it
> contradicted itself.** Trimming the whole `git status --porcelain` blob before splitting eats the
> leading space of the first line and shifts a `slice(3)` by one character; it dropped
> `components/sim/CameraRig.tsx` from the touched set and printed the file in the *opened* and *never
> opened* lists at once. **Four rounds running, the error has been in the join and not in the corpus.**
> Every published figure here is from the corrected reader, which now asserts a known path is present
> before counting.
...
> **Re-run a third time on 2026-08-19 for round 4 by an independent reader** (all 24 files parsed,
> 1,712 records, the re-drive supersession applied to 22 scenarios / 26 records): **1,712 · 26 · 1,686
> · 161 SUMMARY · 1,012 BROKEN (318 critical · 613 major · 81 minor) · 512 UNPOLISHED · 1
> COULD_NOT_TEST · 138 suspect files · 161 lessons** — every figure identical. The corpus is frozen
> and three independent readers now agree on it. **What did NOT reproduce this time was §1's
> never-landed-lane block**, which printed two different measures as one and stated a critical total
> smaller than the two rows listed beneath it; it is corrected in §1.
>
> **Re-run a fourth time on 2026-08-19 for the round-5 gate, by a fourth independent reader**: the same
> **1,712 · 26 · 1,686 · 161 SUMMARY · 1,012 BROKEN (318 critical · 613 major · 81 minor) · 512
> UNPOLISHED · 1 COULD_NOT_TEST · 138 suspect files · 161 lessons**, and additionally **5 records
> naming no frame** and **the top 18 lanes carrying 365 findings / 201 criticals** — every published
> figure identical. Four readers now agree; the corpus is not in doubt. **What did NOT reproduce this
> time was §1's coverage union and §2.2's file count.** Recomputed mechanically against git history,
> the union is **63 opened / 68 never opened / 7 unopenable**, not 57/74/7: **six files §2.2 listed as
> never opened were edited by wave 1 itself**, including the two largest buckets in the whole corpus
> (`LessonPlayShell.tsx` 71 findings, `RouteGuidance.tsx` 23). Corrected in both places. **This is the
> third consecutive round in which the frozen tally reproduced perfectly and the hand-maintained
> coverage arithmetic did not** — the tables that are *computed* are reliable and the tables that are
...
echoed in the same shell immediately after. **The vitest run WAS backgrounded this round, and the trap
§0.4 recorded fired exactly as it is written there**: the harness announced the job „completed (exit
code 0)" — which is the *wrapper's* code, the `echo` after the run — while the file the run wrote ends
`VITEST_EXIT:1`. The wrapper's word was discarded and the file was read. `[gated]`

| gate | result | exit |
|---|---|---|
| `npx tsc --noEmit` | **clean — no output at all**, 0 lines | **0** |
| `npx vitest run --maxWorkers=2` | **931 files** — 928 passed · **2 failed** · 1 skipped · **14,643 tests** — 14,471 passed · **2 failed** · 170 skipped · 430.6 s | **1** |
| `node platform/scripts/validate-content.mjs` | 1,089 questions · 16/16 topics · 17 answer-leak scopes gated, **0 blocking** | **0** |
| `node platform/scripts/tools-tests.mjs` — the runner partition **and** the node:test half, run in full **in addition** to the three | **partition OK: 953 test files, 931 vitest / 22 node:test, none shared, none orphaned**; `vitest.config.ts` still spreads `VITEST_INCLUDE`. Then **390 tests, 390 pass, 0 fail** | **0** |

**BOTH failing files are the two that need a signature no agent may give** (§8), and both are **still
RED, with the same arithmetic as every round before**: `exam/__tests__/content-bank.test.ts`
(`ptp-i-parva-pomosht` 31/64 = **48 %** against a 50 % floor, `REVIEW_DEBT`) and
`lesson/__tests__/compose.test.ts` (`l-accidents-first-aid` carries no quiz beat). **Both were checked
to have RUN, not merely to have failed** — see §0.6.1, where they were red for the wrong reason for an
entire gate run. This round they executed **13 and 15 tests respectively, failing 1 each**, and the
failure text is the assertion's own diff (`REVIEW_DEBT … 31/64 (48%)`, `[ 'l-accidents-first-aid' ]`),
not a collection error. A green there would not be good news: it would mean a content `status` field
had been flipped to clear a gate.

**THERE IS NO THIRD FAILING FILE. Round 9's delta on every gate is ZERO** — §0.9.

**The fourth gate stayed green** — `node scripts/tools-tests.mjs`, the repo's SECOND test runner and
the one invisible to the brief's list of three, holds the **390 / 390** round 6 won it, and its
partition audit again confirms the config still spreads `VITEST_INCLUDE`.

**Toolchain integrity was checked BEFORE the gate was believed**, which is §0.6.1's standing remedy:
`platform/node_modules/.bin` holds **96 shims**, `node_modules/.package-lock.json` is **present**, and
all **41** declared dependencies resolve. An empty `.bin` invalidates the run and fails in the
reassuring direction.

### 0.9 — Round 9: no collision, no regression, and the four heaviest never-opened criticals opened at last

**8 concurrent lanes, 15 files modified and 9 new files (7 test files + 2 tools) — and the delta
against `25c2143` on all four gates is ZERO.** `[gated]`

| | baseline `25c2143` | round 9 (working tree) | delta |
|---|---:|---:|---:|
| vitest test **files** | **924** ‡ | **931** | **+7** |
| node:test **files** (second runner) | 22 | **22** | 0 |
| vitest **files** failed | 2 | **2** | **0** |
| vitest **tests** failed | 2 | **2** | **0** |
| vitest tests passed | — | **14,471** | — |
| vitest tests skipped | 170 | 170 | 0 |
| `tsc` errors | 0 | **0** | 0 |
| content-validation blocking | 0 | **0** | 0 |
| tools-gate failures | 0 | **0** | 0 |

‡ **Measured at the commit under test, not quoted.** `npx vitest list --filesOnly` returns 931 files
today; each was resolved to a repo-relative path and checked against `git ls-files`, and **exactly 7
are untracked**, so `25c2143` collects **924**. It reconciles independently against §0.7: round 7's
gate measured **911**, round 8 committed **13** new test files, and 911 + 13 = **924**.

**THE BRIEF'S FILE COUNT WAS WRONG AGAIN, AND THIS IS THE FOURTH CONSECUTIVE ROUND.** The round-9
brief states a baseline of **878 files**, predicts **881** „if the orphaned-runners lane succeeded",
and warns that 878 would mean that lane did not deliver. As §0.5, §0.6 and §0.7 each record, **878 is
`platform/scripts/tools-tests.mjs`'s own header figure measured at `85ca415`** (the string is still at
that file's line 33), and **the orphaned-runners fix landed in `ed2dd6f`, five commits and four rounds
ago.** `../tools/mobile/{ladder,selectors,settle}.test.mjs` are in `VITEST_INCLUDE` at HEAD, all three
were collected this round and all three are green — **settle 23 tests, ladder 8, selectors 6, plus
budget 22 (1 skipped), 0 failures.** There was no orphaned-runners lane in round 9, it contributed no
findings, and **there is therefore no separate count to report: the „report their number separately so
it is not misread" instruction has nothing to separate.** Had 878/881 been used as the gate, a round
with a zero delta would have read as a **+53 catastrophe**.

**NOTHING IS CLASS (a), A CROSS-LANE COLLISION.** The signature is a test failing in a file no lane
owned, and there is no failing file outside the two content rows at all. **Nothing is class (c), a
lane shipping a test file it never ran**: all 7 new test files were located in vitest's own
collected-file list and then found in the run output with a test count beside them —
`flow-sweep161-truth` 41 · `reverseAssist-audit-harness` 8 · `island-wall-is-a-collider` 7 ·
`rail-crossing-every-district` 5 · `lesson-world-spawn-resolution` 4, with `specs-banner-truth` and
`low-beam-duty-one-source` likewise collected and green — **144 `expect(`s across 44 test blocks, and
0 `.skip` / `.only` / `.todo` / `xit` / `xdescribe` in any of the eleven changed or new test files.**
And nothing is class (b) in the sense of a new test correctly catching a live defect: **every new test
is green**, which is the round's one soft spot and is recorded as such in §0.9.1.

**ONE MODIFIED FILE DESERVED A SECOND LOOK AND SURVIVED IT.**
`world/__tests__/expected-failures.json` is the gate's own allowlist, and a lane rewriting it is the
cheapest way to turn a red gate green without fixing anything. `git diff --numstat` on it is **2
insertions, 2 deletions**: the `owner` and `reason` strings of the O42 entry only. `"scenarios": 1`,
`"rungs": 4`, `"ledger": "T14"` and `"allowlist": []` are byte-identical. **No expectation was
loosened.**

#### 0.9.1 — What the round found instead: three findings refuted from evidence, not from argument

Round 9 is the first round whose lanes mostly **disproved** the findings they were handed, and each did
it on a measurement rather than on a reading of the code:

* **`world/builders/roundabout.ts` (1/1c)** — the sweep says *„the car ends up driving on the central
  island"*. The lane read the drive's own debrief: «Настъпи сблъсък · 1 опасна грешка · 10 наказателни
  точки». **The car was stopped BY the island, not carried over it.** The finding's last sentence
  contradicts its first. The battery it built asserts the *index buffer* rather than the prose, because
  `buildIsland`'s header made the same claim in words and was false for months — vertices no triangle
  references are a wall you can see and drive through.
* **`world/builders/railTrack.ts` (1/1c)** — the sweep says *„the data is there and nothing draws it"*.
  The lane settled it on pixels: a scan across `public/gallery-stills/sc__sc-pk-rail-ban.webp` reads
  asphalt / ballast / sleeper / rail alternating on a ~14 px period whose 8:6 split is
  `SLEEPER_SPACING_M` 1.5 : `SLEEPER_WIDTH_M` 0.6 to within a pixel. **The deck is drawn.** What the
  sweep frames show is a car averaging 5.2 км/ч for 198 s — a camera question, not a drawing one. The
  battery it left takes **no list of map ids**: it walks `public/world/*.json` and asserts a deck for
  every `railCrossing` zone it finds, because the two existing batteries pin four ids by hand and map
  number five inherits no coverage at all.
* **`scene/lessonWorldRecipe.ts` (5/1c)** — four „the world does not match the briefing" findings route
  here. The lane found the real mechanism (a `spawn.pointId` absent from its district falls through
  `p?.x ?? explicit?.x ?? 0` and puts the car at district ORIGIN, mid-junction, silently) and then
  **measured it over every playable spec — LESSONS + POLIGON_LESSONS + EXAM_LESSON plus all 167
  scenario templates at every authored rung, 819 lessons: 0 unresolved.** It is latent, not live. The
  test carries its own positive control, because a census that silently stopped finding lessons would
  report the same 0. The lane also disproved **its own** hazard: the `?? []` half was written up as a
  second failure mode and then killed, because `parseDistrict` throws `district: missing spawnPoints[]`
  before that line is ever reached.

**A REFUTATION IS NOT A CLOSE, AND THE COVERAGE COUNT CANNOT SEE THE DIFFERENCE.** `roundabout.ts` and
`railTrack.ts` were **not edited** — only new test files beside them were — so both remain in §2.2's
never-opened row and inside its 7 standing criticals, correctly. That is the honest bookkeeping, and it
means **two of the seven remaining never-opened criticals have now been adjudicated without moving a
single number in this document.** §7's argument for a `closedBy` / `refutedBy` field is the remedy;
there is still no field to write either into, and this is the third distinct way the programme has now
been bitten by that absence.

### 0.8 — Round 8's gate table was never written into this section

`[read]` §0 skipped a round: it described round 7 at `7404468` until this edit, while `a4a4bf7` and
`25c2143` both landed. §1 and §2.2 *do* carry round 8's corpus and coverage arithmetic, so the gap is
in the gate table only — but it is the reason no tests-failed count for `25c2143` can be quoted above
and that row of §0.9's baseline column reads „—" rather than a number. **What is recoverable and was
measured today: `25c2143` collects 924 vitest files**, by two independent methods (§0.9 ‡). Whoever
runs round 10 should record the gate table in the same commit as the round, not the round after.

### 0.7 — Round 7: one collision, and it is a lane that read the test that convicted it and shipped anyway

**8 concurrent lanes, 20 files modified and 9 new test files.** `[gated]`

| | baseline `7404468` | round 7 (working tree) | delta |
|---|---:|---:|---:|
| vitest test **files** | **902** | **911** | **+9** |
| node:test **files** (second runner) | 22 | **22** | 0 |
| vitest **files** failed | 2 | **4** | **+2** |
| vitest **tests** failed | 2 | **9** | **+7** |
| vitest tests passed | 14,021 ‡ | **14,186** | +165 |
| vitest tests skipped | 170 | 170 | 0 |
| `tsc` errors | 0 | **0** | 0 |
| content-validation blocking | 0 | **0** | 0 |
| tools-gate failures | 0 | **0** | 0 |

‡ the `8a9a9a9`+round-6 figure; round 6 was committed as `1625325`/`7404468` after that gate ran.

**THE BASELINE IS 902, NOT 878 — AND THIS IS THE THIRD CONSECUTIVE BRIEF TO CARRY THE STALE NUMBER.**
The round-7 brief again stated **878 files**, again predicted **881** "if the orphaned-runners lane
succeeded", and again warned that 878 would mean the lane did not deliver. As §0.5 and §0.6 both
record, **878 is `scripts/tools-tests.mjs`'s own header figure measured at `85ca415`**, and **the
orphaned-runners fix landed in `ed2dd6f`, three rounds ago.**
`../tools/mobile/{ladder,selectors,settle}.test.mjs` are in `VITEST_INCLUDE` at HEAD, were collected
this round, and were re-run alone to be certain: **3 files, 37 tests, 0 failures, exit 0.** There was
no orphaned-runners lane in round 7 to deliver anything, and **there are no findings to report
separately from the rest, because that lane contributed none.** Had 878/881 been used as the gate, a
round that opened nine new corpus files would have read as a **+33 catastrophe**. The real baseline is
**902**, the real count is **911**, and the +9 is nine new test files, one per lane but for the two
that shared one.

**NO LANE SHIPPED A TEST FILE IT NEVER RAN — class (c) is clean.** The nine new files were re-run as
their own suite: **9 files, 155 tests, 0 failures, exit 0**, carrying **347 `expect(`s across 130 test
blocks** (24/12, 46/20, 27/13, 49/17, 39/19, 32/10, 82/24, 22/6, 26/9). A repo-wide sweep again found
**zero** test files under `platform/src` with no `expect(` in them.

**THE SEVEN NEW FAILURES ARE ONE LANE AND ONE CAUSE — class (a), a collision.** Both failing files are
**owned by no round-7 lane**: `lessons/scenario/__tests__/signals2-controller-clock.test.ts` (6 failed
of 15) was committed at `730da10` and `lessons/scenario/__tests__/signals-sweep161.test.ts` (1 of 25)
at `7a0ab52`/`2f5ce8f`; neither appears in `git status`. **But this is not the blind kind of
collision.** The lane that broke them — the traffic-controller-clock lane, `orchestrator/runners.ts` —
**names the file it broke, in its own header, as "the routed prescription"**, and that file's header
states the obligation in one sentence:

> *"§1 THE DEFECT, REPRODUCED THROUGH THE PRODUCTION STACK. … This is the section that must be
> INVERTED (not deleted) when the runner fix lands: the whole point of the fix is that 36 s of
> briefing stops changing the grade."*

The lane landed the fix and did not invert the section. **The fix itself is right, and the failures
prove it is right** — which is why this must not be booked as a regression:

- **4 of the 7 are defect-reproduction rows that now correctly refuse to reproduce the defect.**
  `signals2` §1 × 3 and `signals-sweep161` §4 × 1 assert `crossings[0].controller === "proceed"` —
  i.e. *the reckless 59 км/ч drive is acquitted after 36 s of briefing*. They now read `"halt"`. The
  drive is convicted. **That is the whole point of the lane**, and §1's own sibling row (*"the mechanic
  itself works: with no dead time the drive is convicted 10 т."*) still passes.
- **3 of the 7 are the cure section double-counting itself.** §2 proves the cure *without touching the
  engine*, by pre-baking it into the authored spec: `rebased(dead) = {…, flipAtSec: 30 + dead}`. The
  runner now rebases at runtime as well, so the flip lands at **≈ 30 + 2 × dead**; the careful drive
  waits out the halt, arrives on the authored profile, and finds the officer still halting. **The
  signature is exact: every dead-time-0 row passes** (`rebased(0)` is byte-identical to the shipped
  spec, and that row passes too) **and only 12 / 36 / 60 s fail.** The reckless rows all still pass,
  because a later flip convicts them harder.

**No production defect is implied by any of the seven, and no lane's own test caught anything — so
this is neither class (b) nor class (c).** The work is test work: invert §1 and `signals-sweep161`
§4 to the post-fix contract, and rewrite §2 to stop pre-baking a rebase the engine now performs.
**Round 7 must not be signed off as green, and must not be booked as a regression either.**
...
**8 concurrent lanes, 24 files modified and 8 new test files — and the delta against `ed2dd6f` on the
three named gates is ZERO.** `[gated]`

| | baseline `ed2dd6f` | round 5 (working tree) | delta |
|---|---:|---:|---:|
| vitest test **files** | **893** ‡ | **899** | **+6** |
| node:test **files** (second runner) | **20** ‡ | **22** | **+2** |
| vitest tests **failed** | 2 | **2** | **0** |
| vitest tests **passed** | *not measured* | 13,951 | — |
| vitest tests skipped | 170 | 170 | 0 |
| `tsc` errors | 0 source | **0 raw** | 0 |
| content-validation blocking | 0 | **0** | 0 |
| tools-gate failures | **1** ‡ | **1** | **0** |

‡ **The baseline column is measured, not assumed, and this is the first round where that is true of the
file counts.** 893 vitest / 20 node:test is §0.4's own independently measured figure at the round-4
working tree, which is exactly what `ed2dd6f` committed; it was re-derived here a second way, by
listing the 899 files vitest collects today and checking each against `git ls-tree HEAD` — **exactly 6
are untracked**, so HEAD collects 893. Two independent methods, same number.

**THE BRIEF'S PREDICTED FILE COUNT WAS WRONG, AND THE WAY IT WAS WRONG IS WORTH KEEPING.** The round-5
brief stated a baseline of **878** files and predicted **881** if the orphaned-runners lane succeeded.
878 is a real measurement — it is `scripts/tools-tests.mjs`'s own header figure, **measured at commit
`85ca415`**, which is five commits behind `ed2dd6f`. Rounds 3 and 4 added 15 test files after it, and
**the orphaned-runners fix had already landed in `ed2dd6f` itself**: `../tools/mobile/{ladder,selectors,settle}.test.mjs`
are in `VITEST_INCLUDE` at HEAD, they were collected and they passed. Had the 878/881 prediction been
used as the gate, a correct round would have been read as a catastrophic +18 overshoot. **A baseline is
a measurement at a commit, and a stale one fails in whichever direction the arithmetic happens to
point.** The three files that lane un-orphaned are green; the two new node:test files round 5 adds
(`tools/mobile/__tests__/lane-evidence.test.mjs`, `tools/mobile/lib/__tests__/exit-integrity.test.mjs`)
are green too, and **neither is visible to vitest by design** — `VITEST_INCLUDE` names four
`tools/mobile` files individually precisely because a directory glob swallows the node:test files and
hard-fails.

**No failure in this round is class (a) — a cross-lane collision.** The signature of a collision is a
test failing in a file no lane owned; the only failure outside the two content rows is in
`tools/mobile/insets.test.mjs`, and it is **pre-existing at HEAD, not a collision**. Proven rather than
assumed: `git diff HEAD --stat` is empty for `insets.test.mjs`, for `lib/insets.mjs` and for
`zoom-follows-window.mjs`, the file it convicts — all three are tracked and byte-identical to
`ed2dd6f`, so the same assertion fails at the baseline. The failure is real and worth fixing:

> `insets.test.mjs:140` — *"no probe opens a browser context without the notch — newDeviceContext is
> the one door"*. It convicts `zoom-follows-window.mjs:29`, which calls `browser.newContext({ viewport:
> { width: 402, height: 874 }, … })` directly, so it *"lays the app out at `env(safe-area-inset-*)` = 0
> — i.e. on a phone with no notch and no home indicator — and every number it publishes is about a
> device nobody owns."*

**It is class (b) with a delay** — a real defect a correct test caught, in a runner nobody had run
against this file since it was added (`zoom-follows-window.mjs` last changed in `7e2fd21`, the commit
that added iPhone 16 Pro at 402×874, and the probe it added is the one that bypasses the door). It is
**not** class (c) — a lane shipping a test file it never ran — because no round-5 lane owns either
file. **The lesson is the same one §0.4 keeps paying for: a three-gate brief cannot see a four-runner
repo.** The tools gate must be in the standing gate list, and it is added to the table above.

**Every one of the 8 new test files carries real assertions** — 63, 49, 39, 33, 32 and 27 `expect(`s in
the six vitest files, 46 and 32 `assert`s in the two node:test files, 92 test blocks in total — and a
repo-wide sweep found **zero** test files under `platform/src` with no `expect(` in them. No lane
shipped an empty guard.

### 0.4 — Round 4: no collision, one declared-red guard, and a gate poisoned from outside the repo

**8 concurrent lanes, 18 files touched, 4 of them new test files — and the delta against the
`c72bcc2` baseline is +1 failure, which is not a collision.** `[gated]`

| | baseline `c72bcc2` | round 4 (working tree) | delta |
|---|---:|---:|---:|
| test **files** | 890 †       | **893** | **+3** |
| tests **failed** | 2 †        | **3** | **+1** |
| tests skipped | 170 †        | 170 | 0 |
| tests passed | *not measured* | **13,833** | — |
| `tsc` errors in source | 0 †   | **0** | 0 |
| content-validation blocking | 0 † | **0** | 0 |

† **The baseline column was not re-run for this gate, and is marked so rather than implied.** The
round-4 column is measured — one `vitest` run, its own `$?`. The baseline figures are the round-4
brief's stated starting point (`tsc` 0 · content 0 · vitest 2 failed) plus the file count derived
from it: 893 collected today minus the 3 test files this round adds = 890, which agrees with §0.3's
independently measured 890 at `e9bf33c`+round-3, and `2f68ab5`/`c72bcc2` added no test file. **The
passed-test count is simply left blank**: 13,833 − 61 would have looked like a measurement and would
have been arithmetic on a number nobody ran. A delta is only worth printing when both ends of it
were measured, and only three of these five rows have both.

**The one new failure is class (b) — a real defect a new test correctly caught — and it is the
cleanest example of that class the programme has produced.** It is *not* class (a): the signature of
a collision is *a test failing in a file no lane owned*, and this test fails in
`environment/__tests__/weather.test.ts`, which the weather lane owns and edited in the same change.
It is *not* class (c) either — a lane shipping a test file it never ran — because the lane ran it,
recorded the result, and **wrote the expected red into the file before shipping**:

> *“It is RED until the two-file routing lands; that is the guard working, not the guard broken.”*

| failing test | owned by | verdict |
|---|---|---|
| `environment/__tests__/weather.test.ts` — *“THE GUARD: the snow-bearing mapping is reachable AND actually read”*, expected `reachable=false readsSnowChannel=false passesLiveSnow=false` to be `reachable=true readsSnowChannel=true passesLiveSnow=true` | **the weather lane** (same change) | **(b) real, open defect — must not be cleared by weakening it** |

**The defect it convicts was verified independently of the test, by reading the three files.**
`[read]` `weather.ts:584` exports `roadSurfaceToParams`, the road mapping that finally bears a snow
term; `environment/index.ts` exports `wetnessToRoadParams` and **not** `roadSurfaceToParams`; and
`world/components/StaticWorld.tsx:21` imports `{ useWetness, wetnessToRoadParams }` and calls the
rain-only mapping at **:279, :294 and :313**. The snow term is real, tested and reasoned — and it
**changes not one pixel of the snow lesson the audit filed as critical.** By doc 05, a helper missing
from the barrel does not exist. This is the half-routed state, and the lane's own header says why it
is the dangerous one: *“it reads like a fix.”*

**This is the second half-routed fix found in one round, and the other was found by the lane that had
caused it.** The `SessionEndScreen` lane discovered that round 3's `FaultCard` fix went green at the
unit level while the only surface that mounts the list never passed the prop — *“the prop DEFAULTS TO
BILLED on purpose. So the screen went on printing the catalogue base on every row for another day”* —
and shipped `session-end-ledger-close.test.tsx` to pin it at the screen level. **Two lanes, one
shape: a fix that is complete in its own file and inert in the product.** That shape has now produced
a critical-severity miss in two consecutive rounds and deserves its own line in the review checklist,
because what it looks like from inside the owning file is a passing test.

#### 0.4.1 — The `tsc` red is not this repository's, and `exclude` cannot stop it

`tsc --noEmit` exits **1** on four syntax errors, all in
`platform/.next-harness/dev/types/routes.d.ts`. **No lane touched it, no commit contains it, and it
is gitignored** (`platform/.gitignore:9`, `.next-*/`). It is a **torn write**: 154 lines against the
live `.next/dev/types/routes.d.ts`'s 136, with `interface` truncated to `erface` **twice** — a longer
old file partially overwritten by a shorter new one while a harness dev server was running. Its mtime
falls **inside** this gate's own `tsc` invocation.

**Proven environmental, not asserted.** `[gated]` `platform/next-env.d.ts` — itself gitignored
(`platform/.gitignore:52`) and rewritten by Next on every dev run — carries
`import "./.next-harness/dev/types/routes.d.ts";`. With that one line commented out and **nothing
else changed**, `npx tsc --noEmit` prints **no errors and exits 0**. The file was then restored from a
backup and verified byte-identical with `diff`.

**The part worth keeping: `tsconfig.exclude` gives no protection here, and the config looks as though
it does.** `tsconfig.json` excludes `.next-*`, and that exclusion is real but irrelevant — `exclude`
filters which files the *wildcard include* patterns collect, and never which files are reachable by
**import**. `tsc --explainFiles` names the route in one line:

```
.next-harness/dev/types/routes.d.ts
  Imported via "./.next-harness/dev/types/routes.d.ts" from file 'next-env.d.ts'
```

So **any agent who starts a Next dev server with a custom `distDir` silently repoints the type gate at
that dir for every later session**, and a half-written artifact there fails `tsc` for work that has
nothing to do with it. Four such dist dirs now exist (§9). This is a standing hazard on this box
rather than a round-4 event, and it is why the gate table reports `tsc` as **1 raw / 0 source**
instead of quietly picking one of the two.

**The file count is evidence, and the round-4 brief's file-count premise was two rounds stale.**
`[gated]` The brief predicted **878 → 881** on the strength of the orphaned-runners lane getting three
never-executed `tools/` files collected. That arithmetic was already retired in §0.3: those three —
`tools/mobile/{ladder,selectors,settle}.test.mjs` — were un-orphaned in **round 2** (`e9bf33c`), and
the tree has gated at 888, 889 and 890 files since. **878 has not been this suite's file count for
three rounds**, so “still reads 878” could not have been evidence of anything this round. The real
accounting for round 4's +3:

| source | files | note |
|---|---:|---|
| `hud/__tests__/session-end-ledger-close.test.tsx` | +1 | the screen-level pin described above — 31 `expect(`s |
| `lessons/__tests__/finish-work-site-band.test.ts` | +1 | the `finish.ts` lane — 36 `expect(`s |
| `lessons/__tests__/reach-zone-witness.test.ts` | +1 | the `objectives.ts` lane — 60 `expect(`s across 33 blocks |
| `tools/mobile/lib/__tests__/target.test.mjs` | **+0 to vitest** | 89 assertions, and it declares **`node:test`** — so it is the **node** gate's file, and that gate went 19 → 20. The partition audit confirms it: *“913 test files, 893 vitest / 20 node:test, none shared, none orphaned”* |
| **total** | **890 → 893** | 913 across both runners |

**The orphaned-runners lane added zero failures this round, because its work landed two rounds ago.**
All three formerly-orphaned files are collected and green. What the brief anticipated as this round's
legitimate new red had already been paid for in round 2, where `ladder.test.mjs` was red on its first
ever execution and was fixed there (§0.1.1). **A brief that predicts a number is only useful if the
number is re-derived before it is used as evidence** — this one would have convicted a lane that had
done nothing wrong.

**No assertion was weakened to reach this number**, which is the other way a +1 could have been
bought. `[gated]` Every changed test file gained assertions against `HEAD`: `test-ownership.test.mjs`
48 → 99 (23 → 46 blocks), `tools/mobile/__tests__/lesson-audit.test.mjs` 33 → 77 (10 → 18),
`weather.test.ts` 97 → 113 (31 → 34), `markings-paint-truth.test.ts` 225 → 243,
`stop-claim-gates.test.ts` 36 → 42 (15 → 16). `fault-card-ledger-close.test.tsx` holds at 26/5 and its
diff is **documentation only** — the header retreating from a screen-level claim it could not support.
The single `.skip` in any changed or new test file is
`tools/mobile/__tests__/lesson-audit.test.mjs:364`, `t.skip("no playwright browser on this machine")` —
a runtime capability guard, not a muted assertion.
...
**8 concurrent lanes, 25 files touched, 7 of them new test files — and the delta against the
`4dd1850` baseline is ZERO new failures.** `[gated]`

| | baseline `4dd1850` | round 2 (working tree) | delta |
|---|---:|---:|---:|
| test **files** | 878 | **888** | **+10** |
| tests passed | 13,549 | 13,693 | **+144** |
| tests **failed** | 2 | **2** | **0** |
| `tsc` errors | 0 | 0 | 0 |
| content-validation blocking | 0 | 0 | 0 |

**Not one failure falls into any of the three classes the previous waves produced** — (a) a collision,
whose signature is *a test failing in a file no lane owned*; (b) a real defect a new test correctly
caught; (c) a lane that shipped a test file it never ran. All three appeared in wave 3 (§0.2). None
appeared here. This is the first multi-lane round in the programme to land clean.

**No assertion was weakened to get there**, which is the other way this number could have been bought.
`[gated]` Every changed test file gained assertions against `HEAD`: `weather.test.ts` 40 → 62
(14 → 23 blocks), `coach.test.ts` 35 → 54 (12 → 20), `markings-paint-truth.test.ts` 89 → 138
(18 → 29), `sim-overlay-line-grid.test.ts` 40 → 42, `ladder.test.mjs` 9 → 12, `settle.test.mjs` 73 → 73.
No `.skip`, `.only`, `.todo`, `xit` or `xdescribe` appears in any changed or new test file, and no
test file anywhere in the repository — tracked or untracked — is missing `expect(`/`assert`.

**The file count is the evidence, and all ten are accounted for.** The count is *not* 881, which is
what a three-file orphan rescue on its own would have produced:

| source | files | note |
|---|---:|---|
| `tools/mobile/{ladder,selectors,settle}.test.mjs` | +3 | the orphans — see §0.1.1 |
| `platform/scripts/__tests__/test-ownership.test.mjs` | +1 | the orphan lane's own audit, run inside the vitest gate as well as the node one |
| 6 new test files from the other 7 lanes | +6 | `collision` · `runtime` · `hud` · `lessons` · `orchestrator` · `components/sim` |
| **total** | **+10** | 878 → **888** |

#### 0.1.1 — The orphaned-runners lane: a FINDING, not a regression

The lane got **three test files executed that no runner had run since the vitest include glob was
narrowed off `../tools/mobile/**/*.test.mjs`** — a narrowing that was itself correct (the broad glob
was swallowing that directory's node:test files and hard-failing on them) but that left everything
else in the directory claimed by nobody. `[read]` `platform/scripts/tools-tests.mjs` carries the
measurement: at commit `85ca415`, `npx vitest list --filesOnly` returned 878 files and none of them
was one of these three; asking vitest for them **by name printed nothing and exited 0**. Between them
they held **33 test blocks and 88 assertions, green by absence — which is the same green a passing
suite prints.**

**Their red count in this gate is 0, and that 0 must not be read as "nothing was wrong".** One of the
three *was* red on its first ever execution, and the lane repaired it before shipping:

| file | first-run verdict | what the lane judged, and which way |
|---|---|---|
| `tools/mobile/ladder.test.mjs` | **RED** | **the TEST was stale, not the code.** Its geometry pin listed four device profiles while `DEVICES` held six: commit `96a3ea5` (2026-08-11, "a six-phone ladder with real notches") added the two Samsung `galaxy-gesturebar-*` rows and the pin had been wrong for eight days with nothing able to say so. The lane pinned all six **and added a safe-area band assertion**, because `galaxy-gesturebar-portrait` is 360x780 — the same viewport as `small-portrait`, deliberately, as a controlled A/B in which the inset is the only variable — so geometry alone cannot tell the two rows apart, and a silent inset collapse would leave every "measured against an Android 15 gesture bar" claim false while the geometry pin still passed. +2 blocks, +3 assertions. |
| `tools/mobile/settle.test.mjs` | green | header correction only: the comment claiming the config "already globs `../tools/mobile/**/*.test.mjs`" had been false since the narrowing. 73 assertions unchanged. |
| `tools/mobile/selectors.test.mjs` | green | untouched — correct as written. |

**The structural fix is what makes this non-repeatable, and it is the more valuable half.**
`VITEST_INCLUDE` now lives in `platform/scripts/tools-tests.mjs` — plain `.mjs`, readable by *both*
runners — and `vitest.config.ts` imports it. `auditOwnership()` compares the runner each test file
**declares** (read from its imports, with comments stripped first, because half the files here explain
a defect by quoting the code that caused it) against the runners that will actually **execute** it
(vitest's own `tinyglobby`, resolved *through vitest's own package* so the version that answers is the
version that will run, with vitest's own options; plus this script's own walk). It fails on every
disagreement in either direction — claimed by nobody, by both, or by the wrong one — and it runs as a
**precondition** in the node gate, before a single test, as well as a test inside the vitest gate.
Silencing it now takes two deliberate acts in two runners rather than one narrowed glob.

### 0.2 — Wave 3's gate and its collisions *(historical — every row below is now green)*

**All 20 non-signature failures described in this subsection were cleared by `7a0ab52` and `4dd1850`
and are green in the 2026-08-19 gate above.** The diagnosis is kept because the *process* finding it
records had by then recurred three times, and because §0.1's clean result is only meaningful when read
against it.

At `2f5ce8f` **`vitest` exited 1, and only 2 of the 22 were the deliberate content debt.** The wave-3 commit message
...
#### 0.2.1 — The collisions, each one diagnosed
...
> **SUPERSEDED BY THE ROUND-10 GATE, 2026-08-20.** That sentence was true of round 9 and is not true
> now. `HEAD` is **`4afa999`**, and the working tree carries **29 modified files and 13 new test
> files** from seven concurrent round-10 lanes, none of it committed. Gate results, working tree, exit
...
> · 1 `COULD_NOT_TEST` · 138 suspect files · 161 lessons.** Ninth reader, ninth identical result.
>
> | row | files | findings | critical |
> |---|---:|---:|---:|
> | ever opened | **110** | 950 | 304 |
> | never opened | **20** | 25 | 7 |
> | unopenable as filed | **8** | 37 | 7 |
> | | **138** | **1,012** | **318** |
>
> **21 NEW FILES, 34 FINDINGS, 0 CRITICALS.** The largest file movement in the programme and the only
> large one that leaves the never-opened critical row exactly where it found it: **7 → 7**, the same
> seven files §2.2 already lists by name. Rounds 8 and 9 were aimed at that row and took it 25 → 11 →
> 7; round 10 was aimed at the last of the major/minor bulk. Nothing here contradicts round 9 — it is
> what happens when a round spends itself on breadth.
>
> **BOTH DEFINITIONAL TRAPS THE PREVIOUS TWO ROUNDS RECORDED WERE HONOURED, NOT REDISCOVERED.** The
> reader uses `statSync(...).isFile()` — so the six directory-shaped entries (`…/rules`,
> `…/collision`, `…/vehicle`, `…/world/builders`, `…/environment`, `…/traffic`) stay in *unopenable*
> and the row reproduces at 8 / 37 / 7c rather than silently shedding 6 / 14 / 4c into *never opened*.
> And it does not trim the `git status --porcelain` blob before splitting; it **asserts
> `components/sim/LessonScene.tsx` is present in the touched set and exits non-zero if it is not**,
> which is the guard §1 asked for two rounds ago and round 9 first installed.
>
> **THE UNION WAS BUILT FROM `git diff ec1f56f..HEAD` PLUS THE WORKING TREE — NOT FROM THE LANE
> MANIFESTS — AND THE DIFFERENCE IS 13 FILES.** See the OPEN LIST header: the seven round-10 lanes
> owned 34 corpus files and left 13 of them untouched, and those 13 are precisely the ones this row
> still counts as never opened. **Owned is not opened.** A manifest-derived union would have reported
> 7 never-opened files this round and been wrong by 34 findings.
>
> **ROUND 9, 2026-08-19 — the corpus reproduced for the EIGHTH time, and the never-opened criticals
> nearly halve again.** `[corpus]` All 24 JSONL files parsed, the re-drive supersession applied by the
> header's rule: **1,712 records · 26 superseded (22 lessons) · 1,686 standing · 161 `SUMMARY` · 1,012
> `BROKEN` (318 critical · 613 major · 81 minor) · 512 `UNPOLISHED` · 1 `COULD_NOT_TEST` · 138 suspect
> files · 161 lessons.** An eighth independent reader, eighth identical result. The corpus remains
> frozen and is not in doubt; **it is still everything computed *against* it that moves.**
>
> **ROUND 9 OPENED FOUR NEW FILES — the union moves 85 → 89 — AND ALL FOUR CARRY A CRITICAL, so the
> never-opened critical row falls 11 → 7.** Two rounds ago that row had stood at 25 for seven rounds.
> The four are exactly the four heaviest of the eleven §2.2 listed by name: `scene/lessonWorldRecipe.ts`
> (5/1c), `lessons/specs.ts` (4/1c), `lessons/scenario/templates-flow.ts` (3/1c) and
> `engine/reverseAssist.ts` (2/1c). `41 + 89 + 8 = 138` · `59 + 916 + 37 = 1,012` · `7 + 304 + 7 = 318`.
> The three rows still sum.
>
> **THE ROUND-9 ROW IS THE BIGGEST IN THE TABLE SINCE WAVE 3 AND THAT NUMBER IS MISLEADING ON ITS OWN.**
> It reads `9 | 127 | 57`, but 111 of those 127 findings and 51 of the 57 criticals are two files that
> were already open and are the two largest critical buckets in the corpus — `rules/engine.ts` (65/41c)
> and `components/sim/LessonScene.tsx` (40/6c), both touched again by the low-beam and collider lanes.
> **The honest measure of what round 9 added to coverage is the NEW files: 4 files, 14 findings, 4
> criticals.** Both numbers are given because the row total is the one a summary will quote.
>
> **THE ROUND-8 ROW IS NOW A COMMIT.** It read „working tree, UNCOMMITTED" when this section was last
> written; `25c2143` carries exactly the fifteen corpus files it listed, and the row recomputes to the
> same `15 | 47 | 15` from the commit that it did from the tree. **The round-9 row is the uncommitted
> one now** — 15 modified files and 9 new ones, of which 9 are corpus files.
>
> **THE HAND-SUMMED TABLES WERE RIGHT THIS TIME, AND FOR ONCE THAT IS CHECKABLE RATHER THAN ASSERTED.**
> This round's reader computed the union from scratch — programme commits `ec1f56f`, `730da10`,
> `2f5ce8f`, `7a0ab52`, `4dd1850`, `e9bf33c`, `2f68ab5`, `ed2dd6f`, `8a9a9a9`, `1625325`, `7404468`,
> `a4a4bf7`, `25c2143` plus the round-9 working tree — and landed on **exactly the round-8 published
> figures plus round 9's four new files**: 85 + 4 = 89, 902 + 14 = 916, 300 + 4 = 304, and 45 − 4 = 41,
> 73 − 14 = 59, 11 − 4 = 7. Two readers, two methods, one answer. **That is the first independent
> confirmation the coverage join has ever had**, and it retires §7's specific worry about *this* join
> without retiring the argument for the generator.
>
> **THE READER BUG FROM ROUNDS 5–7 WAS GUARDED AGAINST RATHER THAN HOPED AWAY.** Trimming the whole
> `git status --porcelain` blob before splitting eats the leading space of the first line and shifts a
> `slice(3)` by one character. This round's reader does not trim the blob, and **asserts that a known
> path (`components/sim/LessonScene.tsx`) is present in the working-tree set before it counts
> anything**, exiting non-zero if it is not. It is the check §1 has been recommending for two rounds.
>
> **ONE DEFINITIONAL CORRECTION, AND IT IS THIS DOCUMENT'S OWN.** The *unopenable as filed* row is 8
> files / 37 / 7c only if „unopenable" means **does not resolve to a FILE**. A naive existence check
> passes on the six directory-shaped entries (`…/rules`, `…/collision`, `…/vehicle`,
> `…/world/builders`, `…/environment`, `…/traffic`) because those directories are real, and silently
> moves 6 files / 14 findings / 4 criticals out of the unopenable row and into *never opened* —
> inflating the backlog and, worse, implying six files exist to be opened when what exists is six
> directories. The reader must `statSync(...).isFile()`, not `existsSync(...)`. Recorded because the
> published 8 / 37 / 7 is right and the obvious way to recompute it is wrong.
...
> 26 superseded (22 lessons) · 1,686 standing · 161 `SUMMARY` · 1,012 `BROKEN` (318 critical · 613
> major · 81 minor) · 512 `UNPOLISHED` · 1 `COULD_NOT_TEST` · 138 suspect files · 161 lessons.** Six
> independent readers, six identical tallies; the corpus has never moved and is not in doubt.
>
> **The union is 63 → 72 · never opened 68 → 59 · unopenable 7, unchanged.** After two rounds that
> opened nothing, **round 7 opened NINE new corpus files** — the largest single-round movement in the
> programme. `72 + 59 + 7 = 138` · `863 + 113 + 36 = 1,012` · `287 + 25 + 6 = 318`. The three rows
> still sum.
>
> **AND THE NINE CARRY ZERO CRITICALS BETWEEN THEM — every one of the 56 findings they move is
> non-critical.** That is not an accident and should not be read as one: round 7 was pointed at
> **Wave B, the files with findings but no criticals**, and it hit exactly that. The consequence is
> the number that has not moved for seven rounds: **the never-opened row still carries 25 criticals**,
> unchanged since it was first computed, because no round has yet been aimed at them.
>
> **The hand-summed tables were right this time.** For the first round in five, recomputation found no
> error in §1 or §2.2 beyond the row additions below — but note that the previous four corrections
> were each found in a row *not* validated the round before, so this is weak evidence, not a
> retirement of §7's argument for a committed generator.
>
> **A parsing bug in this round's own reader is worth recording**, because it failed in the reassuring
> direction and nearly cost a file: trimming the whole `git status --porcelain` blob before splitting
> it eats the leading space of the FIRST line only, shifting a `slice(3)` by one character. It
> silently dropped `components/sim/CameraRig.tsx` (3/0c) from the round-7 touched set and left it in
> the never-opened list — where it appeared in *both* lists at once, which is the only reason it was
> caught. The published figures are from the corrected reader, which asserts the CameraRig path is
> present before it counts anything.

`[corpus]` Re-run again on 2026-08-19 (round-5 gate) over all 24 JSONL files by the header's rule —
read every record, then drop every record whose `scenario` was re-driven in `chunk-redrive.jsonl`
except the re-drive's own rows.
...
history**, and it was recomputed mechanically this round — for each of the 138 suspect files, is it in
`git diff --name-only <sha>~1 <sha>` for any programme commit, or in the round-5 working tree.
**Six files that §2.2 lists as never opened were edited by wave 1 (`ec1f56f`) itself**, with
`--numstat` of 335, 303, 202, 182, 156 and 12 added lines — real imports and real constants, not
formatting: `lesson-ui/LessonPlayShell.tsx` (71/0c), `components/RouteGuidance.tsx` (23/0c),
`hud/notifyColumn.ts` (7/0c), `hud/overlayQueue.ts` (5/0c), `scene/guidanceRoute.ts` (2/1c) and
`runtime/laneArrows.ts` (1/1c). **These are exactly the six the round-3 correction marked `[+]` and
moved INTO the never-opened list** — and the wave-1 row of the table below, 30 files / 623 / 216,
already counted them as opened, so the table contradicted itself. The `[+]` correction was right that
the six had been dropped from the repair queue and wrong about why: **they were opened and then
forgotten, which is a worse failure than never being opened, because the first is invisible to a
coverage count.** §2.2's own numbers are corrected in place.
...
| round 8 · the 8 lanes (commit `25c2143`) | 15 | 47 | 15 |
| round 9 · the 8 lanes (**working tree, UNCOMMITTED**) | 9 | 127 | 57 |
| **union — ever opened (a real file, really edited)** | **89** | **916** | **304** |
| **never opened at all** | **41** | **59** | **7** |
| **names no file — unopenable as filed** | **8** | **37** | **7** |

**THE ROUND-7 ROW IS NOW A COMMIT** *(and so, as of round 9, is round 8’s — see the round-9 block at the top of this section).* It read "working tree, UNCOMMITTED" when this section was last
...
| file | findings | critical | |
|---|---:|---:|---|
| `scene/lessonSpeedContract.ts` | 22 | 0 | **+** |
| `lessons/advisor.ts` | 12 | 4 | already open |
| `hud/HudToasts.tsx` | 9 | 0 | **+** |
| `components/sim/lesson-ui/TeachMomentOverlay.tsx` | 8 | 0 | **+** |
| `world/builders/markings.ts` | 6 | 1 | already open |
| `traffic/staged.ts` | 5 | 5 | already open |
| `world/components/worldLabel.ts` | 5 | 0 | **+** |
| `components/sim/lesson-ui/MistakeConsequenceOverlay.tsx` | 4 | 0 | **+** |
| `components/sim/CameraRig.tsx` | 3 | 0 | **+** |
| `cockpit/clusterLayout.ts` | 3 | 0 | **+** |
| `orchestrator/contact.ts` | 2 | 0 | already open |
| `lessons/scenario/compile.ts` | 1 | 0 | **+** |
| `orchestrator/runners.ts` | 1 | 0 | **+** |
| **total** | **81** | **10** | **9 new** |

The nine new files sum to **56 findings and 0 criticals**, which is exactly the movement between the
two rows above (807 → 863, 169 → 113, criticals unmoved at 287 / 25).
...
opened* counts sitting in a column of *touched* counts, which is why the table's own rows could not be
added up. Waves 1–3 and round 2 reproduce their published figures exactly (30/623/216 · 5/174/109 ·
44/581/225 · 3/7/4 · 6/123/60), so the recomputation is validated against the five rows that were
already right.
...
that cross two files*, i.e. to spending §2.6's routing debt, and it is the first round to close rows
older than itself: **six of them** (O13/O19, O17, O20, O21, O23, O25). Its nine corpus files —
`lessons/objectives.ts` (51/32c), `lesson-ui/LessonPlayShell.tsx` (71/0c),
`components/sim/LessonScene.tsx` (40/6c), `lessons/finish.ts` (22/13c),
`lessons/scenario/templates-conditions.ts` (11/3c), `collision/probe.ts` (8/4c),
`collision/index.ts` (6/4c), `tools/mobile/lib/auth.mjs` (5/1c) and
`lessons/scenario/templates-signals2.ts` (2/1c) — carry **216 findings, 64 critical**, the largest
single-round bucket the programme has pointed a lane at.
...
**Round 4 opened NO new file, and that is the wrong axis to judge it on.** `[corpus]` Its five
corpus-known files — `lessons/objectives.ts` (51/32c), `lessons/finish.ts` (22/13c),
`tools/mobile/lesson-audit.mjs` (13/10c), `hud/SessionEndScreen.tsx` (22/3c) and
`environment/weather.ts` (6/4c) — carry **114 findings, 62 critical between them**, and every one of
the five had been *edited* by an earlier wave. What round 4 did was land the two **lanes** that had
never run, which is a different thing from opening a file and is the distinction the next block
...
had opened 31 files, that was the single largest jump the programme had made. **Round 2 added 6 more
files** — a smaller jump, but it reached into the part of the queue that had resisted three waves.

**Round 2 landed five of the lanes the 529 outage killed.** `[read]` `.audit-frames/lanes-crit.json`
holds one lane per suspect file carrying a critical finding — 83 of them. Before round 2, **35 of the
83 had never been opened by any wave**, carrying 100 findings and 44 critical. Round 2 opened five:

| lane the outage killed, opened in round 2 | BROKEN | critical | where |
|---|---:|---:|---|
| `environment/weather.ts` | 6 | 4 | working tree |
| `collision/index.ts` | 6 | 4 | working tree |
| `scenarios/coach.ts` | 8 | 3 | working tree |
| `world/builders/network.ts` | 1 | 1 | `7a0ab52` — **the file §0.2 records the wave-3 gate dying inside** |
| `traces/scHzAccidentScene.ts` | 1 | 1 | `7a0ab52` + `4dd1850` |

**CORRECTED 2026-08-19 (round 4) — this block was two measures printed as one, and it did not add
up.** It read *“30 of the 83 critical lanes still have never landed, carrying 78 findings, 31
critical”* and then listed `objectives.ts` (51/32) and `finish.ts` (22/13) as its two deepest members.
**Those two rows alone carry 45 criticals against a stated total of 31**, which is impossible, and
neither file was ever a member of that 30: both had been edited by waves 1–2, as the paragraph
underneath said in as many words. The 30/78/31 figure counts **files never edited**; the two rows
beneath it count **lanes never run**. Both measures are worth having and they are now kept apart.

**Measure 1 — critical lanes whose FILE has never been edited.** `[corpus]` **29 of the 83**,
carrying **76 findings, 30 critical**. Round 4 did not move this number, because all five of its
corpus files were already open. The deepest that remain: `modules/sim/rules` (5/2c — names no file,
§1 row 3), `lesson-ui/LessonCard.tsx` (12/1c), `lesson-ui/ExamBriefingCard.tsx` (5/1c),
`scene/lessonWorldRecipe.ts` (5/1c), `lessons/specs.ts` (4/1c).

**Measure 2 — critical lanes that have never RUN, whatever the file's edit history.** This is where
round 4 spent itself, and it closed the programme's largest single omission:

| the two deepest lanes that had never run | BROKEN | critical | round 4 |
|---|---:|---:|---|
| `lessons/objectives.ts` | 51 | 32 | **LANDED** — working tree, + `reach-zone-witness.test.ts` (60 `expect(`s) |
| `lessons/finish.ts` | 22 | 13 | **LANDED** — working tree, + `finish-work-site-band.test.ts` (36 `expect(`s) |

Between them **73 findings and 45 criticals** finally had a lane pointed at them — the pair that
waves 1–3 and rounds 2–3 all failed to run. `objectives.ts` is *the file where a green tick is
minted*, so its 32 criticals are the product's core failure mode stated once: a student certified for
a skill nobody measured. **Landing a lane is not closing its findings** (§2.6 O20–O23 record what
these two could not reach), but it is the first time either has been attempted.

**Coverage of 57 files is still not progress toward 138 at a constant rate: the 74 never opened are
the ones nobody has looked at six passes running**, and they are now the *shallow* end — 278
findings but only 26 critical, against 698/286 in the 57 that have been opened. The depth is where
the programme has been; the breadth is what is left.
...
> spent; what is left of it is 20 files carrying 25 findings, 18 of the 20 carrying one finding each.
>
> **THE SEVEN CRITICALS ARE THE SAME SEVEN, WORD FOR WORD, AS LAST ROUND**: `hud/PreDriveTutorial.tsx`
> 3/1c, `scenarios/event-library.json` 1/1c, `world/builders/railTrack.ts` 1/1c,
> `lessons/scenario/templates-pk.ts` 1/1c, `world/builders/roundabout.ts` 1/1c,
> `lessons/scenario/templates-junctions2.ts` 1/1c and `public/world/ov-oneway-v1.json` 1/1c. Two of the
> seven (`railTrack.ts`, `roundabout.ts`) were **refuted on evidence** by a round-9 lane and are settled
> without being edited — §0.9.1 has both measurements — so of the 7 standing criticals here, **2 are
> answered and 5 have still never been looked at.** That count has not moved in two rounds.
>
> **THIRTEEN OF THE TWENTY WERE ASSIGNED TO A ROUND-10 LANE AND NEVER EDITED.** `GlanceEdgePings.tsx`,
> `HeroCarBody.tsx`, `cockpit/index.ts`, `cityModels.ts`, `obstacleSpec.ts`, `ribbonStrip.ts`,
> `scMergeLaneEnd.ts`, `scMergeMotorwayExit.ts`, `traffic/index.ts`, `progression.ts`,
> `briefingOverflow.test.tsx`, `ov-crossing-v1.json`, `mw-v1.json`. Not one lane recorded a reason for
> leaving them, the way round 9's lane recorded refutations for `railTrack.ts` and `roundabout.ts`.
> **An unexplained non-edit and a refutation are indistinguishable in every artefact except the lane's
> own prose, and thirteen files here have no prose.** They are the next round's cheapest work: all
> thirteen are already scoped, already routed, and carry 34 findings between them.
>
> **`public/world/ov-oneway-v1.json` and `scenarios/event-library.json` remain data, not code**, have
> sat in this row since wave 1, and have still never been assigned to anyone. Third round running.

> **UPDATED 2026-08-19 (round-9 gate): 45 → 41 files, 73 → 59 findings, and 11 → 7 critical.** Round 9
> opened **four** of them, and every one carried a critical — they are the four heaviest of the eleven
> the round-8 block lists by name: `scene/lessonWorldRecipe.ts` (5/1c), `lessons/specs.ts` (4/1c),
> `lessons/scenario/templates-flow.ts` (3/1c) and `engine/reverseAssist.ts` (2/1c). **Two consecutive
> rounds have now been aimed at this row's criticals rather than its bulk, and between them they have
> taken it 25 → 11 → 7.**
>
> **The seven that remain**, and there is no ordering left to do because six of them carry exactly one
> finding each: `hud/PreDriveTutorial.tsx` 3/1c, `scenarios/event-library.json` 1/1c,
> `world/builders/railTrack.ts` 1/1c, `lessons/scenario/templates-pk.ts` 1/1c,
> `world/builders/roundabout.ts` 1/1c, `lessons/scenario/templates-junctions2.ts` 1/1c and
> `public/world/ov-oneway-v1.json` 1/1c. **Two of the seven have already been adjudicated and this row
> cannot show it.** `world/builders/railTrack.ts` and `world/builders/roundabout.ts` were both handed to
> a round-9 lane, and both findings were **refuted on evidence** — the island stops the car (the drive's
> own debrief bills a collision), and the level crossing IS drawn (a pixel scan of the shipped still
> resolves sleepers at `SLEEPER_SPACING_M`). §0.9.1 has both measurements. Neither file was edited,
> because there was nothing in either to fix, so both stay in this row and in its critical count. **A
> coverage count measures who was opened, not who was answered**, and the gap between those two is now
> large enough to name: of the 7 standing criticals here, **2 are settled, 5 have never been looked at.**
>
> **`public/world/ov-oneway-v1.json` and `scenarios/event-library.json` are data, not code**, and both
> have sat in this row since wave 1. They are the two entries most likely to be closable in an hour by
> whoever reads them next, and neither has ever been assigned.
...
> `lesson-ui/TeachMomentOverlay.tsx` (8/0c), `world/components/worldLabel.ts` (5/0c),
> `lesson-ui/MistakeConsequenceOverlay.tsx` (4/0c), `components/sim/CameraRig.tsx` (3/0c),
> `cockpit/clusterLayout.ts` (3/0c), `lessons/scenario/compile.ts` (1/0c) and
> `orchestrator/runners.ts` (1/0c) — the first movement in this row for three rounds, and the largest
> in the programme.
>
> **THE CRITICAL COUNT DID NOT MOVE, AND THAT IS THE ONLY NUMBER IN THIS SECTION WORTH READING.**
> All nine carry **zero criticals**; all 56 findings they take with them are major or minor. **25
> criticals across 59 never-opened files have now stood unchanged for seven rounds** — every round so
> far has been aimed at files that were already open, or, this round, at Wave B's non-critical bucket.
> The heaviest of them are listed below and none has ever been assigned: `lesson-ui/LessonCard.tsx`
> 12/1c, `lesson-ui/ExamBriefingCard.tsx` 5/1c, `scene/lessonWorldRecipe.ts` 5/1c,
> `templates-parking2.ts` 4/1c, `templates-conditions2.ts` 4/1c, `lessons/specs.ts` 4/1c,
> `hud/PreDriveTutorial.tsx` 3/1c, `templates-flow.ts` 3/1c, `templates-junctions3.ts` 3/1c.
>
> **And §2.6 O34 puts a caveat under every number in this section:** one round-7 lane measured its own
> assigned bucket and found **14 of 18 findings routed at files that cannot contain them**. Where a
> suspect file was inferred from a finding's title rather than read off its frame, these per-file
> counts are softer than they look.

> **CORRECTED AGAIN 2026-08-19 (round-5 gate): 74 → 68 files, 278 → 169 findings, 26 → 25 critical.**
> The round-3 correction below was right that six files had been dropped out of the repair queue and
> **wrong that they had never been opened.** All six are exactly the ones it marks **`[+]`** —
> `lesson-ui/LessonPlayShell.tsx` 71 · `components/RouteGuidance.tsx` 23 · `hud/notifyColumn.ts` 7 ·
> `hud/overlayQueue.ts` 5 · `scene/guidanceRoute.ts` 2/1c · `lesson-ui/briefingOverflow.test.tsx` 1 —
> and every one was edited by **wave 1 (`ec1f56f`)**, by 12–335 added lines each, with wave 1's own row
> in §1 counting them the whole time. They move to the *opened* row and are struck through below.
> **`runtime/laneArrows.ts` 1/1c is NOT one of them** — it was bolded in the same correction but git
> shows no commit has ever touched it, so it stays. **They remain the most dangerous entries in this document**, because a
> file that was opened once and then forgotten cannot be found again by a coverage count: it is
> `LessonPlayShell.tsx` 71/0c and `RouteGuidance.tsx` 23/0c, the first and second largest buckets in
> the whole corpus, and 94 of the 109 findings that left this class in this correction are theirs.
> **Round 5 opened `LessonPlayShell.tsx` for the second time in the programme** (the clip-affordances
> lane, + `shellClipAffordances.test.ts`, 63 `expect(`s), which is the only reason it is now a file
...
**With criticals (25 files, 63 findings, 25 critical) — this is the repair queue:**
...
`lessons/scenario/templates-speed2.ts` 2/1c · ~~`scene/guidanceRoute.ts` 2/1c~~ **(opened by wave 1 — moved out)** ·
...
**No criticals (43 files, 106 findings):**

~~`components/lesson-ui/LessonPlayShell.tsx` 71~~ **(wave 1, re-opened round 5 — moved out)** · ~~`components/RouteGuidance.tsx` 23~~ **(opened by wave 1 — moved out)** ·
`scene/lessonSpeedContract.ts` 22 · `hud/HudToasts.tsx` 9 ·
`components/lesson-ui/TeachMomentOverlay.tsx` 8 · ~~`hud/notifyColumn.ts` 7~~ **(opened by wave 1 — moved out)** ·
`components/lesson-ui/AdvisorCard.tsx` 6 · ~~`hud/overlayQueue.ts` 5~~ **(opened by wave 1 — moved out)** ·
...
`components/HeroCarBody.tsx` 2 · `hud/tapActivation.ts` 2 · and 26 files carrying one finding each —
...
~~`components/lesson-ui/briefingOverflow.test.tsx`~~ **(opened by wave 1 — moved out)** ·
...
`traces/scHzAccidentScene.ts` (1/1c) and `orchestrator/contact.ts` (2/0c). Three rows still deserve
lifting out, because a reader will otherwise scan past them:

- **`world/builders/network.ts` is off this list** — the previous pass lifted it out as *"1 finding, 1
  critical, never opened, and it is the file the gate now dies in"*. `7a0ab52` opened it and the three
  crashes it caused are green. That callout is now discharged; it is recorded here because a
  disappearing warning is indistinguishable from a forgotten one.
...
O36 the same way. Three rows are now closed out of forty-eight, and all three closed because a lane
DISPROVED something rather than repaired it.** Four of wave 3's are
...
of them states. Round 2's three are prose, and all three share one shape: **the fix is one line in a
file the lane did not own.** Each names that file and line.
...
| O11 | **Weather still leaks between lessons on the LIVE path**, and the first frame of every weather lesson is a CLEAR-weather frame that `SkyDome`/`SimEnvironment` snap their damped rig onto. The lane built `primeWeather()` — the one-step prime that both dev routes had been hand-rolling — but could not put it on the live path. A dry lesson opened after a rain lesson renders a wet road for ~12 s; after a snow lesson, a snow haze for ~8 s | verbatim: *"ROUTED OUT, NOT FIXED HERE: the LIVE path still opens clear and still leaks, because the scene boundary is SimEnvironment's mount effect — one line, `primeWeather` instead of `setWeatherTarget`, in a file this lane does not own (`SimEnvironment.tsx:191`)."* | `environment/weather.ts:185` → `environment/SimEnvironment.tsx:191` |
...
**Round 4's rows, O20–O26.** `[read]` Same rule: each was written into the tree by the lane that
could not close it, and read out of the file named today. **Six of the seven are again “one edit in a
file this lane does not own”** — the shape that has now produced **fifteen of the twenty-six rows in
this table** and is, on the evidence of four rounds, the dominant failure mode of single-file lane
ownership. It is no longer a per-round observation; it is the programme's structural cost, and the
remedy is a lane whose `owns` list is the *fix*, not the file.

| # | finding the lane could not close | reason the lane gave | where |
|---|---|---|---|
| O20 | **Two adverse-conditions gates certify lamps their banner never names** — `sc-ac-fog/sc-acf-zone` and `sc-ac-snow/sc-acs-approach`. Both photographed with the telltales dim throughout and the gate ticked 3/3 | verbatim: *“ROUTED, NOT CLOSED — two rows of the same class this file cannot reach … Both are `templates-conditions.ts`, which this lane does not own: · `sc-ac-fog/sc-acf-zone` … Instruction 1 makes BOTH the dipped beams and the fog lamps a precondition of moving off … and the sweep photographed both telltales dim from arrival through t101s with the gate ticked at 1:56 and 3/3 stars. The banner names only a speed, so no derivation can bind it: add `requireLamps: "fog"` to that objective's params. · `sc-ac-snow/sc-acs-approach` — «Приближи със зимна скорост», same shape, СВЕТЛИНИ dim throughout: add `requireLamps: "low"`. Both are one key in one object.”* | `lessons/__tests__/reach-zone-witness.test.ts:591–604` → `templates-conditions.ts` |
| O21 | **Two gates certify that the student read the OFFICER rather than the lamp, and nothing grades the distinction** — `sc-sig-controller-live/sc-sctl-read` and `sc-sig-controller-postures/sc-sctp-read` | verbatim: *“«✓ Приближи бавно и прочети регулировчика, не лампата 1:16» beside its own evidence line … and a −10 «Неизпълнение на сигнала на регулировчика». **The cap of 20 grades «бавно»; nothing grades «не лампата».**”* and, on the second, *“its own source comment says the quiet part, „slow enough to have actually LOOKED at the officer, not read a (dark) lamp" — **slow enough is not looked**.”* Both `templates-signals2.ts` | `lessons/__tests__/stop-claim-gates.test.ts` `READ_CLAIM_KNOWN_OPEN` |
| O22 | **Eleven stranded lanes cannot be ended from this module at all.** Every stranded drive in the sweep oscillates 0–20 км/ч, so no standstill bar at any duration is ever met | verbatim: *“A GLOBAL IDLE bound would close none of them … What ends those drives is a SESSION-level budget or an off-network test (`SimTick.edgeId === null` is the existing channel for „off-road/unknown"), and both need an arm in `engine.ts`, which is not this lane's file. Adding an unmeasured duration cap here instead would end drives on a timer regardless of what the student is doing — **a false refusal manufactured to answer a missing ending, which is the trade this module exists to refuse.**”* | `lessons/finish.ts:104–112` → `rules/engine.ts` |
| O23 | **The roundabout finish hands back up to 5 m of ring to “no automatic ending”** | verbatim: *“Closing it properly needs the zone to CARRY its work-site bound (a `RouteFinishZone` field, i.e. `lessons/types.ts`), which is not this lane's file; **routed rather than guessed at**.”* The lane states the cost rather than hiding it: a widening of a hole B1 already rules open, not a new class | `lessons/finish.ts:1252–1262` → `lessons/types.ts` |
| O24 | **THE SNOW ROAD MAPPING IS BUILT AND NOTHING READS IT** — and this is the round's one new gate failure (§0.4), red on purpose | verbatim: *“IT CHANGES NO PIXEL UNTIL ITS READER PASSES THE VALUE, and that reader is not this lane's file. **So this finding is NOT closed** — and the state in between is the dangerous one, because it reads like a fix. Measured 2026-08-19: `index.ts` exports `wetnessToRoadParams` … and NOT `roadSurfaceToParams`; `StaticWorld.tsx` imports `{ useWetness, wetnessToRoadParams }` and calls the rain-only mapping at :279, :294 and :313. **By §4's own rule that is a DEAD API** — no author reaches it, no reader calls it — and by doc 05 it does not exist at all.”* | `environment/weather.ts:79–96` → `environment/index.ts` + `world/components/StaticWorld.tsx` |
| O25 | **A node runtime abort can discard four healthy audit lanes, one of which carried a real finding** | verbatim: *“It is NOT fixed here, because nothing in this file causes it and the two remedies that work — `process.exitCode` with no `exit()`, or not using global fetch — belong to the sign-in path (`lib/auth.mjs` `warmFromNode`) and to the dispatcher … What this file can do is say so, and keep writing the truth into the status file, which no abort can rewrite: **READ `exit` OUT OF `_audit-status.json`, and treat a process code that disagrees with it as evidence about node, not about the lesson.**”* | `tools/mobile/lesson-audit.mjs:1885–1902` → `tools/mobile/lib/auth.mjs` + the dispatcher |
| O26 | **Neither half of the runner partition can see the command line** — a `--exclude` or `--dir` at invocation would deselect files with no gate able to look | verbatim: *“NEITHER HALF SEES THE COMMAND LINE. Both read the config file; a `--exclude` or `--dir` passed at invocation would deselect files without either gate having anything to look at. **Checked rather than assumed**: `.github/workflows/ci.yml` runs the bare `npx vitest run` and `npm run test:tools`, no selection flags on either, so **nothing today exploits it**. That file is not this lane's to change — if a flag is ever added there, this comment is the reason it needs a check of its own.”* | `platform/scripts/__tests__/test-ownership.test.mjs:98–104` → `.github/workflows/ci.yml` |

**O20 and O21 are the same class as O2/O3, found in a file that had never been opened** — a gate
certifying something no disc can witness. The `objectives.ts` lane was briefed to *judge that claim
yourself; do not inherit it*, and it did: rather than accept “unclosable”, it made the demand
structural — the gate derives what it must measure from the banner the student reads, so **a promise
cannot be written that nothing enforces** — and then filed only the two rows that derivation
genuinely cannot bind, because their banners name a speed while their briefings name the lamps.
That is the difference between a `KNOWN_OPEN` row used as a debt and one used as a permission.

**And every round-4 lane filed at least one row.** After §0.3 recorded that the round-3 lane with the
widest blast radius filed nothing at all, this round has no silent lane — six lanes filed seven rows,
and the two that filed none (`SessionEndScreen`, the partition lane's node half) each shipped a test
that pins what they could not fix. That is the pattern to keep.
...
**And the critical lanes that have still never run reported nothing at all.** Their findings have
neither a fix nor a reason: absence of a report there means the agent was killed or never launched,
not that the finding was examined and left. **Round 2 added three reasons and closed none of the ten
wave-3 rows; round 3 added six more and closed one of its own (O14), which was a hole in round 2's
fix rather than a corpus finding; round 4 added seven and closed none of the nineteen before it** —
O1–O10 stand exactly as written, O11/O13 are carried forward as O16/O19, and O16 (weather leaking on
the live path) is now joined by O24 in the same file, which is the second time `weather.ts` has
shipped a mechanism whose reader lives elsewhere.

**Nineteen rows became twenty-six and not one older row closed.** That was the honest reading of four
rounds: the programme is very good at *finding and naming* what a lane cannot reach, and had not yet
built the thing that spends those rows — a lane scoped to a fix that crosses two files. Fifteen of
the twenty-six are one edit each in a named file at a named line.

**Round 5 was that thing, and it worked.** `[read]` Round 5's lanes were scoped to *fixes* rather than
to files — each `owns` list was two or three files chosen to contain one crossing — and it is the
first round in the programme to close rows older than itself: **six of them.** Verified in the tree by
the round-5 gate, not taken from a lane's report:

| # | closed by round 5 | the crossing it had to make | evidence read in the tree today |
|---|---|---|---|
| O13 / O19 | NPC cars and staged pedestrians walked at a fifth of the ego car's pace below 10 fps — filed twice, carried through **three** rounds unchanged | `LessonScene.tsx` (round 2's lane) → `traffic/system.ts` (nobody's) | the clamp is gone: `MAX_DT_SEC = 0.1` is now `MAX_SUBSTEP_SEC = 0.1` with `steps = ceil(frameDt / MAX_SUBSTEP_SEC)` (`traffic/system.ts:82,161`), exported as `TRAFFIC_MAX_SUBSTEP_SEC` so the caller can assert the bound, and pinned by the new `traffic/__tests__/substep.test.ts` (15 tests, 27 `expect(`s) which asserts `dt <= MAX_SUBSTEP_SEC` **always** and runs the mutation that removes the sub-stepping. `LessonScene.tsx:3014` now records the close instead of the debt |
| O17 | the collision margin was overstated **at its origin** — the file holding the wrong number was another lane's | `collision/index.ts` → `collision/probe.ts:105` | `probe.ts:120–131` retires the „the margin over reality is the same ~45 %" sentence and states the measurement: *„the overstatement was twelvefold"* |
| O20 | two adverse-conditions gates certified lamps their banner never named (`sc-ac-fog/sc-acf-zone`, `sc-ac-snow/sc-acs-approach`) | `reach-zone-witness.test.ts` → `templates-conditions.ts` | **closed, but NOT by the fix the routing note asked for** — see the note below. Banners now name the lamps; `deriveLampDemand` binds them; the new `scenario/__tests__/conditions-lamp-gates.test.ts` (14 tests, 32 `expect(`s) carries both drives in both directions |
| O21 | two gates certified that the student read the OFFICER rather than the lamp, and nothing graded the distinction | `stop-claim-gates.test.ts` → `templates-signals2.ts` | `READ_CLAIM_KNOWN_OPEN` is now **an empty array** (`stop-claim-gates.test.ts:375`), and its staleness test still stands over it. `HALT_CLAIM_KNOWN_OPEN` (1 row, O1) and `ACTOR_CLAIM_KNOWN_OPEN` (8 rows, O2) are unchanged — the emptying is specific, not a weakening |
| O23 | the roundabout finish handed back up to 5 m of ring to „no automatic ending" | `lessons/finish.ts` → `lessons/types.ts` | `RouteFinishZone.workSiteRadiusM` exists (`types.ts:352`), the three anchors state it (`finish.ts:656,702,729`), and `strandedBeyondM` reads it instead of inferring (`finish.ts:1421`), clamped in both directions. The inference survives as the fallback **so every hand-built zone and recorded session stays bit-identical** |
| O25 | a node runtime abort could discard four healthy audit lanes, one carrying a real collision finding | `lesson-audit.mjs` → `lib/auth.mjs` + the dispatcher | `auth.mjs` no longer touches global `fetch`: `import http from "node:http"` (:100), an `httpGet` with `agent: false` (:135), and a header block titled *„AND THIS FILE'S HTTP IS NOT GLOBAL `fetch`, BECAUSE OF O25"* (:80) carrying the timing table. Pinned by two new node:test files — `lib/__tests__/exit-integrity.test.mjs` and `__tests__/lane-evidence.test.mjs` (32 + 46 assertions) |

**O24 also closed, in round 4's own commit `ed2dd6f`** — `environment/index.ts:146` now exports
`roadSurfaceToParams` and `world/components/StaticWorld.tsx` calls it at :291 and :310, so the snow
mapping has a reader and is no longer a dead API. That was the round-4 gate's one declared-red row.

**THE MOST USEFUL THING ROUND 5 PRODUCED IS A REFUTED ROUTING NOTE, and it is a warning about this
table.** O20's reason, filed by round 4, asked for one specific edit: *„add `requireLamps: \"fog\"` to
that objective's params … Both are one key in one object."* The round-5 lane checked it before doing it
and it **was never compilable** — verbatim from `conditions-lamp-gates.test.ts`: *"`ScenarioObjectiveSpec.params` is the real `ObjectiveParams` union, `ReachZoneParams` lives in `lessons/types.ts`, and a `requireLamps` key there does not compile. (The routing note that opened this row asked for exactly that key. It was never compilable — which is why a lane inheriting a premise has to re-measure it.)"* **A row in this table is a lane's best account of a fix it did not
make, and is therefore evidence about the finding and a hypothesis about the remedy.** The remedy that
shipped is stronger than the one that was routed: the demand is derived from the banner, so *a gate may
refuse only for something the banner said*, which no authored key can guarantee.

**AND ONE OLDER ROW WAS PARTLY REFUTED RATHER THAN CLOSED — O22.** `[read]` Its „eleven stranded
lanes" premise does not survive re-measurement, and the round-5 lane re-measured it **before**
changing anything (`finish.ts:115–145`). Two of the three drives O22 names are not this module's
defect: **the sweep's own driver cannot meet any bar in the file** — `lesson-audit.mjs` stops for
`STOP_MS 3000`, against standstill faces of 12 s, 25 s and 75 s, *„so NO drive it produces can ever
trip one — not because the gate is missing but because the driver never stands still"* — and its
`DRIVE_BUDGET_MS` of 210 s is shorter than `sc-merge-accel-lane`'s 930 m route needs at 12 км/ч.
Measured over the whole sweep instead of over three lanes: of 166 scenarios with a machine summary,
**8 had no lane end by itself and 56 more ended on some lanes and not others**, and three of the eight
end on a bay whose only escape is 25 s motionless in it. **`ended: false` in that corpus is therefore
not by itself evidence that a lesson has no ending, and O22's eleven-lesson framing rested on it.**
What survives is **one class, not eleven lessons: a car that is no longer in the authored world**
(`sc-junction-blind` pc-right, off the map after the impact, 146 s more of session with the task chip
still asking for a left turn). It stays open, with a sharper remedy and the same routing:
`SimTick.edgeId === null` held continuously, armed in `lessons/engine.ts`, *„and it must bring its own
sentence"* — both endings `engine.ts` can speak today say «край на маршрута», which is false of a car
that drove off the map, and **a wrong reason is a bare verdict wearing a sentence, which THEO-4
forbids as squarely as no reason at all.**

**Round 5's own rows, O27–O28.** Two, both from the same lane pair, both written into the tree by the
lane that could not close them. **This is the first round whose new-row count (2) is smaller than its
closed-row count (6).**

| # | finding the lane could not close | reason the lane gave | where |
|---|---|---|---|
| O27 | **The work site's INTERIOR still has no automatic ending, and on `passSignal` that interior is a 40 m disc.** O23's close moved the *outer* edge onto authored data; the inner edge is B1's ruling and unchanged. It costs nothing on a roundabout (a 24 m ring the student is working) or a turn box (the corridor he is turning in), but `passSignal` arms on the objective's **grading** radius rather than on a work site | verbatim: *“WHAT IS STILL OPEN, AND IS NOT THIS BAND'S TO CLOSE. The work site's INTERIOR has no automatic ending either, and never had one — that is B1's ruling and its own test. … `passSignal` arms on the objective's GRADING radius rather than on a work site, so on `sc-sig-green-wave` the ending-free disc is 40 m in every direction from the node — measured above: a car resting 0–40 m past tl3 still runs out the capture with the drive live. Closing that means giving the `passSignal` anchor an arm that describes the junction instead of the acceptance ring, which changes when drives ARM and is a founder-visible call, not a widening.”* | `lessons/finish.ts:437–448` and `:1412–1418` → a founder ruling, not a file |
| O28 | **`rules/engine.ts` arms a lamp detector for rain and one for fog and NONE for snow**, so on `sc-ac-snow` the order «Включи късите светлини» was checked by nothing anywhere in the product — the lamps-off drive finished `passed`, `completedAll`, score 0, three stars | verbatim: *“The rule engine has a lamp detector for rain (`raining && headlights === \"off\"` → HEADLIGHTS\_OFF\_IN\_RAIN) and one for fog (`foggy && fogLightsOn !== true` → FOG\_LIGHTS\_OFF\_IN\_FOG). It has NONE armed by `tick.snow`. … That missing detector is a rules/engine.ts row and is routed, not closed here.”* The lane closed the gate half so the order is graded somewhere today, and says so: *“this gate is what makes the order real today”* | `scenario/__tests__/conditions-lamp-gates.test.ts:30–37`, `templates-conditions.ts:905–911`, `reach-zone-witness.test.ts:638–641` → `rules/engine.ts` |

**O28 is how a closed row should be filed: the lane fixed the half it owned and named the half it did
not, in the same breath.** It is also the third finding in this document whose shape is *the product
never measured the thing it ticked*, found by asking what grades an order rather than what the frames
show — the method §2.6 has been converging on since O2.

**Twenty-six rows became twenty-eight, and six of the twenty-six closed.** Of the eighteen still open:
O1–O10 (wave 3) stand exactly as written and have now survived five rounds; O12 and O16 are round 2's,
O15/O18/O26 round 3's, O22 round 4's — narrowed above — and O27/O28 are new. **The structural lesson
...
**Round 7's rows, O31–O34.** `[read]` Every reason below is **quoted verbatim** from the tree, read out
of the file named, today. **Three of the four are again "one edit in a file this lane does not own"** —
the shape that has now produced **twenty of the thirty-four rows in this table.** Seven rounds have not
dented it, and it remains the dominant failure mode of single-file lane ownership.
...
**O31 and O32 are the two that matter, and they are both grading-truth rows, not polish.** O32 is the
more serious of the pair: **it convicts a student for a marking that was never drawn**, which is the
one failure mode this whole programme exists to find — a green tick for a skill nothing measured, run
in reverse. Neither is closable inside the lane that found it.

**O34 is a new kind of row and deserves its own note: it is a ROUTING defect, not a code defect.**
Fourteen findings were filed against files that cannot contain them. Nothing was wrong with the
findings or with the lane; the *routing* was wrong, and it was wrong because the suspect file was
inferred from the finding's title rather than from its frame. That is a defect in how this document
assigns work, and it is the first time a lane has measured it: **78 % of one lane's assigned bucket
...
### Round 10 — six rows, five of them routed at a file the lane did not own

Every row below is a **verbatim** quotation of the lane's own reason, taken from the shipped source
rather than from a lane report, so each is checkable at the path given. Round 10 filed no closure it
could defend to §2.1's standard; what it filed is six statements of what it could not reach.

**O49 — `world/referents.ts` · the `lightsTelltale` evidence channel certifies itself, and the gate
that says so is RED right now.** `[gated]` The scene-publish lane moved `dash.headlightsRequired` out
of `LessonScene.tsx` into `hud/dashboardStatus.ts`, and `world/__tests__/referent-evidence-reachable.test.ts`
went red on four referents. The lane wrote, at `dashboardStatus.ts`:

> *"⚠ MOVING THIS LINE OFF `LessonScene.tsx` TURNED A GATE RED, AND THE GATE IS RIGHT. …
> `referents.ts lightsTelltale` lists `headlightsRequired` with LessonScene.tsx / TelltaleEdgePings.tsx
> / StatusDashboard.tsx. It only ever passed because THIS ASSIGNMENT lived in the scene — a WRITE being
> read as proof that a student can see the field. Nothing renders `headlightsRequired`; nothing ever
> did. Measured: with HEAD's scene the gate is 10/10 green, with the extraction it fails on this symbol
> alone. The repair is in `world/referents.ts` (not this lane): point `lightsTelltale` at what the
> student actually perceives — the row `armedTelltaleWarnings` raises and the lamp on the bar — and read
> `DashboardStatus.conditions`, not this bit. Do NOT re-introduce the identifier into a component to
> quiet the grep; that is the false certificate this whole audit exists to remove."*

**This is the third consecutive round in which a note's own hedge turned out to be the defect**, and
the sharpest of the three: the gate did not break, it **stopped being fooled**. `HEADLIGHTS_OFF_AT_NIGHT`,
`HEADLIGHTS_OFF_IN_RAIN`, `FOG_LIGHTS_OFF_IN_FOG` and `HIGH_BEAM_NOT_DIPPED` have been convicting
students on evidence no student can see, for as long as the channel has existed. See §4 N-R10.

**O50 — `lessons/scenario/templates-exam.ts` · a one-line data fix is correct, measured, and withheld
because landing it alone would leave a census stale.**

> *"ADDING `maxspeedKmh: 50` HERE WAS TRIED AND MEASURED. No gate moves … Eleven of the twelve
> assertions in `scenario/__tests__/b58-gate-never-over-posted.test.ts` stay green … The twelfth is a
> census that must be RESTATED, not relaxed: the survey grows from 502 cards to 510 … That file belongs
> to another lane, so the change is left here rather than made half-way. Apply both together."*

**O51 — `components/sim/lesson-ui/LessonPlayShell.tsx` · the objective's own speed cap is published on
`StatusDashboard` and nothing on the glass moves until the shell passes it.**

> *"⚠ THE OBJECTIVE'S OWN SPEED DEMAND (km/h) — DECLARED HERE, NOT YET THREADED. … `LessonPlayShell.tsx`
> mounts this bar twice (~line 4489 compact, ~4501 roomy) with `limitKmh={snap.limitKmh}` and nothing
> else, so this arrives `undefined` … **That file is not this lane's**, so the change is published,
> asserted and routed rather than claimed."*

The tripwire it promises is real and **it goes red the moment the thread lands** — but it is not where
the comment says it is. See §7 B-R10.

**O52 — `hud/dashboardStatus.ts` · `dashboardHash` re-commits the whole bar 18.7× per visible change,
for a digit the cockpit camera hides.**

> *"WHY IT IS NOT FIXED HERE, stated rather than left as a silent trade. The two candidate fixes are
> both out of this lane or already refused: (a) coarsen `dashboardHash` to the DERIVED pair — one line,
> but the pair needs `limitKmh`, which `dashboardHash(s)` is not given, so it is a signature change in
> `hud/dashboardStatus.ts`, **not this lane's**; (b) gate the poll on the live camera — refused at the
> site of the fold itself … ROUTED, not dropped: (a), to `dashboardStatus.ts`, with the 56-vs-3 number
> above as the acceptance measurement."*

**O53 — `lesson-ui/LessonPlayShell.tsx` + `PlayAreaStyles.tsx` · the rear-proximity badge is hidden by
the shadow transport on exactly the family of lessons whose subject is the car behind you.**

> *"NOT FIXED HERE, AND THE REASON IS NOT TIMIDITY. This component cannot see the transport … adding a
> prop nobody passes would repeat the `DebriefContext.coachedMistakes` mistake this same sweep found (a
> channel built, tested and never fed). The shell owns the answer … Routed there: the badge needs to
> lift above 213 px while the transport is up."*

**O54 — `lesson-ui/LessonPlayShell.tsx` · two producers print one instruction twice and neither can see
the other.**

> *"That half is not closable from a pure predicate over one item and is ROUTED, not dropped: the two
> producers must be handed one string, which is a change in `components/sim/lesson-ui/LessonPlayShell.tsx`
> (where both are mounted) — **not this lane's file**. This predicate is what that change would then be
> checkable against."*

The same lane also filed the phone half of the toast-moment stamp — *"⚠ NOT YET SPENT ON THE GLASS. The
two edits that spend it are both outside this lane"* (`LessonPlayShell.tsx`'s re-map and
`hud/SimOverlay.tsx`'s last row) — and `hud/tapActivation.ts` closed its own third and routed the other
two: *"So: not closed here, not closable here, and not open on the shipped tree for the reason the first
bullet gives."*

**FIVE OF THE SIX ROWS ROUTE TO TWO FILES.** `LessonPlayShell.tsx` is named by O51, O53 and O54 and by
the toast-moment note; `dashboardStatus.ts` by O52 and O49's repair. **`LessonPlayShell.tsx` is now the
single heaviest routing debt in this document and no round has ever been aimed at it.** It is the
obvious lane for round 11, and it would spend four rows at once.

**ONE NON-CLOSURE IS A CORRECTION AGAINST A FINDING, NOT A ROUTE.** `hud/SpeedCard.tsx` was handed a
finding it could not have caused: *"⚠ THIS COMPONENT IS NOT MOUNTED ANYWHERE … there is no `<SpeedCard`
in the tree. It survives only as a named export from `hud/index.ts`, i.e. as a public API with no
consumer. … This is §2.6 O34's class — „a component none of its files renders" — and it is recorded
here rather than closed here so the count is not spent on a file that could not have caused it."*
That is the second §2.6 O34-class misrouting found by a lane rather than by a gate.

### Round 9 — one row closed, one row that looks closed and is not, and six new rows

**O36 IS CLOSED, AND CLOSING IT REFUTED ITS OWN LAST SENTENCE.** Round 8 filed O36 with the reassurance
that an anonymous contact report was harmless — *"nothing about the BILL changes, because the sentinel's
own per-body key is unaffected."* Round 9's collider lane deleted `npcShellObb` and both retired
constants, moved `LessonScene.tsx:988` to `actorObb(pose, m.profile)`, and then found the reassurance
was false, in its own words:

> *"THE SECOND HALF OF THAT WAS WRONG. The rapier channel and the sentinel are two reporters on one
> wire, and the rule engine keys an episode on `actorId` when it has one and on `kind:<withWhat>` when
> it does not — so anonymous reports from every large body fell under ONE latch, and two tram bodies
> touched in one pass billed ONE «ПТП» instead of two. A missing name is not free; it merges victims."*

**That is the second consecutive round in which a routing note's own hedge turned out to be the
defect** — round 8 found O31 was *„a true finding with a false mechanism"*, and round 9 found O36 was a
true finding with a false consequence. `[read]` Verified today: `npcShellObb` survives only in comments
and in the tests that pin its history; `actorObb(pose, profile)` is the single sizing answer, used by
`NpcColliders.tsx`, `orchestrator/contact.ts:268` and `LessonScene.tsx:988` alike.

**O35 IS NOT CLOSED, AND IT WOULD BE EASY TO REPORT THAT IT WAS.** The low-beam lane did real
structural work — `headlightDutyCode` now *calls* `rules/engine.ts lowBeamDuty` instead of mirroring it,
and `rules/__tests__/low-beam-duty-one-source.test.ts` drives all sixteen (night, rain, snow, fog)
combinations through the grader and through the HUD module and fails on any disagreement in either
direction. **The product behaviour did not change.** `[read]` Both live call sites still pass no
conditions — `TelltaleEdgePings.tsx:61` (`armedTelltaleWarnings(s)`) and `LessonPlayShell.tsx:2199`
(`armedTelltaleWarnings(dashboardStatusRef.current ?? createDashboardStatus())`) — and
`LessonScene.tsx:2914` still writes `dash.headlightsRequired = isNight || rain`. `telltaleWarnings.ts:242`
is explicit about what that means: `conditions === undefined ? s.headlightsRequired : dutyCode !== null`.
The lane wrote the fact down rather than letting the green suite imply otherwise:

> *"WHAT DID NOT CHANGE, MEASURED RATHER THAN ASSUMED: both live call sites still call
> `armedTelltaleWarnings(s)` with no conditions — `TelltaleEdgePings.tsx:61` and
> `LessonPlayShell.tsx:2199`, read on this date — so on `sc-ac-snow` the engine convicts a dark car and
> this channel is STILL SILENT in the product. The legacy bit cannot see snow no matter what this file
> does: it arrives already flattened."*

**The test pins BOTH behaviours on purpose**, which is why the suite is green either way — and that is
the exact shape this document has to keep flagging: **a green gate is evidence about the code that ran,
not about the code that ships.** O35 stands, unchanged in effect, one round older.

**O42 IS NOT CLOSED EITHER, BUT ITS DIAGNOSIS CHANGED AND THE PATCH IS NOW MEASURED.** The world-builders
lane re-wrote the `expected-failures.json` entry from „a sign in the wrong place" to „a predicate that
cannot ask its own question", and measured the fix rather than proposing it: swapping
`checkSPEED_TOO_FAST_FOR_CURVE` for a driven-path version takes `SPEED_TOO_FAST_FOR_CURVE` from 1/4 to
0/0 and total falsehoods 33 → 29 **with no new falsehood anywhere and T14 still 0**, and the
counter-direction was measured too (demand raised to 70 m convicts 3/14; the А1 removed from the build
convicts 3/14). It also names the trap that ate earlier rounds: `bestLead = fromM - sM` with `fromM = 0`
makes `mwx-e-ramp` **unsatisfiable from every sign placement in the plane**, so „move the sign onto the
ramp" can never work. `[read]` The allowlist itself was **not** loosened — `"rungs": 4` and
`"allowlist": []` are byte-identical; only `owner` and `reason` changed.

**Round 9's rows, O43–O48.** `[read]` Every reason below is **quoted verbatim** from the working tree,
read out of the file named, today. **Five of the six are again „one edit in a file this lane does not
own"** — the shape has now produced **thirty-three of the forty-eight rows in this table**. The sixth,
O47, is a new shape and the more interesting one: *the finding cannot be judged because the instrument
that produced it could not perform the act it is about.*

| # | finding the lane could not close | reason the lane gave, verbatim | where |
|---|---|---|---|
| O43 | **The low-beam violation code is named for one of the three conditions it now bills.** `HEADLIGHTS_OFF_IN_RAIN` is raised for the snowfall arm as well, so the ledger prints a rain citation over a snow drive | *"ROUTED, NOT SILENTLY ACCEPTED: renaming the code to something condition-neutral (`HEADLIGHTS_OFF_IN_LOW_VISIBILITY`) is the honest end state and touches `rules/types.ts` + `catalog.ts` + `n38.ts` + `consequences.ts` + `scenarios/mapping.ts` + `world/referents.ts`, none of which is this lane's."* | `rules/engine.ts:575` → six files |
| O44 | **The cockpit's own timers still run on the raw frame delta after the lesson clock was fixed.** O13/O19's traffic half closed this round — `traffic.update()` now SUBDIVIDES a long frame into 0.1 s steps instead of truncating it (measured at seed 7: 8.825 m of ambient travel against 44.109 m for the same half-second at 60 Hz, and `update(0.5)` was bit-identical to `update(0.1)`) — but the cabin did not follow | *"STILL SPLIT, and not this lane's file: `VehicleRig`'s `cabin.update(delta, …)` runs the blink/wiper/stall timers on the raw, unclamped delta — so on a 3.57 s frame the indicator still blinks seven times what the clock says."* | `components/sim/LessonScene.tsx:3072` → `VehicleRig` |
| O45 | **`l6-night-route`'s banner no longer over-claims, but nothing grades the lamps it stopped claiming.** The banner was retitled to promise only the metres the gate counts; the lights duty is now taught in `descriptionBg` and graded nowhere | *"ROUTED, NOT FIXED HERE, and both halves are named in the lane report: (1) the title-derived demand that made this structural for `reachZone` (`deriveLampDemand`) reaches no other kind, and extending it to `driveDistance` is `objectives.ts`'s, not ours; (2) that rule engine channel was measured SILENT on a night drive (`objectives.ts:649`), so until it is fixed L6 grades the lamps nowhere. Silence is worse than nothing said — it is not worse than a certificate that is false."* | `lessons/specs.ts:545` → `lessons/objectives.ts` |
| O46 | **On every rung that adds a complication the first briefing line is not authored in the template at all**, so the ≤ 95-character discipline this lane imposed cannot reach it: `compile.ts` puts `complicationBriefingText` at `briefingBg[0]`, and that string is **309–509 characters on 61 of the 165 rungs these five files compile** — past the ~96 that zeroes the body, so **L4/L5 briefings still show the student nothing but the complication** | *"WHAT THIS WAVE COULD NOT FIX, stated rather than hidden … The fix belongs in `compile.ts` / `complications.ts`, which this lane does not own."* | `lessons/scenario/templates-flow.ts:76–82` → `lessons/scenario/compile.ts` + `complications.ts` |
| O47 | **The two reverse findings routed at `reverseAssist.ts` are inadmissible, and the lane says so instead of closing them.** `tools/mobile/lesson-audit.mjs` sends exactly three keys in its whole run — `KeyW`, `KeyS`, `Escape` — with no `[` / `]`, no touch gear sheet and no cockpit lever; and its `brake(on, kmh)` helper returns early for any press at kmh ≤ 1, which is precisely the standstill press the assist requires. **Every reverse drill in the sweep is unmeasured rather than broken** | *"That refusal is correct and must not be deleted: the gesture it blocks is the one that drove a car backwards into traffic at 16.8 км/ч. What is missing from the harness is a DELIBERATE reverse gesture beside it — stop, lift for > `REVERSE_ASSIST_LIFT_S`, press — issued only where the lesson asks for R. Until it exists, every reverse drill in the sweep is unmeasured rather than broken, and the four parking-depth rows nobody could pass are unexplained rather than explained by this file."* **The lane judged the INSTRUMENT, not the code, and made the judgement falsifiable**: the harness test carries the mutation showing the same frames DO reach R once the standstill press is allowed through, so *"the claim is about the instrument and not about a dead machine. The day the harness gains a gear key or loosens the guard, that file fails and the two findings become admissible again."* | `engine/reverseAssist.ts:90–117` → `tools/mobile/lesson-audit.mjs` |
| O48 | **Nine findings are parked against `modules/sim/collision`, and it can close none of them** — O18 carried and widened. Reading the corpus for the six already known turned up **three more parked on the bare string `platform/src/modules/sim/collision`, a DIRECTORY with no filename**, which is a second way for a finding to be invisible to a wave that groups by file | *"THEY ARE NOW RE-ROUTED, with the proof for each, in `.audit-frames/routing-collision.json` — nine rows … The refutation that applies to all nine is one grep: this directory imports neither `three` nor `@react-three/rapier` nor `react`, and the barrel below exports only constants, functions that return a number or a boolean, three body builders and one pose-memory class. Nothing here can create a body, move one, draw one or end a lesson."* **This is the first row in this table that ships a machine-readable re-routing artifact** rather than prose — `.audit-frames/routing-collision.json`, 9 rows, each with its frame, its new suspect file and the proof | `collision/index.ts:69–84` → `.audit-frames/routing-collision.json` |

**O48 IS THE ONE TO ACT ON, AND NOT BECAUSE OF ITS SIZE.** Six of the eight directory-shaped entries in
§1's *unopenable as filed* row (`…/rules` 5/2c, `…/collision` 3/1c, `…/vehicle` 3/1c,
`…/world/builders` 1/0c, `…/environment` 1/0c, `…/traffic` 1/0c — 14 findings, 4 critical) are the same
defect as the three O48 found: **a finding whose `suspectFile` names a directory disappears from every
per-file view in this document.** O48 is the first lane to go looking for them on purpose. The other
five directories have never been swept the same way.
...
### The round-10 gate — three red test files, and only one of them is news

`[gated]` `tsc --noEmit` **0** · `validate-content.mjs` **0** · `tools-tests.mjs` **0** ·
`vitest run --maxWorkers=2` **1**. 945 files collected, 14,814 tests, **3 failed**. All four exit codes
read from `$?`.

**THE TWO KNOWN-RED FILES ARE STILL RED AND — THIS IS THE PART THAT MUST BE CHECKED, NOT ASSUMED —
BOTH ACTUALLY RAN.** `exam/__tests__/content-bank.test.ts` collected **13 tests, 1 failed**, printing
the full 16-topic supply table and failing on `REVIEW_DEBT: ptp-i-parva-pomosht: only 31/64 (48%)
approved`. `lesson/__tests__/compose.test.ts` collected **15 tests, 1 failed**, on
`['l-accidents-first-aid']` having no quiz beat. Real tables, real ids, real counts — not the
absent-file reading a damaged `node_modules` once produced, where an alarm keyed on *"are those two
still red?"* answers **yes** about a file that never loaded. **Neither is green**, so no content
`status` field was flipped to buy a green gate.

**N-R10 — `world/__tests__/referent-evidence-reachable.test.ts` is the third failure, and it is a
CROSS-LANE COLLISION that exposes a real defect rather than causing one.** The test file is **clean** —
no round-10 lane owned or edited it. It went red because the scene-publish lane moved
`dash.headlightsRequired` from `LessonScene.tsx:2914` into `hud/dashboardStatus.ts`, and the literal
symbol `headlightsRequired` now appears in **none** of the three components
`referents.ts lightsTelltale` names as `renderedBy`. Measured at `HEAD`: `LessonScene.tsx` = 1
occurrence, `TelltaleEdgePings.tsx` = 0, `StatusDashboard.tsx` = 0 — so the channel was passing on a
**single write statement**, never on anything rendered. Four referents fail:
`HEADLIGHTS_OFF_AT_NIGHT`, `HEADLIGHTS_OFF_IN_RAIN`, `FOG_LIGHTS_OFF_IN_FOG`, `HIGH_BEAM_NOT_DIPPED`.

**Classification, because the three categories look identical from a failure list.** Not a lane's own
new test catching its own defect; not a test file shipped and never run. It is a collision — **and the
correct response is to fix `world/referents.ts`, not to re-introduce the identifier into a component**,
which would quiet the grep and restore the false certificate. Filed as O49 in §2.6; the lane that
caused it wrote the repair instructions before it walked away.

**NOTHING ELSE MOVED.** No test file under `platform/src` lacks an `expect(` (939 checked). No
untracked probe, scratch or `zz-` residue: the 13 untracked files are all test files, all in
`__tests__/`, and running the 13 alone gives **13 passed / 155 tests passed / 0 skipped** — so no lane
shipped a test file it never ran.
...
- **B-R10 — a routing note names the wrong test file, and the tripwire it promises is real.**
  `hud/StatusDashboard.tsx`'s `taskCapKmh` docstring routes the shell edit with *"`governor-cap.test.ts`'s
  „the shell has not threaded the task cap yet" block asserts the CURRENT mounts, so it goes red the
  moment the thread lands."* `[gated]` `hud/__tests__/governor-cap.test.ts` is **unmodified this round
  and contains no `taskCapKmh`, no „task cap" and no „threaded"**. The block exists — in
  `hud/__tests__/governor-speed-contract.test.ts:218`, *"…and the shell has not threaded it yet, which
  is stated rather than implied"* — and it does exactly what is claimed: it reads `LessonPlayShell.tsx`,
  finds both `<StatusDashboard …/>` mounts and asserts `expect(mount).not.toContain("taskCapKmh")`, so
  it inverts the instant O51 lands. **The mechanism is sound and the address is wrong.** Whoever lands
  O51 will grep the named file, find nothing, and reasonably conclude no tripwire was left. Fix the
  citation; do not weaken the test.
- **The source-text assertions in `governor-speed-contract.test.ts` were checked against this repo's own
  ban and pass it.** The repo has been burned by tests asserting over comment-stripped source (§2.1 C5;
  eight such tests once left 867 green against dead code), and round 10's own lane brief forbade the
  pattern outright for proving publication. Here it is used only to pin **cross-file routing state**,
  which is the established `touchHintLifetime.ts` ⚠ discipline, and the one count that could be spoofed
  by a comment was verified: both `taskCapKmh={taskCapKmh}` occurrences in `StatusDashboard.tsx`
  (lines 704, 812) are live JSX, neither is inside a docstring. The publication proof itself went the
  other way — `hud/__tests__/dashboard-publication.test.ts` drives the extracted function directly and
  `conditions` is a **required parameter**, so dropping the scene's hand-over is a compile error rather
  than a silent winter lesson with no lights row. That is what the brief asked for.
- **The coverage union was rebuilt from `git diff`, and the manifest would have lied by 13 files.** See
  §1 and §2.2. Recorded here because it is a bookkeeping rule, not a finding: **a lane manifest records
  intent, a diff records work, and only the second is evidence.**
...
- **ROUND 9 COMMITTED A GENERATOR — the WRONG one, and that is worth being precise about.**
  `[read]` `tools/audit/make-wave.mjs` is new in the round-9 tree and its header explains itself:
  *„the generators for rounds 2-9 were written in the session scratchpad and a temp cleanup deleted
  them mid-programme, taking the workflow harness with them. The loop that runs this audit should not
  depend on a temp directory surviving."* That fixes the **wave** generator. **It is not this
  document's generator**, and the bullet above stands untouched: there is still nothing in the tree
  that rebuilds §1's and §2's tables from the corpus, and every round still recomputes them by hand
  from a script written fresh. It does, however, establish the precedent the first bullet has been
  asking for — a programme-critical script belongs in the repo, not in a scratchpad — and the same
  argument now applies to the reader. **Open, and now embarrassing rather than merely outstanding.**
- **A `refutedBy` is needed beside `closedBy`, and round 9 is the round that proves it.** Three of the
  forty-eight §2.6 rows are closed and **all three closed by refutation** (O31 round 8, O36 round 9,
  and O42's diagnosis replaced wholesale). Round 9 additionally disproved three corpus findings
  outright — `roundabout.ts`, `railTrack.ts` and `lessonWorldRecipe.ts` (§0.9.1) — on a debrief, a
  pixel scan and an 819-lesson census respectively. **None of that can be written down.** The corpus
  is frozen, so a false finding cannot be marked false; the coverage join counts *files edited*, so a
  file that needed no edit stays in the never-opened row forever. The result is visible in §2.2 right
  now: **2 of its 7 standing criticals are settled and the row cannot say so.** One field —
  `refutedBy` (the measurement, not an opinion) — makes „answered" and „opened" two different columns
  instead of one wrong one. **Open.**
...
(35 bytes each, counted and measured today) and — **now four** — stale scratch dist dirs:
`platform/.next-rig` · `platform/.next-w2inset` · `platform/.next-simoverlay` · **`platform/.next-harness`
(new, created 2026-08-19 07:27)**. `platform/AGENTS.md` is explicit that these inject phantom `tsc`
errors and that each dist dir runs 0.2–1.1 GB on a box the founder's own notes call chronically
squeezed. They belong to other sessions, which is why they have not been deleted from under a running
one — but they are still here.

**AGENTS.md's warning stopped being theoretical this round, and the mechanism is worse than “phantom
errors”.** `[gated]` `.next-harness` is what made `tsc --noEmit` exit **1** in §0's gate table, on
four syntax errors in a half-written `routes.d.ts` inside it. The reason it can reach the type gate
at all is that **`tsconfig.json`'s `exclude: [".next-*"]` does not and cannot stop it**: Next rewrites
the gitignored `platform/next-env.d.ts` to `import "./.next-harness/dev/types/routes.d.ts";`, and
`exclude` filters wildcard *include* matching, never reachability by import. Full proof and the
`--explainFiles` output are in §0.4.1. **This raises the item from housekeeping to a gate hazard**:
any session that runs a dev server with a custom `distDir` poisons `tsc` for every session after it,
and the failure names files no one has touched.

**AND THE ROUND-5 GATE SHOWS WHY „green" DOES NOT MEAN „disarmed" — the hazard is intact and simply
was not firing.** `[gated]` Re-read 2026-08-19 at the round-5 gate, where `tsc --noEmit` exited **0**
with no output at all. Nothing was fixed: `platform/next-env.d.ts` **still** carries
`import "./.next-harness/dev/types/routes.d.ts";` on line 3, `.next-harness` is **still** there and is
**490 MB**, and the reason the gate passed is that `routes.d.ts` is 4,615 bytes and *complete* right
now (mtime 10:57, minutes before the run) where §0.4 caught it **half-written**. **The type gate's
verdict therefore depends on where another session's dev server happens to be in its write cycle when
the gate runs** — a green that is a race, not a state. It will exit 1 again the next time a gate lands
mid-write, naming files nobody touched, and this is the second consecutive round in which the round's
...
**RESIDUE SWEEP, ROUND 9 — CLEAN, and swept wider than the brief asked.** `[gated]` Re-run over the
whole tree on 2026-08-19 against the round-9 working tree. Every row below reproduces the round-5/6/7
result except the untracked-files row, which is this round's.

| swept for | round-9 result |
|---|---|
| files whose own header says *„TEMPORARY — delete after reading"* | **0.** The pattern has exactly three hits repo-wide and **none is residue**: `platform/scripts/tools-tests.mjs:383` *cites* the five-file incident as the reason its ownership audit exists; this document's §9 cites it; and `docs/education/93_THE_THREE_POINT_SYSTEMS.md` says *„Both temporary probes deleted"* and *„the temporary harness removed"* — records of removal, not headers. Checked by reading all three, not by matching the pattern |
| untracked files (`git status --porcelain -uall`) | **9, and every one is real.** Seven test files — `lessons/scenario/__tests__/flow-sweep161-truth.test.ts` 56 `expect(`s/17 blocks · `rules/__tests__/low-beam-duty-one-source.test.ts` 18/7 · `world/builders/__tests__/rail-crossing-every-district.test.ts` 16/2 · `lessons/__tests__/specs-banner-truth.test.ts` 14/4 · `scene/__tests__/lesson-world-spawn-resolution.test.ts` 11/4 · `engine/__tests__/reverseAssist-audit-harness.test.ts` 20/8 · `world/builders/__tests__/island-wall-is-a-collider.test.ts` 9/2 — and **two real tools**, `tools/audit/make-wave.mjs` (the wave generator, committed on purpose: *„the generators for rounds 2-9 were written in the session scratchpad and a temp cleanup deleted them mid-programme"*) and `tools/mobile/wave-c.mjs` (the Wave C re-drive harness). **No stray probes, no scratch files, no `zz-*`, and no new file that is neither a test nor a named tool** |
| does each new test file actually RUN? | **yes, checked per-file and twice.** All 7 were found in `npx vitest list --filesOnly`'s own output, then found again in the run transcript with a test count beside them. **This is the check that catches class (c) — a lane shipping a test file it never ran — and nothing this round is class (c)** |
...
| `.skip` / `.only` / `.todo` / `xit` / `xdescribe` in any changed or new test file | **0**, across all eleven |
| was any gate expectation loosened? | **no.** `world/__tests__/expected-failures.json` is the only allowlist any lane touched: `--numstat` is **2 insertions / 2 deletions**, the `owner` and `reason` strings of the O42 entry. `"scenarios": 1`, `"rungs": 4`, `"ledger": "T14"` and `"allowlist": []` are byte-identical |
| `*probe*` / `*scratch*` / `zz-*` under version control | unchanged from round 5 — all resolve to **real modules or real gitignored working directories**. The two standing exceptions named below (`b15-roundabout-wait.probe.test.ts`, the two committed `tools/clips/headless/*.png`) are still there and still not this gate's to delete |

**THE TWO STANDING EXCEPTIONS HAVE NOW SURVIVED FIVE SWEEPS, WHICH IS ITSELF THE FINDING.**
`modules/sim/runtime/__tests__/b15-roundabout-wait.probe.test.ts` — *„B15 PROBE — NOT A GATE. Throwaway
measuring rig … **Delete once the row is decided**"* — was collected and passed again this round (7
tests). It is still **the one file in the suite that says it should not be in the suite**, and the doc-87
row it exists to decide is still undecided. Naming it every round without deciding B15 is not a sweep,
it is a habit.

**The rest of the residue sweep is clean, and it was run wide.** `[gated]` Re-run 2026-08-19 **after
round 5** (the rows below are the round-5 results; they reproduce round 4's on every line except the
untracked-files row, which is this round's 8 new test files), over the whole tree and not only the
changed files:

| swept for | result |
|---|---|
| files whose own header says *"TEMPORARY — delete after reading"* | **0.** Two hits repo-wide and both are *citations* of the five-file incident, not residue: `scripts/tools-tests.mjs:383` (the reason its ownership audit exists) and this document's own §9. Checked by reading both, not by matching the pattern |
| test files under `platform/src` with no `expect(` | **0** |
| test files anywhere in the repo, tracked **or** untracked, with no `expect(` and no `assert` | **0** |
| `.skip` / `.only` / `.todo` / `xit` / `xdescribe` in any changed or new test file | **0** |
| untracked files after round 4 | **5, and every one is real** — four test files (`hud/__tests__/session-end-ledger-close.test.tsx` 31 `expect(`s · `lessons/__tests__/finish-work-site-band.test.ts` 36 · `lessons/__tests__/reach-zone-witness.test.ts` 60 across 33 blocks · `tools/mobile/lib/__tests__/target.test.mjs` 89 assertions) and one real module they test (`tools/mobile/lib/target.mjs`). No stray probes, no scratch files, no `zz-*` |
