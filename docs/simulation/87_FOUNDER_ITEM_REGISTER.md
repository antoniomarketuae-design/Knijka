...
Ten passes have now run against these 107 rows: a first look, eight fix lanes, a second look, the
2026-08-02 close-out which re-rendered 56 rows and landed six build lanes, the 2026-08-03 gate pass
which refused three lane claims, the 2026-08-03 LOOK-WAVE which put all 107 in front of eyes, the
2026-08-04 VERIFY-BY-DRIVING gate which refused six claims, the 2026-08-05 INSTRUMENT gate —
seven build lanes reporting, all seven disbelieved, 20 of their frames opened, and the two new
instruments driven by the gate itself rather than taken on trust — the 2026-08-09 SEVEN-LANE
VERIFY gate, which found his ten-point complaint in the code, refused three lane claims, and left the
tree green with exactly the two protected reds — and **the 2026-08-09 SEVEN-LANE VERIFY gate #2
(later the same day), which finally PHOTOGRAPHED the result screen the last gate refused to close
without, and which corrected this register's own diagnosis of the thing swallowing three cockpit
rows.** Eleventh: **the 2026-08-09 SCALES gate**, which opened every screen that renders a scored
number at desktop and phone, re-derived the founder's ticket through the new code path instead of the
hand-authored row, and tried to slip a wrong-offence quote past the new citation guard — **and got it
through.** Twelfth: **the 2026-08-09 ATTACK-THE-CHECK gate**, which took the three lanes that closed
the SCALES gate's findings and **read none of them** — thirteen attacks on the citation loader
...
> rendered surfaces contain **37 „т." tokens across the nine dumps, none of them bare**. The manoeuvre rubric still reads
> „1 / 2 **т. за изпълнение**" and the micro-quiz „2 **т. по теорията**" — neither was relabelled
> „изпитни точки" to satisfy the grep. Frames: `scratchpad/V-frames/01-scales-desktop.png` (the
> t=22 teach card he photographed) and `06-result-end.png`.

> **THE RESULT SCREEN EXISTS NOW.** The last gate refused `THE-TEN-POINTS` → `FIXED-SEEN` because
> nobody had photographed it. It has been photographed:
> `scratchpad/ovsr/TENPOINTS-result-3pts-FINAL.png` reads **„3 наказателни точки · ИЗДЪРЖАН ·
> Основни грешки (3 т.) 1 / 3 · Общо (допустими 9) 1 / 3"** with `phase=completed obj=3/3` burned
> into the telemetry overlay. **The row closes.** And the other half he was owed closes with it:
> `scratchpad/ovsr/ONCOMING-on-M1-20pts-FINAL.png` is the same paint against a closing car reading
> **„20 наказателни точки · НЕИЗДЪРЖАН · Опасни грешки 2 / 20"**. Demoting the paint did not leave
> the danger unmeasured.

> **THE 2026-08-09 GATE DELIBERATELY DOES NOT RE-TALLY THIS TABLE.** It re-rendered the rows its seven
> lanes reported — 20 cockpit tiles across five frame files, plus the two phone frames — **not all
> 107.** A column here would imply a sweep that did not happen, and this register's whole reason for
> existing is that a number produced by something other than looking is what he was right to
> distrust. **The verdict movements this gate did earn are itemised in its own section**, with the
> frame path for each. The one verdict it refused outright is `THE-TEN-POINTS` → `FIXED-SEEN`: the
> law is right, the code is right, 813 tests are green, and **nobody has photographed the result
> screen**, so by this file's own rule the row stays open.

| verdict | 2026-08-02 | 2026-08-03 look-wave | 2026-08-04 verify | 2026-08-05 instrument | 2026-08-05 cockpit | **now (2026-08-09 verify #2)** | change |
|---|---:|---:|---:|---:|---:|---:|---|
| **FIXED-SEEN** — rendered, looked at, symptom gone | 52 | 61 | 72 | 79 | 80 | **84** | +4 |
| **PARTIAL-SEEN** — rendered, better, he would still complain | 32 | 39 | 28 | 23 | 21 | **16** | −5 |
| **BROKEN-SEEN** — rendered, still broken | 11 | 5 | 4 | 2 | 3 | **4** | +1 |
| **NOT-A-DEFECT** — investigated and refuted | 2 | 2 | 3 | 3 | 3 | **3** | — |
| **UNVERIFIABLE** — never got on screen | 10 | 0 | 0 | 0 | 0 | **0** | — |
| | 107 | 107 | 107 | 107 | 107 | **107** | |

> **THE 2026-08-09 #2 COLUMN IS A DELTA, NOT A SWEEP — and the five moved rows are named.** This gate
> did not re-render all 107 either. It opened the frames for the rows its seven lanes reported and
> moved exactly five cells, each with a frame path: **B23 · B50 · B53 · B70 PARTIAL → FIXED-SEEN**,
> and **B58 PARTIAL → BROKEN-SEEN**. The arithmetic above is those five applied to the previous
> column and nothing else. **B58 is a deliberate downgrade, on the B65 precedent:** it was rendered,
...
> notice while playing catalog positions 1–50.** Four defects fixed on 2026-08-05 are in none of the
> 107 rows, and one of them would have driven a student backwards into traffic for pressing the
> brake at a stop sign. They are written up as rows in their own right in
> **the 2026-08-05 VERIFY-BY-DRIVING gate section below (`### D1`–`### D4`)** and are
> deliberately **not** folded into the 107 count, because the count is a count of *his sentences* and
> these are not his:
>
> | | defect | how it was found | state |
> |---|---|---|---|
> | **D1** | A HELD BRAKE became a floored reverse accelerator (−3000 N, 16.8 km/h backwards) — triggered by standing on the brake at a Б2 line | a lane reading the input path | **FIXED-DRIVEN** — 1 955 frames at `brake=1.0000 / throttle=0.0000`, gear `[1]`, drift 1.1 mm |
...
> → BROKEN-SEEN confirmed"* since the gate before last. The cell and the body have now been reconciled
> to what the frame shows. **Nothing has ever regressed from fixed to broken in this register and that
> is still true.** The one row that moved forward is `B34`, closed with a cockpit frame.

> **Two corrections to this table's own arithmetic, made by counting the rows instead of trusting the
> previous header.** The 2026-08-04 column read **71 / 29**; counting every row's verdict cell
> mechanically gives **72 / 28**. One row had been double-counted as PARTIAL when its cell said
> FIXED — a one-row drift, corrected above rather than carried forward. **The only two rows still
> BROKEN-SEEN are `B52` (the children — blocked on your Blender machine) and `B65` (the raw map),
> and B65 is a row this gate *downgraded* from PARTIAL on purpose.**

**THE TWO SENTENCES THAT MATTER MOST THIS ROUND.**

*First: the frame at forty seconds exists, and I made my own rather than believe anyone else's.*
Stopped on the М8 give-way paint at `y=−36.2`, **70.42 seconds of continuous standstill** — the car
moved 6 mm in that time, the gear never left D, `phase` never left `driving`, and **zero events fired
on any of the three channels.** One event in the whole 96-second drive, and it was the approach
objective completing. `scratchpad/VERIFY/vwait2-step07-t82.7-v0.png`.

*Second: the detector that had to survive that fix still convicts.* When a circulating car is
genuinely in the way the fault fires with the car's bumper gap printed in the same frame —
`lead=3.2m = 0.89s @ 13km/h` under the teach card „Непропускане на пътно превозно средство с
предимство". **A detector that never convicts is the same defect mirrored, and this one is not that.**

**The headline, in his terms.**

- **Seven rows moved to clean and one moved the other way.** Clean: **B15** (the roundabout wait —
  the row this register has failed to photograph for three waves), **B29** and **B30** (the two
  sightline rows, both of which go straight from BROKEN to FIXED), **B31**, **B69**, **B72**, and
  **B-NEW-1**. Moved the wrong way on purpose: **B65** — the raw map — **downgraded from PARTIAL to
  BROKEN**, because it was re-rendered and *nothing whatsoever had changed*, and calling that
  "partial" was flattering it.
- **79 rows are clean. 25 are still wrong** — 2 photographed and still broken, 23 photographed and
  improved but not finished. He would still complain about all 25.
- **The four lane claims this gate refused were all about the state of the tree, not about a
  frame** — and that is a change worth naming. Every frame a lane cited this round held up; twenty of
  them were opened and none was a bluff. What did not hold up was **"tsc exits 0"** (it exits 2, with
  seven errors in a *tracked product test*), **"zero PNGs in the repo"** (two are),
  **"tsconfigHygiene is clean"** (six globs, three families), and **"that failing test passes in
  isolation in 903 ms"** (it failed three times out of three, at 22–40 s). *The lanes have learned to
  photograph honestly and have not yet learned to gate honestly — every refusal this round was a lane
  reporting green on something it had not re-checked at the end.*
- **The tree is red, and this time one of the reds is ours.** `pe-districts.test.ts` and two traffic
  suites belong to the world / parked-bodies lane, not to a neighbouring wave. Named with the file and
  the reason in the gate section and again in STILL OPEN.
- **The thing that will decide whether B15 stays closed is not in B15.** For the entire minute the
  student stands correctly at the give-way line, the virtual instructor **says nothing** — the
  roundabout grader is not even armed where the paint is — and the first word it ever says about the
  priority car is **−10**. Against requirement-zero (doc 64 THEO-4, "explain every decision, never a
  bare verdict") that is the sharpest open item in this document.
...
## The gate — ATTACK-THE-CHECK, 2026-08-09 (the third pass of the day). **SIX OF THIRTEEN ATTACKS GOT THROUGH THE LOADER. THREE GET THROUGH THE WHOLE REPO. THE TREE IS GREEN WITH EXACTLY THE TWO PROTECTED REDS.**

Three lanes reported closing what the SCALES gate had found by attacking rather than reading: the
citation loader's blind spot, the bare „Наредба № Iз-2539", and the theory exam's unnamed scale.
**This gate read none of their claims on trust.** It re-ran the known-good attack, invented twelve
more, fed both watermark specimens to every check in the tree, and drove the exam in a browser.

Three things worth saying before the detail:

*First: the fix is real and it holds against the attack it was built for.* The known-good attack —
give a speeding row the traffic-light case in `contextQuoteBg` **and** `offencePhraseBg` — is now
refused by name („the offence named is not the offence this row prices"), and so are six others,
including two the lane never claimed: the whole-article quote and the danger-variant control-points
citation moved onto the no-danger row.

*Second: the survivors are not the ones the lane disclosed.* It disclosed two. There are three, and
the third — `O-3` below — is **live on a shipped row**, not hypothetical: `pen-b2-no-stop-danger`'s
fine phrase can be truncated to drop „ако от това е създадена непосредствена опасност за
движението", which is the entire difference between 200 лв. + 10 к.т. and 50 лв. The lane wrote that
class up as „nothing in the corpus reads that way today". It does.

*Third: the loader and the CI gate are different guarantees, and this is the first gate to measure
both.* Every survivor was re-run as a **committed** mutation of `content/law/penalties.json` against
the repo's own suites. Three are caught by a test even though `verifyCitations` accepts them. Three
are caught by nothing at all.

### 1 · THE ATTACK BATTERY — 13 attacks, run against the real bank, every one reported

`verifyCitations(acts, [tamperedRow])` on a `structuredClone` of a real row. „through" means it
returned `[]`.

| # | attack | verdict |
|---|---|---|
| A1 | **the known-good one.** Speeding row given the traffic-light case in `contextQuoteBg` AND `offencePhraseBg` | **REFUSED** — „the offence named is not the offence this row prices … satisfies none of [превишаване \| превиши], [от 21 до 30 \| с повече от 10 km/h]" |
| A2 | **adjacent tier, same article.** The 21–30 row priced by the 11–20 sentence, on the `not-listed` disqualification figure so no amount contradicts it | **REFUSED** — the tier anchor |
| A3 | **the identical twin.** The in-town row's `paragraphRef` flipped to „ал. 2" — чл. 182 ал. 1 т. 3 and ал. 2 т. 3 are word for word | **THROUGH** → `O-1` |
| A4 | **the tier where the ladders differ.** A 31–40 row quoting ал. 2's 300 лв. under a coordinate saying ал. 1 (in town it is 400) | **REFUSED** — „quote is in ЗДвП чл. 182 but NOT in ал. 1". *The new alinea-span parser earning its keep.* |
| A5 | **right offence, wrong act.** Red-light control points re-cited to the **2025 snapshot**, whose т. 20 is word-identical | **THROUGH the loader, CAUGHT by the repo** — `corpus.test.ts > the whole bank now cites the consolidation, not the 2025 snapshot` fails with `[ 'pen-red-light' ]` |
| A6 | same, on the alcohol row, where the editions disagree on the number | **REFUSED** — „quote is NOT in Наредба № Iз-2539/2012 чл. 6" |
| A7 | **the declaration loosened by DELETION, then the swap.** Two anchor groups replaced by the stem `[["превиш"]]`, numbers spelled out of `statementBg`, then the 11–20 phrase moved in | **THROUGH the loader, CAUGHT by the repo** — 3 tests red, loudest the discrimination matrix (6 pairs, 4 pinned) → `O-5` |
| A7b | the same loosening with the citations untouched | **THROUGH the loader** — all four declaration guards pass |
| A8 | **the declaration as a paraphrase of its own title** — „Превишена скорост … двадесет и един до тридесет километра в час", one generic anchor „скорост" | **REFUSED** — and by the row's *own honest phrases*, which do not contain „скорост". Self-consistency is doing work here. |
| A9 | **`titleBg` rewritten** to another offence, every citation left honest | **THROUGH everything** → `O-2` |
| A10 | **the row `id` rewritten** | **THROUGH the loader, CAUGHT by the repo** — but only incidentally: 12 tests name the id |
| A11 | **`quoteBg` widened to the whole article**, so every phrase is inside it | **REFUSED** — the alinea span |
| A12 | **the danger-variant control-points citation moved onto the no-danger row** | **REFUSED** — „satisfies none of [не спира \| не спре]" |
| A13 | **the offence phrase truncated so its limiting clause falls off** | **THROUGH everything** → `O-3` |

**The discrimination matrix reproduces at 4, independently.** Re-derived with a private
implementation of the anchor matcher rather than the exported one, so a bug in the exported matcher
could not hide inside its own measurement: exactly four ordered (row, foreign-phrase) pairs survive,
all four the single Наредба № 38 exam indent that `pen-b2-no-stop`/`-danger` and the two speeding
tiers genuinely share. **Nothing from ЗДвП or the контролни-точки наредба crosses a row.** The lane's
headline claim is accurate.

**And one structural finding the lane did not name.** Check (5b) requires *every* offence phrase on a
row to satisfy *every* anchor group, so **a row's `conduct` is forced to be the weakest common
denominator of its figures.** That is why `pen-b2-no-stop-danger` cannot declare „непосредствена
опасност": its контролни-точки and наказателни-точки citations come from acts that never say it, and
adding the group would turn the row's own honest citations red. The two rows that differ **only** by
the danger clause are precisely the two rows the mechanism cannot tell apart.

### 2 · THE BARE ACT NAME — answered by resolving every hit, not by grepping

Every `lawRef` in `content/` whose `act` string mentions 2539, put through `actIdForActName`:

| act string as written | resolves to | count |
|---|---|---:|
| `Наредба № Iз-2539` (**bare**) | `naredba-iz-2539-consolidated-dv49-2026` — **the text in force** | 11 |
| `Наредба № Iз-2539 (изм. ДВ, бр. 49 от 2026 г.)` | `naredba-iz-2539-consolidated-dv49-2026` | 8 |
| direct `actId` in `penalties.json` / `sources.json` | the consolidation (7) + registry rows | 11 |

The eleven bare ones are `content/concepts.json` ×2 and nine questions across
`alkohol-i-godnost`, `dokumenti-i-sanktsii` and `osnovni-ponyatia`. **Zero land on the snapshot.**
All eleven cite чл. 2 or чл. 6, which both editions carry, so the move is a strict improvement.

**And the snapshot is still reachable when named — which is the point of keeping it.** All four
documented markers resolve to `naredba-iz-2539`: `(ред. 28.01.2025 г.)`,
`(изм. ДВ, бр. 108 от 2024 г.)`, `(снимка)`, `(редакция към 2025 г.)`. Both the Ukrainian „і" and the
Cyrillic „и" spellings of the Roman numeral resolve too. **Breaking that would be a different defect
and it has not been broken.**

One thing left standing: `lib/content/pointScales.ts:122` renders `"Наредба № Iз-2539"` bare **to the
student** as the контролни-точки scale's source. Harmless today — it names no article and no figure —
and named in STILL OPEN as `O-8` because the next person to put an article number beside it inherits
...
| specimen | `build-naredba-24.mjs` ingest signatures | its stop-line headings | `pageFurniture.test.ts` FURNITURE | its PAGINATION |
|---|---|---|---|---|
| lex.bg „Хумор" sidebar | **5 of 7 fire** | **4 stop lines** | **5 fire** | — |
| „Източник: Правно-информационни системи „Сиела"" + `24/01/2025 г.` | **0 fire** | none | none | **both fire** |

So the answer to „does the new ingest check actually fail on page furniture" is **half**. The Хумор
class is stopped at the ingest *and* at the repo test. The Сиела class is stopped **only by the repo
test's ledger** — and the ingest that has a guard (`build-naredba-24.mjs`) is not the ingest that
produced the damage. **`content/law/tools/build-corpus.mjs`, which emitted all 185 occurrences across
naredba-38 (124 / 34 units, приложение № 5 among them), naredba-iz-2539 (58 / 16) and
naredba-sredstva-za-izmervane (3 / 3), contains no furniture check of any kind** — zero signatures,
one unrelated `throw`.

The ledger was verified by mutation rather than by reading: a Сиела stamp injected into
`naredba-24.json` `чл. 9` turns the ledger test red with `+ "naredba-24": { "чл. 9": 2 }`. The file
was then restored **byte-identical**. `O-4` carries the owner and the reason it cannot be fixed from
here.

### 4 · THE THEORY EXAM — driven, and the „ten" were twenty-two

**Measured against `HEAD` with a scanner written for this gate** (different regexes from the shipped
one, so a blind spot would have to be shared to survive both): the four exam files carried
**8 bare „т." + 14 unqualified „точки" = 22**, not ten. The repairing lane reported 21. **The working
tree is 0 and 0.**

The undercount was of the *defect*, not the abbreviation — twelve of the twenty-two were „точки"
spelled out („точки максимум", „/ 97 точки", „Загубени точки", and „Праг за успех 87 точки" read
aloud by the screen reader). Unqualified „точки" in Bulgarian **is** контролни точки. It is the
founder's own misreading with the abbreviation expanded.

**Driven, on the rig already listening on `:3200`** — `/dev/fold-rig?mode=exam&rank=3`, hydrated
(`data-hydrated="1"`), a full 45-question paper, clock at 39:52, counter „1/45":

- **1280 px** — the visible chip reads **„3 точки от правилни отговори"**; the phone twin is
  `display:none`.
- **375 px** — the visible chip reads **„3 т. по теорията"**; the desktop twin is hidden.
- Both carry the same `title`: „Толкова тежи този въпрос на **ТЕОРЕТИЧНИЯ** изпит. Печели се с верен
  отговор и няма нищо общо с наказателните точки от карането, нито с контролните точки по книжката."

The result view, rendered and read back end to end:
...
> време 33:00 · Точки от правилни отговори на теоретичния изпит (Наредба № 38, чл. 39, ал. 1): 97
> максимум, изпитът е издържан при не по-малко от 87. Печелят се с верен отговор. **НЕ са контролни
> точки по книжката и НЕ са наказателните (изпитни) точки от практическия изпит.**"

Pass, fail and late papers all carry that footing; the review chip reads „0 / 3 т. по теорията"
(and „1 / 1 т. по теорията" at one).

**THE OPPOSITE ERROR WAS NOT MADE, and this gate went looking for it specifically.** A grep for
`изпитни точк` / `наказателн* точк` across `app/(dashboard)/exams/**`, `components/exam/**` and
`modules/exam/**` returns **only the test that forbids them**. Nobody stamped the practical sheet's
10/3/1 label on a 97-point written test to clear a scanner. And it is **not a fifth scale**:
`POINT_SCALES.theory` already counted this unit for the simulator's micro-quiz, so it was grounded
rather than duplicated — the inline form stayed „N т. по теорията" so no sim frame moves, and doc 93
§2 now names all five systems with the act each rests on.

**Two bare units remain in the repo** and neither is the student's exam:
`app/(dashboard)/review/ReviewClient.tsx:635` and `app/(dashboard)/admin/page.tsx:415`. Both are
outside the shipped guard's directories — `O-6`.

### 5 · The gate itself — every command, with the numbers
...
- `npx vitest run --maxWorkers=4` → **755 files: 2 failed | 752 passed | 1 skipped. 11 518 tests:
  2 failed | 11 346 passed | 170 skipped.** 180.01 s.
  **Exactly the two protected reds, and this gate traced them to a file and a line rather than
  recognising them:** `src/modules/exam/__tests__/content-bank.test.ts:103` (`REVIEW_DEBT` —
  `ptp-i-parva-pomosht` 31/64 approved) and `src/modules/lesson/__tests__/compose.test.ts:73`
  (`l-accidents-first-aid` has no quiz beat). **No third red.** The `law-citations.test.ts` failure
  the last gate had to arbitrate between three lanes is gone — the `lawRef: "к"` typo in the B35
  lane's own fixture was fixed, and no lane-attribution dispute arose this round.
- **Nothing was left changed.** Both probes deleted
  ...
  `*probe*` / `*__gate*` under `platform/src` returns only the pre-existing
  `b15-roundabout-wait.probe.test.ts`. `content/law/penalties.json` (44 036 B, sha256
  `a682f0e1fd353833…`) and `content/law/acts/naredba-24.json` were mutated for the committed-attack
  runs and restored **byte-identical**, verified by buffer equality against pre-mutation copies.
- **No dev server started and no dist dir created.** The drive reused the rig already listening on
  `:3200`, read-only. `platform/tsconfig.json` untouched. The four orphan `platform/.next-*` dirs
  (`c5rsw`, `harness`, `rig`, `ttlane`) are still on disk and are **not this gate's** — they are the
  same four the SCALES gate reported and nobody has claimed them.

### 6 · Claims this gate REFUSES

| claim | who said it | what the measurement shows |
|---|---|---|
| „A FIFTH ATTACK STILL GETS THROUGH" *(one survivor: the row's own label)* | the citation lane | **Three survive the whole repo, not one.** The label (`A9`), the identical-alinea twin (`A3`) and the truncated phrase (`A13`). Three more survive the loader and are caught only by a test — a distinction that lane did not draw. |
| „No substring check can see negation or limitation … **Nothing in the corpus reads that way today**" | the citation lane | **It does.** `pen-b2-no-stop-danger`'s own fine phrase truncates cleanly to a general clause that drops the danger limb, satisfies the row's conduct, and turns nothing red. That is not a class of defect waiting to happen; it is the shipped row. |
| „the new ingest check … refuses to emit" *(implied general)* | the watermark lane | Only for the lex.bg class, and only on the ingest that never produced the problem. **`build-corpus.mjs` — the tool that produced 185 of the 185 — has no check.** The lane says the fix belongs in the extraction and it is right; what is not stated is that the guard it did add cannot see the Сиела stamp at all. |
| „the ten bare „т."" *(as an inventory)* | doc 93 §7 G-5 (3), and the repairing lane's own count | **22 defects at `HEAD` on those four files** by an independent scanner — 8 abbreviations and 14 spelled-out. The repairing lane found and fixed the spelled-out half itself and said so; the *register* still said ten. |
...
## The gate — SEVEN-LANE VERIFY gate #2, 2026-08-09 (later the same day). **THE RESULT SCREEN IS PHOTOGRAPHED. THE CAMERA ROWS HAVE THE WRONG CULPRIT IN THIS FILE, AND THE FRAME PROVES IT.**

Seven lanes reported: the cockpit camera (**B58 · B27 · B55 · B67 · B35**), the ten penalty points
(**THE-TEN-POINTS** + the corridor + `EMERGENCY_LANE_DRIVING`), the phone (**C1 · C8**), six lesson
rows (**B5 · B23 · B40 · B41 · B64 · B70**), the seven pedestrian streets (**B50 · B53 · B54**), the
look/anisotropy brief, and the citations/tooling lane. **Every headline was disbelieved.** This gate
opened 16 frame files with its own eyes, re-derived the draw-call census itself rather than quoting
either the register or the lane, read the load-bearing predicates out of the shipped source, and ran
the full four-command gate.

### 1 · THE NUMBER HE ASKED FOR TWICE — photographed, both directions

The last gate refused this row for the right reason. It is now closed for the right reason.

| frame | what it reads |
|---|---|
| `scratchpad/ovsr/TENPOINTS-result-3pts-FINAL.png` | **„3 наказателни точки · ИЗДЪРЖАН"** · Опасни грешки (10 т.) **0 / 0** · Основни грешки (3 т.) **1 / 3** · Второстепенни **0 / 0** · Общо (допустими 9) **1 / 3** · ★★☆ · 88 с при ориентир 85 с. Telemetry burned into the frame: `t=87.70s phase=completed obj=3/3 events=2`. `sc-ov-solid-return` @L3. |
| `scratchpad/ovsr/ONCOMING-on-M1-20pts-FINAL.png` | **„20 наказателни точки · НЕИЗДЪРЖАН"** · Опасни грешки **2 / 20** · „Настъпи сблъсък — реалният изпит се прекратява незабавно". Same М1 paint, a closing car, `sc-animal-hazard` @L3. |
| `scratchpad/ovsr/TENPOINTS-teach-first-FINAL.png` | The teach card, in the product's own Bulgarian: **„Първа среща — не се брои в резултата. При повторение: −3 т. (основна грешка)"**. |

**Read those three together, because the first one alone would mislead the lead.** It takes **two**
crossings to put 3 on that screen — the first raises the card, the second bills it. **A single
crossing reads 0.** That is A12 teach-first working as designed, and it is also exactly why still-open
row 6 below (the recommender hole) matters: the encounter we are surest represents a knowledge gap is the
one that reaches the theory recommender as nothing at all.

**Demoting the paint did not leave the danger unmeasured, and that is verified in source, not
inherited.** `runtime/worldRuntime.ts` no longer carries `tick.solidCenterLine !== true` in the
corridor's arming: the predicate is split at **:1824 `ocBase` → :1832 `ocArmed = ocBase`** for the
head-on measurement and **:1834 `orArmed = ocBase && tick.solidCenterLine !== true`** for the OV-09
return tracker, so the corridor widens while the М1 seam stays byte-for-byte. Read here, in the file.

### 2 · THE CAMERA CONSTANT — this register's own diagnosis is wrong, and a lane would have lost a day to it

**Still-open row 7 of the previous gate is measurably false and is corrected below.** It attributes
B58 / B27 / B55 to CameraRig's `rv` quad and concludes *"one camera constant sits between the product
and three rows."* Read out of the shipped source by this gate:

- `LessonScene.tsx:704` — `const [cockpit, setCockpit] = useState(true)`. **Cockpit is the default view.**
- `CameraRig.tsx:964–965` — `const rvSide: MirrorGlanceKind | null = mode === "chase" ? (heldSide ?? "rear") : null;` and at `:966` `if (rvSide === null) { rv.glass.visible = false; rv.bezel.visible = false; … }`.

**In the view he plays, that quad is never drawn.** A fix landed in `chaseRearView.ts` would have moved
nothing he can see. The real occluder is the cabin's own interior mirror — `hotspot_mirror_rear` plus
the code-built pod in `VitokCockpit.tsx` — **which is the very window he asked for in B74/B76.** Same
object, two row numbers.

**And there is no camera constant to turn.** Three independent hard stops, each checked against the
shipped constants: `COCKPIT_CAM_OFFSET = { aft: 0.375, inboard: 0.1, up: 0.05 }` (`vehicle/tuning.ts:555`)
sits inside a band the contract test pins at `inboard ∈ [0.05, 0.15]` (`cockpit-camera-contract.test.ts:156–157`)
— outboard makes the sign's emergence *later*, inboard pushes the graded left door mirror (already at
`fx 0.001`) off the frame; vertically `ROOF_Y = 0.9` (`VitokCockpit.tsx:322`) leaves ~16 mm above the
mirror hood and an unmovable authored GLB casing below; and pitch moves mirror and sign by the same
angle. **~1–2° of travel against a ~7° problem.** The lane refused to ship a cosmetic change so the row
could be closed on it, and that refusal was correct.

**The cheap real fix is not in the camera at all.** `SCENARIO_SIGN_SCALE = 1.5`
(`world/builders/constants.ts:335`, his own „those signs must be big" ruling) scales the sign post
**about its base**, so making the disc 1.5× also raised its centre from ~2.2 m to ~3.30 m — straight
into the mirror's box. Legal Bulgarian mounting is a 0.6 m disc with its lower edge at 2.0–2.2 m.

### 3 · The three camera rows, each against the rule „is the thing he called missing IN the windscreen?"

| row | verdict | the frame, and what is actually in it |
|---|---|---|
| **B58** | **BROKEN-SEEN** *(downgraded from PARTIAL)* | **The x-ray pair settles it beyond argument.** Same pose, same pixels: `scratchpad/camlane/zoom-b58-xray-sign.png` (mirror assembly hidden) shows the red-ringed **«50»** disc plainly on the right verge; `zoom-b58-live-sign.png` (mirror present) shows **a reflection of a white van** in exactly that place. Annotated full cockpit frame at the shipped 16:9: `camlane/b58f-annotated.png` — red box = mirror body `[873,64,1155,304]`, cyan = the sign `[973,258,1007,291]`, 100% inside it. Swept the approach: 100% / 100% / 81% occluded at 28.5 / 24.8 / 21.8 m. **He was told twice the world was empty when it was not. NOT FIXED.** |
| **B27** | **PARTIAL-SEEN** | „there is no stop sign" is false of the seat as well as the world: `camlane/b27-spawn-frame.png` is a cockpit frame with the **Б2 red octagon on the right verge**, clear of the mirror. What the mirror *does* eat at that pose is the В26 «40». **Not closed, and the lane said so itself:** that frame was shot before the aspect defect below was found (canvas 1248×520, aspect 2.40, not the shipped 16:9), and **nobody has photographed the 12–28 m band where the Б2 enters the mirror's box** — the last three seconds, which is exactly when the student decides whether to stop. |
| **B55** | **PARTIAL-SEEN — and re-attributed** | `camlane/b55f-annotated.png`, cockpit frame, shipped 16:9. **The mirror is not the occluder here.** The two pedestrian heads are 139.5 m away and project **2–3 px wide**; the register's „dark bar with a 1–2 px lens sliver" reproduces exactly and its cause is distance plus the head's own lens-vs-housing geometry, in `WorldProps`. Of the three rows this register bundled as one camera problem, **B55 is not a mirror row at all.** |
| **B67** | camera half **refuted, photographed** | `camlane/b67g-s1.png`, real product shell, 1440×900 → 16:9: at **111.1 km/h** on `mw-v1` the binnacle reads **„110 км/ч D"** with the wheel, the full dash, the 140 roundel and the telltale row all in frame. No headrest, no seats-from-behind, at any speed; the cluster rect moves 17 px and shrinks 5% across 37→113 km/h. **His actual sentence — the 160–180 ceiling — is untouched and is a curriculum call.** |
| **B35** | **untouched — NOT ATTEMPTED** | The lane says so in its own words and this gate repeats it rather than dressing it up: not rendered, affordance not built. What it did establish is that the prescribed pattern costs more than this register assumes — the B42 bubble is a billboarded canvas plane **inside `TrafficLayer.tsx`** serving the traffic ACTOR layer, while a signal head is world geometry in `WorldProps.tsx`. **There is no shared world-label channel to reuse.** |

### 4 · The phone — C1 answered by sweep, C8 answered with a real number

**C1 — the four-surface stack is genuinely gone, and the row's own quoted evidence is a
misdiagnosis.** Landing state, WebKit, real device insets, world pixel-gated before anything was read:
iPhone 16 portrait **2 named surfaces, 0 intersections, 0 covered controls, road 93.6%**; landscape
**1 surface, road 95.4%**; small-portrait 2 / 0. Against the row's 0%. Frame opened:
`scratchpad/p0p7/c1-frames/iphone16-portrait__landing.png` — the road dominates the picture and the
seatbelt pill is now inside the notification column instead of floating free.

The row quoted „«Разбрах» was not tappable (something is painted over it)". **That sentence is the
harness's, not the screen's** — `tools/mobile/lib/routes.mjs:248` prints it for *any* Playwright
refusal, and this gate confirmed the string is hard-coded there. Measured, `elementFromPoint` at the
button's own centre returns the button; the refusal is the actionability STABLE check, because the
notification column swaps content underneath it. **Two things really are still stacked, both named
with coordinates** — see still-open rows below.

**C8 — there is now a real number and it is nowhere near the budget.** 4 of 4 requested runs produced
a full row, zero lost, each gated on pixels showing a rendered street (253–303 distinct colours) before
a phase was read, each number `atRestMs` timestamped **inside the page** so the instrument is excluded
by construction rather than subtracted afterwards, with the probe's own share printed beside it:

```
phase          n   min  median  worst  spread  wall(med)  probe%   over 1200 ms
base(idle)     4   152    340    421     269      570      18%       0 / 4
overlayOpen    4   275    337    507     232      514       2%       0 / 4
overlayClose   4   235    296    382     147      544      10%       0 / 4
worst STATE-CHANGE phase per run: min 275 / median 346 / worst 507 ms   →  0 / 4 over
```

**693 ms of margin, wider than both the run-to-run spread (232 ms) and the instrument's own resolution
step (336 ms).** The „1 198 ms" was never about the driving shell. One honest caveat the lane
volunteered: a fifth idle sample from a colder start read **1 680 ms**, so 1 of 5 *idle* samples is over
— and idle is the phase in which the probe changes nothing.

### 5 · The seven streets — opened at the SPAWN, and he could tell them apart

`E:\AI driver\scratchpad\sf\frames\SHEET-SEEN-spawn.png`, opened by this gate. All seven tiles are
cockpit frames with wheel, cluster and A-pillars. **No pair reads as the same street.**

**And the differentiator is not the weather.** Weather does exactly one of the seven (pe-rain is night
and snow). The other six separate on furniture and traffic: pe-bus a blue panel van plus pickups at the
right kerb and a kerbed median standing in the road; pe-cane both kerbs parked and **every car tail-on**
(the one-way tell); pe-child dark green estates at the bonnet with the left bay empty; pe-dart the only
street whose **near** kerb is the empty one; pe-slow one saloon far out against an open forecourt;
pe-clear both kerbs dense from behind the bonnet.

**The cause was worse than „similar" and it is fixed at source.** `computeParkedCars` hashed
`(edgeIndex, slot)` with **nothing naming the district**, so pe-cane / pe-bus / pe-child seated *the
same three cars, in the same paint, in the same order*, three consecutive lessons.
`districtParkedSalt` (`traffic/TrafficLayer.tsx:410`) now salts the **appearance only** — placement is
deliberately untouched, and this gate confirmed the graded side by diffing the JSON: the seven PE
districts changed **1–7 lines each and not one graded key** (`nodes`, `edges`, `crossings`,
`spawnPoints`, `zones`, `geometry`, `points` — zero hits across all seven diffs).

**What all seven still share, named rather than smoothed over:** the tarmac tone, the dashed centre
line and the white edge lines are one picture in every frame. That is honest, and it is the next lever.

### 6 · The look — it is filtering, not base maps, and this gate re-measured the draw calls itself

`scratchpad/lookfall/frames/PAIR-8m-20m.png` and `AFTER-low.png` opened (the latter a full cockpit
frame, so the crops came from a seat). **The honest reading is PARTIAL and the lane said so first:** at
4× the 8 m pair is nearly indistinguishable; the 20 m pair is visibly grainier but the gain is modest.
It is **not** another base-map pass — the change is one texture filter. A cockpit eye 1.3614 m up meets
the road at 3.9° at 20 m, a 17:1 pixel footprint, and the ground was bound at anisotropy 4, so two mip
levels of detail were fetched and then thrown away. Ground anisotropy now 16, clamped to the device
ceiling: pixel-scale structure +44.9% at 20 m, +51.7% at 12 m, and the 8 m→20 m fall goes −46% → −27%.
Zero draw calls, zero triangles, zero bytes.

**THE DRAW-CALL CENSUS, RE-MEASURED BY THIS GATE THROUGH `buildWorldGeometry` OVER ALL 100 SHIPPED
DISTRICTS, IN BOTH COPIES.** The brief was right to demand it:

```
low-tier soft cap 70 · hard cap 100
  67  hz-roadworks-v1     67  ov-crest-v1     67  pe-zone-v1     67  pk-rail-v1
  66  d2-v1               65  mw-exit-v1      65  ov-oneway-v1   65  pe-cane-v1
WORST 67  ·  HEADROOM 3  ·  over soft cap 0  ·  max triangles 81 528 (32.6% of 250 k)
content/world and platform/public/world agree exactly — no mirror drift
```

**The previous list’s still-open row 11 is wrong twice, and is replaced by still-open row 7 below:** the worst is **67, not 66**, on **four maps
this register does not name**, and headroom is **3, not 4**. `pe-cane` 65 and `d2-v1` 66 both confirmed.

### 7 · Traces and generator idempotency — both clean, and checked before being believed

`node tools/maps/gen_streetwall.mjs --check` → **exit 0, „files rewritten 0 (--check; 0 would change)"**
across 24 maps / 681 buildings. **The previous list’s still-open row 18 closes.** The two districts that drifted were
`d2-v1` (four `sw-` plots removed, two added) and `tj-emerge-v1` (one removed) — and the reason is worth
keeping: **all five vanished plots stood inside a spawn keep-out** (15.08 / 14.51 / 15.80 / 15.07 m
against 18 for CITY; 11.98 against 12 for JUNCTION) because a later pass moved the `spawnPoints` and
nobody re-stamped the wall. The generator was never non-deterministic; the repo was stale.

Five traces are modified in this tree. **One belongs to the lanes reporting here** —
`sc-animal-hazard/mistake-cross-line`, re-recorded on purpose because the corridor found a real oncoming
car at **2.39 s** in the demo whose whole lesson is „the oncoming was clear and it is STILL forbidden".
The other four (`sc-jx-blocked-exit`, `sc-rb-busy-gap`, `sc-rb-exit-signal`, `sc-roundabout-entry`) belong
to a lane not in this report set; their recording scripts are modified alongside them.
**`npx vitest run src/modules/sim/traces` → 160 files / 2 039 tests, all green in 37.5 s.** Every trace is
consistent with the world as it stands.

### 8 · The gate itself — every command run here, with the exact numbers

| command | result |
|---|---|
| `cd platform && npx tsc --noEmit --incremental false` | **exit 0, zero error lines** (the run produced no output at all) |
| `npm run validate:content` | **OK — all structural and referential checks passed**, exit 0. 1 089 questions / 152 concepts / 16 topics |
| `npx vitest run --maxWorkers=4` | **745 files: 2 failed · 742 passed · 1 skipped. 11 331 tests: 2 failed · 11 159 passed · 170 skipped.** 221.71 s |
| `npm run test:tools` | **290 tests · 289 pass · 1 fail** |

**Exactly the two protected reds, and they are the two that were expected:**
`exam/__tests__/content-bank.test.ts > has no dark, threadbare or under-represented topic`
(`ptp-i-parva-pomosht` 31/64 approved) and
`lesson/__tests__/compose.test.ts > gives every lesson at least one quiz beat` (`l-accidents-first-aid`).
**Both are the 29 quarantined first-aid questions waiting on his medical ruling. Neither was touched.**

**The third red is real, is in `test:tools`, and is traced to a file and a commit rather than waved
at:** `tools/deploy/deploy.test.mjs:588 — every shell script is committed executable` fails on
`content/medical/tools/fetch.sh` and `content/sources/tools/fetch.sh`, both mode `100644`.
`git ls-tree HEAD` confirms **both were added at mode 100644 in HEAD itself (`d7ec746`)** — it is
pre-existing, not one of these seven lanes, and the fix is one `git update-index --chmod=+x` each.

**Hygiene, checked not asserted.** `platform/tsconfig.json` is **unmodified** (`git diff` empty) and its
only `.next-*` glob is in **`exclude`**, which is the committed anti-phantom-tsc guard, not an injected
`include`. **Zero untracked PNGs in the repo** (`git status --untracked-files=all` → 0). All frames are
in scratchpads. **Nothing was committed.**

### 9 · A TENTH AND AN ELEVENTH INSTRUMENT DEFECT — and one of them invalidates the aspect of earlier cockpit measurements

1. **`LessonPlayShell` sizes the picture to `PLAY_ASPECT` (16:9) only once its ResizeObserver fires.**
   In a scripted page nothing ever resizes, so the canvas sat at the reading column's full width —
   **1248×520, aspect 2.40** — for entire 60-second drives. At 2.40 the hFOV lock drops vFOV from 47 to
   35.7, which is **not** the frame `cockpit-camera-contract.test.ts` is authored for, and `playArea.ts`'s
   own table says the bands stop holding past aspect 2.00. **Every pixel rectangle in this register that
   cites a 1264×620 or 1248×520 canvas — including still-open row 7's — was measured on that frame.**
   Someone should check whether a real first paint in a real browser also lands there; if it does, it is
   a product defect, not a harness one.
2. **In Playwright's WebKit, once a WebGL canvas's drawing buffer changes SIZE, every subsequent
   screenshot of it is a flat fill, permanently** — isolated to one variable on a page containing none of
   this app, with `gl.readPixels` in the same rAF tick returning a bright frame and `isContextLost()`
   false. That is what „the canvas dies at the resize step, 5 of 5 runs" always was: **the camera, not
   the world.**
3. **`page.evaluate` has no timeout in Playwright** and `setDefaultTimeout` does not apply to it, so the
   reads inside `tools/mobile/lib/ready.mjs` can hang a sweep for ever (observed: 391 s, zero CPU).
4. **An unbuckled car raises a teach modal ~2 s into every drive** that both pauses the scene and shrinks
   the play area — four frames of one lane's first sweep are photographs of that modal.
5. **A scene remount still eats drives under load.** One attempt teleported the car 88 m in 1.06 s while
   vitest was running at 2 workers. Anyone driving for evidence on this box should have nothing else
   running.

### 10 · Claims this gate REFUSES
...
Seven lanes reported: the founder's phone (**HIS-ACTUAL-PHONE**), the ten penalty points
(**THE-TEN-POINTS**), seven cockpit rows (**B27 · B35 · B55 · B58 · B60 · B64 · B67**), the seven
pedestrian streets (**B50 · B53 · B54**), the five doors and the sim citations, the look/texture
brief, and the street furniture (**B65**). This gate disbelieved all seven, opened their frames,
re-derived every headline number from raw with predicates written here, and **read Наредба № 38 out
of the ingested act itself rather than out of any lane's table.**

### 1 · The frames — 20 cockpit tiles opened, none refused for a missing seat

**Every row that is about what he sees from the driving seat was answered with a frame that has a
seat in it.** Checked before acceptance: A-pillar, dashboard, steering wheel, windscreen.

| frame opened | what is in it |
|---|---|
| `b50frames/SHEET-CROSS-L3.png` | Seven tiles, each with both A-pillars, dash, wheel, cluster at **0 км/ч D** and the interior mirror. |
| `b50frames/SHEET-SPAWN.png` | Seven tiles, same furniture, all at `y=15.0`. |
| `b50frames/L3__28-pe-bus-CROSS__y76.png` | Full cockpit; pedestrian, zebra, blue bus, kerb railing. |
| `b65furn/B65-before-after.png` | Four cockpit frames (2 before / 2 after) at `y=119` and `y=239` on `sp-creep-v1`, telemetry burned in. |
| `cockpitlane/b67/b67-step01-t36.1-y577-v119.png` | Both A-pillars, wheel, full dash row, door mirror, binnacle reading **„120 км/ч D"**, 140 roundel lit, at 119.9 km/h. |

**Two frames were correctly NOT cockpit frames and are accepted as such**
(`mlfold/mlBEFORE|mlAFTER3/iphone16-portrait--practice--text4-q-predimstvo-042.png`): the phone rows
are about the theory app's own chrome, no simulator is involved, and a cockpit would be the wrong
evidence. That is not the `/dev/scene-still` failure the earlier gates refused three times.

**B67's premise is dead, and the frame kills it.** The look-wave recorded „at motorway speed there is
NO instrument cluster in frame at all". Opened: at **119.9 km/h** the cluster, wheel, dash and both
A-pillars are all in shot. The lane's reverse-view-glance hypothesis (a held or leaked `KeyF`
producing every one of those headrest frames at any speed) is the first explanation anyone has
offered that also survives a throttle-shut coast. It is a hypothesis, it is unproven, and it is
cheap to test.

### 2 · THE TEN POINTS — his complaint is real, and this gate found it independently

**Read out of `content/law/acts/naredba-38.json` here, not taken from any lane.** приложение № 5
към чл. 36, ал. 1, т. 1 is the act's only fault classification. Its б. „в" reads „за опасна грешка
се поставят 10 наказателни точки **в следните случаи:**" and then enumerates **exactly six** cases —
counted in the ingested text: the light/controller signal, entering against traffic at a junction or
**one-way** road, failing to stop at **Б2**, the commission intervening, creating a **предпоставка за
допускане на ПТП**, and exceeding the limit by **more than 10 km/h**.

`CROSSED_SOLID_LINE` charged 10 and matched **none of the six**. Verified clause by clause:

* **Case 2 is expressly excluded by its own words** — the clause names „път с **ЕДНОПОСОЧНО**
  движение", and `engine.ts` arms the detector only when `tick.oneway === false`.
* **Case 5 was the only candidate and the detector establishes no предпоставка** — authored М1 span,
  opposing bank, moving, forward gear, sustained 0.6 s, **no other road user required and none
  queried**. `solidLineCrossSustainSec: 0.6` is documented in `types.ts` as a paint-flicker guard: it
  answers „did he really cross", never „was it dangerous".
* **The catalogue convicted itself, and this gate re-read both entries to confirm it.**
  `OVERTAKING_IN_BAN_ZONE` — знак **В24**, the explicit statutory ban on overtaking — requires a
  denoised lane change **and a real lead vehicle** (`overtakeBeat(cfg.banOvertakeLeadGapM)`,
  `engine.ts:852-857`) and bills **3**. The paint out-charged the sign it merely repeats.

**Now основна (3).** `SPEEDING_DANGEROUS` was checked against case 6 and matches to the letter:
`dangerousSpeedOverKmh: 10`, detector `speed > bands.dangerousAbove`, strictly greater.
`PEDESTRIAN_CROSSING_TOO_FAST` requires `z.pedestrianSeen` — a real pedestrian, a real предпоставка.

**The brief's own number was wrong and the lane was right: 53 codes, not 52.** Parsed from
`catalog.ts`: **14 опасна / 22 основна / 17 второстепенна** after the ruling (15/21/17 before).

### 3 · The phone — insets verified applied, then the counts re-derived

The defect this check exists to catch is a green number produced with the notch switched off. It is
**not** present. Measured over both raw runs, 160 rows each: `saRewrites > 0` on **160/160**, and
`<body>`'s padding reads back **equal to the inset asked for on every single row** — B34 portrait,
L59/R59/B21 landscape, B0 on the two deliberately zero-inset Android profiles. **0 mismatches.**

Re-derived with a predicate written for this gate (countable = settled · fonts loaded · no loading
word · surface actually mounted · a pinned bar found structurally). **160/160 countable, 0 refused,
in both runs.**

| surface | profile | BEFORE | AFTER | worst doc scroll B→A | stem clamped B→A | worst hidden stem B→A |
|---|---|---:|---:|---|---:|---|
| practice | iPhone 393×852 | **0/20** | **20/20** | 34 → 0 | 20 → 0 | 173 → 0 |
| practice | iPhone 852×393 | **0/20** | **19/20** | 32 → 11 | 10 → 0 | 74 → 0 |
| practice | Android 360×780 | 20/20 | 20/20 | 0 → 0 | 1 → 1 | 16 → 16 |
| practice | Android 780×360 | 19/20 | 19/20 | 1 → 1 | 0 → 0 | 0 → 0 |
| exam | iPhone 393×852 | **0/20** | **20/20** | 34 → 0 | 20 → 0 | 173 → 0 |
| exam | iPhone 852×393 | **0/20** | **20/20** | 21 → 0 | 11 → 2 | 74 → 24 |
| exam | Android 360×780 | 20/20 | 20/20 | 0 → 0 | 1 → 1 | 36 → 36 |
| exam | Android 780×360 | 20/20 | 20/20 | 0 → 0 | 0 → 0 | 0 → 0 |

**Every figure matches the lane's table.** The two zero-inset Android columns are byte-identical
BEFORE and AFTER — the control that shows the iPhone columns moved because of the insets and not
because of drift.

**And the frames say the same thing.** `mlBEFORE/…q-predimstvo-042.png`: the stem is sliced
**mid-glyph** after two lines, lines three to five are simply not there, and roughly 900 device px of
empty card sits below the answers. `mlAFTER3/` same name: **all eight lines and all four answers.**

### 4 · The seven streets — answered honestly, and not upgraded

**At the crossing they are seven different crossings.** Opened `SHEET-CROSS-L3.png`: three refuge
islands (2.4 / 2.0 / 2.2 m, all three present in the shipped JSON — re-read here), three raised-table
plateaus, two skewed zebras (+18°, −12°). He could tell these apart.

**Two things this gate will not let the row claim.**
1. **The strongest differentiator on that sheet is the weather, not the furniture.** `pe-rain` reads
   as a different street instantly because it is night and snowing. Strip the weather and the skew
   and the remaining separation is thinner than the table implies.
2. **`pe-bus`'s refuge island is in the data and is NOT legible from the seat.** Opened its own
   full-resolution frame at 11 m: the zebra reads as unbroken white bars. The island exists
   (`island: {widthM: 2, approachM: 9, departM: 9}`) — it is 2.0 m with a 9 m nose, the most compact
   in the family, and at that distance and angle it does not read. **A field in the JSON is not a
   thing he can see.**

**At the spawn `pe-bus`, `pe-child` and `pe-cane` still read as one composition** — same right-hand
parked row, same white-car-then-red-car, same mid-rise slabs, same dashed centreline. The lane said
three of seven and **three of seven is what the sheet shows.** `pe-clear` is close behind them.
The rows stay **PARTIAL-SEEN**. This is not an upgrade.

### 5 · The look — it is object work, it is real, and the cost is measured

Opened `B65-before-after.png`. **This is not another lighting constant, and it is not texture work
either.** The road surface is visually identical between the two columns; what changed is that
objects now exist. At `y=239` BEFORE, the left of the windscreen is open green field with one lone
tree and the carriageway running to a bare horizon. AFTER: frontage on both kerbs to the end of the
street, lamp columns, an overhead line, a pavement parapet, varied tree silhouettes.

**The texture lane shipped nothing and said so.** Its measurement stands and is the more useful
deliverable: the surfaces are *not* untextured — road, ground and sidewalk all bind PBR
colour+normal+roughness — but detail **collapses with distance** (against a validated flat control
of 0.16 and a detailed control of 5.02, the carriageway measures 1.34–1.60 at 8 m and **0.72–0.93 at
20 m**). The next lane should fix falloff, not author new base maps.

**THE COST — measured in this gate, through `buildWorldGeometry`, against `environment/perfBudget.ts`.**

| map | draw calls | triangles | lamps | poles | railings | skid marks |
|---|---:|---:|---:|---:|---:|---:|
| `sp-creep-v1` | 59 | 25,778 | 11 | 11 | 34 | 6 |
| `sp-danger-v1` | 59 | 25,844 | 12 | 12 | 36 | 6 |
| `pe-clear-v1` | 63 | 25,634 | 9 | 7 | 24 | 0 |
| `pe-cane-v1` | **65** | 25,828 | 7 | 8 | 20 | 0 |
| `d2-v1` (exam) | **66** | 75,826 | 280 | 0 | 0 | 66 |
| `district-v1` | 59 | 81,532 | 134 | 0 | 0 | 32 |

`low` tier (the phone — **the tier his FPS complaint is about**): **70 draws soft / 100 hard,
250 000 tris soft / 300 000 hard.**

**Verdict on cost: it does not drop frames, but it has eaten most of the remaining headroom.**
Triangles are comfortable everywhere — the worst map is at **33% of the low-tier soft budget**. Draw
calls are not: `pe-cane` at **65** and `d2-v1` at **66** sit **5 and 4 calls under the soft cap**.
The whole furniture pass cost **+3 draws**, each mounted only when its list is non-empty, and
**`public/` gained zero bytes** — verified by `git status`: not one new binary asset, only world JSON.
**One more instanced kind added to the PE family breaches the soft cap.** That is the number the next
lane needs and it is why this table is here rather than a sentence saying „within budget".

*(Correction to the B65 lane's own table: it reported 59–63 draws. Measured here, the range is
**59–66** — `pe-cane` 65 and `d2-v1` 66 were not in it.)*

### 6 · The fourteen surfaces — Door 6 is closed, and nothing else opened

Re-run with the forbidden set built **from `content/`**, never from the gate: **290 of 1 089**
questions forbidden. Every one of the 1 089 rows driven through `buildReviewRow` in all three pin
states:

| pin state | integrity distribution | rows speaking a forbidden string |
|---|---|---:|
| current | `verified 799` · `withdrawn 290` | **0** |
| stale | `moved 799` · `withdrawn 290` | **0** |
| absent | `unpinned 799` · `withdrawn 290` | **0** |

**Exactly the distribution the lane reported.** The verdict never moves: points, maxPoints and
correct are identical across all three states.

**THE PROBE HAD TO PROVE ITS OWN REACH BEFORE THE GREEN WAS WORTH ANYTHING** — a check that reads
nothing passes trivially. Pinned: a `verified` row **must** carry a stem, an explanation and option
text; a `withdrawn` row must carry a claim-free placeholder („Този въпрос не се показва в
прегледа."), an empty explanation, empty options and **no** `lawRefs`. Both hold.

### 7 · A NINTH INSTRUMENT DEFECT — this gate's own, and it is the same bug for the third time

**My first enumeration reported 34 leaks. All 34 were false.** Naive `includes()` over the serialised
row matches a forbidden row's whole option text against a *fragment inside a longer approved one*:
„Движение на заден ход" (21 chars), „Около 10 метра" (14), „На кръстовище на равнозначни пътища" (35).
All three rows were `status=approved`. Exact field comparison takes it to **0**.

This is the third time in this programme that the same class of instrument error has produced a
confident wrong number — the predecessor's 38 phantom leaks, its 331 phantom citation refusals from
an ASCII `\b` against Cyrillic, and now these 34. **The lesson is not „be careful"; it is that a
probe reporting a defect must be made to fail on a known-clean input before its output is quoted.**

*(A second, smaller one: this gate's first perf probe read `world.skidMarks` and reported 0 for
every map. The field is `world.stats.skidMarks`. Re-measured before anything was written down —
`sp-creep-v1` is **6** and `d2-v1` is **66**, exactly what the B65 lane claimed. The lane was right
and the instrument was wrong.)*

### 8 · Re-recorded traces

**Four, and they are clean.** `sc-jx-blocked-exit/mistake-impatient-red`,
`sc-rb-busy-gap/mistake-barge-lead`, `sc-rb-exit-signal/mistake-barge-entry`,
`sc-roundabout-entry/mistake-barge-entry` (eight files — each has a `content/` and a
`platform/public/` copy). Spot-checked `sc-rb-busy-gap` against its scenario by parsing both
revisions: **`meta` IDENTICAL, `samples` IDENTICAL (361 → 361), only `events` changed** — the
annotation string the citation wave rewrote. No geometry moved.

**And the districts whose geometry DID move needed none, which this gate checked rather than
accepted.** The seven PE districts gained islands, tables and skews, but the crossing point does not
move and no edge changes width, so the rail at `x=4.06` that every committed PE trace drives is
untouched. All 22 PE traces replay green in the full run. **The four determinism failures that
parked the previous attempt at this wave are gone.**

### 9 · The gate itself — run here, with the numbers

```
cd platform && npx tsc --noEmit --incremental false   → EXIT 0, ZERO OUTPUT
npm run validate:content                              → OK, all structural and referential checks passed
                                                        1 089 questions (799 approved / 290 needs-review / 0 draft)
                                                        152 concepts · 77 signs · answer-leak sweep 17 scopes gated, 0 blocking
npx vitest run --maxWorkers=4                         → Test Files  2 failed | 734 passed | 1 skipped (737)
                                                        Tests       2 failed | 11 039 passed | 170 skipped (11 211)
npm run test:tools                                    → 192 tests, 191 pass, 1 fail (PRE-EXISTING — see below)
```

**THE TWO REDS ARE EXACTLY THE TWO PROTECTED REDS AND NOTHING ELSE.**
`modules/exam/__tests__/content-bank.test.ts` („has no dark, threadbare or under-represented topic")
and `modules/lesson/__tests__/compose.test.ts` („gives every lesson at least one quiz beat") — both
the 29 quarantined first-aid questions awaiting the founder's medical ruling. Neither was touched and
neither was made green.

**Every red the seven lanes reported mid-session is gone.** The six `pe-districts` failures from the
lamp-column collision, the four trace-determinism failures, the `clearanceProvenance` pair — all
green in this run. Confirmed by targeted re-run: `pe-districts` + `b65-street-furniture` +
`naredba-38-classification` + `law-citations` + `review-integrity` = **5 files, 178 tests, all pass.**

**The one `test:tools` failure is pre-existing at HEAD and this gate proved it rather than asserting
it:** `content/medical/tools/fetch.sh` and `content/sources/tools/fetch.sh` are mode `100644` in
commit `d7ec746` (`git ls-tree HEAD`) and are **not modified by this wave** (`git status` is empty for
both). No lane added a shell script.

**Hygiene.** `platform/tsconfig.json` is byte-identical to HEAD — no `.next-*` glob. No PNG is
untracked anywhere in the repo; the only untracked files are 13 intended source/content additions.
Both verification probes this gate wrote were deleted and `git status` confirms it. Four scratch dist
dirs (`.next-c5rsw`, `.next-harness`, `.next-rig`, `.next-ttlane`) are live from other lanes; they are
gitignored and excluded from `tsconfig`, so they cost disk, not correctness.

### 10 · Claims this gate REFUSES

| claim | refused because |
|---|---|
| **`THE-TEN-POINTS` · `FIXED-SEEN`** | **The finding is right, the work is right, the verdict label is not.** The row is about a number he **saw on a result screen**, and nobody has opened one reading „3 наказателни точки". The lane says so itself in its own hand-over. By this register's own rule — *„`FIXED-CODE-ONLY` is not a verdict"* — the row stays open. One drive of `sc-ov-solid-return` L3 with the result screen photographed closes it. |
| **`B50`/`B53` · „all seven differ at the crossing"** | **True as far as it goes, and it goes less far than the sentence implies.** Held to `PARTIAL-SEEN`, which is where the lane already put it. The refuge island on `pe-bus` is in the JSON and cannot be seen from the seat, and the loudest differentiator on the sheet is `pe-rain`'s weather. |
| **`B65` · „draws 59–63"** | Measured here at **59–66**. `pe-cane` (65) and `d2-v1` (66) were absent from the lane's table, and they are the two closest to the low-tier soft cap of 70. |

---

## The gate — VERIFY-BY-DRIVING, 2026-08-05 (the fourth pass of the day). **ALL SIX CHECKS DRIVEN. `tsc` IS 0. THE SIMULATOR TREE IS GREEN AND EVERY RED IN THE REPO BELONGS TO ANOTHER WAVE.**

The last gate refused five claims, three of them „`tsc` exit 0" from three different lanes. So this
one ran every command itself and pasted the counts, and it drove every check instead of reading a
diff: **21 browser drives on the shipped `LessonPlayShell` — two of them on the real keyboard
channel rather than the rig's injected axes — and 12 frames opened and read, not merely written.**
Nothing below is quoted from a lane's report.

**The four defects this wave fixed are recorded as `D1`–`D4` below, as rows in their own right.**
None of them is one of his 107. The reverse bug, the silent minute, the six-metre gap between the
grader and the paint, and the two lessons that could not be finished were all found by measuring or
by driving — and **a register that only tracks what he happened to notice will keep missing the
things he did not.** The 107-row count is deliberately left alone; these are a separate ledger.

### D1 — a held brake became a floored reverse accelerator, and the input that triggered it is the single most correct thing this product teaches

| | |
|---|---|
| **What was wrong** | `ReverseAssist` toggled D→R after 0.35 s of a HELD brake at a standstill; the in-R pedal swap then handed that same held pedal to the reverse throttle. `VehicleSim`'s R branch does `engineTotal = −REVERSE_FORCE_N · throttle` = **−3000 N, capped at 25 km/h**. Standing on the brake at a Б2 line — the behaviour a whole lesson exists to build — accelerated the car backwards at 16.8 km/h into whatever was behind it, and stripped its service brake at the same instant. |
| **Fix** | Two laws in the INPUT PATH, not in the trigger. **LAW 1:** a press arms only on its rising edge, only at a standstill, and only after `REVERSE_ASSIST_LIFT_S` = 0.25 s of a genuinely lifted pedal at rest. **LAW 2 (`ReversePedalMapper`):** whichever channel inherits the throttle role at a remap flip is DISOWNED until genuinely released — zero throttle, and it goes on braking with the value the driver is applying. Deliberately asymmetric: a held pedal may always gain „stop", never „go". `GatedSimInput.read()` now goes through the mapper, so the `[`/`]` keys, the touch gear sheet and the cockpit lever are covered too. |
| **Driven, not asserted** | `sc-junction-stop@L1`, brake held continuously for **34.98 s / 1955 frames**: `brake` **1.0000 on every one of them**, `throttle` **0.0000 on every one of them**, gear values seen **`[1]` — D only, never −1**, max \|v\| **0.0156 km/h**, position drift **0.0011 m**, **0 collisions, 0 violations, 0 session-clock resets.** It then drove off and earned `FULL_STOP_AT_STOP_SIGN` „Правилно спиране на знак Б2". |
| **Provoked the other way** | Real keyboard, `KeyS` pressed at 20.6 km/h and **never lifted for 52 s**: `[` tapped twice mid-hold → D→N→**R** — **925 frames in R, max \|v\| 0.0010 km/h, y span 0.0000 m**; `]` twice back to D → **456 frames, same zeros**. The measured-pedal repeat (brake injected by the rig so the value is in the record): **1227 frames in gear R at `brk` 1.00, y span 0.0000 m.** Then foot off 1.5 s, select R, press again → the car reverses to 8.9 km/h. **The feature the founder asked for on 2026-07-17 still works; only the gesture changed: stop, lift, press.** |
| **Frames** | `scratchpad/verify/b2hold-step01-t35.9-v0.png` — cockpit, **0 км/ч, D**, `thr=0.00 brk=1.00 events=0`, step „HOLD BRAKE 35s". · `scratchpad/verify/prov2-D-R-plus20s.png` — reverse camera swung round, `gear=−1 v=0.0 brk=1.00`, and **„Кола отзад · 2 м"** on screen: a car two metres off the rear bumper, which is exactly what the old code drove into. |
| **Pinned** | `engine/__tests__/reverseAssist.test.ts` — 30 tests, including the 30 s Б2 hold, the rolling press, the sub-threshold lift, the pre-drive handover, 600 frames of a brake held into R, and the mirror case (braking to a stop mid-reverse must not pop the car back into D). |

### D2 — the minute in which the student is most correct was the minute the product said nothing

| | |
|---|---|
| **What was wrong** | A lawful give-way wait produced no words at all. The engine knew the driver was holding — `YieldWaitState` published a boolean — but nothing downstream could say WHAT he was waiting for, so the screen was silent for the whole wait and then graded him on the way out. That is a requirement-zero violation (doc 64 THEO-4) in the one place it matters most. |
| **Fix** | `YieldWaitState` now publishes a `reason`; `advisor.ts` gained `YIELD_VOICE_COPY` (five reasons × four stages), `yieldWaitAdvisorPrompt`, and the pure `stepYieldVoice` fold, folded by the engine only when `!examMode` — the advisor's own single gate, not a new one. Three stages: **name the duty at 1.2 s**, **confirm at 10 s that the waiting IS the manoeuvre**, **judge the gap 4 s after the wheels turn** — and the verdict is dropped silently if a yield-family fault is graded inside that window. |
| **The first thing it says — transcribed from the screen, not from the source** | Wheels stopped at t = 24.27 s. At **t = 25.24 s, 0.97 s later**, verbatim: „**📚 НАУЧИ · Защо чакаш: в кръга имат предимство** — Спрял си правилно. На входа на кръгово кръстовище не може да стои знак „Път с предимство" — там винаги е Б1 или Б2, тоест ти си на пътя без предимство и пропускаш движещите се в кръга. Гледай НАЛЯВО. Интервалът, който чакаш, е такъв, че да влезеш и да набереш скоростта на кръга, без движещият се в него да намалява заради теб." + the chip „ЗДвП чл. 50, ал. 1; Наредба № РД-02-21-1/2023 правила за поставяне на знак Б3". **It is not a number. It names the duty, the reason, where to look and what a good gap is.** |
| **And it does not nag** | **60.01 s of standing still produced exactly two lines.** The second, at t = 33.94 s: „Чакането Е маневрата — Стоиш вече 10 секунди и това е правилно… Тези секунди не ти струват нито точка и се изваждат от ориентировъчното време на урока". Then **nothing for the remaining 50 s.** The advisor card is constant for the whole wait by design: „Чакаш правилно — в кръга имат предимство. Гледай НАЛЯВО и тръгвай, когато можеш да влезеш, без някой в кръга да намалява заради теб." After the wheels turned, one verdict at t = 88.48: „Интервалът беше добър". |
| **All three other controls driven too** | **Б2** (`sc-junction-stop`): „Защо чакаш: знак Б2 „Спри! Пропусни движението!"" at t = 36.62 (0.83 s after the stop) → „Чакането Е маневрата" at t = 45.42 → 25 s of silence → „Спря докрай и пропусна" at t = 74.99. **Б1** (`sc-jx-giveway-b1`): „Защо чакаш: знак Б1 „Пропусни движението"" t = 75.85 → „Чакането Е маневрата" t = 84.63 → „Пропусна и тръгна в истински интервал" t = 102.24. |
| **Exam mode is silent, and that was driven, not trusted** | `sc-roundabout-entry@L4` („Изпитни условия · ИЗПИТ"), stopped on the same paint, **45.01 s at a standstill: ZERO new lines of text on screen.** Grepped the transcript for every voice string — „Защо чакаш", „Чакането Е маневрата", „Интервалът беше добър", „Чакаш правилно", „НАУЧИ" — **all absent.** |
| **Frames** | `scratchpad/verify/rbfull2-step01-t26.1-v0.png` — cockpit at the paint, 0 км/ч D, `brk=1.00`, the advisor card in the top rail and the circulating car visible in the ring. · `scratchpad/verify/exam-A-wait-t30.png` — the same pose in exam mode: **no card, no toast, no shadow line, no route line, `events=0`.** |
| **One thing the transcript caught that the source does not** | During the wait the **shadow-demo subtitle track keeps talking on a 49 s loop** — „Приближаваме кръговото — намали отрано и гледай наляво" / „Кола се движи в кръга — тя е с предимство. Спри и я пропусни." / „Тя премина — влизаме плътно след нея, в нейния интервал." Correct in content and it is the demo's own voice, not the advisor's, but he asked for EVERY word and these are words. Recorded here rather than filtered out. |
| **Pinned** | `lessons/__tests__/yield-voice.test.ts` — 21 tests, including two minutes of continuous waiting producing exactly two lines, the creep-and-restop episode rule, the rolling give-way that is never lectured, the exam silence, the verdict dropped on a graded fault, and the citation rules (retrieved byte-identical to the graded card; no article number on an act the repo does not hold; the roundabout never cites the retracted чл. 50а). |

### D3 — the roundabout grader was armed six metres inside the paint, so the student's whole decision happened where the instrument was blind

| | |
|---|---|
| **What was wrong** | The entry tracker armed at `ringRadius + ROUNDABOUT_ENTRY_MARGIN_M` — a constant bolted to the RING. The give-way paint is bolted to the MOUTH, and on `rb-mini-v1` it is at **35.73 m** from the ring centre against an armed reach of **30.00 m**. A student who stops exactly where the road tells him to stop stands **outside the instrument** — so his stop, his wait and the circulator he let past were all invisible, and `YIELDED_TO_PRIORITY` was unreachable by construction. **19 of 22 mouths across the six shipped rings were outside.** |
| **Fix** | `roundaboutGiveWayReachM` derives an OBSERVATION reach per ring from that ring's own М7 line plus the М18 triangle run. `ROUNDABOUT_ENTRY_MARGIN_M` keeps its old job as the COMMIT radius and nothing else: a violation may still only fire, and its sustain clock may still only start, inside it. **The change is additive on the punishing side.** |
| **Driven — stop on the paint, wait, pull out clean** | `sc-roundabout-entry@L1`. Stopped at (4.06, −37.31) = **37.53 m from the ring centre** (the origin sample; the bumper is on the 35.73 m paint). Stood **30.01 s**. Pulled out on a clear gap, circulated, and `YIELDED_TO_PRIORITY` „Правилно отстъпено предимство" landed at (27.29, −12.51) = **30.02 m — exactly the unchanged commit radius.** **Zero yield violations on that drive.** |
| **Driven — the barge still convicts** | Same map, same rung: stop 10 s, then floor it. `FAILED_TO_YIELD <roundabout>` **опасна, 10 т.** at (4.06, −24.31) = **24.65 m** — inside the commit radius, outside the ring — followed **0.74 s later** by `COLLISION <vehicle>`, which is the independent proof that the car really was there. Frame `scratchpad/verify/barge3-card-t47.0.png`. |
| **Honest caveat, because it looks like a miss and is not** | Two barges that **never slowed at all** (21.6 and 13.6 km/h straight through) produced **no conviction — and no collision either.** The staged circulator was not in the band at the moment of entry, so entering was lawful and the detector was right to stay quiet. It does mean a live barge is not reproducible on demand; see the open list. |
| **The traces: the answer is SIX, not five, and none of them moved** | The margin did **not** move — the watch radius was added BESIDE it — so nothing needed re-recording. Verified rather than argued: `RECORD_TRACES=1` over all six roundabout gates (`sc-roundabout-entry`, `sc-rb-busy-gap`, `sc-rb-circulate-priority`, `sc-rb-exit-signal`, `sc-rb-lane-choice`, `sc-rb-ped-exit`) → **6 files, 81 tests, green**, and `git status content/traces platform/public/traces` afterwards shows **zero roundabout traces changed.** All 18 (6 shadow + 12 mistake) came back byte-identical. **This register has been saying „five shadow traces" and there are six.** |
| **New finding, and it only shows up if you open the frame** | The conviction is now produced by the right adjudicator and **explained by the wrong law.** See the open list, row 1. |

### D4 — two lessons could not be finished at all, and both now finish clean

| | |
|---|---|
| **`jxg-giveway-b1` — the junction mouth he collided in** | Driven end to end at L1 with glances at both Б1 lines: **3/3 objectives, `phase=completed`, 0 violations, 0 collisions, gears `[1,2]` — never R.** `YIELDED_TO_PRIORITY` at t = 77.52. **198 consecutive frames at a steady 15.6 km/h through y = 140…155 — the exact band of his y = 146.00 collision — with no contact of any kind.** Result screen: **„0 наказателни точки · ИЗДЪРЖАН"**, Опасни 0 / Основни 0 / Второстепенни 0, ★★★, 86 с при ориентир 80 с. Frames: `scratchpad/verify/jxg-step08-t98.2-v1.png` (pulling away from the second Б1 with the mouth ahead **empty**, the advisor card „Чакаш правилно — знак Б1…") and `scratchpad/verify/jxg-end-t109.5-v17.png` (the result). |
| **`sc-follow-standstill` — the queue drill** | Driven at the drill's own nominal pace, easing and resuming with the lead's authored profile: **2/2 objectives, `phase=completed`, 0 violations, 0 collisions**, minimum lead gap over the whole drive **6.66 m**, at rest **6.7 m = 4.02 s**. Result screen: **„0 наказателни точки · ИЗДЪРЖАН"**, ★★★, 88 с в ориентира от 100 с. Frame `scratchpad/verify/queue2-end-t87.6-v6.png`. |
| **And it now TEACHES, which is the half that was missing** | Held at a flat 19.6 km/h while the lead eased — the fault the drill exists for — the new основна `CLOSING_ON_LEAD_TOO_FAST` raised „**Настигаш колата отпред** — Колата пред теб намалява, а ти не намаляваш с нея… Точно така се стига до удар в спряла колона: не защото си карал бързо, а защото си влязъл в чужда спирачка със своята скорост. **Дистанция в момента: 1,2 с (6 м) — дръж поне 2 с**" (ЗДвП чл. 23; чл. 20, ал. 2) at t = 57.24 — **1.97 s and 10.7 m before the first contact** at t = 59.21. Before this detector existed the entire rule log for that drive was a `CLEAN_DRIVING` commendation on the way into the queue. Frame `scratchpad/verify/queue-card-t57.2.png`, with the telemetry burned into the same PNG: `v=19.6km/h y=243.9 lead=6.4m = 1.17s`. |
| **Honest caveat** | A rig that holds 20 km/h and **never lifts** still ends in two collisions — correctly. The drill's whole subject is lifting; the pass above is a drive that lifts, and the warning above is what the student gets 2 s before if he does not. |

### The gate itself — every command run by me, with the counts pasted

- **`cd platform && npx tsc --noEmit --incremental false` → exit 0, ZERO error lines.** `--incremental false` deliberately, so no stale `tsconfig.tsbuildinfo` could hide anything. Run three times across the session (start, mid, final) — 0 every time. **The red three lanes reported and the last gate carried — 2 errors in `lessons/__tests__/b5-crash-pin-rescue.test.ts` (`preDriveRequired`, `.done`) — is GONE:** that file now compiles and its 4 tests pass.
- **`npx vitest run src/modules/sim --maxWorkers=2 --testTimeout=180000` → 528 files, 8 058 passed, 164 skipped, 0 FAILED, 230.49 s.** The whole simulator is green in one run.
- **`npx vitest run` (the whole tree) → 737 files: 731 passed, 5 failed, 1 skipped. 11 169 tests: 10 994 passed, 5 failed, 170 skipped. 305.35 s.** Every failure named and owned, below.
- **The four the last gate called ours are ALL GREEN, run by name and alone:** `pe-districts.test.ts` **110/110** · `traffic/__tests__/ped-through-parked.test.ts` **4/4** · `traffic/__tests__/scenery-sightline.test.ts` **8/8** *(it lives in `traffic/`, not `world/` — the last two gates printed the wrong path)* · `app/dev/__tests__/dev-surfaces-gated.test.ts`, the `KNIJKA_DIST_DIR` timeout, **40/40 in 5.95 s.** Plus `src/lib/tsconfigHygiene.test.ts` **4/4**.
- **This wave's own eight batteries: 8 files, 99 tests, all green** — `reverseAssist` (30), `yield-voice` (21), `roundabout-armed-at-the-paint`, `roundabout-mouth-not-right-hand-rule`, `b5-crash-pin-rescue` (4), `b5-junction-mouth-clear`, `b5-queue-drill-survivable`, `junction-scan-control-copy` (3).
- **`npx eslint` on the twelve source files this wave's four defects live in → exit 0, no output.** *(The repo-wide `npx eslint` reports 47 562 problems / 2 804 errors because it sweeps generated and build output; `npx eslint src/modules/sim src/components/sim src/app/dev` reports 144 problems / 47 errors, all long-standing React-hooks rules such as `WorldProps.tsx:1116`. Neither is this wave's and neither is the CI gate — said here so nobody claims eslint is clean.)*

#### Every failing test in the repo, and whose it is

| # | failure | owner | why it is theirs |
|---:|---|---|---|
| 1–2 | `../tools/mobile/navigation.test.mjs` and `../tools/mobile/ready.test.mjs` — **„No test suite found"** | the mobile-harness lane | Both are **untracked** (`??`) and both do `import test from "node:test"`. `vitest.config.ts`'s third include glob, `../tools/mobile/**/*.test.mjs`, sweeps them in; vitest cannot run node:test and counts them as failed suites. **Their own sibling `settle.test.mjs` carries the comment saying node:test files in this directory belong to `scripts/tools-tests.mjs` — the glob does not honour it.** `navigation.test.mjs` additionally burns **three 180 s Playwright navigations against `127.0.0.1:9`** inside the unit run. |
| 3–5 | `src/app/api/review/route.test.ts` ×3 — **„No `parseQueue` export is defined on the `@/modules/content-admin` mock"** | the review / api wave | `src/app/api/review/route.ts` is **modified in the working tree** and calls `parseQueue(params.get("queue"))` at line 45; the test's `vi.mock` was not updated to return it. |
| 6 | `src/modules/exam/__tests__/content-bank.test.ts` — **`REVIEW_DEBT: ptp-i-parva-pomosht: only 31/64 (48%) approved`** | the theory / content wave | `content/questions/ptp-i-parva-pomosht.json` is dirty in the working tree. Supply audit, not code. |
| 7 | `src/modules/lesson/__tests__/compose.test.ts` — **`l-accidents-first-aid` has no quiz beat** | the same content wave | `content/lessons/l-accidents-first-aid.json` is dirty in the working tree and was already dirty when this session opened. |

**Nothing in `src/modules/sim` fails. Nothing this wave touched fails.** *(5 failed files = 2 failed suites + 3 files carrying 5 failed tests.)*

#### Hygiene, checked rather than claimed

- **`platform/tsconfig.json` is byte-identical to HEAD** — `git diff` is empty. Its `include` carries only `.next/types` and `.next/dev/types`; **zero injected `.next-*` glob families**, and `exclude` still holds `[".next-*"]`.
- **No dist dir was created by this gate.** Five `.next*` directories exist and all five predate it — every drive ran against the **pre-existing shared rig server on :3200** (`.next-rig`).
- **Untracked file count: 87 at the start of the session, 87 at the end.** Every one of ~60 frames and both telemetry dumps went to the scratchpad.
- **Exactly two untracked binaries exist in the whole repository** — `tools/clips/headless/sheets/sc-jx-giveway-b1__m0.strip.png` and `__m1.strip.png`, **mtime 2026-08-04 23:55**, i.e. before this session opened.

#### On „is `tools/clips/headless/sheets/` ignored now?" — no, and this gate refuses to ignore it

**48 of the 50 files in that directory are TRACKED**, and `.gitignore` already carries an explicit
carve-out saying so (`# NOTE: tools/clips/headless/sheets/ IS tracked — those contact sheets are
referenced from the docs`). **This register cites the two untracked ones by path in four places**
(the reserved-files note, the refused-claims table, the clip-strip table, and the B29 row, where
`sheets/sc-jx-giveway-b1__m0.strip.png` is the „opened" evidence that closed the row FIXED-SEEN).

Ignoring the directory would not untrack the 48, would not delete the 2, and **would break the
citation that closes B29.** The previous gate's instruction to „move them to a scratchpad" would
delete that evidence outright. **The right action is to COMMIT those two, deliberately, 738 KB, and
the row below says so.** Until someone does, the honest answer to the check is: *the directory is
not ignored, on purpose, and two PNGs are untracked in it.*

---

## The gate — THREE-LANE COCKPIT gate, 2026-08-05 (later the same day). **EVERY FRAME HAD A SEAT IN IT. ONE LANE WAS RIGHT TO REFUSE ITS OWN BRIEF, AND THE TREE IS GREEN.**

Three lanes reported — the roundabout shape (**B16**), the priority road's speed repeat (**B34**), and
the seven pedestrian streets (**B50 / B53 / B54**). This gate disbelieved all three, opened every frame
they cited, and **re-derived the two headline measurements from the shipped code rather than reading
their tables.**

### First, the thing the last gate had to refuse three times: the frames

**Not one frame this round was shot through `/dev/scene-still`, and none had to be refused.** The last
gate threw out B27, B34 and B60 because a world fact proved in a wide-angle still does not answer a row
about what a driver sees. Every frame below was checked for the furniture before it was accepted —
**both A-pillars, the dashboard, the wheel, the instrument cluster and the interior mirror.**

| frame | what is in it |
|---|---|
| `b16round/shots/b16-before-canvas-y-45.png` | Both A-pillars, dash, wheel, cluster at **0 км/ч D**, interior mirror, the Б1 give-way triangle on the right kerb. Stopped 23.7 m off the ring. |
| `b34lane/shots2/b34lane-xm19-t59.3-v14.png` | Both A-pillars, dash, wheel, cluster at **14 км/ч D**, left wing mirror, telemetry burned in: `t=59.62s v=13.6km/h x=-23.0 y=4.2 hdg=279°`. The В26 «50» repeat standing in the windscreen. |
| `zebra/frames/SHEET-PE-7-CARRIAGEWAY.png` | Seven tiles, each with A-pillar, wheel, dash and cluster. |
| `b16round/shots/b16-after46-canvas-y-73.8.png` · `b16-r31-canvas-y-58.8.png` | Same cockpit, two candidate ring radii at an equal 23.7 m to the kerb. |

### The measurement I re-derived instead of believing — and it refutes this register's own number

**Every district in the corpus was swept, not the six the lane chose.** `content/world` holds 100
districts and **exactly six register a ring.** Running the shipped `analyzeNetwork` +
`analyzeRoundabouts` over all of them, sampling the circle in 0.1° buckets:

| district | ring R | arm half-width | kerb arc (sum) | kerb arc (union) |
|---|---:|---:|---:|---:|
| `rb-mini-v1` | 17.85 m | 8.13 m | **152.9°** | 152.8° |
| `rb-ped-v1` | 17.85 m | 8.13 m | **152.9°** | 152.8° |
| `rb-2lane-v1` | 25.78 m | 16.25 m | **109.1°** | 108.8° |
| `rb-single-v1` | 37.68 m | 8.13 m | **254.1°** | 254.4° |
| `district-v1` `rb-1` (real Sofia OSM) | 19.01 m | **4.06 m** | **252.8°** | 252.8° |
| `d2-v1` `rb-1` (OSM, island refused) | 26.69 m | 4.06–12.13 m | **127.1°** | 187.3° |

Those reproduce the 153 / 254 figures this register has carried since 2026-08-03, so it is the same
measure. **Two things fall out of it that no lane had to be trusted for.**

**1. This register's own fix number is wrong, and it has been wrong in print for two gates.** Line
"`rb-mini` needs radius ≈ **31 m** instead of 18" does not reach the 270° bar it was written to
satisfy. At R = 31 the mouths are `4 × 2 × asin(9.625 / 35.06)` = 127.5°, leaving **232.4° of kerb —
37.6° short.** Solving `asin((armHalf + 1.5) / (R + ringHalf)) = 11.25°` gives the real break-even:
**R = 45.3 m** for the three single-lane families (ship 46 → 271.3°) and **R = 83.0 m** for `rb-2lane`
(ship 84 → 271.1°). That is **2.6× and 3.2×**, not 1.7×. Corrected in place below.

**2. The builder is innocent and the proof is in the table.** `district-v1`'s real-Sofia ring gets
**252.8°** from the same code at a nearly identical **R = 19.01** — because its arms register at
**4.06 m** half-width where `rb-mini`'s register at **8.13 m**. The generators author the arms at
`PERCEPTUAL_ROAD_SCALE` 2.5 and the ring radius at real-world scale. **The mouths are not too wide;
the ring is too small for the roads we drew into it** — and 18 × 2.5 = 45 is within half a metre of
the measured break-even.

### What I refused, and what I refused to reverse

- **B16 stays BROKEN-SEEN and nothing was landed — which was the right call, not a failure.** The lane
  proved the change needs three `lessons/scenario/templates-*.ts` files that pin the ring **by value**
  (`templates-flow.ts` hard-codes `acceptBeforeMarkM 1.725` against the district's М8 bars at
  `y=-35.725`; `templates-roundabout.ts` declares `RING_R = 18`), all of `content/world`, **36 trace
  files**, and that it would red two **untracked** runtime probes another lane is writing this minute
  which hard-code `MOUTH = { x: 0, y: -18 }`. I verified the blast radius is untouched: all four
  `rb-*` districts and their public copies, all 18 committed roundabout traces and their 18 published
  copies, the four generators and the three templates are **byte-identical to HEAD**.
- **A lane's cockpit note that outranks its own brief.** I opened both candidate frames at an equal
  23.7 m to the kerb. **At R = 46 the top-down is a perfect annulus and the windscreen reads worse
  than at R = 31** — a 41.5 m island fills the horizon as parkland and the ring's curvature leaves the
  glass, so it reads as a T-junction. **270° is a top-down metric.** Which shape he meant is a ruling,
  and this gate will not guess it. Both frames are cited on the row.
- **B50 / B53 / B54 stay PARTIAL. I opened the contact sheet and I am not reversing the last gate out
  of politeness.** `pe-rain` (night, rain, bare verge) and `pe-clear` (widest at the seat, low tan
  arcade) are unmistakable. **`pe-dart`, `pe-slow`, `pe-cane`, `pe-bus` and `pe-child` still read as
  one composition** — grey-and-gold slab blocks either side, one dashed line down the middle, one row
  of parked cars on the right at varying distance. The differences are real and you have to look for
  them. Five of seven is not seven.
- **Two numbers in the lanes' own reports are wrong and are corrected here rather than reprinted.**
  (a) B34 reported "**four** new В26" on `tj-emerge-v1`; its own enumeration lists three and its own
  6 → 9 total is +3. I measured **three**, all at station 35.13 m from the node. (b) B50 reported the
  spawn "byte-identical to HEAD on all seven"; **x, y and heading are — the record is not.** On six of
  the seven the spawn's `edgeId` moved `pe-e-street` → `pe-e-street-s1` (`-s2` on `pe-child`), and
  `pe-e-street` itself lost its first 18–55 m to the new chain. Not a defect — see the traces below —
  but the register will not carry "byte-identical" for a field that changed.

### The trace question, answered by opening a trace instead of counting files

**PE: 0 of 22 committed traces re-recorded, and that is correct.** The rule is that a district whose
geometry moved must have its traces re-recorded, and this geometry did move — the street is now a
chain of collinear segments and six districts carry 4 m kerbside bays. I opened
`content/traces/sc-crossing-white-cane/shadow-correct.trace.json`: **918 samples, and `x` is 4.0600 on
every one of them**, y running 15.00 → 135.95, eight events that read exactly as the drill does (the
white cane steps out, full stop, „търпение", he clears, done). `edgeParkingWidthM` adds the band
**outside** `edgeTravelHalfWidth`, so the rail the trace drives never moved — and the traces prove it
by replaying green through the production stack. **Roundabout: 0 of 36 needed, because nothing landed.**

### The gate itself — exact numbers, run by me

- **`cd platform && npx tsc --noEmit` → exit 0, no output.** The last gate's red #1 — *"exits 2 with 9
  errors, seven of them in `pe-districts.test.ts`"* — **is closed.** *(My first run showed one error,
  `lessons/engine.ts(750): Cannot find name 'stepYieldVoice'`. It was a mid-write snapshot: the import
  landed on line 53 seconds later and the second run was clean. Named here because anyone gating
  against a shared tree at 01:37 will see it too.)*
- **`npx vitest run src/modules/sim/world src/modules/sim/traffic src/modules/sim/traces` → 284 files,
  4 035 tests, 4 034 passed, 1 failed — and the one failure was my own throwaway probe.** Zero product
  reds. `pe-districts.test.ts` green, `ped-through-parked.test.ts` **green** (it was red at the last
  gate and red while B34 ran), `scenery-sightline.test.ts` green, `streetwall.test.ts` green, every
  roundabout suite green, all 22 PE traces and all 18 roundabout traces green.
- **`npx vitest run src/modules/sim/{lessons,rules,runtime,engine,orchestrator,scene}` → 196 files,
  3 521 tests, 3 355 passed, 164 skipped, **2 failed** — and both belong to a fourth lane that is
  writing right now.** `orchestrator/events-integration.test.ts` *"yielding at the entry while the car
  circulates earns the commendation"* and `lessons/scenario/s-w3-bot-completion.test.ts` *"the LIVE
  session earns `YIELDED_TO_PRIORITY`"*. **Traced, not assumed:** the commendation is emitted at
  `rules/engine.ts:1910`, and `rules/engine.ts` was modified at **01:44:10** — after all three of these
  lanes had finished (00:32 / 00:38 / 01:13) — with `lessons/engine.ts` (02:12), `lessons/advisor.ts`
  (01:37) and `runtime/worldRuntime.ts` (02:20) still moving as this was written. That is the
  yield-voice lane, in two reserved files. **Neither failing test loads a PE map, and B16 landed
  nothing at all**, so no path exists from these three lanes to either red.
- **Across both runs: 480 files, 7 556 tests, 7 389 passed, 164 skipped, 3 failed — one mine, two a
  fourth lane's, none from the three lanes under review.**
- **One number I could not verify and will not reprint as if I had.** B34 reports "31 of 100 districts
  gain 77 signs". Its *scenario-only* half is proven **structurally** — the pass sits inside
  `if (lessonScale !== undefined)` and `scenarioSignScale` returns `undefined` unless
  `meta.mapKind` starts with `"scenario"`, so no city / exam / полигон map can move. **The +77 itself I
  could not re-derive:** the only pre-edit compile available already contained the junction pass, and
  my own recompile lost signs on 42 districts, so both baselines were unsound. The two districts the
  row is actually about were measured through the real stack and are reported below.

### Reserved files — every hit named, with who did it

`git status` shows **182 paths** dirty across four concurrent waves. Of the reserved list, these are
dirty, and only one of them belongs to these three lanes:

| reserved path | dirty? | whose |
|---|---|---|
| `content/**` | **yes — and it is one of these three** | The PE lane wrote `content/world/pe-*.json` ×7 + `platform/public/world/pe-*.json` ×7 at 00:57–00:58. That is its whole output and it is on the list. *(The other `content/**` churn — `jxg-giveway-v1.json`, `sc-follow-*` / `sc-lane-change` traces, the question banks, `content/law`, `content/medical` — belongs to the give-way, follow-brake and theory waves.)* |
| `lessons/scenario/templates-*.ts` | yes | **Not these three.** `templates-junctions.ts` (00:30) is the give-way lane — its diff moves `SC_JX_GIVEWAY_CONFLICT.hold` −34 → −45 against `CameraRig.GLANCE_OFFSETS.right`, and ships with `gen_jx_giveway.mjs` + `jxg-giveway-v1.json`. `templates-following.ts` (22:04) is the follow-brake lane. |
| `traffic/TrafficLayer.tsx` | yes (01:34) | Another wave, **after** all three lanes had finished. The PE lane named it as the owner of the residual it could not fix and did not touch it. |
| `runtime/**`, `lessons/engine.ts`, `components/sim/CameraRig.tsx`, `world/components/WorldProps.tsx`, `components/exam/**`, `components/dashboard/**`, `tools/mobile/**`, `modules/lesson/**` | yes | Four other waves, by mtime and by diff content. None of these three lanes appears in any of them. |

**Two live-tree hazards worth recording, because they will bite the next gate.** (1) A lane is
**deleting files in `world/__tests__` mid-run** — my own probe was removed from under a running vitest
process. (2) `tools/clips/headless/sheets/sc-jx-giveway-b1__m0.strip.png` and `__m1.strip.png` are
**untracked PNGs sitting in the repo**; 626 MB of frames have been committed by accident three times
already. Neither is these three lanes'.

---

## The gate — SEVEN-LANE INSTRUMENT gate, 2026-08-05. **THE INSTRUMENTS ARE REAL. THE TREE IS STILL RED, AND THIS TIME ONE RED IS OURS.**

Seven lanes reported. This gate disbelieved all seven, **opened 20 of their frames**, and — because
the whole point of this wave was two new instruments — **drove those instruments itself three times
instead of reading anyone's account of them.**

### The two instruments, driven by the gate rather than believed

**1. Does `/dev/drive-rig` really put per-tick telemetry and a fault card on the same screen? YES.**
I drove `sc-roundabout-entry@L1` with my own script and deliberately barged the mouth. The card that
came up carries the live readout burned into the same PNG:
`scratchpad/VERIFY/vbarge-card-t24.0.png` — the teach modal „Пътнотранспортно произшествие" (ЗДвП
чл. 20, ал. 2) with `t=23.96s v=0.3km/h x=4.2 y=-15.3 gear=1 phase=driving obj=1/2 events=1 thr=1.00
step=8 "BARGE +5.6"` in the bottom-left corner. **That is the thing no route in this product could do
before: the conviction and the car's state, in one image, at one instant.**

**2. Does the finish gate now survive a 60-second wait at a give-way line? YES — I waited it out.**
`scratchpad/VERIFY/vwait2-step07-t82.7-v0.png` and its telemetry: **70.42 s of continuous
standstill** on the М8 paint at `y=−36.2`; `y` moved **6 mm** across the whole wait (−36.177 …
−36.171), `x` never moved at all, `gear` stayed 1 (D) so the reverse-assist never grabbed it, `phase`
stayed `driving` on every one of 4 077 samples, **0 clock resets**, and **0 events during the wait**.
The entire 96-second drive fired **one** event — the approach objective completing at t=18.96. His
„about 40 seconds" now has a 70-second answer.

**3. And the detector still convicts, which is the other half nobody may skip.** My own barge did
*not* draw a yield fault — I mistimed the circulator and hit the central island instead
(`COLLISION` at `y=−15.37`, no `FAILED_TO_YIELD` anywhere in the run). **That is corroboration, not a
miss:** it says the fault does not fire when there is no car to yield to. The other direction is
photographed in the B15 lane's own frame, which I opened — `b15-ROUNDABOUT-FINAL/b15a-card-t76.9.png`
— „Непропускане на пътно превозно средство с предимство", ЗДвП чл. 47, with `lead=3.2m = 0.89s @
13km/h` and `events=1` inside the same frame. **Fires with a car, silent without one.**

### Something the instrument caught that no lane reported

My **first** 70-second wait was silently destroyed and the rig said so out loud:

> `!! SESSION CLOCK RESET 2× — the scene remounted mid-drive. This telemetry is`
> `!! more than one drive spliced together; re-run before trusting any row of it.`

Measured: at `t=56.07 s`, standing still at the paint, the session clock jumped to 0 and the car
teleported `y=−36.17 → y=0` (the district-origin placeholder pose) and then to `y=−93` (spawn). It
happened twice in one run. **The re-run was clean — 0 resets — so the cause is almost certainly a
Fast Refresh from another lane editing source on the shared `:3200` dev server, not a product
defect.** I am recording it anyway for one reason: **every long drive on a shared dev server can be
spliced without the driver noticing, and this guard is the only thing standing between that and a
confident wrong answer in this register.** Two waves of B15 evidence were produced on exactly that
server. The guard is the best thing either instrument lane built.

### What I refused

| # | the claim | why it is refused |
|---:|---|---|
| 1 | **"`npx tsc --noEmit` → EXIT 0"** — asserted by three separate lanes | **It exits 2, with 9 errors.** Seven are in `src/modules/sim/world/__tests__/pe-districts.test.ts` — a **tracked, modified product test**: `PeMapCase` gained `carriageway`, `seatCurbToCurbM`, `widthChangesAtM` and `parkedBodies`, and seven of its `CASES` were never updated to match. Two more are in untracked `src/modules/lesson/__tests__/zzprobe-{citations,resolve}.test.ts`. The `zz-b29-*` files three lanes blamed are **gone** — that half of their story was true and has been cleaned up. **This red is ours, it is in the parked-bodies/world lane, and it is the first time in four gates that the tree's red belongs to us rather than to a neighbouring wave.** |
| 2 | **"no PNGs in the repo" / "zero PNGs added to the repo"** — B29/B30/B31 lane | **Two are.** `tools/clips/headless/sheets/sc-jx-giveway-b1__m0.strip.png` and `__m1.strip.png`, both written 23:55 by that lane's clip render, both showing as `??`. **In fairness, and this matters:** that directory already held 48 untracked strips dating from 21–26 July, so it is an un-gitignored render-output convention rather than a new sin. The claim as written is still false, and **the fix is one line in `.gitignore`, not a scolding.** |
| 3 | **`tsconfigHygiene`** — claimed clean by one lane, "four scratch globs" by the register | **Six globs, three families, and a family that is new since the last gate.** `platform/tsconfig.json` `include` now carries `.next-harness/**`, `.next-rig/**` **and `.next-exhdr/**`. Item 10 of the last STILL OPEN said four; it is six. Reverting is futile while those dev servers run — they re-inject on the next compile — so this needs the servers stopped and the file stripped in the same minute. |
| 4 | **"`KNIJKA_DIST_DIR` … passes in isolation in 903 ms — not a code failure"** — the drive-rig lane | **Half true, and the false half matters.** It *is* a timeout rather than an assertion failure, so the safety property still holds — that much was honest. But **it does not pass in isolation**: run alone, three times out of three, it failed at **22.2 s / 26.8 s / 39.5 s** against vitest's 5 s default. The lane wrote off a permanently-red test as a flake on the strength of one lucky run. **On this box it fails every time, and one `testTimeout` argument fixes it.** |
| 5 | **B15-finish-gate "FIXED-SEEN", proven by the 61-second gw-shell frame** — the finish-gate lane | **Downgraded in its reasoning, not in its verdict.** I read the frame's own sidecar (`frames/B15AFTER-11-wait61-SIXTY-SECONDS.json`): `yieldWaitSec: 66.57`, `yieldHolding: true`, `phase: "driving"`, `resultScreen: false` — **but `finishGate.armed: false` and `finishRescueGate.armed: false`.** The session survived 61 s at a **disarmed** stop, which is not the same as surviving the armed gate the fix targets. **The lane said this itself, in its own words, unprompted — that is the behaviour this gate wants and it is why the row still closes.** The armed path is pinned by unit test, not by a photograph, and the row says so now. |

### What held up, frame by frame — 20 of theirs, opened by me

| frame I opened | what the lane said it shows | holds? |
|---|---|---|
| `driverig/b15c-card-t37.5.png` | teach card + live readout, one screen | **yes** — ЗДвП чл. 47 card over `x=4.1 y=-27.5 obj=2/3 events=2 step=1 "brake to 0"` |
| `b15-ROUNDABOUT-FINAL/b15a-step07-t54.1-v0.png` | the 40-second frame | **yes** — `t=54.07s v=0.0 y=-36.2 phase=driving events=0`, ЗАДАЧА 2/2, no card |
| `b15-ROUNDABOUT-FINAL/b15a-card-t76.9.png` | the conviction, with the car's gap in frame | **yes** — `lead=3.2m = 0.89s @ 13km/h` |
| `b15-ROUNDABOUT-FINAL/b15top-step16-t57.8-v15.png` | top-down: 40 s waited, ring empty, no fault | **yes** — whole junction in one frame, `events=0` |
| `frames/B15AFTER-11-wait61-SIXTY-SECONDS.png` + sidecar | 61 s stopped, no result screen | **yes**, with the armed-gate caveat above |
| `frames/PARK-L3-AFTER-06-t40.png` | B-NEW-1: parked at spawn, still alive | **yes** — L3 cockpit, ЗАДАЧА 1/2, no result screen |
| `b29/ZOOM-clean-t45.png` → `b29/ZOOM-fix-car-t41.png` | B29 before/after | **yes** — nose-to-tail parked wall → one blue estate on open road |
| `b29/ZOOM-b29b-circle.png` | B30: the priority car readable from the green circle | **yes** — estate broadside, lorry, van and two more cars beyond, nothing parked in front |
| `b29/b31-t60.0-y95.7-v0.png` | B31: yield credited from the rear of the band | **yes** — banner has moved to **ЗАДАЧА 3/3**, `y=95.7 v=0.0 obj=2/3` |
| `tools/clips/headless/sheets/sc-jx-giveway-b1__m0.strip.png` | 5 keyframes, actors 1/1 | **yes** — approach·pre-fault·FAULT·consequence·end, Б1 triangle, red X on the ghost, traffic on the boulevard |
| `newdef/ZOOM-b58-gate-y162.png` | the gate reads 50, not 57 | **yes** — „Карай дотук / не по-бързо от **50** км/ч" |
| `newdef/b69-step03-t31.2-v39.png` | instruction 1 and the measured gap in ONE image | **yes** — «дръж поне 2 секунди» in the panel, `lead=26.4m = 2.42s @ 39km/h` in the readout |
| `newdef/b72-card-t57.7.png` | the truck taught an unlifting rig a distance fault | **yes** — ЗДвП чл. 23, «Дистанция в момента: 0,8 с (5 м)», `lead=4.4m = 0.81s` |
| `newdef/b67probe-v137-err0.00.png` | the cockpit cluster IS in frame at motorway speed | **yes** — binnacle reads **137 км/ч D**, wheel and dash and the 140 roundel all in frame |
| `newdef/b5gw-card-t24.4.png` | the scan fault names **Б2** on a **Б1** lesson | **yes, and it is worse in the picture than on paper** — the title bar reads «**Б1 не значи спри винаги**» directly above a card reading «Премина стоп-линията на знак **Б2**» |
| `newdef/ZOOM-b5gw-junction-t58.png` | a van standing in the student's own lane | **yes** — white box van, brake bar lit, squarely on the blue path, blue SUV alongside |
| `newdef/b64-step02-t33.4-v26.png` | B65: the map is still raw | **yes** — two grey slabs, one parked row, no kerbs, no crossings, no furniture, nothing moving |
| `driverig/ftfix-card-t42.7.png` | the truck drill can finally grade following distance | **yes** — ЗДвП чл. 23 card over `lead=3.3m = 0.62s @ 20km/h` |
| `driverig/fsq-end-t52.4-v-1.png` | the queue drill is unsurvivable at its own pace | **yes** — «20 наказателни точки · НЕИЗДЪРЖАН», Опасни 2, Основни 0, Второстепенни 0 |
| `qr-lane/B35-dead-vs-live.png` | lamps off, but still unreadable | **yes** — dead green `#266d34` vs live-OFF `#286f36` vs live-LIT red `#e7614b`; head is 7 × 18 CSS px |

### The crossings — every lane reached outside its files, and every reach was checked

| crossing | smallest change that unblocked them? | verdict |
|---|---|---|
| `components/sim/lesson-ui/LessonPlayShell.tsx` | **Yes, and I read the diff to be sure.** The telemetry lane's contribution is exactly: one optional prop `onDevTelemetry?`, one `applyTick` result named instead of destructured inline, one optional call, one `useCallback` dep. **The file's `+81` lines are mostly a *different* lane's A6 mobile work** (`HudCloseButton`, `advisorDismissed`) — worth knowing before anyone blames the rig for that diff. `/simulator` passes no such prop, so the production tick path is byte-identical. | **clean** |
| `sim/scene/stop-line-grading.test.ts` | **Yes, and in the right direction.** One default parameter `windowM = ROUTE_TOUCHES_LINE_M`, and one call site passing `Math.max(ROUTE_TOUCHES_LINE_M, params.radiusM)`. **It widens the sweep to cover the reach being policed — it does not lower the `>= 11` floor**, which is the change a lazier lane would have made. | **clean** |
| `sim/orchestrator/__tests__/scheduled-cruise.test.ts` | **Yes.** One default parameter `cruiseSec = 30`, 60 passed at the single call site that needed it after another lane's B79 station-walk changed the convergence time. **No assertion touched.** | **clean** |
| `sim/lessons/{finish,types,index}.ts`, `scenario/rubric.ts` | Additive: new optional fields (`yieldWait`, `yieldWaitSec`), new pure functions, barrel exports. Zero-wait rubric output asserted byte-identical. | **clean** |
| `sim/lessons/scenario/templates-following.ts` | One `ruleConfig: { followMinSpeedKmh: 10 }` on `SC_FOLLOW_TRUCK` — per-lesson, moves no other drill's floor. | **clean** |
| `sim/orchestrator/runners.ts` | +5 lines in `BrakingLeadCarRunner.stage()` issuing `setIndicator` only when an actor authors a resting indicator. | **clean** |
| `tools/maps/gen_jx_giveway.mjs` + both generated JSONs | `parkingBand:false` authored in the generator (this map's source of truth) and regenerated, not hand-edited. | **clean** |

**No lane crossed a file it did not name, and no lane's crossing was larger than its row required.**
That is a first for this register and it deserves to be said as plainly as the failures.

### The gate itself — exact numbers

```
cd platform && npx tsc --noEmit          →  EXIT 2, 9 errors
   7 ×  src/modules/sim/world/__tests__/pe-districts.test.ts   TS2739
          PeMapCase missing carriageway, seatCurbToCurbM, widthChangesAtM, parkedBodies
          (TRACKED + MODIFIED — this one is OURS)
   1 ×  src/modules/lesson/__tests__/zzprobe-citations.test.ts TS2339  (untracked scratch)
   1 ×  src/modules/lesson/__tests__/zzprobe-resolve.test.ts   TS2339  (untracked scratch)
   0 ×  zz-b29-replay / zz-b29-sightline — DELETED since the last gate, as promised

npx vitest run src/modules/sim src/components/sim src/app/dev --maxWorkers=2
   Test Files   4 failed | 529 passed  (533)
   Tests        3 failed | 8117 passed | 164 skipped  (8284)
   Duration     1055.09 s          EXIT 1
   Four files failed, and two of the four are ours:
     × traffic/__tests__/ped-through-parked.test.ts
         "the geometry that produced his sentence still fails the same measurement"
     × traffic/__tests__/scenery-sightline.test.ts
         "is clean on all 90 committed districts"          ← the FR-21 remainder
     × app/dev/__tests__/dev-surfaces-gated.test.ts
         "KNIJKA_DIST_DIR never reaches application code"
         Error: Test timed out in 5000ms.   ← a real timeout, NOT an assertion failure.
         The drive-rig lane called this "not a code failure … it passes in isolation in
         903 ms". THE SECOND HALF IS FALSE: run alone it still fails, three times,
         at 22.2 s / 26.8 s / 39.5 s. The test file-walks the whole app source tree
         against vitest's 5 s DEFAULT testTimeout, and on a 7200 rpm E: that walk
         costs 20-40 s. It will fail for anyone on this box, every time.
         The fix is one `testTimeout` argument, not a re-run.
     × world/__tests__/zzz-zebra-scratch.test.ts  (0 tests — another lane's empty scratch file
                                                   left in the tree; vitest counts it as a failure)

tsconfigHygiene                          →  FAIL
   platform/tsconfig.json `include` carries SIX scratch globs in THREE families:
     .next-harness/{types,dev/types}  .next-rig/{types,dev/types}  .next-exhdr/{types,dev/types}
   The last register said "four". A third family has appeared since.

untracked binaries in the repo           →  FAIL (2 new, 50 total)
   tools/clips/headless/sheets/sc-jx-giveway-b1__m0.strip.png
   tools/clips/headless/sheets/sc-jx-giveway-b1__m1.strip.png
   `git check-ignore` exit 1 — the directory is NOT ignored and holds 48 older strips too.

NOTHING COMMITTED. All of this gate's own frames and telemetry are under
scratchpad/VERIFY/ and none of them went near the repo.
```

**Read the vitest result honestly: two of those four failures are the parked-bodies work in flight,
one is a disk, and one is somebody's empty scratch file.** None of them is in the instrument lanes,
the give-way lanes, the following-drill lanes or the roundabout lane — **every suite that this wave's
own changes can reach came back green when run scoped.** But the whole-tree gate is red, and this
register does not get to call a red tree green because the red is in a neighbour's half.

---

## The gate — SIX-LANE VERIFY-BY-DRIVING, 2026-08-04. **THE TREE IS RED, AND NOT ONE RED IS OURS.**

Six lanes reported closing the last of his open rows. This gate disbelieved all six and opened
**36 frames** — **26 of theirs and 10 I cropped myself**, plus pixel-column sampling on 5 frames and
a lit-lens cluster scan on 3, because a lane that hands you its own zoom has chosen the crop.
**Six claims were refused, one verdict is stale, and the two rows he checks first — the lights — hold
up under measurement.**

### What I sampled myself, rather than taking a lane's word for it

The lights are the headline, so I re-derived them from the pixels. I wrote a lens scanner that finds
lit heads **by colour cluster anywhere in the windscreen**, instead of sampling where a lane pointed.

| lesson | station | what the scanner found | verdict |
|---|---|---|---|
| **B37** мигащо жълто, approach y −45.8 | the drill's own first objective | amber lens `rgb(218,172,87)` against unlit red `(141,52,49)` and green `(51,116,65)` | **lit** |
| **B37** at the graded stop line y −29.8 | 10 s later | same head, amber now `rgb(136,94,30)` — the same band as the unlit red beside it | **dark** |
| **B39** спане на зелено, y −46.8 | where instruction 1 says «свети ЗЕЛЕНО» | green `rgb(113,193,115)` vs red `(134,46,44)`, amber `(136,97,44)` | **green** |
| **B39** at the stop line y −29.8 | ten seconds on | amber `rgb(218,171,81)`, red and green unlit | **amber** |

Two frames ten seconds apart therefore photograph **the blink itself**. „Мигащо жълто" is a thing on
screen now, not only in the card text. **B37 and B39 are FIXED-SEEN and I confirm them.**

### The six claims this gate refused

1. **The signs lane never photographed a cockpit.** B27, B34 and B60 were all shot through
   `/dev/scene-still` — no A-pillar, no dashboard, no windscreen frame, no lesson UI, and a wider
   field than the cockpit's `COCKPIT_HFOV_RAD`. The sub-claim those frames *do* support is real and
   worth keeping — the Б2 octagon is unmistakable at 45 m on `tj-stop-v1`, two signs stand in
   `tj-emerge-v1`'s opening windscreen — but the question each row actually asks is *can he see it
   from the seat*, and a frame with no seat in it cannot answer that. Their frames stay in the record
   as world evidence; the rows stay open.
2. **B60's evidence belongs to B59.** The signs lane wrote up „no actual Sign stating 30 exists on the
   sidewalk" as B60. That sentence is **B57/B59**, and B59 has been FIXED-SEEN since 2026-08-02.
   **B60 is „no kids are playing on the sidewalks and we should do that"** — a different complaint, on
   the same catalog position, which that lane did not touch. B60 is unchanged.
3. **The B55 mirror mechanism is not in the frame that was cited for it.** The lights lane reported
   that at `pe-jay-v1` y −2.8 the far-side head „falls behind the interior rear-view mirror housing",
   and used it to corroborate B27's cause. I opened that frame: **there is no mirror in it.** What
   actually hides the left-hand head there is the lesson's **own keyboard-help overlay panel**, and
   the forward-right quadrant is filled by a billboard with the advisor card drawn across it. The
   interior mirror does exist — I photographed it in the B24 cockpit pair — so the mechanism is
   possible; it simply was not demonstrated, and B27 gains no support from it.
4. **„Stands clear of the buildings" is overstated for B55.** On the approach the driver's own head is
   a **5 × 5 px** lit green lens against a **building facade**, not against sky as on `sx-v1`. It is
   present, it is the colour instruction 2 asserts, and you can find it — but only if you know where
   to look. The frame that genuinely closes „a traffic light for us" is the one **inside the box**,
   where the head is a 22 × 13 px unmistakable RED with the jaywalker in her red jacket on the zebra
   in the same picture.

5. **The zebra family was reported FIXED and it is not.** The seven pedestrian districts really are
   seven distinct files with seven distinct rooflines now, and the cause that lane found is excellent
   (see B50). But the **carriageway is identical on all seven** — same 16.25 m, same осева, same edge
   line, same kerb — so the bottom ~45 % of every frame is one picture, and four of the seven still
   read as one family of dark banded canyons. **B50 / B53 / B54 stay PARTIAL.**
6. **B46 and B49 were signed off on a frame of a different lesson.** Their evidence line reads „frame
   evidence is the zb-v1 pair above" — a different district. I checked them at data level instead and
   the closure survives, but for a reason nobody had stated: **`pe-clear-v1` and `pe-rain-v1` both
   declare `parkingBand: false`, so no parked car exists on either map.** Which also means whatever
   car he saw on those two lessons **was not a parked one**, and nothing in this wave looked at that.

### One verdict is stale, and it is the one that says we failed

**B70 cannot be reported BROKEN on a constant that no longer exists.** The traffic lane photographed
275 m of empty carriageway and named the file it was not allowed to touch: `templates-following.ts`,
`FS_LEAD_CAR.actor.hold.offsetM: 290` on a 360 m road with the spawn at y 15. That constant is now
**48** in the working tree — „(4.06, 48) — 33 m of centres ahead of the spawn" — `FS_QUEUE_AHEAD` has
been re-authored with a moving tail, and the three `sc-follow-standstill` traces have been re-recorded
to match. Another wave took the fix. **The lane's frames are honest and now out of date; B70 needs one
fresh drive before anyone calls it either way.**

### What held up

| row | claim | this gate |
|---|---|---|
| **B37** | flashing amber visible from the seat | **CONFIRMED** — pixel-sampled, both phases |
| **B39** | green then amber, from the seat | **CONFIRMED** — pixel-sampled, both states |
| **B55** | „a light for us" closed, ped head not | **CONFIRMED as PARTIAL** — the lane's own 14× zoom shows the ped lens as a 1–2 px sliver and it says so |
| **B81** | the 16.25 m grey plaza gets edge lines | **CONFIRMED** — before/after at an identical pose, both kerbs |
| **М18** give-way triangle | 0 → 108 symbols | **CONFIRMED, independently** — visible in the *roundabout* lane's top-down, apex pointing at the driver, М7 line behind it |
| **B33** | the encounter survives the instructed speed | **CONFIRMED, and this is the one that mattered** — trace max **21.9 / 22.0 km/h**, **zero** samples over 22, and a van crosses his windscreen while he is stopped at the mouth at 0 км/ч |
| **B73** | indicator on early, not late | **CONFIRMED** — amber lit at the cutter's right rear corner mid-glide, dark on the next frame |
| **B80** | the barge is now detected | **CONFIRMED** — «Непропускане на пътно превозно средство с предимство», ЗДвП чл. 47, at x −2.3 / y 111.9 |
| **B79** | no longer convicts a lawful gap | **CONFIRMED** — 0 наказателни точки · ИЗДЪРЖАН · 0/0/0 · ★★★ photographed at 29 km/h |
| **B21** | the unhurried beginner is not convicted | **CONFIRMED** — 0 точки · ИЗДЪРЖАН · ★★★ · 2/2 on a 51 s drive |
| **B24** | no brightness snap at the paint | **CONFIRMED** — two cockpit frames **9 cm** apart across the flip, ribbon identical |
| **B14** | the walker clears the parked row | **CONFIRMED** — 6× zoom, her shoulder stops 0.06 m short of the bodywork |
| **B41** | the officer is bigger | **CONFIRMED as PARTIAL** — he measures ~van height, and the same zoom shows he is still a capsule rig with a sphere head and scarecrow arms |
| **B16** | the roundabout is not round | **CONFIRMED as BROKEN** — the island is a circle, the outer boundary funnels open at every mouth |

**Half of B79 is measurement, not photograph.** „Still convicts at an unlawful gap" rests on a
headless ladder (30 / 40 / 44 km/h clean, 48 km/h fires at 1.05 s). The 48 km/h frame I opened shows a
**collision** card, not a following-distance card. The detector is demonstrably alive; the specific
fault card at the specific gap has not been photographed.

**B23 was not driven at the taught pace.** Its trace peaks at **26.6 km/h** with 61 samples over 22.
The row is about how many cars there are, so this does not overturn it — but it was not the obedient
drive B33 got.

**B46 and B49 were closed by arithmetic, and I checked the arithmetic instead of accepting it.** Both
districts — `pe-clear-v1` and `pe-rain-v1` — declare **`parkingBand: false`**, so **no parked car
exists on either map**. The parked-car defect could not occur there, which makes the closure sound. It
also means **whatever car he saw on those two lessons was not a parked one**, and nothing in this wave
looked at that. Only `zb-v1` (B14) actually has a parked row, and that is the one that got
photographed. The global guard `ped-through-parked.test.ts` covers all 100 districts with a legacy
witness so the before-number cannot rot — it passes.

### The gate itself

| check | result |
|---|---|
| `npx tsc --noEmit` | **EXIT 2 — 4 errors**, every one in an untracked `zz-` scratch test another wave left behind: `lesson/__tests__/zz-verify-scratch.test.ts` (×2), `sim/orchestrator/__tests__/zz-b29-replay.test.ts`, `sim/traffic/__tests__/zz-b29-sightline.test.ts`. **Zero in any file the six lanes touched.** A run six minutes earlier showed **9** errors in the same three files — the set churns while waves write and delete them, so any lane quoting a tsc error count is quoting a moment, not a state. |
| `npx vitest run --maxWorkers=4` | **730 files (722 passed / 7 failed / 1 skipped) · 11,008 tests (10,830 passed / 8 failed / 170 skipped) · 238.89 s** |
| expected reds | **2** — `exam/__tests__/content-bank.test.ts`, `lesson/__tests__/compose.test.ts` (the quarantined first-aid questions). |
| the six lanes' own new guards | **7 files / 135 tests, all pass** — `signal-head-forward-view`, `give-way-marking`, `witness-rehold`, `lane-change-beginner-window`, `routeGuidanceAnnouncement`, `ped-through-parked`, `pe-districts`. |
| untracked PNG / WebM / MP4 in the repo | **Zero from any of the six verify lanes** — every one of their frames lives in a scratchpad, and that is the one hygiene rule they all kept. **But not zero overall, and the timestamp matters:** this check was clean at 23:47 and dirty at 00:02, because a concurrent wave wrote `tools/clips/headless/sheets/sc-jx-giveway-b1__m0.strip.png` and `__m1.strip.png` at **23:55**, mid-gate. They are not ignored by `.gitignore` (which is itself modified in the working tree). Clean them or commit them deliberately. |
| `platform/tsconfig.json` | **NOT clean.** It carries `.next-harness/types/**`, `.next-harness/dev/types/**`, `.next-rig/types/**`, `.next-rig/dev/types/**`. |

**The six unexpected reds, each traced to a file and an owner — none of them a verify lane.**

| red | traced to | owner |
|---|---|---|
| `lib/tsconfigHygiene.test.ts` ×2 — 4 scratch globs, `include` is 10 entries against a limit of 8 | `platform/tsconfig.json` | whoever ran the `.next-harness` / `.next-rig` dev servers — the untracked `app/dev/drive-rig/` + `app/dev/popup-rig/` wave. Each of the six lanes named **and deleted** its own dist dir (`.next-lights`, `.next-signs`, `.next-samemap`, `.next-ttlane`, `.next-b14lane`); one started no server at all. |
| `app/api/review/route.test.ts` ×3 — „No `parseQueue` export is defined on the `@/modules/content-admin` mock" | `platform/src/modules/content-admin/*` (8 files modified, `queues.ts` untracked) + `app/api/review/route.ts` | the content-admin wave |
| `sim/scene/stop-line-grading.test.ts` — expected ≥ 11 declared stop-line cuts, got 10 | the count is re-derived from `SCENARIO_TEMPLATES` on every run | `templates-junctions.ts` (+135) / `templates-following.ts` (+217) — **the lane-change lane predicted this exact red and named this exact owner before it fired** |
| `tools/mobile/navigation.test.mjs`, `tools/mobile/ready.test.mjs` — „No test suite found in file" | both untracked | the mobile wave |

### The parallel wave's files — twelve are open, and two are load-bearing

`git status` against the do-not-touch list. **`templates-flow.ts` and `world/TrafficLayer.tsx` are
clean — nobody went near them.** The rest are not:

`app/dev/fold-rig/fold-rig-client.tsx` · `app/dev/gw-shell/gw-shell-client.tsx` (+54) ·
`app/dev/drive-rig/` *(new, untracked)* · `app/dev/popup-rig/` *(new, untracked)* ·
`components/sim/CameraRig.tsx` (+41) · `modules/clips/clipPlan.generated.ts` ·
`modules/clips/clipPlanBuilder.ts` (+84) · `sim/contracts.ts` (+114) · `sim/lessons/engine.ts` (+89) ·
`sim/lessons/scenario/templates-following.ts` (+217) ·
`sim/lessons/scenario/templates-junctions.ts` (+135) · `sim/runtime/worldRuntime.ts` (+71)

There are no per-lane commits, so **git cannot tell you which hands wrote which hunk** — that is the
real finding here, and it is a process problem, not a code one. Two of these files are load-bearing
for rows in this register: `templates-following.ts` carries the B70 fix a lane said it could not make,
and `templates-junctions.ts` is why `stop-line-grading` is red. **Whoever merges must read those two
diffs before trusting either row.**
...
| **B21** | 7 Смяна на лента (7→9) | "he must press almost at the same time few buttons … just a second" | **FIXED-SEEN** | Not reached in either pass. Needs one timed drive of catalog 9.<br><br>**RE-LOOK 2026-08-02 → PARTIAL-SEEN.** The GRADER does not demand a second — held Q for 1.2 s, released, pressed „,“, steered ~0.4 s later (three inputs over ~2.5 s) and earned „БРАВО · Правилна смяна на лента“. Windows are authored generously: `indicatorLookbackSec: 3`, `mirrorLookbackSec: 5`. `scratchpad/lanegw/frames/B21-09-i-y191.png`. **Where his feeling comes from, seen on screen:** the L1 demonstration is 0:29 for a 400 m drill and compresses two taught steps into one bubble — „Ляв мигач, после бърз поглед през рамо към мъртвата зона.“ — arriving as one prompt at 13 m/s. Owner: templates-flow.ts instructionsBg / demo pacing.<br><br>**→ 2026-08-04 VERIFY-BY-DRIVING GATE → FIXED-SEEN.** The fix was already in the tree; what was missing was somebody driving it. An unhurried beginner drive of `sc-lane-change` — 1.81 s on the left mirror, 2.03 s to find the stalk, 1.52 s on the indicator, 1.80 s over the shoulder, 2.01 s back to the wheel, a 4.7 s gentle diagonal — now finishes **0 наказателни точки · ИЗДЪРЖАН · ★★★ · obj 2/2**, 51 s against the drill's own 50 s par whose text reads «спокойно, точността е преди скоростта» (`scratchpad/laneSL/b21/B21beg-end-t50.8-v28.png`). Telemetry puts **5.53 s** between the last graded glance latch and the lane-id flip — **0.53 s over the 5 s window that shipped**, so that exact drive was a conviction a day ago. The old number was upside down: the lane is drawn at `PERCEPTUAL_ROAD_SCALE` and the beats are strictly serial (KeyQ pins the camera to the mirror, so a student physically cannot steer while looking), which means the slower and more carefully a 17-year-old drove, the more certainly he was convicted. |
...
| **B23** | 8 Предимство отдясно (8→10) | "the traffic car is quite quick and its only 1 … should there be at least 1 more" | **FIXED-SEEN** *(was PARTIAL-SEEN — 2026-08-09 verify #2)* | **→ 2026-08-09 VERIFY #2 → FIXED-SEEN. There is a second car now, and it is photographed from the stop line.** `SC_JUNCTION_RHR_CONFLICT_2` (`templates-junctions.ts`) stages a second priority car 55 m behind the first on the same east→west path, mounted on all five rungs through `stagedAdd` and deliberately **not** in `staged`, so the three committed recordings read `spec.staged` and stay byte-identical — no graded gate reopens. Measured through the real director, ambient off: lead crosses the node at **t = 18.87 s**, follower at **t = 28.37 s**, a **9.50 s** window. Frames opened by the gate: `scratchpad/lsn6/B23b/B23b-step03-t55.6-v0.png` (stopped at y = −20.05, priority road EMPTY — the first car gone) and `B23b-step04-t61.6-v0.png` (a second car crossing in front of him). New instruction 5 — „НЕ тръгвай в мига, в който първата кола отмине… зад нея идва втора" — is legible on the card in both. Gate: `b23-second-conflict-car.test.ts`, 5 tests pinning the measured crossing times and that `staged` still holds exactly one car.<br><br> The timing machinery is real (hold at nodeIndex 1 offset −95 m, witness-arm eta 8 s) and the spawn was moved off the осева so the drive is no longer paused before the junction. **His explicit ask is not met:** L1 stages exactly **one** conflict actor and carries no ambient traffic; the only `vehicleCount` in the template is 8, at L5. At the moment he arrives the priority road is empty across the visible span. `scratchpad/laneB23/rig/B23-rhr/t005__16.09s.png`. **Owner:** `templates-junctions.ts` — the second car is a data change, not a rules change.<br><br>**→ 2026-08-04 VERIFY-BY-DRIVING GATE → PARTIAL-SEEN.** The same one-shot-ETA fix as B33 reaches this drill: two vehicles now stand on the priority road in one windscreen at the mouth — a dark estate crossing away and a yellow van coming **from his right** (`scratchpad/ttframes/B23-b06-t21.4s-y-17.9-v0.png`). **Two honest limits.** The staged conflict is still exactly **one** car — a second needs `templates-junctions.ts`, which the wave was told not to touch — and unlike B33 **this drive was not at the taught pace: its trace peaks at 26.6 km/h with 61 samples over 22.** |
...
| **B49** | 24 | "ghost crossing trough stopped car" | **FIXED-SEEN** | Fourth occurrence; not rendered. Covered by the B14 clamp in code, pinned, not photographed.<br><br>**RE-LOOK 2026-08-02 → FIXED-SEEN.** Catalog 26 at night in rain on pe-rain-v1: mid-walk at x 7.12, at rest on the east pavement at x 9.94, nearest parked car 9.3 m south with empty footway between, headlights throwing a cone up the lane. `tools/clips/headless/r0-ped/zoom/B49-restpoint-night.png`. Note for B48's owner: at night the walker on the far half sits OUTSIDE the headlight cone and reads as a dark blob — realistic, and taught nowhere in the copy.<br><br>**→ 2026-08-04 VERIFY-BY-DRIVING GATE → SAME AS B46.** **`pe-rain-v1` also declares `parkingBand: false`** — no parked car exists on this district either, so the row was never at risk from the clamp that produced B14. The 2026-08-02 frame of her at rest at x 9.94 was harmless *here*; that same x would have been **inside a parked body** on any district that has a row, which is exactly the defect B14 fixed. The unexamined reading — a moving or stopped **traffic** car rather than a parked one — is untouched by this wave. |
| **B50** | 25 Внезапен пешеходец (25→27) | "same map, same engineering, everything same" | **FIXED-SEEN** *(was PARTIAL-SEEN — 2026-08-09 verify #2)* | **→ 2026-08-09 VERIFY #2 → FIXED-SEEN, and he was being literal about „same engineering".** Contact sheet at the SPAWN pose, all seven, opened by the gate: `E:\AI driver\scratchpad\sf\frames\SHEET-SEEN-spawn.png`. The parked row on pe-cane / pe-bus / pe-child was not merely similar — it was **the same three cars, in the same paint, in the same order**, because `computeParkedCars` hashed `(edgeIndex, slot)` and **nothing in that hash named the district** (`m=1 s=654` · `m=1 s=35` · `m=4 s=133`, three consecutive lessons). Fixed at source: `districtParkedSalt` (`traffic/TrafficLayer.tsx:410`) salts the **appearance only**, placement untouched — proven over all 90 shipped districts with the pass run salted and unsalted, not one body moved. The trio now reads apart at the spawn: pe-bus a tall blue panel van at the right kerb 18 m out plus two pickups and a kerbed median from 29 m; pe-cane both kerbs parked and every car **tail-on** (one-way, чл. 94); pe-child dark green estates at the bonnet from 8 m with the left bay empty. **Graded side provably still:** 1–7 changed JSON lines per district and **zero graded keys** across all seven diffs, 0 of 22 PE traces re-recorded, 160 trace files / 2 039 tests green. The guard bites — with the salt disabled the new family test fails naming the shared cars.<br><br> **→ 2026-08-03 GATE — the maps really did change, and from the seat it is still one street.** `gen_pe_crossings.mjs` grew a fourth axis (TERMINUS — what the street runs INTO) and all seven were regenerated: topology is now **2n/1e, 3n/2e and 4n/3e** where it was 1 shape × 7, four carry 70–126 m of real extra road and three close the vista with frontage. `pe-districts.test.ts` now FAILS if two of the seven share a terminus, and requires ≥3 distinct node/edge shapes across the family. That is real road work and it is why this is no longer BROKEN. **Why it is not FIXED, looking at the lane's own sheet** (`scratchpad/wm/frames/SHEET-PE-7-spawns.png`, opened at the gate): all seven differences are **100 m+ away**. The near field — the half of the frame a student looks at for the whole drill — is still the same dead-straight two-lane carriageway, same dashed markings, same kerb, same block typology, in all seven. He wrote "same map, same engineering"; the engineering changed, the map he is looking at did not.<br><br>**Earlier:** Never reached.<br><br>**RE-LOOK 2026-08-02 → BROKEN-SEEN.** He is right and here is the picture. Seven consecutive catalog positions (24–30), driver's seat, identical stand-off (21.3–22.1 m), tiled into one sheet: every one is a dead-straight two-lane street to a flat horizon, one zebra, a parked row on the right kerb, an overhanging billboard right, blocks either side. The 80 m overhead sheet says it with no ambiguity — seven straight ribbons, no junction, no side road, no bend, no gradient, no sign, no signal. Data agrees: all seven are **1 edge / 2 nodes / 1 crossing / 0 signs / 0 signals**; only street length (138–155 m) and zebra position differ. `tools/clips/headless/r0-ped/sheets/B50-B53-cockpit-at-22m-seven.png`. Cause: `gen_pe_crossings.mjs` emits one recipe and only `approachM` varies. Fixing this is ROAD work, not facades.<br><br>**→ 2026-08-04 VERIFY-BY-DRIVING GATE → PARTIAL-SEEN — the lane reported FIXED and this gate will not sign that.** What genuinely moved is real and the cause is excellent: `gen_pe_crossings.mjs`'s `block()` stamped **`heightSource: "default"`** on every volume, and `resolveBuildingHeightM` reads that as *throw away the authored height, substitute a hash in [15,25] m* — so the 1.8 m park wall, the 3 m kiosk, the 2.8 m garage row and the 17 m slab all rendered as the same mid-rise block. **Three previous variety passes had been authoring massing into a renderer that was deleting it.** With `heightSource: "height"` plus a NEARFIELD axis authored against the **spawn** (all four existing axes were authored relative to `crossingY`, i.e. 50–100 m past the seat), near-field frontage went pe-cane 0 / pe-clear 1 / pe-slow 2 / pe-rain 2 / pe-bus 2 / pe-child 2 / pe-dart 3 → **4–6 apiece**, seven distinct rooflines, no two districts planting the same tree in the same metre. **Why it is still PARTIAL:** I opened the seven-up sheet and the carriageway is **identical on all seven** — same 16.25 m, same dashed осева, same edge line, same kerb — so the bottom ~45 % of every frame is one picture, and 26/28/29/30 still read as one family of dark banded canyons. `scratchpad/samemap/frames/SHEET-AFTER-PE-7.png`.<br><br>**→ 2026-08-05 COCKPIT GATE → PARTIAL-SEEN. The sentence that refused the last two gates is now FALSE, and the row still does not close.** **What is no longer true:** "the carriageway is identical on all seven". `gen_pe_crossings.mjs` grew a sixth axis — CARRIAGEWAY — and the street is now emitted as a chain of collinear segments joined at degree-2 nodes, with the crossing-bearing segment keeping the id `pe-e-street`, `lanes: 2`, the same class, limit and `parkingBand: false` while the segments south of it carry 4 m kerbside bays with a real parked row. Curb-to-curb **at the seat** was 16.25 m seven times; it is **24.25 m on `pe-clear` and `pe-child` and 16.25 m on the other five**, six carry 29–46 m of bayed carriageway, and no two share a width profile or put the taper at the same metre (`[+31]` / `[+3,+35]` / none / `[+11,+40]` / `[+1,+37]` / `[+7,+43]` / `[−3,+29]`). The difference resolves from the driving seat: `scratchpad/zebra/frames/ZOOM-24-pe-clear-nearfield.png` has asphalt running 4 m past the edge line on both sides with cars standing in the right bay; `ZOOM-26-pe-rain-nearfield.png` at the same crop has the pavement starting 4 m closer on each side. **WHY IT IS STILL PARTIAL — I opened the sheet and I am not reversing the last gate to be kind.** `scratchpad/zebra/frames/SHEET-PE-7-CARRIAGEWAY.png`, seven cockpit tiles: only **`pe-rain`** (night, rain, bare right verge, no bay) and **`pe-clear`** (widest at the seat, low tan arcade, lamp column, trees, cars at your shoulder) are unmistakable at a glance. **`pe-dart`, `pe-slow`, `pe-cane`, `pe-bus` and `pe-child` still read as one composition** — grey-and-gold slab blocks either side, one dashed white line down the middle, one row of parked cars on the right at varying distance. The differences are real and you have to look for them. **The leading cause is named and is not this lane's to fix:** `traffic/TrafficLayer.computeParkedCars` computes `const nx = dy; const ny = -dx` — the right-hand normal, with no side alternation — so all 16 bodies across the six bayed districts stand at `x = +10.13`. **A bay is drawn on both sides and filled on one; the left bay is always empty tarmac.** **The grading is provably unmoved and this gate opened a trace to prove it rather than counting files:** crossings are byte-identical on all seven; `sc-crossing-white-cane/shadow-correct.trace.json` holds **`x = 4.0600` on all 918 samples** with y 15.00 → 135.95 and eight events that read as the drill does — because `edgeParkingWidthM` adds the band *outside* `edgeTravelHalfWidth`, so the rail every trace drives never moved. **0 of 22 PE traces needed re-recording and all 22 replay green.** *One correction to the lane's own wording: the spawn is byte-identical in `x`, `y` and `heading`, but on six of the seven its `edgeId` moved `pe-e-street` → `pe-e-street-s1` (`-s2` on `pe-child`) and `pe-e-street` itself lost its first 18–55 m to the chain. The register will not print "byte-identical" for a field that changed.* **The test that could never have caught this was fixed too:** `pe-districts.test.ts` measured `edges[0]` — the crossing segment, which is frozen — so `expect(widths.size).toBe(1)` was true when written *and was the defect*; it now measures at the seat and refuses the family if any two districts share a carriageway signature. |
...
| **C5** | `theory-practice` landscape: 479 px of scroll, options overflow 334 px | **FIXED-SEEN** | The whole question fits on iPhone 16 landscape — question, four options in two columns, «Провери» and the back link, no scroll. Run JSON: fold pass true, `legend` overflow **0**, `label:has(input)` found 4 overflow **0**, `.btn-accent` overflow **0**, against 479 and 334. Content 85.1%. `tools/mobile/.out/run-…/theory-practice__iphone16-landscape.png`. **Residual the mobile lane measured and stated rather than hid:** 12 of the bank's 18 heaviest questions still need 22–104 px of scroll at 852×393, and 6 of those put one option 4–79 px under the pinned bar. That is a genuine limit of 393 px minus a 48 px topbar, not an oversight.<br><br>**RE-LOOK 2026-08-02 → downgraded to PARTIAL-SEEN.** The specific overflow this row was opened for is gone (fold pass true, legend overflow 0). But the 18-question fold sweep, run properly for the first time, says **12 of 18 heaviest questions still need 22–104 px of scroll at 852×393**, and 2 of 18 at 360×780. Portrait passes 18/18, which was his approved acceptance — but his own sentence is „it all have to be on the screen without scrolling“ and his point 8 names orientation differences explicitly. Byte-identical to the previous measurement. Owner: the practice/exam lane.<br><br>**GATE 2026-08-05 → FIXED-SEEN on the device this row was opened about. The top bar stands down on a question, and the 48 px it hands back closes all four.** Measured by the gate itself on WebKit against the working tree (fingerprint `autoHideTopbar.ts 8b91be568fbb`, `DashboardShell.tsx 21118470ad06`), not read out of the implementing lane's log — and measured twice, on two different dev servers, with identical results.<br><br>**THE FOUR, NAMED, WITH BOTH NUMBERS.** The pre-auto-hide landscape column (852×393, 14/18 clean, `scratchpad/c5ladder/c5l-MIXED-run1.log`) missed on exactly four: `q-eco-062` doc **3→0**; `q-vehicle-063` (six options) option **15→0** / doc **26→0**; `q-vehicle-058` (five) option **37→0** / doc **48→0**; `q-ptp-062` (five) option **37→0** / doc **48→0**. All four now clean on every axis — option overflow 0, button 0, legend 0, document scroll 0 — **and the question stem is no longer clipped into its own scroller either** (`qScroll` 22–26 px on two cases before, **0** on all eighteen now; that clip was the earlier passive-effect version arming ~6 frames after `useQuestionBudget` had already clamped). Full column: **18/18 clean at 852×393, 20/20 including both six-option items.** Frames: `…\scratchpad\c5verify\frames\F1-q-eco-062.png`, `F2-q-vehicle-063-6opts.png`, `F3-q-vehicle-058.png`, `F4-q-ptp-062.png`.<br><br>**PORTRAIT DID NOT REGRESS: 18/18, still.** 393×852, all twenty cases, bar `sticky` at y=0 at its full 64 px, `main` at y=64, no edge handle — the auto-hide cannot arm there, because its media query is `(max-height: 520px)` and the shortest portrait viewport tested is 745 px. `…\scratchpad\autohide-final\05-portrait-unchanged.png`.<br><br>**THE WAY BACK EXISTS AND WAS EXERCISED THREE WAYS.** Edge tap → bar to y=0 and the drawer opens with all nav links (`N2-after-edge-tap.png`, `N3-drawer-open.png`); **after a reload** the bar hides again, the handle is redrawn, and the tap still returns it (`N4-after-reload-edge-tap.png`); the **first Tab** lands on «Отвори менюто» inside the bar and focus alone reveals it (`N5-first-tab-keyboard.png`). Scroll-up was verified where scrolling still exists (780×360 exam, 19 px): down keeps it away, up returns it — `…\scratchpad\autohide-final\09-returned-by-scrolling-up.png`. **On his own phone that gesture is unreachable**, because after this change nothing at 852×393 scrolls at all; the tap and the visible 3 px cyan handle are the only ways back there, which is the whole argument against an invisible hotspot.<br><br>**NO LAYOUT SHIFT, MEASURED MID-SLIDE.** Caught with the bar at y=−46 in flight: `main` 0→0, legend 17→17, last option 299→299. The bar stays `position: fixed` while revealed — it slides *over* the first line of the question rather than pushing the page down (visible in `N2`, and the trade to know about: the revealed bar covers words, never a control — first control at y=99 vs bar bottom 48). `transition-property` is `transform, translate, scale, rotate` in default motion and **`none`** under `prefers-reduced-motion`; the mobile harness's `contextOptions()` forces `reducedMotion:"reduce"` on every profile, so both modes were asked in a purpose-built context (`…\scratchpad\c5verify\vfy-motion.json`).<br><br>**RESIDUAL, CARRIED NOT CLOSED:** the Android floor. 780×360 **14/18** (worst: `q-vehicle-058`, an option 45 px under the bar) and 360×780 **15/18** (`q-krastovishta-029`, 28 px). Those two columns are the implementing lane's sweep (`…\scratchpad\c5resweep\rsw-FINAL-report.txt`); the gate did not reproduce them. Owner: the practice lane.<br><br>**VERIFY 2026-08-05 → all four columns re-derived from the raw JSON by a third party, and they hold: 18/18 · 18/18 · 15/18 · 14/18.** Recomputed row-by-row out of `scratchpad/c5resweep/FINAL/rsw-measure.json` with an independently written predicate rather than read from any lane's report; the file's own coherence evidence checks out too (start/end fingerprints of the 7 geometry-deciding files byte-identical, canary reproduced on all four devices). **BUT THE BASELINE THIS ROW STILL QUOTES IS UNDERSTATED, IN BOTH HALVES.** Re-derived from `scratchpad/c45/before/measure.json`, the before-state at 852×393 was **3 of 18 clean — 15 failing, 13 hiding an answer by 4–79 px, 15 needing 22–104 px of scroll**. The sentences above say „12 of 18 failing“ and „6 … put one option 4–79 px under the pinned bar“. The true starting point was worse than the row has ever claimed, which makes the recovery to 18/18 larger than the row has ever claimed. **AND THE SEVEN ANDROID FAILURES SPLIT INTO TWO KINDS THAT ONE COUNT ERASES.** *Genuinely hides text* (4): 780×360 `q-vehicle-058` **45 px** and `q-ptp-062` **22 px**; 360×780 `q-vehicle-063` **10 px** and `q-krastovishta-029` **28 px**. *Nudge only, every word visible* (3): 780×360 `q-vehicle-063` and `q-vehicle-056` (11 px each); 360×780 `q-krastovishta-062` (20 px). The second group fails his literal „no scrolling“ sentence while hiding nothing from him — worth putting to him as its own question rather than folding into the residual above.<br><br>**AND THE FOUNDER WAS RIGHT THAT EXAMS WAS THE SAME THING — IT WAS WORSE, AND IT HAD NEVER BEEN IN A FOLD SWEEP AT ALL.** Both runners measured by one instrument on one day, 20 cases per surface per profile (the 18 heaviest bank questions + both six-option items again with the under-five-minutes banner up), **160 cases**, fold = top of the pinned action bar, re-derived here from the raw `after1.jsonl`/`after2.jsonl` rather than read from the lane's report:<br><br>`surface   profile            exam BEFORE   AFTER`<br>`practice  iPhone 393×852          —        20/20`<br>`practice  iPhone 852×393          —        20/20`<br>`practice  Android 360×780         —        16/20`<br>`practice  Android 780×360         —        15/20`<br>`exam      iPhone 393×852          —        20/20`<br>`exam      iPhone 852×393     >>> 0/20 <<<  17/20`<br>`exam      Android 360×780         —         8/20`<br>`exam      Android 780×360         —         9/20`<br><br>Pooled: **PORTRAIT practice 36/40, exam 28/40 · LANDSCAPE practice 35/40, exam 26/40.** **The before-control (this tree, only `ExamRunner.tsx` reverted to HEAD) is 0 of 20:** the action bar was not pinned on a single case, the 45-button navigator was in flow on all 20, and I opened the frame — an iPhone held sideways was served the **desktop** exam, full desktop header and 3 of 6 answers on screen. Worst option **228 px** past the bottom edge, document scroll up to **490 px**. **The exam is still worse than practice everywhere**, and the three remaining 852×393 failures are named and photographed: `q-vehicle-058` +25 px, `q-ptp-062` +5 px, `q-vehicle-063` with the banner up +15 px — practice is clean on all three at the same viewport, and the only structural difference is ExamRunner's own header card, whose question counter and countdown the pinned bar below **already duplicates**. That is the next lever, and it needs the aria-live region moved out of that header first. **This is the surface under a 40-minute clock.** Owner: the practice/exam lane.<br><br>**VERIFY 2026-08-05 (round 3) — PARITY IS MET. The exam now fits no worse than practice on any profile in the ladder, and on one it fits better.** Re-measured and re-derived by a third party from raw jsonl with an independently written predicate (a case is clean only if every option AND the question text are at or above the fold, the document does not scroll at all, and the pinned bar does not overflow across; a row counts only if the page reported `settled`, the rig published the surface asked for with a 45-question paper, fonts loaded, every image decoded with real pixels, and nothing on screen said „Зареждане/Compiling"). **160 rows, 0 untrustworthy.** `profile        practice      exam` / `393x852        20/20         20/20` / `852x393        20/20         20/20` / `360x780        20/20         20/20` / `780x360        19/20         20/20` — worst exam option margin -9 / -42 / -8 / -9 px (negative is slack). The one non-clean case in the whole sweep is PRACTICE at 780x360 (`q-ptp-062`, document scrolls 1 px); the exam is clean there. **The exam column had to be re-taken: `ExamRunner.tsx` moved again after the implementing lane's last sweep (`8e356b7f1971` -> `c94af2186d2a`), so none of that lane's exam numbers describe the current tree. These do.** **THE FOUR CASES THIS ROW WAS OPENED ABOUT — NO ANSWER TEXT IS HIDDEN ANYWHERE, on either surface, on any of the four profiles.** Worst option position, current tree: `q-vehicle-058` practice 780x360 **-22 px** (was +45) · `q-ptp-062` practice 780x360 **0 px with 1 px of document scroll** (was +22 / +33) · `q-vehicle-063` practice 360x780 **-25 px** (was +10) · `q-krastovishta-029` practice 360x780 **-2 px** (was +28). Exam is clearer than practice on all four (-39, -19, -50, -29). **ONE PIECE OF TEXT IS STILL HIDDEN AND IT IS NOT AN ANSWER.** On `q-krastovishta-029` in PRACTICE at 360x780 the question stem is clamped: 244 px shown of 260, **16 px behind the scroller**. I opened it (`scratchpad/vfy/look/B1-clamp-practice-360x780-q-krastovishta-029.png`): the last line „движението от тук?" is cut horizontally through the glyphs. It is readable but visibly truncated — it does NOT read as „there is more", it reads as a cut line. The other clamped case, `q-vehicle-063` in the exam at 360x780 with the banner up, reports 36 px clipped but that is the clamp's own 12 px tail padding plus slack — the last line is complete and merely dimmed by the fade (`A1-clock-360w-timelow-q-vehicle-063.png`). **„Clipped px" is therefore not the same as „hidden text px", and only the 16 px case cuts a word.** **THE CLOCK DID NOT SHRINK.** At 360 px wide with the under-five-minutes banner up, measured in the rendered page and then looked at: exactly **one** visible clock, `font-mono` 18 px, weight 900, `rgb(255,106,88)` red, 43x28 px, inside the pinned bar. The committed baseline drew the phone clock at the same 18 px (`text-lg … sm:hidden`) — its 24 px header clock was `hidden … sm:block` and had never appeared on a phone at all. Exactly one counter (`1/45`), 45 navigator cells. Frames: `scratchpad/vfy/look/A1..A4`. **THE 58 px FOLD DEFECT IS CONFIRMED AND CLOSED IN THE INSTRUMENT THIS ROW IS NOW MEASURED WITH:** the fold is the top of whatever is `sticky`/`fixed` with its bottom on the viewport's bottom edge, found structurally; `barPinned` is true on 160 of 160 rows, so no row in this measurement fell back to the viewport edge. **`tools/mobile` has NOT been ported** — its `.btn-accent`-parent definition is still there, so any future exam fold number out of the shared harness will still be 58 px optimistic. **CARRIED, NOT CLOSED:** with the iPhone 16's real safe-area insets both runners score **0/20 in both orientations** — see C4 for the mechanism and the frames. That is the same defect for this row and it is the one that still makes his sentence false on his own phone. |
...
| **C8** | Portrait driving shell settles in 1,198 ms against a 1,200 ms budget | **NOT-MEASURED — previous pass RETRACTED** | Not re-measured in either pass. `tools/mobile/stability-probe.mjs` needs its own WebKit sign-in, and `auth.mjs:22` calls `page.goto('/login')` with no timeout argument — Playwright's 30 s default — which never completed on this box (one cold `/login` compile took 6.8 minutes). **Two millisecond margins should not be measured on a box like this.** To retry: warm `/login` and `/simulator` server-side with the session cookie first, then run the probe.<br><br>**RE-LOOK 2026-08-02 → PARTIAL-SEEN — re-measured for the first time.** The blocker is fixed at source: `tools/mobile/lib/auth.mjs` called `page.goto('/login')` with no timeout, inheriting Playwright's 30 s default; it now warms from Node, uses domcontentloaded, an explicit 180 s budget and a 120 s hydration wait, with per-step logging. `tools/mobile/lib/user.mjs` gained a 10 s pg connect deadline (its „bounded“ 20-try loop was unbounded in practice — one run sat at zero CPU for 17 minutes). **Result: portrait worst settle 1,190 ms against the 1,200 ms budget = 10 ms of margin** (worst phase overlayClose, not the toolbar); landscape 996 ms. Everything else clean: 0 edge violations, 0/97 and 0/121 elements moved, 0 overlaps, 0 occlusions, symmetric margins, EXIT 0. It passes. It passes on a coin flip.<br><br>**VERIFY 2026-08-05 → RETRACTED. THERE IS NO VALID MEASUREMENT FOR THIS ROW, AND THE „1,190 ms" WAS NEVER ABOUT THE DRIVING SHELL.** I opened the frame the number was taken from. `tools/mobile/.out/stability/simulator-drive__iphone16-portrait.png` is **a black canvas** — no street, no car, no cockpit — **with the lesson menu still open and the „Пауза" modal stacked on top of it**. Two defects in one image. The landscape half (996 ms) is the same shell. **Proved from pixels, not from looking:** measuring the centre 50 % band, that frame has **110 distinct colours and 86.4 % near-black**; the landscape one **101 / 85.1 %**; the bare loading shell **35 / 99.7 %**. A frame in which the world has actually rendered measures **410–463 distinct colours and 11–19 % near-black** (`.out/overlays/iphone16-portrait__driving.png` — street, buildings, traffic, steering wheel, 28 км/ч). Re-derived independently on 12 captures: the gate accepts **6/6 real worlds and rejects 6/6 shells**. **Why the menu was still open:** the phase named `overlayClose` pressed **Escape**, and `platform/src/modules/sim/engine/input.ts:272` binds Escape to `onTogglePause()` (documented at line 14 as „Escape — pause menu"). The phase that decided the verdict was measuring **a second dialog OPENING**, not a layout coming to rest.<br><br>**THE HONEST NUMBERS, RE-DERIVED FROM THE RAW SWEEP** (`.out/stability/stability.json`, 2026-08-04T12:27Z — still a loading screen, so these bound the instrument, not the app): worst-per-run **946 / 2,568 / 32,144 ms** → **min 946 · median 2,568 · worst 32,144 · SPREAD 31,198 ms** against a 1,200 ms budget. **2 of 3 surviving runs are over budget**, `overlayClose` never settled in one of them, and **25 % of the sample was lost** (`iterations 4, iterationsOk 3, "page.goto: Timeout was reached"`) while the table printed no mark. So even the register's own most recent raw data contradicts the pass — on a shell.<br><br>**A FOURTH DEFECT, AND THE DEEPEST:** `settle.ms` charges the probe's own `page.evaluate` latency to the app. In the worst recorded sample **31,881 of 32,144 ms was the instrument** (99.2 %), while `atRestMs` says the layout had actually stopped moving at **1,477 ms**. Instrument cost across the sweep: min 5 · median 376 · max 31,881 ms. That is why this metric could pass a black canvas by 10 ms on a quiet box and fail anything on a busy one. The verdict now prints **INSTRUMENT-BOUND** when the probe's share exceeds half.<br><br>**WHAT IS ACTUALLY DELIVERED:** the probe can no longer PRODUCE a false number — it refuses to record a sample unless the canvas passes a five-statistic pixel gate on two consecutive reads, the close is now the control a student clicks (with a „Затвори" fallback and **deliberately no Escape fallback**) and must prove the overlay count went DOWN, and a row that lost runs now FAILS. 23 tests pin all of it (`tools/mobile/ready.test.mjs` 16, `navigation.test.mjs` 7), including a negative control proving the source-scan can tell code from prose about code. **STATUS: NO VALID MEASUREMENT EXISTS — not a pass, and not a new figure.** A live re-measurement with the world required is still owed and is blocked on the box, not the code (Turbopack cannot compile `src/app/globals.css` while something spawns `tsc --noEmit` every ~30 s). Re-run on an idle box: `node tools/mobile/stability-probe.mjs --base-url http://localhost:3460 -r simulator-drive -d iphone16-portrait --repeat 4`. Owner: the mobile/probe lane.<br><br>**VERIFY 2026-08-05 (round 3) — THE INSTRUMENT IS NOW HONEST AND THE ROW STILL HAS NO VALID MEASUREMENT. The reason has moved from the probe to the product.** **1. THE INSTRUMENT IS OUT OF THE NUMBER — verified independently, not read.** The polling loop now runs inside the page (`tools/mobile/lib/settle.mjs`, `settleBody`) and timestamps itself with `performance.now()`; the driver bridge is crossed once, outside the window. I tested it against a page whose layout stops moving at a known moment while the renderer's main thread is deliberately jammed BEFORE the measurement starts: across a 0 -> 3,000 ms jam the APP number read **480 / 372 / 160 / 164 ms (spread 320)** while wall clock read **592 / 485 / 1,707 / 3,204 ms (spread 2,719)** and `bridgeMs` correctly absorbed 1,437 and 2,938 ms. Live re-run of the real probe on a real route with six CPU hogs on the box: instrument share rose 2% -> 10%, the app number did not move (151-163 -> 154-167 ms). **The old metric moves with the box; this one does not.** **2. INSTRUMENT-BOUND FIRES ON THE RIGHT CONDITION, AND THERE IS ONE PLACE IT CANNOT FIRE.** Threshold is a literal 25% and both trigger expressions are present. My jam samples reached **84% and 92%** instrument share; **4 of the 25 recorded C8 samples are over the line (31.9%, 26.5%, 50.3%, worst 68.5%)**. But the table line carrying the marker is at source line 1459 and the `if (!r.ok) … continue` bail-out is at 1414, and the detail block is guarded by `!r.settleRuns?.length` — so **on the C8 row itself, which was refused, the marker was unreachable and never printed.** The refusal is the louder signal, but „it prints when it should" is true only for rows that survive. **3. THE LOSS IS FIXED AND IS NOW IMPOSSIBLE TO MISS.** `iterationErrors` is recorded unconditionally; the `LOST n/N RUNS` line is at source line 1411, BEFORE the bail-out that used to skip it; and an unconditional `SAMPLE INTEGRITY` block at line 1752 re-states asked-vs-produced and fails the sweep. The recorded artefact proves the behaviour (`iterations 5, iterationsOk 0`, all five reasons present). **Caveat: the test pinning this is a regex over the probe's own source, not a behavioural test.** **4. THE WORLD IS NOT RENDERED, AND I OPENED THE FRAMES.** `ENDED-BLANK__simulator-drive__iphone16-portrait__i01.png` and `…-landscape__i01.png` show a dead canvas with only the HUD painted on it — „Завърти телефона хоризонтално", a „Демонстрация" button, a live „Compiling …" badge — no street, no car, no cockpit. Re-derived from pixels over the whole frame by a separate script: those frames measure **115 and 130 distinct colours, top-colour share 0.98, dark share 0.99, busy 0.008-0.012, std 11-14**, against **536-630 distinct, top 0.26-0.30, dark 0.37-0.42, busy 0.11-0.18, std 50-63** for a frame in which the street actually rendered. The gate accepts 4 of 4 real worlds and rejects 6 of 6 shells. **The canvas dies at the layout-viewport resize step, in 5 of 5 portrait runs and in landscape, and never comes back.** **5. WHAT CAN AND CANNOT BE REPORTED.** 0 of 5 requested runs produced a scoreable row. The phases taken while the canvas passed the pixel gate — base(idle), overlayOpen, overlayClose — are supported by the recorded statistics (269-393 distinct colours, dark 0.085-0.334, against 35-54 / 0.987-0.997 after the resize), **but no capture was saved at those checkpoints, so „I opened the frame" applies to the refusals only.** Of those phases, the two genuine STATE CHANGES run **156-575 ms** — comfortably inside 1,200 ms. The only overruns are in `base(idle)` (**1,275 and 1,512 ms**), the phase in which the probe changes nothing, on a screen whose HUD is alive by design; the same phase on the static `exams` route reads 151-167 ms with a 1 ms spread, which is this instrument's floor. `toolbarShown`/`toolbarHidden` — the phases the retracted „1,190 ms" came from — were taken over a dead canvas and cannot be reported at all. **6. THE FOUNDER'S CALL, AND IT SHOULD NOT BE READ AS A PASS OR A FAIL. The 1,200 ms budget was set against a measurement now known to be fiction:** the figure it was ratified against was 99.2% instrument, taken on a loading screen, with the pause modal open. Nothing about the product was ever tested against it. Two questions only he can settle: **(a)** should `settle` be defined over STATE CHANGES only, excluding the idle phase — because on the driving shell the number that busts the budget is the phase with no state change, and a live HUD is not a layout failing to settle; **(b)** is a 90 px LAYOUT-viewport shrink the right stand-in for iOS's collapsing toolbar at all, given that it destroys the WebGL world while a real toolbar collapse changes only the VISUAL viewport. **The budget has not been moved and no new figure is claimed.** Raw: `tools/mobile/.out/stability/stability.json`; my re-derivations and jam experiment: `scratchpad/vfy/`.<br><br>**7. AN INSTRUMENT DEFECT I FOUND BY TRIPPING OVER IT: the probe writes its artefact to a FIXED filename, so any later run on any route silently destroys the previous sweep's raw evidence.** `stability.json` is guarded against `--inject-css` negative-control runs (source line 1385) but not against SCOPE: my own two-run check on the `exams` route overwrote the five-run `simulator-drive` artefact this row is evidenced by. **Restored byte-identical from the implementing lane's own backup** (`scratchpad/mp-c8-portrait-raw.json`; my `exams` run is preserved at `scratchpad/vfy/stability-my-exams-run.json`). The filename should carry the route/device scope, or the write should refuse to clobber a sweep taken over a different scope. This is the same class as the reference frame the implementing lane destroyed by running the probe, and it is how a register citation quietly stops pointing at anything. |
---
...
it opened screens and re-derived numbers through the shipped code, and it disbelieved every lane
report above it. **It supersedes the two lists below it only where it names a row; everything else in
them is still open.** Three of the lanes' own disclaimers turn out to be wrong, and they are named
below rather than repeated.

### What this gate actually did

Nine surfaces photographed at desktop **and** phone through the real components on the real app CSS
(`/dev/popup-rig?state=scales|column|end`, `/dev/micro-quiz`), plus **three live cockpit drives** on
`/dev/drive-rig` at a measured 16:9 (`sc-speed-dangerous` L1 · 1281×720 aspect 1.779; `lex-exam-1` ·
1280×719). Frames and text dumps:
`scratchpad/V-frames/` (`01-scales-desktop.png` … `12-calibration-before.png`, `b58/`, `exam/`).
Every figure below was re-derived by running the shipped function, not by reading it.

### Closed by this gate — looked at, not taken on trust

| what | evidence |
|---|---|
| **G-5 · „т." unqualified on fourteen in-drive surfaces** | **CLOSED — 0 of 14 remain.** An independent scanner (not the shipped one) over `modules/sim/hud`, `components/sim/lesson-ui` and `app/(dashboard)/simulator` returns **0 bare hits across 56 source files**, and the nine rendered text dumps contain **37 „т." tokens, 0 of them bare**. The founder's own screen — the teach card at t=22 — is photographed at `V-frames/01-scales-desktop.png` reading „При повторение: **−10 изпитни т.** (опасна грешка)" with **two** chips („правило: ЗДвП чл. 21, ал. 1" / „оценка: Наредба № 38 приложение № 5, т. 10, б. „в“") and the sentence „НЕ са контролни точки по книжката". |
| **The opposite error — the manoeuvre rubric and the micro-quiz mislabelled as изпитни точки** | **DID NOT HAPPEN.** `V-frames/06-result-end.png`: the rubric renders „1 / 2 **т. за изпълнение**" under a chip „оценка на симулатора — не е закон" and the note „Тук точките се ПЕЧЕЛЯТ (2 е най-доброто) — обратно на наказателните". `V-frames/08-microquiz.png` / `09-microquiz-landscape-phone.png`: „2 **т. по теорията**". Neither was relabelled to clear the scanner. The legend above them agrees at one: „(по 1 изпитн**а** т.)". |
| **The founder's ticket, re-derived through the NEW code path** | **RECONCILES, all four values.** `deriveSpeedingBand({78, 50, "urban"})` → tolerance **flat 3** → charged **75** → excess **25** → **ЗДвП чл. 182, ал. 1, т. 3** → **100 лв. = 51,13 €** → **0 контролни точки** (`status: "not-listed"`, with the reason) → instruments **фиш · електронен фиш**. Not the hand-authored row: `penalties.json` was not consulted. |
| **The ±3 % branch is real, not a hard-coded 3** | `(140, 130)` → `branch: "percent"`, **4,2 km/h**, charged 135,8, excess 5,8 → floored to 5 → **ал. 2, т. 1**, 20 лв. = 10,23 €. `(96, 50, urban)` → 93 → 43 over → **ал. 1, т. 5**, 600 лв. = **306,78 €**, **18 к.т.**, два месеца, **АУАН only**. `deviceToleranceKmh`: 100 → 3 · 100,5 → 3,015 · 140 → 4,2 · 200 → 6. |
| **The six penalty entries no longer share one enumerated case** | **4 distinct `contextQuoteBg` across 6 rows** (the two pairs that share are the same statutory case — both Б2, both speeding), each with its own `offencePhraseBg`. The **header** `quoteBg` is still 1 of 6, and that is correct: the act states the „10 наказателни точки" once. |
| **`verifyCitations` now asks whether the quote NAMES THE OFFENCE** | Three attacks caught: wrong enumerated case with the right phrase → *„the quotes shown never name the offence"*; `offencePhraseBg` deleted → *„a grounded figure must name the offence it prices"*; our paraphrase „превишена скорост" substituted → *„offencePhrase is NOT in Наредба № 38/2004 приложение № 5"*. A superseded-snapshot repoint is also caught. |
| **No new ungrounded figure entered a gated surface** | `penalties.json` — 41 fields, **0 refused** (55 quoted · 5 derived · 2 stipulated · 1 constant · 7 digits-only, 173 citation coordinates masked). `content/questions` — **exactly 296**, the ratchet unmoved. `withEurBg` verified empirically: „…с глоба **10,23 € (20 лв.)**" outside quotation marks, and „Актът казва „с глоба 300 лв.“" left **byte-identical** inside them. |
| **B58 — the В26 «50» is in the windscreen, and the mirror still works** | Both frames opened. `scratchpad/b58/zz-AFTER-y25-annotated.png`: the plate (canvas rows 226–274) sits plainly below the mirror assembly (rows −22–198) at 23 m, the pose the register measured as 100 % occluded. `scratchpad/b58/zz-tail-mirror.png`: a full, unclipped rectangle of the road behind. **Re-verified live on this tree, not on the lane's word:** GLB `hotspot_mirror_rear` y = **1.458** (authored 1.353), `MIRROR_STATION_RAISE_M` 0.105, all three glTF extensions intact; and `V-frames/b58/V58-at-y25-*.png` shows the mirror **carrying the following car** („Кола отзад · 7 м" on the dash) while the forward view is clear. |

### Opened by this gate — named, owned, and none of them a percentage

| # | what is wrong | owner (the actual file) | why it is not fixed |
|---:|---|---|---|
| 1 | **`verifyCitations` proves a citation is INTERNALLY consistent, not that it is about the row it prices — and the exact defect the wave says it closed still ships** | `lib/content/law/corpus.ts` — `verifyCitations` check (3) | Reproduced: take `pen-speeding-urban-21-30` („Превишена скорост в населено място с 21 – 30 km/h"), set `contextQuoteBg` to the **traffic-light** case and move `offencePhraseBg` to match. **`verifyCitations` returns `[]`.** The check compares the phrase to the *citation's own quotes*; nothing compares it to the row's `id`, its `titleBg`, or the violation code it prices. The wave's summary says the check answers „is this sentence ABOUT THIS OFFENCE?" — it answers „is this sentence about the offence this citation claims", which is a different question. **The fix is cheap and mechanical:** require each row to declare the conduct once and assert the phrase against *that*, or key `offencePhraseBg` off a per-row enum so two rows about different conduct cannot carry the same phrase. |
| 2 | **Every „0 контролни точки" verdict on the fault card is proven against a copy of the exhaustive list that is missing an item and carries a vendor watermark inside the statute text** | `modules/sim/rules/consequences.ts` — `CP_LIST_HEADER` (line 339), plus `CONTROL_POINTS_BUDGET.max` / `.newDriver` (323, 330) and the т. 20 / т. 21 quotes (1034, 1057) | `penalties.json` is clean — **0 snapshot citations** — but `consequences.ts` still carries **five** bare „Наредба № Iз-2539", which `actIdForActName` resolves to the **2025 SARS snapshot**. Two measured facts about that file: (a) its чл. 6 has **21 enumerated items, not 22 — т. 3 is truncated mid-sentence** („…откаже да му") by a „Източник: Правно-информационни системи „Сиела“ / 24/01/2025 г." pagination watermark, which appears **inside 16 units** of the act; (b) its чл. 2, ал. 6 is the **repealed** restoration rule. The 0-point *claim* is still true under the consolidation (new т. 12 reaches only чл. 182, ал. 1, т. 5 и 6, ал. 2, т. 6, ал. 3, т. 6) — **but the evidence link a student taps opens a superseded, physically incomplete list.** |
| 3 | **The version guard is sound for positive claims and structurally blind to negative ones** | `modules/sim/rules/__tests__/citation-version.test.ts` — „a quote cut from the 2025 snapshot is wording the current text still carries" | It requires only that the **quoted words** survive into the consolidation. `CP_LIST_HEADER` quotes the list's *header line*, which is identical in both editions — so the guard passes while the **list underneath it** was amended. A claim of the form „X is not in this exhaustive list" cannot be secured by checking that the list's title is unchanged. **One clause closes it:** when a quote is a list header and the figure is `not-listed`, require the whole cited unit to be byte-identical across editions, or forbid `not-listed` from citing a superseded act at all. |
...
| what | why |
|---|---|
| **`app/(dashboard)/review/ReviewClient.tsx:673` prints `{q.points} т.`** | It is the **dev-only** founder review tool: `page.tsx` calls `notFound()` when `NODE_ENV === "production"` and it is never linked from navigation. Not a student surface. |
| **„×1.5 / ×2.0" on the teach card** | Pre-existing — the wave only re-indented the line (`git diff` shows the string unchanged). Not a figure this wave introduced. |
| **The mirror assembly's top 22 canvas rows sit above the frame at 16:9** | Measured by the B58 lane and confirmed here. A real interior mirror sits at the top of the driver's view; the **glass** is fully in frame and unclipped. |

---

## STILL OPEN after the 2026-08-09 SEVEN-LANE VERIFY gate #2 — every item has a NAME, an OWNER and a REASON

**No percentage closes this section and this report does not end in one.** This supersedes the list
below it, which is kept for its reasoning; where the two disagree, **this one is right and the
disagreement is deliberate** — two of that list's entries turned out to be false against the tree and
are corrected here. **Four things are waiting on the founder and are named as his, not counted
against us.**
...
| what | evidence |
|---|---|
| **`THE-TEN-POINTS` — a student's exam ended for driving around a car door on an empty street** | **CLOSED, PHOTOGRAPHED, AND THE DANGEROUS CASE PHOTOGRAPHED WITH IT.** `scratchpad/ovsr/TENPOINTS-result-3pts-FINAL.png` reads **„3 наказателни точки · ИЗДЪРЖАН · Основни 1 / 3"**; `ONCOMING-on-M1-20pts-FINAL.png` reads **„20 наказателни точки · НЕИЗДЪРЖАН · Опасни 2 / 20"** on the same paint against a closing car. The last gate refused this label for exactly the missing frame; the frame exists. **Caveat that must travel with it: a SINGLE crossing reads 0** — the first raises the teach card (`TENPOINTS-teach-first-FINAL.png`), the second bills 3. |
| **`worldRuntime.ts:1797` — nothing measures oncoming traffic on a solid centre line** | **CLOSED, verified in source by this gate.** The clause is gone; the predicate is split `ocBase` (:1824) → `ocArmed = ocBase` (:1832) for the head-on measurement and `orArmed = ocBase && solidCenterLine !== true` (:1834) so the OV-09 return seam stays byte-for-byte. Proven from the seat by the 20-point frame above, where the corridor's 10 lands **0.98 s before** the collision that used to be the only thing that spoke. |
| **`EMERGENCY_LANE_DRIVING` is the weakest remaining 10** | **NOT-A-DEFECT — the 10 is kept, and a real defect underneath it is fixed.** The лента за принудително спиране is travel space for *nobody*; the opposing half of a two-way road is lawful travel space for somebody else, which is precisely why the М1 detector needed a conflict test and this one does not. But чл. 58 opens **„При движение по автомагистрала"** and the detector armed on an authored span alone — a span on an urban street would have billed 10 citing a motorway-only article. `rules/engine.ts:1279–1280` now requires `tick.motorway === true`. **Verified byte-identical on shipped content:** all three `emergencyLane` spans sit on `motorway: true` edges. |
| **`sc-vu-door-zone`'s mistake demo passes the drill** | **CLOSED.** `sc-vud-row` was a radius-6 disc 19 m *past* the door, accepting the whole carriageway including the oncoming bank. It is now `(2.6, 156) r 2.2` — **on** the door, accepting the student's own lane. The swerve demo now fails for leaving its lane at the door instead of on a 10-point charge Наредба № 38 does not support. |
| **`gen_streetwall.mjs` is not idempotent at HEAD** | **CLOSED, re-run by this gate:** `--check` → exit 0, **„files rewritten 0 (0 would change)"** across 24 maps / 681 buildings. The plan and the byte transform now live in one shared module the driver writes with and the test asserts on, `--check` exits 1 naming drifted districts, and a CRLF trap was closed on the way (a byte-literal fixed-point test would have reported all 24 as drift on a fresh Windows clone). |
| **`LessonPlayShell`'s fullscreen request is non-deterministic headless** | **CLOSED.** `drive-rig.mjs` now presses the real key through CDP (a synthetic event carries no user activation), waits on `document.fullscreenElement`, **verifies**, and stamps the run THIS DRIVE IS NOT EVIDENCE if it did not settle; the achieved geometry is published per run. **Correction earned on the way:** on `/dev/drive-rig` the canvas is the *same* size fullscreen-on and -off, and it varies by SCENARIO (1264×662 vs 1264×620) — so this register's „1028×577" did not come from this rig. |
| **The controller bubble's painted `lawRef` was never rendered** | **CLOSED from the seat.** `traffic/TrafficLayer.tsx` now carries a shrink-to-fit painter (2 `measureText` calls where HEAD had 0), and `scratchpad/lsn6/B41/ZOOM2-B41-bubble.png` shows all five lines — including the 43-char **„ППЗДвП сигнали на регулировчика; ЗДвП чл. 7"** — inside the card with margin. It shrinks, never ellipsizes: ADR-002 makes this the one place a law reference is read from the driving seat. |
| **`pe-bus`'s refuge island exists in the JSON and cannot be seen from the seat** | **CLOSED — the schema field is now an object.** `crossingKit` went `refuge-compact` → `refuge-nose-long`; the median nose is standing in the road at 29 m in `scratchpad/sf/frames/SEEN__29-pe-bus__y15.png`, which this gate opened. |
| **At the spawn, `pe-bus` / `pe-child` / `pe-cane` read as one street** | **CLOSED at the spawn pose.** `SHEET-SEEN-spawn.png` opened; no pair of the seven reads as one street, and only one of the seven (pe-rain) is separated by weather. The cause was literal: `computeParkedCars` seated the *same three cars* on three consecutive lessons. Graded geometry proven unmoved — 1–7 changed lines per district, **zero graded keys** across all seven diffs, and 160 trace files / 2 039 tests green. |
| **`B23` · `B50` · `B53` · `B70`** | **PARTIAL → FIXED-SEEN**, each with a frame this gate opened. See the row cells. |

### Still open — with a name, an owner and a reason

| # | name | owner | reason it is still open |
|---:|---|---|---|
| 1 | **`B58` — the В26 «50» the lesson tells him to read is behind the interior mirror, at every distance on the approach** | **lead ruling — choose FIX A or FIX B** | Photographed with an x-ray pair, downgraded to BROKEN-SEEN. **There is no camera constant** (three measured hard stops in §2 above). **FIX A, cheap:** stop `SCENARIO_SIGN_SCALE = 1.5` scaling the sign's MOUNTING HEIGHT along with its size — scale about the face, or translate the instance so the disc's lower edge lands at the legal 2.0–2.2 m. Owner `world/builders/constants.ts` + `props.ts` + `WorldProps.tsx`. Blast radius: **every scenario sign in every district**. **FIX B, correct and expensive:** re-mount the mirror where a real one lives — glass bottom edge at ≥7° above the horizon (it is at 3.2°), ~94 mm higher, with the authored casing and stalk moved and `ROOF_Y` raised. That is `hero_interior.glb` + `VitokCockpit`'s ROOF_Y/POD block. |
| 2 | **THIS REGISTER'S OWN row 7 named the wrong culprit and must not be re-inherited** | this file | The previous list attributes B58/B27/B55 to CameraRig's `rv` quad. In the cockpit view — the default at `LessonScene.tsx:704` — that quad is `visible = false` (`CameraRig.tsx:964`). **A lane sent to `chaseRearView.ts` would move nothing he can see.** Corrected in §2; left here so the correction is not lost. |
| 3 | **`B27` and `B55` have not been photographed at the poses that matter** | cockpit camera lane | B27's Б2 enters the mirror's box between ~12 m and ~28 m — the last three seconds before the stop decision — and no frame exists there; the one that does exist is at the un-measured 2.40 aspect. B55's residual is **not** a mirror problem: the pedestrian head's own lens barely clears its housing and is viewed 65–77° off-normal, in `WorldProps`. |
| 4 | **`B35` — a dead signal head still reads as three coloured discs** | world/HUD label layer | **Not attempted this pass and not counted as looked at.** Costs more than assumed: the B42 bubble lives inside `TrafficLayer.tsx` and serves traffic ACTORS, while a signal head is world geometry in `WorldProps.tsx`. There is no shared world-label channel — the pattern must be extracted before it can be reused. |
| 5 | **FIVE STUDENT-FACING STRINGS STILL TELL THE LEARNER A SOLID-LINE CROSSING IS ОПАСНА — two of them that it „проваля изпита" — while the result screen from that very lesson now reads ИЗДЪРЖАН** | founder's call on the copy, then `templates-lanes*.ts` | Verified by this gate at `templates-lanes2.ts:1144` („Пресичане на непрекъснатата осева при прибирането е **опасна грешка и проваля изпита**", the examiner background of `SC_OV_SOLID_RETURN` itself) and `templates-lanes.ts:913 / 920 / 928 / 931`. **This is the loudest remaining contradiction in the product and it is on the same lesson as the frame that closed the ten-point row.** `templates-lanes.ts` is modified by another lane — coordinate; `templates-lanes2.ts` is clean. |
| 6 | **A first-encounter основна is taught, not scored — so it never reaches the theory recommender** | **lead ruling, then `lessons/engine.ts` + the learning module** | `engine.ts` pushes to `scoredEvents` only when `coachStep` returns `scored: true`; `rules/summary.ts` derives `conceptId`s from the scored events. So a first-encounter `CROSSED_SOLID_LINE` contributes **no conceptId** — the one time we *know* the student did not know it. Teach-first is right for the SCORE and wrong for the RECOMMENDER, and the two are fused only because the recommender reads the score. Not implemented here because it crosses a module boundary (docs/architecture/05). **2026-09-17 — the SCORE half is ruled for a lesson's own mistake (founder Ruling A, [ADR-009](../architecture/07_ARCHITECTURE_DECISION_RECORDS.md); decided, implementation pending).** On a scenario practice rung, the first occurrence of the lesson's own mistake is still taught with no points (option B, charging it, was rejected), but the lesson is not taken. `CROSSED_SOLID_LINE` is such a mistake in sc-ov-solid-line, sc-ov-solid-return, sc-vu-door-zone, sc-mv-uturn-ban and sc-animal-hazard; everywhere else, teach-first is unchanged. **The RECOMMENDER half stays open and stays this row:** ADR-009 deliberately leaves `recordSimObservations` reading charged events and commendations only (its follow-up 1), so a coached first occurrence still gives the learner model no evidence. For a lesson's own mistake, ADR-009 adds only the mistake's concept to that session's debrief theory chips. |
| 7 | **Draw-call headroom at `low` — the register's own numbers are wrong, and the budget's meaning needs a ruling** | **lead ruling, then the next world lane** | **Re-measured here over all 100 districts, both copies: worst 67, on `hz-roadworks-v1` / `ov-crest-v1` / `pe-zone-v1` / `pk-rail-v1` — four maps this register does not name. Headroom 3, not 4.** `pe-cane` 65, `d2-v1` 66, max triangles 81 528 (32.6%). **The bigger question:** `drawCallEstimate` is static-mesh arithmetic, while `perfBudget.ts`'s own docstring says the budget is *per frame, including the mirror pass and the composer*. Measured live at tier `low` the frame is **168–191 draws / 308–401 k triangles** — past the hard caps. Either the 70 means static meshes and the docstring is wrong, or the budget has been breached for a long time. |
| 8 | **The look: the falloff is narrowed, not removed** | surfaces/materials lane | 20 m now reads 74% of 8 m (was 54%), but both are far under the source photograph's own ceiling. The remaining gap is the geometry of a 1.36 m eye — it cannot be filtered away, only authored around, and that needs a road-direction vertex attribute the mesh builder does not carry. **Also unfixed and named with file:line: the identical dead clamp `Math.min(8, Math.max(4, anisotropy))` is still live on the buildings at `world/textures/facadeTextures.ts:211` and `:226`**, in a file whose own header cites „grazing street views". |
| 9 | **The phone cost of the anisotropy change has never been measured on a phone** | whoever holds the handset | Everything about cost is a GTX 1060 extrapolated to a Mali-G57. `?simPerf=1` + `__simPerf.gpu(20)` over `chrome://inspect`, once before and once after, closes it — and would settle the 168-draws-per-frame finding on the device it was budgeted for. |
| 10 | **C1 landscape: the settings gear is painted over** | whoever places the settings control on the landscape shell | ⚙ „Контроли на автомобила" at 747,44 resolves to the «Разбрах» pill at its own centre — **986 px² of hit-box overlap**. A thumb aimed at settings dismisses the instructions. Portrait and small-portrait are clean. |
| 11 | **C1: the founder's fourth surface is still on the road, and its only control reads as a caption** | `LessonScene.tsx:1568` + `PlayAreaStyles` GHOST_SURFACES | `[data-hud="touch-hint"]` paints „Завърти телефона хоризонтално" across the centre in portrait. Verified here: the button is authored `bg-accent text-background`, and `[data-hud="touch-hint"]` **is** in `GHOST_SURFACES` (`PlayAreaStyles.tsx:556`), so `UNPANEL_CSS` forces its background transparent — dark caption text on a bright road. Either add the existing `data-hud-ink` opt-out to that one button, or take touch-hint off the ghost register. |
| 12 | **The harness sentence that produced a phantom occlusion bug** | `tools/mobile/lib/routes.mjs:248` | It prints „«Разбрах» was not tappable (something is painted over it)" for **any** Playwright refusal. Measured, the refusal is the actionability STABLE check and `elementFromPoint` returns the button itself. Until it says *which* condition failed, this register will keep quoting an occlusion that is not there. |
| 13 | **C8: the toolbar phase is unmeasurable by this method, twice over** | mobile harness | A 90 px layout-viewport shrink (a) permanently blinds Playwright-WebKit's canvas screenshot path and (b) stopped the page answering entirely in 1 of 1 attempts. **A 90 px layout shrink is not a stand-in for iOS's collapsing toolbar** — that phase should be removed or replaced with a visual-viewport method before anyone scores the driving shell against it. `page.evaluate` also needs a ceiling in `lib/ready.mjs`. |
| 14 | **C8 landscape was never re-measured, and the fold numbers were never re-swept** | mobile harness | Only iphone16-portrait produced a clean 4-run sample. Separately, `probe.mjs` now measures the fold against the pinned bar instead of the viewport edge, but **nobody has looked at the new numbers** — `npm run mobile:sweep` against `tools/mobile/baseline.json`; every route's fold column will move down by its bar height and some may cross `foldMustPass`. |
| 15 | **`B64` owes exactly one frame** | B65 furniture pass | The shelter is built (`kind: "busStop"` is now a placeable district-building kind, `props.ts:1677`) and photographed on the pavement — but only through the right A-pillar on a held glance, and the parked-clear-zone change postdates that drive. **One forward-view frame at the graded stop point closes it.** |
| 16 | **`B40` needs a world-anchored label, and its other half is a map job** | renderer + maps | The sleeper is **nose-on**, so no rear cue can work; it needs the B42 machinery generalised from a pedestrian owner to a staged actor id. And his actual sentence was sameness: catalog 10 / 17 / 18 / 19 / 21 are all `sx-v1` from `sx-spawn-south`. |
| 17 | **`B5` is unreachable by the rig, and its mechanism is now named** | drive rig, then a give-way lane | `DriveStep` has **no steering channel** (only a constant `steer`), so Урок 2's own junction is behind a turn the rig cannot make. ~20 lines of opt-in heading controller unlocks it and every other row behind a turn. **Correction to this register:** the give-way predicate is **`conflictNearFor`** (`traffic/system.ts:718`, called from `worldRuntime.ts:1004` at a 26 m radius), which tests only distance-to-node, speed and bearing — **no side test, no arm test, no already-cleared test.** It was never narrowed to the player's right. |
| 18 | **`B54`'s task-panel half has never been photographed** | a rig that mounts the objective panel | `/dev/ghost-demo` mounts the bare `LessonScene` with no objective list, so no frame shows that PE-14 is a different DRILL (three tasks including a graded full stop). No lane has built that rig. |
| 19 | **The repaired `sc-animal-hazard` demo has not been looked at** | reels/clip lane | The fix is measured and gated, but `reelClipStaged` returns null for `mistakeIndex ≠ 0`, so nobody has watched the repaired ghost cross at y 132→158. One replay look closes it. |
| 20 | **The 74 rewritten citations (the previous list's row 16) have never been seen on a student surface — and 34 + 19 more of the same shape are ungated** | `content/questions` owner, then a screenshot | The 74 `прил. № N` refs are rewritten, re-pinned and gated four ways, but no dev route renders a real bank row's `lawRefs` (both `/dev/micro-quiz` and `/dev/media-demo` use hardcoded fixtures) — **one authed why-panel screenshot closes it.** Measured and out of scope: **34 more** in `content/signs/signs.json` and the semicolon-form refs across **16** `content/lessons/*.json`, neither covered by any freeze script. |
| 21 | **A third red in `test:tools`, pre-existing at HEAD** | one command | `tools/deploy/deploy.test.mjs:588` — `content/medical/tools/fetch.sh` and `content/sources/tools/fetch.sh` are committed `100644`. **`git ls-tree HEAD` proves both were added non-executable in HEAD (`d7ec746`)**, so it is not this wave's. `git update-index --chmod=+x` on each. |
| 22 | **Every cockpit pixel rectangle in this file that cites a 1264×620 or 1248×520 canvas was measured at the wrong aspect** | this file, and whoever next measures from the seat | `LessonPlayShell` only reaches `PLAY_ASPECT` 16:9 once its ResizeObserver fires; a scripted page never resizes, so the canvas sits at 2.40 and vFOV drops 47 → 35.7. Horizon-relative angles are aspect-invariant and hold; **pixel rects do not.** Worth checking whether a real first paint in a real browser lands there too. |
| 23 | **A scene remount still eats drives under load** | sim lane | One attempt teleported the car 88 m in 1.06 s and re-raised the seatbelt lesson mid-drive while vitest ran at 2 workers. Same signature as `B-NEW-1`. Also seen once in passing: on iphone16-portrait the `driving` state came back as the loading shell at road 0% while landscape was 95.9% — one observation, dev server, Next's „Rendering…" badge live in the frame, so it may be Fast Refresh. |

### Waiting on the founder — named plainly, not counted against us

| row | what is needed from him |
|---|---|
| **B16** | His ruling on two cockpit frames. Unchanged this gate. |
| **B52 / B60's bodies** | His Blender machine (FR-43). The behaviour half is done and photographed; the body half is blocked on hardware, not on work. **B41's officer is the same blocker** — the law line now fits inside his bubble, but he is still a primitive capsule rig with a sphere head. The cheap wins if his machine frees up are a peaked cap and a white палка, both on the elder's-cane instancing precedent. |
| **B67 — the speed ceiling** | **A curriculum decision, now asked for a fifth time.** The camera half of the row is closed with a frame (110 км/ч on the cluster at 111.1 km/h telemetry). His sentence — „I cant go more than 100–105 … must go beyond 160–180" — is untouched, and it is a syllabus call, not a bug. *(An earlier gate recorded a 138.3 km/h top on a different drive; this lane's `sc-mw-discipline` run reached 111.1 before the участък ended. Both are below what he asked for.)* |
| **The 29 quarantined first-aid questions** | His medical ruling. They are **the sole cause of both protected reds** — `content-bank.test.ts` (`ptp-i-parva-pomosht` 31/64 approved) and `compose.test.ts` (`l-accidents-first-aid` has no quiz beat). |
| **`B58` — which of the two real fixes to buy** | FIX A moves every scenario sign in every district and undoes half of his own „those signs must be big" ruling by lowering the mount; FIX B is an asset job on the hero interior. **Both are named with numbers in still-open row 1. Neither should be started without his call**, because A changes what he already ruled on. |
| **The five „опасна" strings** | Rewording what students are told about exam practice is a content-truth call, not a side effect of a severity edit. Still-open row 5. |

---

## STILL OPEN after the 2026-08-09 SEVEN-LANE VERIFY gate *(earlier the same day — SUPERSEDED by the list above; rows 7, 17 and 18 of this list are now known to be wrong or closed)* — every item has a NAME, an OWNER and a REASON

**No percentage closes this section and this report does not end in one.** Everything the sections
below list is still open unless it is named here as closed. What follows is only what *this* gate
closed, found, or is handing over. **Two rows are waiting on the founder and are named as his, not
counted against us.**
...
| what | evidence |
|---|---|
| **`THE-TEN-POINTS` — a student's exam ended for driving around a car door on an empty street** | **THE COMPLAINT IS FOUND AND THE CAUSE IS NAMED.** Наредба № 38 приложение № 5 т. 10 б. „в" is a closed list of six cases, read out of the ingested act by this gate. `CROSSED_SOLID_LINE` matched none of them and is now **основна (3)**. The decider was the catalogue contradicting itself: знак **В24** requires a measured overtake against a real lead car and bills **3**; the paint required only position and billed **10**. All 53 codes grounded clause-by-clause in `rules/n38.ts`; a 22-test pin re-derives the act's numbers at run time and **forbids any ten-point charge that rests on geometry alone**. *(The verdict label stays open — see „Still open", row 1.)* |
| **`HIS-ACTUAL-PHONE` — both runners scored 0 of 20 on the founder's iPhone, in both orientations** | **CLOSED, with the insets proven applied rather than assumed.** One token: `app/layout` ships `viewport-fit=cover`, `globals.css` pays the inset back on `<body>`, and `DashboardShell` then asked for a full `min-h-dvh` *inside* that padded box — so every page was permanently 34 px (portrait) / 21 px (landscape) taller than the screen, and `useQuestionBudget` cut the stem against a floor that could never be reached. Re-derived here: practice **0/20 → 20/20** portrait, exam **0/20 → 20/20** both orientations; `saRewrites > 0` on 160/160 and `<body>` padding reading back the asked inset on every row. The two zero-inset Android columns did not move — the control. |
| **`B65` — no lamps, wires, poles, fences, tyre marks; one repeated tree; a street ending in a field** | **CLOSED from the seat**, `b65furn/B65-before-after.png`. One predicate: `buildProps` gated all street furniture on `ARTERIAL_CLASSES`, and every authored teaching street is `residential` — so `sp-creep-v1` had **0** lamp columns (not the 1 the register printed), and because every bench, bin, planter and bollard derives from the lamp run, the product had no street furniture on any teaching street at all. |
| **Door 6 — post-exam review re-read the question text** | **CLOSED, and without paying either price §10.3 assumed.** A 16-hex fingerprint of exactly the bytes the grader taught from — 720 B per 45-question paper, 1.8% of what audit M-1 removed. Re-run here over all 1 089 rows in all three pin states: **0 rows speak a forbidden string**, and the probe was made to prove its own reach first. |
| **Four trace-determinism failures that parked the previous attempt at this wave** | **GONE.** Four traces re-recorded; spot-checked `sc-rb-busy-gap/mistake-barge-lead` by parsing both revisions — `meta` identical, `samples` identical (361 → 361), only the annotation string moved. The seven PE districts gained islands/tables/skews and needed **no** re-recording: the crossing point does not move and no edge changes width, so the `x=4.06` rail every PE trace drives is untouched. All 22 replay green. |
| **The tree was red in six places across the reporting lanes** | **GREEN.** `11 039 passed / 2 failed / 170 skipped` across 737 files, and the 2 are exactly the protected reds. The `pe-districts` lamp-column collision, the trace determinism four and the `clearanceProvenance` pair all resolved as the lanes converged. |

### Still open — with a name, an owner and a reason

| # | name | owner | reason it is still open |
|---:|---|---|---|
| 1 | **`THE-TEN-POINTS` has never been read off a result screen** | whoever next drives `sc-ov-solid-return` L3 | The row is about a number he **saw**. The act is right, the code is right, 813 tests are green — and no one has photographed a result screen reading „3 наказателни точки". One drive closes it. This gate refused the `FIXED-SEEN` label for exactly this reason. |
| 2 | **`worldRuntime.ts:1797` — nothing measures oncoming traffic on a solid centre line** | `runtime/worldRuntime.ts` | `tick.solidCenterLine !== true` disables the corridor tracker precisely where sight distance is worst. Before the ruling a geometric 10 masked it; now it is a visible hole — **a genuinely dangerous overtake across a solid line bills 3 unless it ends in a collision.** Deleting the clause changes `ocArmed` for every М1 scenario and requires the affected traces re-recorded, so it was not smuggled in behind a severity edit. |
| 3 | **`EMERGENCY_LANE_DRIVING` is the weakest remaining 10, and its detector is the same shape as the one just demoted** | lead ruling | Read here: position in an authored emergency lane + `laneCount > 1` + moving + forward gear + sustained, **with no test for another road user** — structurally identical to what `CROSSED_SOLID_LINE` was. It survives on a legal distinction (the lane's *purpose* is to hold stopped vehicles; the opposing half of a two-way street is empty most of the time). The lane declared this itself in `contestedBg` rather than hiding it. If the lead rejects the distinction, the code falls to основна with it. |
| 4 | **`sc-vu-door-zone`'s mistake demo now PASSES the drill** | `templates-vru.ts` | Uncovered by the fix, not caused by it: the „late dodge" counter-proof used to fail on the unlawful 10 points alone. Strip the charge and the swerve ticks every objective the template authored — the drill never encoded „не пресичай осевата" as a gate; the severity was doing the objectives' job. Pinned deliberately in `s10-vru-pack-bot-completion.test.ts` so closing the hole turns those assertions red on purpose. |
| 5 | **`q-vehicle-058` in practice at 852×393 needs 11 px of scroll** | founder's call | Nothing is hidden — every word and all five options are visible. The lever is measured: practice draws 14 px answer text at **22.75 px** line-height where the exam draws it at **20.3 px**, so three wrapped rows cost practice ~21 px more. Not pulled, because tightening answer leading across the whole practice runner in the tightest orientation is a readability decision. Two exam cases at the same size (`q-krastovishta-029`, `q-vehicle-063` with the under-5-minutes banner) still clip one stem line; same lever. |
| 6 | **C6 — the exam bar's „Следващ →" is 38 px tall** | lead ruling | **PRE-EXISTING and provably so:** the className is byte-identical at HEAD, carries no `min-h-11`, and the bar is `flex items-center` so no parent padding can set it. Also „→ Следващ въпрос" 40×38 on small-portrait and „Отбележи за преглед" 43×44. Growing them to 44 px takes 6 px off the answers on the surface under a 40-minute clock — a trade, not a commit. |
| 7 | **The in-scene rear-view quad swallows three separate rows** | `scene/CameraRig` | **The single most useful finding of the cockpit lane.** Not a DOM element — it is CameraRig's `rv` quad parked in front of the camera, at roughly x 784–945, y 222–305 on a 1264×620 canvas, i.e. ~224 px right of centre and ~80 px above the horizon: exactly where a 3 m sign on the right verge at 20–60 m projects. Measured landing inside it: B58's В26 «50» at (874, 291), B27's Б2 at (876, 245), B55's right-hand pedestrian head at (852, 287). Its rect is identical across four districts and eight poses, so it is camera-parked, not world geometry. **One camera constant sits between the product and three rows.** |
| 8 | **`pe-bus`'s refuge island exists in the JSON and cannot be seen from the seat** | PE crossings lane | Found by this gate. `island: {widthM: 2, approachM: 9, departM: 9}` is the most compact in the family and at 11 m the zebra still reads as unbroken bars. A schema field is not a thing he can see. |
| 9 | **At the spawn, `pe-bus` / `pe-child` / `pe-cane` still read as one street** | PE crossings lane | Three of seven, down from five, but not zero. The measured lever is counter-intuitive and worth acting on: the driver sits at `x=+4.06` and the left bay centre is `x=−10.13`, so a left-kerb car 6–19 m ahead is 37–67° off-axis, behind the A-pillar. **What pays is which kerb is EMPTY, not which is full.** |
| 10 | **The look: detail collapses past ~15 m** | surfaces/materials lane | The road is *not* untextured — PBR colour+normal+roughness, detile, UDN detail normal, macro noise, wear decals. Against validated controls (flat 0.16 / detailed 5.02) it measures 1.34–1.60 at 8 m and **0.72–0.93 at 20 m**: the exact failure `roadSurface.ts`'s own header claims to have fixed. **Attack MIP/anisotropy/detail-tile falloff and missing macro features — not new base maps, which cost bytes and change nothing at 20 m.** |
| 11 | **Draw-call headroom at `low` is nearly gone** | next world lane | Measured in this gate: `pe-cane` **65** and `d2-v1` **66** against a **70** soft cap. Triangles are fine (33% of budget). **One more instanced kind on the PE family breaches the soft cap.** Also: at `low` the tier is `colorOnly`, so the three normal maps that are 75% of the texture weight are never fetched — **any fix expressed as a normal-map improvement is invisible on the tier his FPS complaint is about.** |
| 12 | **`B58` (b) — `ov-keepright-v1` still ends into a field with a flow vehicle on open grass** | TERMINUS pass over `gen_ov_keepright.mjs` | In the windscreen for the last two seconds of the graded drive (ends `y=321.0` of 360 m). Same family as B65's terminus. Should run in the **same sweep** as the PE terminus work, not as a second convention. |
| 13 | **`B64` — the bus stop is still three grey volumes** | B65 furniture pass | Needs a placeable street-furniture kind: `world/types.ts` + builder + renderer + district authoring. One entry in that pass, not a lane of its own. |
| 14 | **`B35` — a dead signal head still reads as three coloured discs** | world/HUD label layer | The pixels are right (every lens at the unlit tint, junction unoccluded), but a student cannot tell a dead head from a working one at 14×. The prescribed affordance (the B42 bubble anchored to the head) is unbuilt. |
| 15 | **The controller bubble's painted `lawRef` grew 32 → 43 chars and was never rendered** | `traffic/TrafficLayer.tsx` | `drawControllerBubble` paints it with a bare `fillText`, centred, **no wrap and no `measureText` clamp**, on a 1024 px canvas. Guarded with a 48-char budget derived from a line that *has* been looked at — but that is arithmetic, not a frame, and it is a what-can-he-see question. |
| 16 | **74 bank citations hide an annex number behind „прил."** | `content/questions` owner | `normaliseUnitRef` knows only the full word „приложение", so „прил. № 2, знак Б2" reads as numberless to every check in the repo — including the citation freeze that reports „0 with a number on an act we cannot open". 40 distinct, **100% on Наредба № РД-02-21-1**, which the repo does not hold. The sim scanner now refuses the shape; 74 bank rows need re-pinning. |
| 17 | **No 2026-consolidated Наредба № Iз-2539 exists to ingest** | law corpus | Proven, not assumed: a HEAD on the pinned SARS URL returns the same **226 435 bytes, last-modified 2025-01-28** — re-fetching cannot produce the 2026 text. Both amending acts are held verbatim instead, but the consolidated act is not, so `pen-alcohol-05-08` **withholds** its control-points figure rather than serving the superseded 8. Erring honest, by design. |
| 18 | **`gen_streetwall.mjs` is not idempotent at HEAD** | maps tooling | A landmine for everyone: it is a mandatory post-pass to `gen_pe_crossings.mjs`, but running it also silently rewrites `d2-v1.json` and `tj-emerge-v1.json`. **Those are exactly the two files and the two diff shapes that reddened the previous attempt at this wave.** Reverted here; any agent who runs any map generator will re-trip it. |
| 19 | **`LessonPlayShell`'s fullscreen request is non-deterministic headless** | `tools/` rigs | Requested from a mount effect relying on the start click's transient activation; headless there is no gesture, so it is granted or denied **between identical runs**, and the two outcomes give different canvas geometry (1264×620 vs 1028×577). Every projected pixel and crop rectangle moves with it — it nearly cost the B55 verdict. The cockpit rig now forces and verifies the state; `drive-rig.mjs` does not. |
| 20 | **`tools/mobile/lib/probe.mjs` measures the fold against the viewport edge** | mobile harness | It now runs with real insets so its safe-area column is honest for the first time, but any fold number out of the shared budget sweep is still optimistic by the bar's height. **`fold-sweep.mjs` is the instrument to trust for fold questions.** |
| 21 | **A first-encounter основна is taught, not scored** | lead ruling | Shipped A12 behaviour for the whole class, not something this wave invented — but `CROSSED_SOLID_LINE` has now joined it, so the sim→theory recommender will **miss a student's first solid-line crossing entirely**. Worth a ruling: is teach-first right for a marking offence taught from lesson one? **2026-09-17 — the teach-first half is answered for a lesson's own mistake by founder Ruling A ([ADR-009](../architecture/07_ARCHITECTURE_DECISION_RECORDS.md); decided, implementation pending).** `CROSSED_SOLID_LINE` is one of the lesson's own mistakes in sc-ov-solid-line, sc-ov-solid-return, sc-vu-door-zone, sc-mv-uturn-ban and sc-animal-hazard. On those lessons' practice rungs, the first crossing still costs no points (option B, charging it, was rejected), but the lesson is not taken. In every other lesson, and in exam mode, the rule stays as it is today. **The recommender half stays open:** ADR-009 deliberately leaves the learner model reading only charged events and commendations (its follow-up 1); it adds only a debrief theory chip for the mistake's concept. The live copy of this row is row 6 of the list above. |
...
|---|---|
| **B16** | His ruling on two cockpit frames. Unchanged this gate. |
| **B52 / B60's bodies** | His Blender machine (FR-43). Three of the six school children now **run** at 2.4–2.9 m/s on crossing paths and the behaviour is rendered and photographed — but the placeholder rig has no run pose, so a still frame still shows standing capsules. The behaviour half is done; the body half is blocked on hardware, not on work. |
| **B67 — the 138 km/h ceiling** | A curriculum decision, asked for a fourth time. The drive tops at 138.3 km/h and the lesson fires its result screen at `y=923.1` of a 2 600 m carriageway. He will not see 160 because of the tier governor and the finish line, and that is a syllabus call, not a bug. |
| **The 29 quarantined first-aid questions** | His medical ruling. They are the sole cause of both protected reds. |

---

## STILL OPEN after the 2026-08-05 DRIVE gate — every item has a NAME, an OWNER and a REASON

**This report does not end in a percentage and no percentage closes this section.** Everything the
sections below list is still open unless it is named here as closed. What follows is only what *this*
gate closed, found or is handing over.

### Closed by this gate

| what | evidence |
|---|---|
| **`tsc --noEmit` exits 2 with 2 errors in `b5-crash-pin-rescue.test.ts`** (`preDriveRequired` / `.done`) — carried by the last gate and by three lane reports | **CLOSED.** `npx tsc --noEmit --incremental false` → **exit 0, zero error lines**, three runs across the session. That test file compiles and its **4 tests pass**. |
| **`pe-districts.test.ts` (7 errors / red), `ped-through-parked.test.ts`, `scenery-sightline.test.ts`, `dev-surfaces-gated.test.ts` (`KNIJKA_DIST_DIR` 5 s timeout)** | **ALL FOUR GREEN**, run by name and alone: **110/110 · 4/4 · 8/8 · 40/40 in 5.95 s.** The whole of `src/modules/sim` is **528 files / 8 058 tests / 0 failed.** |
| **„were the five shadow traces re-recorded?"** | **There are SIX, not five, and none of them needed it.** The commit radius did not move; the observation radius was added beside it. Proven by re-recording all six with `RECORD_TRACES=1` (81 tests green) and showing `git status` clean on all 18 roundabout traces afterwards. |
| **A held brake can become throttle in any gear** | **CLOSED — D1 above.** 1 955 frames of `brake=1.0000 / throttle=0.0000` in D, 925 + 1 227 frames of a held pedal carried into R with **y span 0.0000 m**, driven on both the scripted and the real-keyboard channel. |

### Opened or carried by this gate — named, owned, and not one of them a percentage

| # | what is wrong | owner (the actual file) | why it is not fixed |
|---:|---|---|---|
| 1 | **The roundabout conviction is produced by the right adjudicator and explained by the WRONG LAW — and it is now photographed twice** | `modules/sim/rules/catalog.ts`, entry `FAILED_TO_YIELD` (+ the override channel in `rules/engine.ts`) | The barge card reads „**На кръстовище без светофар пропускаш идващите отдясно**" with the chip „**ЗДвП чл. 47**" — the right-hand rule — **at a roundabout entry, where the duty is чл. 50, ал. 1 toward the ring on your LEFT.** The L4 result screen repeats it: „Непропускане… (ЗДвП чл. 47) → Правилното действие: … **огледай дясно**". Frames `scratchpad/verify/barge3-card-t47.0.png` and the exam transcript. **Not fixed here because `catalog.ts` is a flat code→copy map with no per-situation variant:** the fix touches the catalogue type, the engine's override branch, the teach-moment pipeline, the fault card and the end-of-session разбор. Its own change with its own gate. It was flagged by the previous wave and it is still on screen; a student is being taught the wrong rule at the moment he is being billed 10 points. |
| 2 | **The drive rig publishes the SCRIPT's pedals, not the car's — so a keyboard drive's `thr`/`brk` columns are a constant** | `modules/sim/devrig/rig.ts:401` — `throttle: this.command.throttle, brake: this.command.brake` | A real-keyboard drive reports `thr=0.00 brk=0.00` **while the driver is standing on the pedal**, because the rig echoes its own injected command and no key was injected. This gate hit it and worked around it by re-running the same provocation with the brake injected through the rig's own `holdBrake` step so a measured `1.00` reached the record. **Any row closed on a keyboard drive's pedal columns is closed on a number that carries no information.** One line: publish the merged `VehicleInput` the scene actually consumed. |
| 3 | **Two untracked `node:test` files break `npm test` for everyone** | the mobile-harness lane · `platform/vitest.config.ts` include glob | `../tools/mobile/**/*.test.mjs` sweeps in `navigation.test.mjs` and `ready.test.mjs`, both `import test from "node:test"`, which vitest counts as failed suites. Their own sibling `settle.test.mjs` documents that node:test files here belong to `scripts/tools-tests.mjs`. **`navigation.test.mjs` also runs three 180 s Playwright navigations against a dead port inside the unit run**, which is most of that run's wall clock. Either commit them under `scripts/tools-tests.mjs` or narrow the glob. |
| 4 | **`api/review/route.test.ts` mocks a module that gained an export** | the review / api wave | `route.ts` (dirty in the tree) calls `parseQueue` at line 45; the test's `vi.mock` of `@/modules/content-admin` does not return it. Three failing tests. |
| 5 | **`content-bank` REVIEW_DEBT and `compose` quiz-beat** | the theory / content wave | `ptp-i-parva-pomosht` is 31/64 (48 %) approved and `l-accidents-first-aid` has no quiz beat. Both driven by `content/**` files that were already dirty when this session opened. |
| 6 | **The two untracked contact strips want COMMITTING, not ignoring** | whoever stages next | `sheets/sc-jx-giveway-b1__m0.strip.png` + `__m1.strip.png`, 738 KB, mtime 2026-08-04 23:55. 48 of the 50 files in that directory are already tracked, `.gitignore` carries a deliberate carve-out for it, and **this register cites those two by path in four places, including the B29 row's FIXED-SEEN evidence.** Ignoring or deleting them breaks the citation. |
| 7 | **A live barge at a roundabout is not reproducible on demand — only from the committed trace** | `modules/sim/orchestrator/runners.ts` — `RoundaboutEntryRunner`'s sync | The runner places the circulator `conflictLeadM` upstream of the entry **relative to the player's predicted arrival**. A driver who never slows arrives on a different schedule: two barges at 21.6 and 13.6 km/h met a genuinely clear ring — **no conviction and no collision, which is correct** — and the conviction only reproduced after a 10 s pause. **Not a defect. It is a testability gap:** the mistake this drill teaches cannot be provoked from a live drive without first standing still, so anyone verifying it from a rig will report a false „the detector is dead". |
| 8 | **The queue drill's route marker sits 40 m past the moving tail** | `lessons/scenario/templates-following.ts` — `sc-fs-stopped` at `y = 281 r = 12` vs `FS_LEAD_CAR`'s rest at `y = 290` | Found by driving. The green objective marker is at y = 281 while the queue tail is still **rolling through y ≈ 250** — so a student who aims at the marker and holds the nominal 20 km/h drives into the back of it at y ≈ 254. That IS the fault the drill teaches and the new detector warns 2 s out, so it is arguably the lesson working. **But the guidance marker is the one thing on screen telling him where to go, and it points past the hazard.** Worth a ruling: either the marker follows the tail, or the copy says „stop where the queue is, not where the circle is". |
| 9 | **Two zero-assertion scratch probes are still in the test tree** | unknown — both untracked | `lessons/scenario/__tests__/zz-b58-survey.test.ts` and `traffic/__tests__/zz-fr21-measure.test.ts` contain **0 `expect(` calls**. They pass, they measure, and they are indistinguishable from an abandoned probe. *(`runtime/__tests__/b15-roundabout-wait.probe.test.ts` has 1 assertion and is named „probe" too.)* Not deleted here: a previous gate recorded „a lane is deleting files in `world/__tests__` mid-run" as a live hazard, and this gate will not become that lane. |
| 10 | **This wave's entire proof is untracked** | the lead who commits | `reverseAssist.test.ts`, `yield-voice.test.ts`, `roundabout-armed-at-the-paint.test.ts`, `roundabout-mouth-not-right-hand-rule.test.ts`, `b5-crash-pin-rescue.test.ts`, `b5-junction-mouth-clear.test.ts`, `b5-queue-drill-survivable.test.ts`, `junction-scan-control-copy.test.ts` and the whole `modules/sim/devrig/` package are `??`. **99 green tests and the instrument that produced every frame in this section are one `git clean` from not existing.** |
| 11 | **`POOR_LANE_KEEPING` fires on any rig drive that steers round the ring** | nobody — recorded so it is not misread | The drive script only supports a CONSTANT steer per step, so a scripted circuit of a roundabout is a polygon and the lane-keeping detector says so. **It is the rig, not the product.** Anyone reading a roundabout telemetry dump will see it and should not file it. |

---

## STILL OPEN after the 2026-08-05 COCKPIT gate — every item has a name, an owner and a reason

**No percentage closes this section and this report does not end in one.** Everything the previous
section lists is still open unless it is named here as closed. Below is only what *this* gate changed
or found.
...
| what | evidence |
|---|---|
| **`B34` — the priority road never repeats its limit** | **CLOSED.** Three new В26 at station 35.13 m from `tj-emerge-v1`'s junction node, measured through `buildWorldGeometry`, one of them photographed from the seat 23 m west of the junction: `scratchpad/b34lane/shots2/b34lane-xm19-t59.3-v14.png`. The owner this register named for two gates (`gen_t_junction.mjs`) was the wrong file and could never have carried it — it does not emit that district. |
| **`tsc --noEmit` exits 2 with 9 errors, seven of them in `pe-districts.test.ts`** | **CLOSED.** Exit 0, no output. The seven `PeMapCase` rows were brought into line with the four new fields. |
| **`traffic/__tests__/ped-through-parked.test.ts`** and **`scenery-sightline.test.ts`** | **GREEN** in a 284-file run of `world` + `traffic` + `traces`: 4 034 of 4 035 tests passed and the one failure was this gate's own throwaway probe. |

### Opened by this gate — named, owned, and none of them a percentage

| # | what is wrong | owner (the actual file) | why it is not fixed |
|---:|---|---|---|
| 1 | **A ruling is owed before anyone spends B16's re-record, and it is his, not ours** | the founder | `270°` of kerb is satisfied only from **above**. At R = 46 the top-down is a perfect annulus and the **cockpit frame reads worse than at R = 31** — a 41.5 m island fills the horizon as parkland and the ring's curvature leaves the windscreen, so it reads as a T-junction. Both frames are on the B16 row at an equal 23.7 m to the kerb. **If his sentence is about the shape of the map, 46 / 84 is right; if it is about what he sees on approach, the target is nearer 31–39 and the 270° threshold should be retired.** The spend behind the wrong answer is 36 trace files, four generators, a `gen_streetwall.mjs` re-run and three `templates-*.ts` files. |
| 2 | **The parked row is right-of-travel only — every bay is drawn on both sides and filled on one** | `modules/sim/traffic/TrafficLayer.tsx` — `computeParkedCars` | `const nx = dy; const ny = -dx` is the right-hand normal with **no side alternation**, so all 16 bodies across the six bayed PE districts stand at `x = +10.13` and the left bay is always empty tarmac. **This is the single biggest reason five of the seven pedestrian streets still read as one composition.** The file was on the do-not-touch list and another wave has it open. |
| 3 | **No median, refuge island, raised table, staggered or angled crossing is expressible at all** | `world/types.ts` + `runtime/district.ts` (mirrored) + `builders/markings.ts`, `decals.ts` | `DistrictCrossing` is `{id, x, y, kind, signalized, edgeId}` and `CrossingKind` is `signals\|marked\|unmarked\|unknown` — there is no field for any of it, so that half of the PE brief was not deliverable from the generator. **A central reservation cannot be faked with a building either:** `cityBuildings.DATA_HEIGHT_MIN_M` clamps every authored volume up to 3 m, so a 0.3 m island renders as a 3 m wall across the road. A schema change, and `runtime/**` is reserved. |
| 4 | **Наредба № РД-02-21-1/2023 is cited constantly by the content bank and its text is not in the repo** | `content/law/sources.json` + `content/law/README.md` (the re-fetch recipe is already written) | The row is catalogued `coverage: "index-only"`, `extraction: null`, **no sha256**, while five `content/audits/*.audit.json` files carry the rule line *"every sourceLine is quoted verbatim from … the locally extracted ППЗДвП and Наредба № РД-02-21-1 text (sha256-pinned in `content/law/sources.json`)"*. `sources.json` holds three full-text acts and this is not one of them, **so every `lawRef` citing that ordinance is currently unverifiable against the corpus — and the signs bank cites it constantly.** Ingesting it would also settle whether a statutory В26 repetition distance exists and replace the authored 120 m with a cited number. *(Flagged only: `content/**` is reserved.)* |
| 5 | **The В26 repeat is a junction restatement, not a mid-block cadence — and the deep block is still silent** | `world/builders/props.ts` — `LIMIT_REPEAT_MAX_KMH` | The new plate stands 35 m past the junction; the cadence that would fill the rest stops **below 50 km/h on purpose** (50 is ЗДвП чл. 21's inferable built-up default; 30 and 40 are reductions a driver cannot infer). So on `tj-emerge-v1`'s 160 m westbound arm he sees one plate at 35 m and then nothing for ~125 m. **Deliberate and measured** — widening to 50 buys these two districts nothing and over-signs `sp-creep2-v1` by 5 plates — but it is a judgement call worth revisiting if he drives a long junction-less 50 stretch and still feels blind. |
| 6 | **A streetwall canopy hides the new В26 until ~15 m** | `tools/maps/gen_streetwall.mjs` (the occluder), not the sign pass | `b34lane-xm16` shows the pole but not the disc; it clears at about `x = −20`. Fine at 14 км/ч; roughly one second of reading time at the posted 50. The post's station is correct by construction — the occluder is another lane's building. |
| 7 | **`gen_streetwall.mjs` rewrites three districts it was not asked to touch, on every run** | `tools/maps/gen_streetwall.mjs` / `tools/maps/lib/streetwall.mjs` | `d2-v1`, `rb-mini-v1` and `tj-emerge-v1` are rewritten even when only the PE maps are the target — `d2-v1` loses two procedural volumes (380 → 378) and `streetwall.test.ts` then fails on a district the runner never intended to touch. The PE lane reverted them after each of three runs; **the next person who follows the generator header's own instruction to re-run the street-wall pass will hit it.** *(Already carried as "no longer idempotent"; now it has a reproduction.)* |
| 8 | **Two untracked PNGs are sitting in the repo** | whoever generated them | `tools/clips/headless/sheets/sc-jx-giveway-b1__m0.strip.png` and `__m1.strip.png`. **626 MB of frames have been committed by accident three times.** Move them to a scratchpad before anyone stages. |
| 9 | **A lane is deleting files in `world/__tests__` mid-run** | unknown | This gate's own probe was removed from under a running vitest process. Harmless here; it will not be harmless when it happens to a tracked test. |

---

## STILL OPEN after the 2026-08-05 instrument gate — every item has a name, an owner and a reason

**No percentage closes this section, and this report does not end in one.** Twenty-three rows are
improved-but-not-finished and two are still broken outright. Below is every remaining thing, who owns
the file it lives in, and why it is not done. Where the block is on you rather than on us, it says so
plainly instead of counting it against us.

### The two rows that are still broken outright

| row | what he sees | owner | why it is not fixed |
|---|---|---|---|
| **B65** | "I see many issue with the Map its very Raw, boring" | the scenario-street generators (`tools/maps/gen_sp_*.mjs`) + `world/builders/props.ts` | **Downgraded from PARTIAL by this gate, on purpose.** Re-rendered along `sp-creep-v1` and opened: two grey slabs with orange window rectangles, one parked row, no kerbs, no crossings, no arrows, no stop bars, no side turnings, no bins or benches or wires — **one lamp post in 360 m** — and the carriageway runs on into an empty green plain. Nothing has changed since the look-wave, so PARTIAL was flattering it. This is a **street-furniture build lane**, not a drive, and **nobody has owned it in three waves.** |
| **B52** | the children on the pavement | **your Blender machine** | See "Blocked on you" below. No amount of code closes it. |

### The tree is red — and for the first time in four gates, one of the reds is ours

| # | what is red | owner | detail |
|---:|---|---|---|
| 1 | **`tsc --noEmit` exits 2 with 9 errors** | **ours — the world / parked-bodies lane** | Seven are in `platform/src/modules/sim/world/__tests__/pe-districts.test.ts`, a **tracked, modified** product test: `PeMapCase` gained `carriageway`, `seatCurbToCurbM`, `widthChangesAtM` and `parkedBodies`, and **seven of its `CASES` were never updated to match**. Three lanes reported "tsc EXIT 0"; it does not. *(The other two errors are untracked `zzprobe-*` scratch probes in `modules/lesson`. The `zz-b29-*` files that blocked the last gate are **gone** — that cleanup did happen.)* |
| 2 | **`traffic/__tests__/scenery-sightline.test.ts` — "is clean on all 90 committed districts"** | same lane | The 90-district census is red while the *give-way* district it was opened on is green. This is the FR-21 remainder below, showing up as a test rather than as a paragraph. |
| 3 | **`traffic/__tests__/ped-through-parked.test.ts` — "the geometry that produced his sentence still fails the same measurement"** | same lane | Same family. |
| 4 | **`app/dev/__tests__/dev-surfaces-gated.test.ts` — `KNIJKA_DIST_DIR never reaches application code`** | whoever owns the dev-surface gate test | `Error: Test timed out in 5000ms` — a real timeout, not an assertion failure, so the *safety property* it guards is fine. **But the drive-rig lane's claim that "it passes in isolation in 903 ms" is false:** run alone it failed three times out of three, at **22.2 s / 26.8 s / 39.5 s**. It file-walks the whole app source tree against vitest's **5 s default** `testTimeout`, and on this 7200 rpm E: that walk costs 20–40 s. **It will fail for anyone on this box, every time. One `testTimeout` argument fixes it; a re-run does not.** |
| 5 | **`world/__tests__/zzz-zebra-scratch.test.ts` — 0 tests** | another lane's leftover | An empty scratch file left in the tree; vitest counts "no test suite" as a failure. Delete it. |

### FR-21 — the pavement parking, measured rather than estimated

**68 of 100 committed districts still place at least one procedurally parked body off every
carriageway.** 58 of them need **only a `parkingBand` tag** — the frontage headroom is already there,
from +0.03 m to +101 m. **Ten are genuinely blocked, and they are four different problems:**

- **Six generators share one building constant and one number** — a b-approach/corner block at
  14.12 m against a 15.975 m need, deficit ~1.85 m: `mv-uturn-v1`, `pk-ban2-v1`, `pk-banx-v1`,
  `pk-busstop-v1`, `pk-double-v1`, `pk-rail-v1`. **152 bodies come off the pavement for a 2 m nudge
  of one constant in six `tools/maps/gen_*.mjs` files** — the same nudge 57 other generators already
  have. *This is the cheapest 150-body win left in the product.*
- **`pk-drive-v1`** (8 bodies): the blocker is `pkd-b-garage` at 12.000 m and **it cannot move — the
  drill is a driveway meeting the street.** Needs a parking band with a gap at the driveway.
- **`rx-tram-stop-v1` and `rx-tram-island-v1`** (26 bodies): the blockers are a tram shelter and a
  tram island at 0.400 m. **A tram island is 0.4 m off the centreline because that is what a tram
  island is.** The row must be suppressed on the island side.
- **`district-v1`** (80 bodies) — the OSM-derived Sofia map, the only non-micro one. 13 of its 18
  offending edges are blocked, worst frontage **3.139 m**. **Nothing can move: those buildings are
  surveyed from the real city.** That map needs the row placed from each edge's *actual*
  kerb-to-frontage distance instead of a fixed cross-section.

### Found by driving, filed with numbers, owned by nobody yet

| # | what is wrong | owner (the actual file) | why it is not fixed |
|---:|---|---|---|
| 1 | **The roundabout grader is not armed at the give-way line it tells him to stop at** | `runtime/worldRuntime.ts` — `ROUNDABOUT_ENTRY_MARGIN_M` | The tracker arms only within `radius + 12` = **30 m** of the ring centre; the М8 paint is at **35.96 m** and the drill's own green marker at 34.24 m — **both outside.** So the stationary-clear repair shipped *for B15* never executes for a student standing where the paint tells him to stand, and when the grader does wake 6 m later it convicted at **0.96 s and 0.92 s** after arming — exactly `YIELD_CONVICT_SUSTAIN_SEC`, conflict already true on the first armed tick. **From the seat that is indistinguishable from “the instant the wheels turn”.** Deliberately not changed: widening it re-arms the tracker earlier across the whole roundabout family against **five gated shadow traces**. That is a grading change, not a photograph. |
| 2 | **Nothing coaches him during the wait — the first word this drill ever says about the priority car is −10** | same file, plus the advisor | Full event dump across a 40.57 s wait with the circulating car crossing the mouth in front of him: **nothing on rule, card or teach.** Against requirement zero (doc 64 THEO-4) that is the sharpest open item in this register: the student does the most important thing a learner does at a junction and the virtual instructor is silent. |
| 3 | **Holding the brake at a standstill auto-shifts D→R and the held brake becomes a floored reverse accelerator** | `engine/reverseAssist.ts` | Measured: gear flipped 1→−1 at t=40.363 (0.35 s after the wheels stopped = `REVERSE_ASSIST_HOLD_S`), car accelerated **backwards to −16.8 km/h under `brake=1.00`**, logged COLLISION, ended 6.8 m back; the frame shows „Кола отзад · 0 м". **A student doing the correct thing at a Б2 sign — full stop, count to three — reverses into whatever is behind him.** The rig works around it by pulsing the brake; the product defect is untouched. |
| 4 | **A stationary van stands in the student's own lane in the mouth of `jxg-giveway-v1`'s second junction** | `templates-junctions.ts` + `traffic/system.ts` | Found by driving B5's sequence. The correct drill — stop at Б1, wait, „щом пътят е чист, потегляш" — ends in a **10-point COLLISION at `y=146.00`**, and then the car is **jammed at full throttle for 40 s**, third objective unreachable, lesson unfinishable. Not the same defect as B29/B30: this body is not on the pavement, it is in the carriageway the student is told to drive up. |
| 5 | **The scan fault tells him he crossed «знак Б2» — in the lesson titled «Б1 не значи спри винаги»** | `rules/catalog.ts:499` + `rules/types.ts:484` | `rules/engine.ts:1696` arms the code on `giveWay` deliberately and correctly; the single catalogue string names Б2 unconditionally. **One string plus a control-aware branch.** It is the cheapest correctness fix in this whole section and it is embarrassing in a screenshot. |
| 6 | **`STOP_SIGN_NO_FULL_STOP`, `FAILED_TO_YIELD` and the stop-line objective all fire 27.7 m out, while the car is still doing 19.6 km/h** | the give-way / stop-line lane | On `sc-junction-stop`: convicted and credited at `y=−27.69`, then the car went on to actually stop **17.1 m further along**. Whether `y=−27.69` matches the painted Б2 line is for that lane to adjudicate — **the point is the measurement now exists**, which is exactly what B15 died without for three waves. |
| 7 | **The queue drill is unsurvivable at its own nominal speed** (his 40) | `templates-following.ts` `FS_LEAD_CAR` + `rules/types.ts` `followMinSpeedKmh` | Driven at the drill's own pace: **two collisions, zero основни, zero второстепенни, no distance fault at any point** — result screen «20 наказателни точки · НЕИЗДЪРЖАН». The truck drill was fixed with a per-lesson floor of 10; **this one was deliberately left alone**, because its subject *is* a queue rolling at 8 km/h and the 20 km/h floor is right for its standstill phase. What its approach needs is a gentler final leg or a **closing-rate** detector — a curriculum call, not a 2 a.m. tuning. |
| 8 | **B58's two residuals** | the TERMINUS family + the sign-placement lane | The gate lie is fixed (it reads «не по-бързо от 50 км/ч» now), but `ov-keepright-v1` still **ends into a field with both flow vehicles standing on open grass**, and the В26 «50» that instruction 2 tells him to read is **never in the windscreen** — it is 30 m ahead behind the rear-view inset. Same occlusion class as B57. |
| 9 | **FR-50 — every `mw-v1` lesson finishes at 860–940 m of a 2 600 m road** | the motorway templates | Photographed: the result screen fires at `y=923.4` at **138.6 km/h, still accelerating in 5th**. 1 677 m of carriageway is never driven, and it is systematic — `sc-mw-discipline`, `sc-mw-min-speed`, `sc-fo-motorway-gap`, `sc-ac-truck-spray`, crosswind all finish at 860–940. **The map grew from 1 006 m to 2 600 m and not one lesson's finish line moved with it.** Flat out on the whole road on the default tier gives **145.2 km/h**. *He still will not see 160.* The road is no longer the limit; the tier governor and the 923 m finish line are. |
| 10 | **The two drills that do not exist** | `contracts.ts` (unblocked) + a new grader + new sign art | **ЗАОБИКАЛЯНЕ (ЗДвП чл. 43б):** its structural blocker is gone — `StagedActorPathSpec.indicator` now exists, so a car can stand still with its left blinker lit, which is the drill's defining image. What remains is a **чл. 43б ал. 2 grader that does not exist** (no adjacent-lane-occupancy conflict in the catalogue), plus a host map (`ln-arrows-v1`) and three traces. **Т13 is not buildable at all:** all 36 `SIGN_KINDS` are standalone faces, and Т13 is a plate mounted *under* Б3 — it needs a new kind, new face art, an under-post mounting rule, and a district whose priority road physically bends. **An art job plus a map job, not a template.** |

### Carried forward, still true, still nobody's

`B27` never photographed from the cockpit · `B34` the priority road never repeats its limit
(`gen_t_junction.mjs`) · `B60` no children playing on pavements · `B16` the roundabout is not round
(map-and-trace job) · `B50/B53/B54` seven pedestrian streets share one carriageway · **the authored
building height discarded on 93 districts** (332 volumes on `heightSource:"default"`, 111 in the
flagship `d2-v1` — *this should be the next wave*) · `DEFAULT_SEED = 1337` shared by all 90 districts
· `gen_streetwall.mjs` no longer idempotent · `B79`'s lead car cannot be followed lawfully at the
posted limit · **Д15/Д16, В23, В2, В25, Г12 named in lesson copy and unplaceable by the world.**

### Housekeeping this gate is handing over rather than fixing under other lanes' running servers

- **`platform/tsconfig.json` carries six scratch globs across three families** — `.next-harness/**`,
  `.next-rig/**` and now `.next-exhdr/**`. The last register said "four"; it is six, and a new family
  has appeared since. `src/lib/tsconfigHygiene.test.ts` fires on it. **Reverting is futile while those
  dev servers run — they re-inject on the next compile — so it needs the servers stopped and the file
  stripped in the same minute.**
- **`tools/clips/headless/sheets/` is not gitignored** and holds **50 untracked PNGs**, two of them
  written by this wave. One line in `.gitignore` closes it; the lane claim "zero PNGs in the repo" was
  false, and the directory has been like this since 21 July.
- **`drive-rig.mjs:145` swallows its own timeout** — `page.waitForFunction(fn, { timeout })` passes the
  options object as Playwright's `arg`, so the **30 s default** applies. Needs
  `waitForFunction(fn, null, { timeout })`.
- **`dev-surfaces-gated.test.ts` is permanently red on this hardware, and was written off as a flake.**
  `KNIJKA_DIST_DIR never reaches application code` file-walks the whole app source tree against
  vitest's **5 s default** `testTimeout`; the walk costs **22–40 s** on E:. It failed three times out
  of three in isolation. **The safety property it guards is fine — the test is not.** One
  `testTimeout` argument closes it, and until then every gate on this box reports a red it will be
  tempted to explain away.
- **`world/__tests__/zzz-zebra-scratch.test.ts` has 0 tests** and vitest counts that as a failed file.
  Somebody's leftover; delete it.
- **A long drive on the shared `:3200` dev server can be silently spliced by a Fast Refresh.** This
  gate's first 70-second wait was destroyed twice that way and only the rig's own clock-reset guard
  caught it. **Two waves of B15 evidence were produced on that server.** The guard is the best thing
  either instrument lane built; nobody should run a long drive without reading its warning line.

### Blocked on you, not on us — said plainly

- **FR-35 the traffic officer and FR-43 the children (B52) are blocked on your Blender machine.** Both
  are still procedural capsule rigs and previous gates photographed them as such: the officer measures
  ~2.1 m — taller than any car in the fleet, which is what you asked for — but the same zoom shows a
  sphere head, no face, and capsule arms fused at the shoulder. **No amount of code closes these.**
- **FR-19, the 13 pre-drive tutorial clips, is blocked on a payment.** Atlas returns 402, FAL is
  locked at −0.21, Poyo has no entitlement. **The seam is built and empty by one constant** — the grey
  box with a gear icon in the middle of every tutorial card is where the clip goes. One entry per step
  upgrades it with no other edit.
- **B19 and everything at L2–L5 is out of scope because you ruled it so.** Your entire review is L1 —
  22.6 % of the product. **No L5 frame has been opened and this gate claims none.**

---

## STILL OPEN after the 2026-08-04 verify gate — *(previous pass, kept for reference)*

**No percentage closes this section.** Each line below is a thing that is still wrong, who owns the
file it lives in, and why it did not get fixed. Where the block is on you rather than on us, it says
so plainly instead of counting it against us.

| # | what is still wrong | owner (the actual file) | why it is not fixed |
|---:|---|---|---|
| 1 | **The pedestrian's own signal head is unreadable from the seat** (open half of B55) | `PED_LAMP_OFFSETS` in `sim/world/components/WorldProps.tsx` | Measured, not guessed: the lens sphere (r 0.105) sits at z +0.12 inside a housing whose front face is at z 0.19, so **0.035 m of sphere clears the box**, and the head faces across the carriageway so the driver sees it 65–77° off-normal — a 1–2 px sliver. One-line remedy (raise the offsets to ≈ 0.20) was **not applied because a parallel wave has that file open**, and a second pair of hands in it is the exact failure this wave was told to avoid. |
| 2 | **B27 has never been photographed from the cockpit** | `components/sim/RouteGuidance.tsx` (marker pillar) + `components/sim/CameraRig.tsx` (eye point) | The sign is provably on the street. The two occlusions this register blames — the Б2 head behind the interior mirror housing at y −44, and the objective marker pillar washing the windscreen at the stop line — need a cockpit frame, and `CameraRig.tsx` is on the reserved list and currently modified by another wave. **One drive closes it.** |
| 3 | **B34: the priority road never repeats its limit** | `tools/maps/gen_t_junction.mjs`, or a repeat rule in `world/builders/zoneSigns.ts` | `tj-emerge-v1`'s two В26 «50» posts sit at district x = ±115 — the far ends of a 320 m boulevard. Same shape on `tj-stop-v1` (x = ±105). Nobody owned it this wave. |
| 4 | **B60 — no children playing on the pavements** | `traffic/TrafficLayer.tsx` + `world/builders/props.ts` | **Untouched this wave, and the evidence filed against it belonged to B59.** Four staged school-yard children exist; nobody has added play behaviour on the pavement he was looking at. |
| 5 | **B70 needs one drive to know what it is** | `templates-following.ts` | The constant this register condemned (`offsetM: 290`) is already **48** in the working tree, from another wave. Neither "broken" nor "fixed" is currently earned. |
| 6 | **B16 — the roundabout is not round, and the owner in this register was wrong** | `tools/maps/gen_mini_roundabout.mjs`, `gen_rb_2lane.mjs`, `gen_rb_ped.mjs`, `gen_rb_single.mjs` — **not** `world/builders/roundabout.ts` | The builder is correct: the same code draws a 254° kerb on `rb-single-v1`, a 153° one on `rb-mini-v1` and **252.8° on the real-Sofia `district-v1` ring at a nearly identical R = 19**, because mouth width is fixed by **arm width over ring radius** and the generators author the arms at `PERCEPTUAL_ROAD_SCALE` 2.5 against a ring radius written at real-world scale. ~~`rb-mini` needs radius ≈ 31 m instead of 18~~ **— THAT NUMBER WAS WRONG AND THE 2026-08-05 COCKPIT GATE MEASURED THE REAL ONE. R = 31 gives 232.4°, 37.6° short of the 270° bar it was written for.** The break-even is **R = 45.3 m** for `rb-mini` / `rb-ped` / `rb-single` (ship 46 → 271.3°) and **R = 83.0 m** for `rb-2lane` (ship 84 → 271.1°). **And 270° is a top-down metric: at R = 46 the cockpit frame reads WORSE than at 31** — see B16 for both frames. **A ruling on which shape he meant comes before the spend**, because the spend is 36 trace files, four generators, a `gen_streetwall.mjs` re-run (8 `sw-rbm-*` buildings the roundabout generator does not emit would otherwise be deleted, and at R = 46 the old footprints sit on the ring), six gallery stills, and three `templates-*.ts` files that pin the ring by value. |
| 7 | **The seven pedestrian streets still share one carriageway** (B50/B53/B54) | `tools/maps/gen_pe_crossings.mjs` + whoever re-tunes doc 86 T11 | All seven are 16.25 m curb-to-curb with `parkingBandM 0`, so the bottom ~45 % of every frame is the same picture. **The width lever is blocked, not forgotten:** a 4 m parking band grows the staged crossing to 24.25 m, adds 5.7 s at 1.4 m/s, and drops `sc-crossing-slow-crosser` and `sc-crossing-white-cane` from 3 stars to 1. It needs the two drills re-tuned in the same commit. |
| 8 | **The authored building height is discarded on 93 other districts — the biggest thing this wave found** | `cityBuildings.resolveBuildingHeightM`, plus every generator except `gen_pe_crossings.mjs` | Across the 100 shipped districts, **332 buildings carry `heightSource: "default"` and 4 carry `"height"`** — i.e. almost every authored height in the product is thrown away and replaced by a hash in [15, 25] m, including **111 volumes in the flagship `d2-v1`**. Fixed for 7 files only. Fixing it properly moves every map in the product and invalidates pinned frames. **This should be the next wave.** |
| 9 | **The prop RNG is one constant for all 90 districts** | `DEFAULT_SEED = 1337` in `buildWorldGeometry` | Two same-class straight streets get byte-identical planting at byte-identical metres — which is why four of the seven pedestrian maps had trees at exactly (14.4, −11.0) and (−14.0, −33.0). Cured indirectly on that family; the real fix is a per-district seed, and `DEFAULT_SEED` is shared by every pinned prop census in the tree. |
| 10 | **`platform/tsconfig.json` carries four scratch globs** | `platform/tsconfig.json` | `.next-harness/**` and `.next-rig/**` are in `include`. `src/lib/tsconfigHygiene.test.ts` fires on both of its assertions, and `AGENTS.md` names this as the source of phantom `tsc` errors. Left alone rather than break another wave's running server — **strip it before the tree is gated.** |
| 11 | **`gen_streetwall.mjs` is no longer idempotent** | `tools/maps/gen_streetwall.mjs` | Its header promises a re-run is a zero-byte diff. It now deletes one `sw-` volume from `d2-v1.json` and 23 lines from `tj-emerge-v1.json`. Both were reverted, but the drift will bite the next person who runs the pass. |
| 12 | **`drive-rig.mjs` swallows its own timeout** | `tools/clips/headless/drive-rig.mjs:145` | `page.waitForFunction(fn, { timeout: 120_000 })` — Playwright's second argument is `arg`, not options, so the object becomes the arg and the **30 s default** applies. It killed drives on this box twice. Needs `waitForFunction(fn, null, { timeout })`. The file is untracked and owned by another wave. |
| 13 | **B79's residual: the lead car cannot be followed lawfully at the posted limit** | `templates-lanes.ts` — but the lever is proven closed | `OVC_LEAD_CAR.maxMatchSpeedMps` 12 (43.2 km/h) on a street posted 50, and `followGapM` 20 of centres puts the band's fixed point under the 1.26 s fire line above 45.4 km/h. Ledger T18 already bisected the gap lever and proved raising it destroys both `OVERTAKING_AT_CROSSING` demos. **Closing this needs the two mistake demos re-choreographed** — a wave of its own. |
| 14 | **Signs the lesson copy names that the world still cannot place** | `world/types.ts` + `world/components/signFaces.ts` | **Д15/Д16** (10 mentions — `templates-pe2.ts:503` says so out loud), **В23** (7), **В2** (3), **В25** (1), **Г12** (1). Also worth an owner's eye: the copy names **А34/А35** for rail crossings while `types.ts` describes the shipped faces as „А32-style" and „А33-style" — **the code the student reads and the face he meets may not be the same sign.** |

### Blocked on you, not on us — said plainly

- **FR-35 the traffic officer and FR-43 the children (B52) are blocked on your Blender machine.** Both
  are still procedural capsule rigs and this gate photographed them as such: the officer now measures
  ~2.1 m — taller than any car in the fleet, which is what you asked for — but the same zoom shows a
  sphere head, no face, and capsule arms fused at the shoulder. **No amount of code closes these.** The
  child's height, head ratio, jacket palette, silhouette and ball mesh all live in one file, and what
  *is* ours in that drill (speed 2.6 m/s, `ballLeadSec` 0.5, `triggerDistM` 38) is tuned choreography
  that the committed traces grade against — changing it would be a grading change wearing a costume.
- **FR-19, the 13 pre-drive tutorial clips, is blocked on a payment.** Atlas returns 402, FAL is
  locked at −0.21, Poyo has no entitlement. The seam is built and empty by one constant. You can see
  the hole: the grey box with a gear icon in the middle of every tutorial card is where the clip goes.
  **One entry per step upgrades it with no other edit.**
- **B19 and everything at L2–L5 is out of scope because you ruled it so.** Your entire review is L1 —
  22.6 % of the product. The B50 height fix reaches L5 because it lives in district data, but **nobody
  has opened an L5 frame** and this gate claims none.

---

## STILL OPEN after the 2026-08-03 gate — no softening, with owners *(previous pass, kept for reference)*
...
**Verdicts (2026-08-04 verify gate): 107 · FIXED-SEEN 71 · PARTIAL-SEEN 29 · BROKEN-SEEN 4 · NOT-A-DEFECT 3 · UNVERIFIABLE 0.**
All 107 rendered and looked at. **33 rows are still wrong and every one of them is named, with an
owner and a reason, in § STILL OPEN after the 2026-08-04 verify gate.** Three of those 34 are blocked
on you — the officer and the children on your Blender machine, the tutorial clips on a payment — and
they are recorded as blocked, not as ours failing.
