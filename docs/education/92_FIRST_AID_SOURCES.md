...
> редактирана на четири места (тази навигация, статусът „ЗАТВОРЕНО“ на §7.1, една поправена мека
> тирета в цитат от ERC и затварянето на находки 1, 2 и 5 в §7.3 на 2026-09-12 — генераторът още
> носи СТАРИЯ им текст, `tools/theory/gen_first_aid_sources.mjs:855` и наоколо), инструментът
> **отказва да пише**, ако главата на диска се разминава с
...
1. ⚠️ **ЧАСТИЧНО ЗАТВОРЕНО — `content/concepts.json` вече НЕ изрича обърнатото указание; остава
   декоративният `ЗДвП чл. 123`.** Прочетени на 2026-09-12, и четирите обобщения учат линията на
   ERC/RCUK 2025: `c-victim-handling` казва „човек в безсъзнание, който диша нормално, не се обръща“
   и носи изхода за дихателния път; `c-cpr-basics` открива с изречението, че прагът не е „не диша“, а
   „не реагира и не диша нормално“. Втората половина на поправката от §7.1 — махането на `ЗДвП чл. 123, ал. 1,
   т. 2, б. „в“` от `c-first-aid-priorities`, `c-cpr-basics`, `c-bleeding-control` и
   `c-victim-handling` — НЕ е направена, и не е чиста промяна само в съдържанието: цитатите на тези
   четири концепции са ЗАКОВАНИ в `platform/src/modules/lesson/clearanceCitations.ts`
   (`CARRIED_CONCEPT_CITATIONS`), а пинът се проверява от
   `__tests__/clearanceCitations.test.ts`. Измерено: махането мести и четирите хеша
   (`c-cpr-basics` 544fde20797c56f0 → 2d2efab543cb3a64, `c-victim-handling` d5222581714fd762 →
   a967e1a0bbc0a391, `c-bleeding-control` fbc260f64f20802c → 7dbfd32092e89cf5,
   `c-first-aid-priorities` 4ffae7bbb0feb0a4 → 5604323b375ee88d), тоест съдържанието и пинът трябва
   да се сменят В ЕДИН PR: `node scripts/freeze-lesson-citations.mjs` пре-заковава.
   (Бележка от 2026-09-12: самият скрипт днес пада с `TypeError` на `resolves()` за акт, който не е
   в `ACT_IDS` — трябва да се оправи преди пре-заковаването.) И четирите концепции остават с поне
   един `lawRef` след махането (Наредба № 24), тоест `checkConcept`'s `min: 1` е спазен.
2. ✅ **ЗАТВОРЕНО на 2026-09-12 — прагът е „не реагира и не диша НОРМАЛНО“ и на трите места.**
   Беше: `q-ptp-017`, опция (b) градираше ВЯРНО „щом установиш, че пострадалият НЕ ДИША“, докато
   собственото обяснение на реда казва друго; същата неточност стоеше в СТЪБЛАТА на `q-ptp-016` и
   `q-ptp-060`. Агоналното хъркане Е дишане за неопитно око; това е точно моментът, в който хората
   задържат масажа. Сега: опция (b) на `q-ptp-017` иска „не реагира и не диша нормално“; стъблото на
   `q-ptp-016` е „Пострадал при ПТП не реагира и не диша нормално — започваш сърдечен масаж…“;
   стъблото на `q-ptp-060` е „Пострадал не реагира и не диша нормално, но ти не си обучен…“.
   **Ключовете не са пипани** — поправката е в условието и в обяснението, не в градираните опции;
   нито един верен отговор не става грешен от нея, защото и трите реда градират нещо друго (къде и
   с каква честота; масаж без обдишване; трите изхода).
   И трите обяснения казват ЗАЩО думата е там, дословно от източника: `rcuk_bls.txt:162`
   „If any person is unresponsive with abnormal breathing, cardiac arrest should be assumed.“ в
   `q-ptp-016`, `rcuk_bls.txt:160` „Slow, laboured breathing, as well as other abnormal patterns such
   as agonal gasping or panting, must be recognised as signs of cardiac arrest.“ в `q-ptp-060`, и
   двете вече цитирани в `q-ptp-017` и в `med-breathing-check`.
   **ЗА ПОДПИСА:** `contentHash` на `q-ptp-016` (e8c5bd08… → 09952fdb…) и на `q-ptp-060`
   (f7963e14… → 6ff8960f…) се мести. Нито един подпис не пада: и двата реда бяха ИЗРИЧНО изключени
   от партидата от 26 подписа на 2026-09-12 точно заради тази находка
   (`content/review/approvals.json`, `noteBg`), а `validate:content` отчита „stale signatures 0“
   след промяната.
...
5. ✅ **ЗАТВОРЕНО на 2026-09-12 С РЕШЕНИЕ НА ОСНОВАТЕЛЯ — `med-recovery-position`.** Беше: статус
   `contested-content-affected` и бележка към конфликта „q-ptp-022 и q-ptp-037 инструктират точно
   това“ — невярно от мига, в който двата ключа бяха обърнати по ERC/RCUK 2025. Находката отказа да
   се поправи редакционно, защото регистър не се пренаписва, за да съвпадне със съдържание, чието
   клинично решение не е ратифицирано: това беше решение на основателя. **Решението е дадено на
   2026-09-12** — следваме ERC/RCUK 2025, НЕ практиката на БЧК. Сега статусът е
   `contested-founder-ruled-erc-2025` (тоест казва КАК е решен, а не че е спорен), бележката
   записва кой, кога и защо, и изрично назовава невярното изречение, което е стояло на нейно място.
   Разминаването с БЧК ОСТАВА записано и в двата реда — решено е коя страна учим, не че разминаването
   е изчезнало — заедно с приетата цена (ученик може да бъде отбелязан като сгрешил на изпита към
   12-часовия курс, Наредба № 24, чл. 10). Обратната грешка е заключена от двете страни: и `q-ptp-022`,
   и `q-ptp-037` носят изхода, че отварянето на дихателния път изпреварва обездвижването
   (`rcuk_fa.txt:231`, дословно), и че при непроходим път се обръща КАТО ЕДНО ЦЯЛО.
   Проверено, а не предположено: `node content/medical/tools/build-claims.mjs`, пуснат в отделна
   директория, произвежда байт по байт същия `claims.json` — поправката е в генератора, не е ръчна
   кръпка, която следващото пускане ще върне.
