...
- [x] **Asset hygiene** — 210 clip PNG keyframes (**247.3 MB**) pruned after verifying the manifest names only the WebP set, and `sky_clear_1k.hdr` (**1.5 MB**, referenced by nothing) removed from the deploy. `public/` fell 492.5 MB → 255.2 MB; the *deployed* half 181.0 → 179.6 MB. Also `sim/streetscape/palm_tree.glb` + its `prop_palm_tree()` generator deleted (2026-07-26) — orphaned by the palm fix, and the generator was the live path to re-creating it.
- [x] **Doc 70 SOFIA LOCALISATION** (2026-07-26) — the visual brief's REF 1 is a **Gulf/waterfront photograph**, which is how palms got planted on a Sofia boulevard. Doc 70 now opens with the rule (*replicate REF 1's craft, never its geography*) + 4 forced substitutions marked `[SOFIA]` inline: greenery, no waterfront, **horizon = Vitosha not a supertall spire**, skyline ~25–30 floors not 25–80. The same re-import was closed in `quality-gap/10` and `/13`.
