...
import { corridorFor, ARM_ROLL_ALLOW_M } from "../lib/path-plan/policy.mjs";
import {
  SAME_PROBE,
  armPoseFromSamples,
...
describe("§12.2 path-evidence", () => {
  ...
  /* THE IN-BAY LATERAL CHECK IS RETIRED (2026-09-16). `bayLateralOfRun` measured every in-bay
   * sample against `policy.mjs BAY_FLANKS` — a PROXY for body clearance, from before this harness
   * could measure body clearance. Both sides measure it now (`path-plan/body-screen.mjs` for a
   * plan, `lib/drive-clearance.mjs` for a drive), and the proxy had begun refusing the planner's
   * own clearance-maximising witnesses, so it is retired rather than re-tuned for a third time.
   *
   * The witness it fed — contact branch 5.1 in `path-follow.mjs contactEvidence` — is now the
   * INJECTED drive-clearance record, pinned in `path-follow.test.mjs` T11.2b. This file still may
   * not compute that record (its import allow-list is guidance/policy/geom, pinned below), which
   * is exactly why it is injected. What is pinned HERE is the consequence: `within` is the
   * corridor alone again, and it says so. */
  it("routeBySegment `within` is the CORRIDOR alone — the retired in-bay proxy no longer rides on it", () => {
    const segs = routeBySegment(samplesAlong(trace, { lat: 0.2 }), trace, lesson);
    for (const sg of segs) {
      assert.equal("bay" in sg, false, `seg ${sg.k} still carries a bay field — the proxy came back`);
      if (sg.measured) assert.equal(sg.within, sg.maxM <= sg.corridorM, `seg ${sg.k}: within is not the corridor verdict`);
    }
    assert.ok(segs.some((sg) => sg.gear === -1 && sg.measured), "no measured reverse segment to check");
  });
