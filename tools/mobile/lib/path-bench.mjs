...
import { DISARM_BRAKE, brakeReadOf, chainStep, createPathState, headingUnit, pathApplyActions, pathDisarmHoldActions, pathDisarmLandActions, pathFollowBooks, pathObservation, pathOnPauseDrain, pathRunnerBudgetMs, pathRunnerEntry, pathSafeKmh, pathStep, pathYieldActions, pathYieldState, PATH_TUNE } from "./path-follow.mjs";
 ...
 */
export function createPlant({ x, z, psi, gear = "D", kappaScale = 1, revKappaScale = BENCH.revKappaScale, aRev = 1.2, aCoast = 0.23, camLocal = { x: -0.24, z: 0.35 }, steerSign = 1, hudPollMs = BENCH.hudPollMs, hudPhaseS = 0, camNoise = true, noiseSeed = 1, creepMps2 = 0, creepAfterX = null, assistThrottleVeto = true } = {}) {
...
function stepFrame(pl) {
  ...
  const throttleCeil = 0.45 + 0.55 * ramp(vKmh, 4, 12);
  let accel = Math.min(pl.pedal[accelKey], throttleCeil);
  // a toggle in flight: the pressed key is still the OLD gear's brake
  if (pl.toggle || pl.ft < pl.motionHoldUntil) accel = 0;
  const brakeIn = Math.min(pl.pedal[brakeKey], 0.6 + 0.4 * ramp(vKmh, 6, 14));
  const dir = pl.gear === "D" ? 1 : -1;
  let aDrive = pl.gear === "D" ? accel * BENCH.aFwdFullMps2 : (accel / 0.45) * pl.aRev;
  const creeping = pl.creepMps2 > 0 && pl.gear === "R" && brakeIn < 0.02 && (pl.creepAfterX === null || plantCentre(pl).x <= pl.creepAfterX);
  if (creeping) aDrive = Math.max(aDrive, pl.creepMps2 + pl.aCoast);
  const resist = brakeIn * BENCH.fullBrakeMps2 + (accel > 0.02 || creeping ? 0 : pl.aCoast) + (pl.extraDecelMps2 ?? 0);
...
export function runBench({
  ...
  disturbances = [], crashAt = null, tune = null, liftBeforeDisarm = false, disarmBrake = true,
} = {}) {
  const R = rng(seed);
  ...
  // lesson-audit.mjs pathDisarmLand: «D» landed. The SAME two planners (path-follow.mjs §10b, «W
  // ENCLOSES S») decide every key; this only executes them in order, gathers the frame timestamps and polls.
  const disarmLand = () => {
  ...
  const disarmReverse = () => {
    const entry = { segIndex: state.segIndex, ok: false, skippedStopPress: false };
    ...
    if (speedNow() !== 0) { reverse.disarmed = false; throttle(false); return false; }
    for (let attempt = 1; attempt <= BENCH.armAttempts; attempt++) {
      throttle(false);
      wait(BENCH.reverseLiftMs);
      throttle(true);
      // disarmBrake: false is HEAD's landing (W lifted the instant «D» reads, nothing braking the
      // change) — kept only as the mutation T9.arm measures the brake against
      if (holdMode === "poll") selectorHold("D", disarmBrake ? disarmLand : throttle);
