...
import { chainStep, createPathState, headingUnit, pathApplyActions, pathFollowBooks, pathObservation, pathOnPauseDrain, pathRunnerBudgetMs, pathRunnerEntry, pathSafeKmh, pathStep, pathYieldActions, pathYieldState, PATH_TUNE } from "./path-follow.mjs";
 ...
 */
export function createPlant({ x, z, psi, gear = "D", kappaScale = 1, revKappaScale = BENCH.revKappaScale, aRev = 1.2, aCoast = 0.23, camLocal = { x: -0.24, z: 0.35 }, steerSign = 1, hudPollMs = BENCH.hudPollMs, hudPhaseS = 0, camNoise = true, noiseSeed = 1, creepMps2 = 0, creepAfterX = null } = {}) {
...
function landKeys(pl) {
  while (pl.queue.length && pl.queue[0].at <= pl.ft + 1e-9) {
    ...
    for (const k of ["W", "S"]) {
      ...
      if (!pl.keys[k] && nk[k]) {
        pl.downEdges.push({ key: k, t: pl.ft, vKmh, gear: pl.gear });
        if (k === functionalBrake && vKmh < BENCH.standstillKmh && pl.ft - pl.liftAt[k] >= BENCH.liftS && pl.toggle === null) {
          pl.toggle = { at: pl.ft + BENCH.toggleS, key: k, to: pl.gear === "D" ? "R" : "D" };
        }
      }
    }
...
export function runBench({
  ...
  disturbances = [], crashAt = null, tune = null, liftBeforeDisarm = false,
} = {}) {
  const R = rng(seed);
  ...
  const selectorHold = (want, releasePedal) => {
    const t = now();
  ...
  const disarmReverse = () => {
    ...
    for (let attempt = 1; attempt <= BENCH.armAttempts; attempt++) {
      ...
      wait(BENCH.reverseLiftMs);
      throttle(true);
      if (holdMode === "poll") selectorHold("D", throttle);
