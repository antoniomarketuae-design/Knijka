...
import { ARM_ROLL_ALLOW_M, CAVEATS, D_FLOOR_KMH, INSERTED_DWELL_S, PRODUCT, R_BAND_KMH, STAGED_ACTORS, acceptAlongOf, bandFor, bayFrame, bayLateralExceeds, bayLateralLimits, corridorFor, reversePolicyFor, stopTargetOf } from "./path-plan/policy.mjs";
...
export function routeBySegment(samples, trace, lesson) {
  ...
  return segs.map((sg) => {
    ...
    const bay = sg.g === -1 ? bayLateralOfRun(run.idx.map((i) => samples[i]), lesson, sg.k) : null;
    if (!dev) return { ...base, measured: false, driven: true, bay, why: sg.micro ? "micro segment — unmeasured by construction" : "too few moving samples" };
    const corridorOk = Number.isFinite(base.corridorM) ? dev.maxM <= base.corridorM : null;
    return { ...base, measured: true, driven: true, maxM: dev.maxM, medianM: dev.medianM, p90M: dev.p90M, n: dev.n, bay, within: corridorOk === null ? null : corridorOk && (bay === null || bay.within === true) };
  });
}

/**
 * THE IN-BAY LATERAL CHECK (policy.mjs BAY_FLANKS): every sample of the R run whose centre
 * lies inside the bay's painted length, against the side limits the product's own bodies
 * leave. null where the lesson has no flank; UNMEASURED (measured false, within null) when
 * no sample of the run was inside the bay — never a pass.
 */
export function bayLateralOfRun(runSamples, lesson, k) {
  const lim = bayLateralLimits(lesson, k);
  if (!lim) return null;
  let n = 0;
  let worst = null;
  const over = [];
  for (const s of runSamples ?? []) {
    ...
    const f = bayFrame(lim.bay, s.wx, s.wz);
    if (Math.abs(f.lonM) > lim.lonHalfM) continue;
    n += 1;
    if (worst === null || Math.abs(f.latM) > Math.abs(worst.latM)) worst = { latM: r3(f.latM), lonM: r3(f.lonM), tSec: s.tSec ?? null };
    if (bayLateralExceeds(lim, f.latM)) over.push({ latM: r3(f.latM), lonM: r3(f.lonM), tSec: s.tSec ?? null });
  }
  return {
    measured: n > 0, n, limitNegM: lim.negM, limitPosM: lim.posM, worst, over: over.slice(0, 5), within: n > 0 ? over.length === 0 : null,
    why: n > 0 ? lim.why : "UNMEASURED — no sample of this reverse run had its centre inside the bay",
    basis: SAME_PROBE,
  };
}
 ...
 */
export function computePathEvidence({ samples = [], trace, lesson, debrief = null, mistakes = null, rubric = null, routeHold = null, routeHoldBySegment = null, selfReportMayOnlyBlame = null }) {
