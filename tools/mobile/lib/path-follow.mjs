...
import { ACQUISITION, ARM_ROLL_ALLOW_M, D_FLOOR_KMH, R_BAND_KMH, bayFrame, bayLateralExceeds, bayLateralLimits } from "./path-plan/policy.mjs";
...
export const PATH_TUNE = Object.freeze({
  ...
  // 0.267 → 0.226 m; 2000 ms added nothing on the wall (0.141 vs 0.145 m median) and left the
  // reverse's INDEPENDENT route UNMEASURED through a card drain (fewer than five moving
  // outer-tick samples). See pathRunnerBudgetMs.
  ...
  // THE IN-BAY LATERAL REFUSAL (policy.mjs BAY_FLANKS; refuse false only to measure tracking
  // without it). It applies where the centre is BAY_MOUTH_INSET_M inside the paint (the limits'
  // lonHalfM) and the body within yawGateDeg of the bay axis. The limit itself is derived at the
  // park's 10° tolerance; the gate is 20° so a wide car that is still rotating into the bay is
  // seen before it aligns: at 10° the calibrated bench's card-drain runs were first seen at up
  // to 0.62 m and stopped 0.68 m wide; at 20° every one stopped ≤ 0.43 m, and no clean run of the
  // five bay lessons × 20 seeds refused. Evaluating the formula at the MEASURED yaw instead
  // refused 11/20 clean sc-park-left runs (the extreme corner is not beside the neighbour).
  bay: Object.freeze({ refuse: true, yawGateDeg: 20 }),
 ...
 * `bay-lateral` refusals (branch 1) can miss a contact made under the final brake. Three
 * witnesses, none of them the controller's, each enough on its own:
 *   5.1 `evidence.routeBySegment[k].bay.within === false` — path-evidence.mjs bayLateralOfRun,
 *       the outer-tick samples against the room the product's own bodies leave (same pose probe);
...
export function contactEvidence(k, rt, evidence = {}) {
  ...
  let branch = null;
  const bay = rt?.bay;
  if (bay && bay.within === false) {
    branch ??= 5.1;
    const w = bay.worst ?? {};
    reasons.push(`the reverse left the lateral room the bay's own bodies leave (${Array.isArray(bay.over) ? bay.over.length : "?"}${Array.isArray(bay.over) && bay.over.length >= 5 ? "+" : ""} of ${bay.n ?? "?"} in-bay sample(s) over; worst ${w.latM ?? "?"} m off the bay axis at lon ${w.lonM ?? "?"} m${Number.isFinite(w.tSec) ? `, t=${w.tSec}s` : ""}, limits ${bay.limitNegM ?? "none"} / ${bay.limitPosM ?? "none"} m; independent of the controller, same pose probe)`);
  }
...
export function pathReverseOutcome({ segments = [], refusals = [], evidence = {}, sign = null, endPoses = {}, credited = null, creditBySegment = null, disarmFailed = false }) {
  ...
  for (const seg of rSegs) {
    const k = seg.k;
    const ref = refusals.find((rf) => (rf.k === k || rf.k === undefined) && ["reverse-not-armed", "arm-gate-stalled", "arm-roll-out-of-band", "disarm-failed", "lost-R", "end-overrun", "collision", "bay-lateral"].includes(rf.code));
...
export const REFUSAL_CODES = Object.freeze([
  ...
  "end-overrun", "collision", "bay-lateral",
]);
...
export function createPathState(plan, tune = PATH_TUNE) {
  ...
  return {
    ...
    bayLimits: null,
    books: {
      nearLockM: 0, nearLockBudgetEntries: 0, impacts: [], holds: [], bay: [],
...
function reverseSettleStep(state, obs, cmd, row, pose, psi, vAbs, now, keys) {
  ...
  state = { ...state, witness: sel.witness, poly, cursor, leadM: sel.leadM, mode: "reverse-follow", settle: null, latch: { brake: false, coast: false, wBrake: false, hold: false }, yaw: resetYawHistory(state.yaw), lostCount: 0, stops: buildStops(seg, poly), currentStop: null, bayLimits: (state.tune ?? PATH_TUNE).bay?.refuse === false ? null : bayLateralLimits(state.plan.lesson, seg.k), impact: { prev: null, reads: 0, peakMps2: 0 }, nearLock: false };
...
function reverseStep(state, obs, cmd, row, pose, psi, v, vAbs, stepM, now, keys) {
  ...
  // ── THE BAY'S OWN LATERAL ROOM, BEFORE CONTACT (policy.mjs BAY_FLANKS) ──
  // The centre's lateral in the bay frame, extrapolated along its recent motion over the
  // distance the car needs to stop from here, against the limit the product's bodies leave.
  if (state.bayLimits) {
    const lim = state.bayLimits;
    const fr = bayFrame(lim.bay, pose.x, pose.z);
    const hist = state.yaw.history;
    let latRate = 0;
    if (hist.length >= 2) {
      const a = bayFrame(lim.bay, hist[hist.length - 2].x, hist[hist.length - 2].z);
      const b = bayFrame(lim.bay, hist[hist.length - 1].x, hist[hist.length - 1].z);
      const d = Math.hypot(b.lonM - a.lonM, b.latM - a.latM);
      if (d > 1e-6) latRate = (b.latM - a.latM) / d;
    }
    const vm = vAbs / 3.6;
    const stopM = vm * (tune.pedals.landS + brakeAttackDeadS(vAbs, -1) + (Number.isFinite(obs.blindP90S) ? obs.blindP90S : 0)) + (vm * vm) / (2 * brakeCapMps2(vAbs));
    const latPred = fr.latM + latRate * stopM;
    // WHERE THE LIMIT HOLDS: the centre BAY_MOUTH_INSET_M inside the painted length (lim.lonHalfM)
    // AND the body within tune.bay.yawGateDeg of the bay axis — see both for the measurements.
    // Mid-swing at the mouth the laterally extreme corner is still out past the neighbours'
    // bumpers (canary-path-s2 sc-park-left at lon −2.48 m, yaw 21.7°: lat 0.36 m with 0.60 m of
    // air to the fleet box).
    const axisDeg = Number.isFinite(psi) ? Math.abs(wrapDeg(psi - lim.bay.headingDeg)) : null;
    const yawAxisDeg = axisDeg === null ? null : axisDeg > 90 ? 180 - axisDeg : axisDeg;
    const inBay = Math.abs(fr.lonM) <= lim.lonHalfM && yawAxisDeg !== null && yawAxisDeg <= tune.bay.yawGateDeg;
    if (inBay) {
      const bb = state.books.bay;
      let worst = bb.find((x) => x.k === seg.k);
      if (!worst) { worst = { k: seg.k, worstPredLatM: 0, worstLatM: 0, limitNegM: lim.negM, limitPosM: lim.posM }; bb.push(worst); }
      if (Math.abs(latPred) > Math.abs(worst.worstPredLatM)) worst.worstPredLatM = r3(latPred);
      if (Math.abs(fr.latM) > Math.abs(worst.worstLatM)) worst.worstLatM = r3(fr.latM);
      if (bayLateralExceeds(lim, latPred) || bayLateralExceeds(lim, fr.latM)) {
        const side = latPred < 0 ? lim.neg : lim.pos;
        return refusedStep(refusePathState(state, "bay-lateral", `R${seg.k}: in the bay at lon ${r2(fr.lonM)} m the centre is ${r2(fr.latM)} m off the bay axis and would stop at ${r2(latPred)} m (over ${r2(stopM)} m of stopping) — past the ${latPred < 0 ? lim.negM : lim.posM} m the product's own bodies leave on that side (${side?.body ?? "?"}; ${lim.why})`), obs, cmd, row, vAbs, keys, now);
      }
    }
  }
