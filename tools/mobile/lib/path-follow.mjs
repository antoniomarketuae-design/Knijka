 ...
 * latencies. Then at EVERY `SimInput.read()` — hence at every ReverseAssist update, paused or not,
 * whichever read it consumes — S > REVERSE_ASSIST_PEDAL_ON implies W > REVERSE_ASSIST_PEDAL_ON:
 *  · while the S key is held, the W key is held, and W's pedal has been above 0.1 since the flip
 *    (the flip itself was the assist reading W above 0.1; a held pedal never falls, input.ts:147-156);
 *  · after S's key-up and before W's, W's key is still held — same;
 *  · after both key-ups, S has been falling at least as long as W (its key-up was processed first)
 *    and falls faster (BRAKE_RELEASE_S 0.2 < THROTTLE_RELEASE_S 0.25), so S > 0.1 needs less than
 *    0.18 s of release, in which W (at ≥ 0.82 when released) cannot reach 0.1.
 * A rising edge of the functional brake in D that meets W above 0.1 is disarmed in the same update
 * (reverseAssist.ts:265-268) and an S that never drops below 0.1 until its key is up makes no second
 * edge; W's own lift is only a lift (in D the functional brake is S). So the S press can never be an
 * armed D → R gesture — at any HUD, key or evaluate latency, any frame schedule, any stall and any
 * pause — and nothing in that argument depends on the frame evidence below.
 * WHAT IT ASSUMES, named: the keys reach the product in the order they were awaited (each
 ...
 * latencies, stalls and pauses, and shows the first 2026-09-17 landing and the swapped release order
 * DO shift the gear.
 *
 * WHAT IT COSTS, MEASURED — the release is no longer creep-free. S's ramp runs out before W's, so for
 * a few frames W pushes with nothing braking. On the bench plant (T6.7c, coast 0.23 m/s², both pedals
 * full at rest) the roll after the release is 0.005 m with the two key-ups 1–14 ms apart, 0.011 m at
 * 30 ms, 0.031 m at 60 ms, 0.103 m at 120 ms and 0.328 m at 250 ms; at the canary's key timing the whole
 * disarm rolls p90 0.011–0.012 m on T9.arm's grid at every HUD phase. So at a slow key round trip the
 * creep this landing exists to remove comes back, and it is not hidden: T6.7c prints it. That residue
 * is the price of the ordering; the only orders that avoid it lift W while S is held, and those are
 * exactly the ones a pause or a stall turns into a shift back to R.
 *
 ...
 *
 * CONSUMED BY: `lesson-audit.mjs disarmReverse` → `pathSelectorHold("D", …, pathDisarmLand)`
 * and `lib/path-bench.mjs` disarmReverse → `selectorHold("D", disarmLand)`. Both only execute
