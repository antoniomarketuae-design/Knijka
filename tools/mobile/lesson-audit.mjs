...
async function disarmReverse() {
  ...
  for (let attempt = 1; attempt <= REVERSE_ARM_ATTEMPTS; attempt++) {
    ...
    // pc-path: on «D», S (D's brake) goes down INSIDE this held W — now D's
    // accelerator — and comes up BEFORE it (pathDisarmLand, §10b «W ENCLOSES S»).
    if (STEER_BY === "authored-path") await pathSelectorHold("D", REVERSE_HOLD_MS, pathDisarmLand);
...
async function pathSelectorHold(want, holdMs, releasePedal) {
  ...
  while (Date.now() - t0 < holdMs) {
    ...
    if (g.length === 1 && g[0] === want) {
      // stamped when the gear was SEEN: the «D» landing below holds a brake for ~0.3 s, which is not the lift
