...
while (!ended && Date.now() - t0 < budgetMs) {
  ...
  if (now - lastFrame >= (readDear ? EXPENSIVE_SHOT_MS : FRAME_MS) && !beatLabelsTaken.has(periodicLabel) && (STEER_BY !== "authored-path" || pathFollow.shutterOk || now - lastFrame >= 3 * FRAME_MS)) {
    lastFrame = now;
