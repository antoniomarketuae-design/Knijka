...
//     used to measure against the viewport edge, which scores an answer painted
//     UNDER an opaque sticky strip as fitting — 58px of optimism on the exam.
//     (Fixed there on 2026-08-09: the shared probe now runs this same finder
//     and publishes `foldPx`/`barPinned`/`barAnchor`, so there is no longer a
//     flattering tool sitting next to this one waiting to be picked up. What
//     still makes THIS the instrument for the two question runners is the
//     other two items — real insets, and the refusal below.)
//     Here the fold is whatever is `sticky`/`fixed` sitting at or past the
//     bottom of the USABLE area, and a row where no such element was found is
//     REFUSED rather than quietly measured against the viewport.
//
//     „Usable", not „viewport", because there are two bottom edges on a phone
//     and the first cut of this only knew about one. A `fixed` bar's bottom
//     lands on `vh`; a `sticky` bar lives inside <body>'s padded box, so once
//     <body> pays the home-indicator inset its bottom lands on
//     `vh − padding-bottom` — 35px higher on an iPhone 16 in portrait. Testing
//     only against `vh` found NOTHING the moment the runners stopped paying
//     that inset twice, and 80 of 160 rows came back with `barPinned:false`,
//     a fold of `vh` and numbers that looked exactly like data. The refusal in
//     the retry loop is what turned that into a stopped run instead of a
//     published one; it was documented here and not implemented.
//     `barAnchor` records which of the three states was found — `viewport`,
//     `safe-area`, or `overflowing` (the page genuinely does not fit and the
//     bar has been carried below the usable bottom, which is the case the
//     sweep most needs to count rather than discard).
//
...
const PROBE = () => {
  ...
  //
  // THERE ARE TWO BOTTOM EDGES ON A PHONE, AND THIS ONLY KNEW ABOUT ONE.
  // `position: fixed` resolves against the VIEWPORT, so a fixed bar's bottom
  // lands on `vh`. A `sticky` bar lives inside <body>'s padded content box, so
  // once <body> pays the home-indicator inset its bottom lands on
  // `vh - padding-bottom` — 35px short of the viewport edge on an iPhone 16 in
  // portrait. A ±4px test against `vh` alone therefore found NOTHING the moment
  // the runners stopped paying that inset a second time, and the whole iPhone
  // half of a sweep silently fell back to measuring against the viewport edge —
  // which is the 58px-of-optimism defect this fold-finder was written to end,
  // reappearing through the other door. Both anchors are accepted, and the row
  // records which one it found so a fallback can never masquerade as a bar.
  const bodyCs = getComputedStyle(document.body);
  const bodyPadBottom = Math.round(parseFloat(bodyCs.paddingBottom)) || 0;
  const usableBottom = vh - bodyPadBottom;
  //
  // AND THERE IS A THIRD STATE, WHICH IS THE ONE WORTH MEASURING. When the page
  // genuinely does not fit, the document overflows and the bar is carried
  // BELOW the usable bottom — it is still the bar, it is still the fold, and a
  // finder that only knows „pinned exactly here" throws away precisely the
  // cases the sweep exists to count. So the test is „at or past the bottom of
  // the usable area", and the row records which of the three it was.
  let fold = vh;
  let barEl = null;
  let barAnchor = null;
    ...
    for (const c of ALL_CASES) {
      ...
      for (let attempt = 0; attempt < 4 && m === null; attempt += 1) {
        try {
          ...
          // The header of this file claims a row with no pinned bar is REFUSED
          // rather than quietly measured against the viewport edge. It said so
          // and did not do it: 80 rows came back with `barPinned:false`, a fold
          // of `vh`, and numbers that looked exactly like data. Now it refuses.
          if (!probe.barPinned) {
            throw new Error(
              "no bottom-pinned bar found — the fold would silently fall back to the " +
                "viewport edge, which is the 58px of optimism this rig exists to remove",
            );
          }
      ...
      console.log(
        `${bad ? "FAIL" : " ok "} ${deviceId.padEnd(19)} ${mode.padEnd(8)} ` +
          ...
          `bar ${row.barPinned ? (row.barAnchor === "safe-area" ? "saf" : row.barAnchor === "overflowing" ? "OVF" : "vpt") : "---"}@${String(row.fold).padStart(3)} ` +
