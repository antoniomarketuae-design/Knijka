...
// Malformed lines are reported by verdict-coverage; here they are skipped.
const rows = parseVerdictRows(fs.readFileSync(VERDICTS, "utf8"));
// EFFECTIVE VERDICT — THE POSTER'S RULE, NOT A LOOKALIKE. This was two passes,
// originals then every verify line, so a verifier's STILL from w45 outranked
// the fresh w46 round's UNJUDGED forever — and three rows the poster counted
// UNJUDGED entered wave 47 as "confirmed STILL", against this file's own rule
// that UNJUDGED never enters a repair wave. See effective-verdict.mjs.
const eff = effectiveVerdicts(rows);
...
// AN ADDRESS A LANE CAN OPEN. apply-reroute.mjs already refuses to route a row
// anywhere but an existing file under platform/src/; this generator did not
// check the same thing, and wave 47's first build handed one lane the address
// `unknown` and another `platform/public/world/mw-v1.json` — a file generated by
// tools/maps and duplicated in content/world, where a hand edit is overwritten
// by the next generator run. Both stay OPEN and are printed; they wait for a
// routing pass, not a repair lane.
const addressable = (r) => {
  const f = String(r.suspectFile || "");
  return f.startsWith("platform/src/") && fs.existsSync(path.join(REPO, f));
};
const unaddressed = still.filter((r) => !r.unrepairable && !addressable(r));
const repairable = still.filter((r) => !r.unrepairable && addressable(r));
...
if (unrepairableSkipped) {
  ...
  console.log("skipped, unaddressed  : " + unaddressed.length + "   (STILL, but the address is not a file under platform/src — route them first)");
  for (const r of unaddressed) console.log("   " + r.findingId.padEnd(36) + " " + (r.suspectFile || "(none)"));
}
...
const BRIEF = [
  ...
  "",
  "    ⚠ AND IF YOU WRITE ALREADY-FIXED YOU MUST NAME THE COMMIT THAT FIXED IT, with the diff",
  "    hunk you read. This is not bookkeeping. «Already fixed» without a named commit is",
  "    indistinguishable from «I could not reproduce it in the ten minutes I looked», and the two",
  "    have opposite consequences: one retires a row, the other leaves a live defect in front of",
  "    a seventeen-year-old with a CLOSED beside it. A check on twelve such claims refused NINE,",
  "    several with the verifier finding the row LIVE AT HEAD after the reader had called it",
  "    fixed. `git log -S '<the changed text>' -- <your file>` finds it in one command. If you",
  "    cannot find a commit, your verdict is NOT already-fixed — it is «cannot reproduce», and",
  "    you should say exactly that, because it sends the row somewhere different.",
  "",
  "    A judge downstream will run `git merge-base --is-ancestor <your commit> <the swept",
  "    commit>`: if your repair landed AFTER the sweep that photographed this lesson, no drive",
  "    has ever exercised it and the row needs a re-drive rather than a verdict. Naming the",
  "    commit is what makes that question answerable at all.",
  "",
  ...
  "",
  "YOUR ANSWER IS A REPORT, not a message. Per row exactly one of:",
  "",
  "  REPAIRED        — and name the LIVE CONSUMER you verified reads it.",
  "  ALREADY-FIXED   — and name the REPAIRING COMMIT (sha + subject) and quote the diff hunk.",
  "                    Without a commit this verdict is not available to you; use CANNOT-REPRODUCE.",
  "  CANNOT-REPRODUCE— you looked and could not make it happen, and you could not find what",
  "                    changed. An honest and useful answer. It is NOT already-fixed.",
  "  MISROUTED       — and name the correct address as a path that EXISTS on disk, with the",
  "                    import chain proving it is on a live path.",
  "  REFUTED         — not a defect, with proof.",
  "  BLOCKED         — needs another lane's file: give file, line, exact edit.",
  "",
  "State plainly if you changed nothing. A lane that changed nothing and said so precisely is",
  "worth more than one that wrote code at the wrong address.",
].join("\n");
