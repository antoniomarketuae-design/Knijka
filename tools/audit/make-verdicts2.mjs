...
const HOW = [
  ...
  "",
  "BEFORE YOU RULE ON ANY ROW, READ docs/development/94_ROW_EVIDENCE_2026-09-14.md. It carries, per row, the",
  "measurements taken since the last judging pass — which legs stayed on their authored route AND drove it, the",
  "three error-class counts recovered from every debrief, five rows whose filed arithmetic the newest sweep",
  "contradicts, the audio witness (49/49 mobile drives built zero WebAudio; 40/40 pc drives built ~21 sources;",
  "simAudio.ts unlock() reads window.AudioContext only and returns silently), and the recovery caveat: a",
  "\u00abBACK ON A CARRIAGEWAY\u00bb line means the PRODUCT stopped saying off-road — on every such leg in w45 the car was",
  "still 13–296 m from its authored line, so ROUTE FIDELITY, not that line, decides whether a leg can witness",
  "anything along its route. Each debrief sidecar now carries `severity`, `route` (with droveIt), `routeHold`,",
  "`recovery` and `audio` as FIELDS — parse them; never grep run.log prose for a number.",
  ...
  ...adr009JudgeBrief(),
  "-- A `-path` LEG IS GRADING EVIDENCE ONLY --",
  "An `-path` leg (run.log headline STEERED BY: THE LESSON'S AUTHORED LINE) was steered by the lesson's",
  "authored parking line through the dev-only pose probe. It is judgeable for GRADING only — objective",
  "credits, «подравняване» / «ъгъл», booked faults, HUD, cards and debrief — and ONLY where its ROUTE",
  "(independent) and ARM lines are within corridor and band. Any row whose claim or repair is about the",
  "guidance ribbon, legibility, lane choice, route-keeping, a wrong drive, parity or determinism is UNJUDGED",
  "on it, never STILL and never FIXED. Codes listed under ATTRIBUTION as PROCEDURAL-OMISSION or HARNESS",
  "TIMING/ACTUATION are not convictions of a correct drive; COLLISION CANDIDATES need the frame and an",
  "in-process replay before filing. A ROUTE, ARM or REVERSE OUTCOME line that reads UNMEASURED or UNJUDGED",
  "is within nothing: rows resting on that segment stay UNJUDGED. A `-path` leg's sidecars carry `route: null`",
  "BY DESIGN — its whole-drive distance is `harnessRoute`, the harness following its witness, and it is",
  "NEVER evidence that a leg stayed on its route; do not read `droveIt` / `onRoute` from a `-path` folder.",
  "",
...
const RULES = [
  ...
  "   EVIDENCE was read, and the frame was then described rather than opened.",
  "   One closure quoted a dark SPECULAR carriageway with the lit building faces",
  "   MIRRORED in it; a 2x crop of that exact frame shows matte asphalt with lane",
  "   paint and droplet rings and no reflection anywhere. Another quoted a",
  "   task chip that is not in the frame it cited. Both would have survived any",
  "   review that did not open the PNG. So: open every frame you cite, crop in",
  "   when the claim is about a detail, and quote only what you have SEEN.",
  "",
  " . CHECK WHETHER THE DRIVE WAS ENTITLED TO ANSWER, BEFORE YOU READ WHAT IT",
  "   SAID. run.log discloses its own competence and will refuse on your behalf:",
  "   BLIND, NOT-STEERED, NOT-INVOKED, WANDERED, a straightness near 1.000 on a",
  "   lesson whose task is to turn, and in the plainest case the sentence",
  "   NO LANE-POSITION FINDING MAY BE DRAWN FROM THIS DRIVE. In w32 nine STILL",
  "   verdicts were downgraded to UNJUDGED for drawing an inference the lane's",
  "   own log forbids. A STILL resting on a drive that could not see is not a",
  "   measurement of the product, and it inflates the open list with rows nobody",
  "   can act on. This cuts BOTH ways: a fault booked on such a lane (a shoulder",
  "   incursion by a car with steering liveness DEAD) is the instrument's own act",
  "   and may not be cited as the product convicting anyone.",
  "",
  " . COMPARE AGAINST THE LESSON THE ROW NAMES. A row citing a trio of lessons is",
  "   settled against a member of that trio, not against a neighbour that shares a",
  "   template. w32 closed sc-junction-stop by comparing it with sc-junction-LEFT,",
  "   whose whole subject is that it turns the other way, while the correct",
  "   comparison lesson had been re-driven in the same frame pack.",
  "",
