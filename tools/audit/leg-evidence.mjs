...
export function legEvidence(dir) {
  ...
  if (legMode === "path") {
    ...
    // THE DRIVE'S OWN BODY CLEARANCE, recomputed here from the pose record on disk
    // rather than trusted from the sidecar - the same measurement `path-canary.mjs`
    // G10 gates on, through the same predicate. A missing or truncated
    // `_audit-path.json` comes back as a NAMED REFUSAL and prints as UNMEASURED.
    const driveClearance = driveClearanceFromSidecar(scenario, readJson(`${dir}/_audit-path.json`));
...
export function renderPathEvidence(e) {
  ...
  // BODY CLEARANCE — the safety number. The corridor above is a plausibility check:
  // a clearing witness deviates ~0.47 m from the authored line on purpose, so the
  // reverse corridors are 0.82 m wide and a drive can wander most of a metre inside
  // one. This line is what the drive's own body actually kept.
  {
    const dc = e.path?.driveClearance ?? ev.driveClearance ?? null;
    const g = driveClearanceGate(dc);
    L.push(
      dc?.measured === true
        ? `      BODY CLEARANCE (${g.pass ? "OK" : "FAILS"}): worst ${dc.worstM} m vs ${dc.body}/${dc.model} at ledger row ${dc.atRow} — required ${dc.requiredM} m · plan floor ${dc.planFloorM} m · verdict «${dc.verdict}» · ${dc.basis}.`
        : `      BODY CLEARANCE: UNMEASURED — ${dc?.why ?? "no record"} (rows resting on this leg's safety are UNJUDGED).`,
    );
  }
