/**
 * gen_vu_cyclist.mjs — the VULNERABLE-ROAD-USER scenario micro-map (Scenario
 * Studio, doc 76 §3; doc 72 §7 „Family VU") → content/world/vu-cyclist-v1.json
 * (+ byte-identical publish to platform/public/world/).
 *
 * VU-01 „Десен завой през велосипедист / Cyclist right hook" host: an
 * uncontrolled T-junction where the driver rides the through road (west→east)
 * and turns RIGHT onto the south stem, while a curb-side cyclist continues
 * STRAIGHT through the junction — the #1 car-cyclist urban kill geometry.
 *
 * Layout (local meters, x = east, y = north, origin at the junction node):
 *
 *   vu-n-w ───────────── vu-n-c ───────────── vu-n-e   through road (W–E, the
 *                           │                            cyclist rides its south
 *                           │  south stem (the driver    curb; the driver rides
 *                        vu-n-s   turns right onto it)    its eastbound lane)
 *
 * The whole point is that the cyclist travels the DRIVER'S OWN direction
 * (eastbound, curb-side), so the runtime's conflictFromRight same-direction
 * filter correctly ignores it (orchestrator/runners.ts CyclistRightHookRunner):
 * the ONLY thing that can grade the right hook is the staged cyclistRightHook
 * director (prioritySituation "cyclist-right-hook" → FAILED_TO_YIELD /
 * YIELDED_TO_PRIORITY, collision(cyclist) → COLLISION). Every edge is
 * residential (rank 2) → control "none" → the runtime derives ZERO stop lines
 * (an UNCONTROLLED junction), so no signal/sign/priority code pollutes the demo.
 *
 * No signals, no crossings, no roundabouts. Deterministic: same params →
 * byte-identical JSON. No randomness, no OSM. Run:
 *   node tools/maps/gen_vu_cyclist.mjs
 *
 * Contract battery: platform/src/modules/sim/world/__tests__/vu-cyclist-district.test.ts
 */
 ...
 * }} params
 */
export function buildVuCyclistDistrict(params) {
  const errors = [];
  ...
  if (errors.length > 0) throw new Error(`gen_vu_cyclist params invalid:\n  - ${errors.join("\n  - ")}`);
  ...
  const EDGES = [
    edge("vu-e-w", "vu-n-w", "vu-n-c", throughMaxKmh, "Права улица — запад"),
    edge("vu-e-e", "vu-n-c", "vu-n-e", throughMaxKmh, "Права улица — изток"),
  ...
  // Visual anchor, clear of both carriageways + sidewalks (SW quadrant).
  const BUILDINGS = [
    {
      id: "vu-b-corner",
      height: 5,
  ...
  const SPAWN_POINTS = [
    {
      ...
      heading: 90,
      edgeId: "vu-e-w",
      name: "Права улица — подход от запад",
    },
    {
      id: "vu-spawn-east",
      x: r2(throughArmM - 15),
      ...
      heading: 270,
      edgeId: "vu-e-e",
      name: "Права улица — подход от изток",
    },
    {
      id: "vu-spawn-south",
      x: 0,
  ...
  const district = {
    ...
    meta: {
      ...
      generator: "tools/maps/gen_vu_cyclist.mjs",
      boundsLocalMeters: bounds,
      attribution: {
        text: "Учебно кръстовище за велосипедист — оригинален параметричен дизайн (без данни от OpenStreetMap)",
  ...
  // Self-validation (mirrors gen_t_junction / gen_poligon invariants).
  const post = [];
  ...
  if (post.length > 0) throw new Error(`gen_vu_cyclist self-validation FAILED:\n  - ${post.join("\n  - ")}`);

  return district;
}

// ---------------------------------------------------------------------------
// Committed instance (S3 batch 6: the VRU scenario family)
// ---------------------------------------------------------------------------

const INSTANCES = [
  {
    districtId: "vu-cyclist-v1",
    label: "Учебно Т-кръстовище с велосипедист покрай бордюра (сценарий VU-01)",
    throughArmM: 130,
    stemArmM: 90,
    throughMaxKmh: 50,
    stemMaxKmh: 50,
...
for (const params of INSTANCES) {
  const district = buildVuCyclistDistrict(params);
  ...
  console.log(`=== vu-cyclist build: ${params.districtId} ===`);
