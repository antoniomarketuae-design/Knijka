...
const CLAIMS = [
  ...
  },
  {
    ...
    corroborating: [
      q("src-erc-2025-layperson", "If they are in cardiac arrest position them lying flat on their back for CPR."),
      // THE EXIT, and it belongs in the register because the ruling of
      // 2026-09-12 made it load-bearing. „Do not roll a trauma casualty" on its
      // own is the reverse killer (doc 92 §3): a student who remembers only the
      // prohibition watches someone aspirate. Both q-ptp-022 and q-ptp-037 now
      // quote this sentence for the way out, so the sentence has to be pinned
      // to a locator like every other quote here, not typed into an explanation
      // where nothing re-checks it.
      q("src-rcuk-2025-first-aid", "Airway opening, if required, always has priority over in-line immobilisation"),
    ],
    conflicts: [
      {
        ...q("src-rcuk-2025-first-aid", "In cases of agonal breathing or trauma, do NOT move the person into the recovery position."),
        natureBg:
          "Пострадал при ПТП по дефиниция е травма, а ERC/RCUK 2025 казват при травма ДА НЕ се поставя в странично положение. РАЗМИНАВАНЕТО ОСТАВА, но вече не е между насоките и нашето съдържание: на 2026-09-12 основателят се произнесе за ERC/RCUK 2025 и q-ptp-022 и q-ptp-037 следват тях — ключовете им са обърнати, обръщането не се инструктира. ВНИМАНИЕ: бележката, която стоеше тук до 2026-09-12, твърдеше „q-ptp-022 и q-ptp-037 инструктират точно това“. Това беше фактически невярно от мига, в който ключовете бяха обърнати, и рецензентът щеше да го прочете в конзолата като вярно.",
      },
      {
        ...q("src-bchk-first-help-bls", "Поставете пострадалия в стабилно странично положение."),
        natureBg:
          "БЧК учи страничното положение за всеки в безсъзнание, който диша, без изключение за травма. Това НЕ подкрепя нашето съдържание — след решението от 2026-09-12 нашите два реда следват ERC/RCUK 2025, а БЧК остава от другата страна. Практическата последица е приета съзнателно и се казва на ученика вътре в самите редове: ученик, който отговори по нашата банка, може да бъде отбелязан като сгрешил на изпита към 12-часовия курс на БЧК (Наредба № 24).",
      },
    ],
    statusBg: "contested-founder-ruled-erc-2025",
    noteBg:
      "РЕШЕНО ОТ ОСНОВАТЕЛЯ на 2026-09-12: следваме ERC/RCUK 2025, НЕ практиката на БЧК. Конфликтът между източниците остава записан — решено е коя страна учим, не че разминаването е изчезнало. ОСНОВАНИЕ: ERC 2025 и RCUK 2025 съвпадат дословно и изключват травмата, а пострадал при ПТП е травма по определение; БЧК е `not-a-grounding-source` в този регистър и на същата страница носи жива грешка за мястото на натиска. ПРИЕТА ЦЕНА: ученик, който отговори по нашата банка, може да бъде отбелязан като сгрешил на изпита към курса на БЧК — затова и двата реда казват открито къде БЧК учи друго. ОБРАТНАТА ГРЕШКА Е ЗАБРАНЕНА: „настрани е грешно“ само по себе си убива — и двата реда носят изхода, че отварянето на дихателния път изпреварва обездвижването и че при непроходим път се обръща, като едно цяло. КАК Е ДАДЕНО РЕШЕНИЕТО (записано дословно, защото това е разликата между запис и твърдение): основателят каза „approve them“ в чат на 2026-09-12 върху ОБОБЩЕНИЕ на решенията, изготвено от агент. Той НЕ е чел 29-те реда един по един. Затова решението важи като клинична посока, а подписите по редовете носят същата уговорка — виж `content/review/approvals.json`.",
  },
  {
