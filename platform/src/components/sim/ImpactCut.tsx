 ...
 *
 * GATE: `IMPACT_MIN_KMH` = `VehicleRig.COLLISION_MIN_KMH`, ON PURPOSE and not
 * merely by symmetry. Parking drills pass `collisionMinKmh: 0` so that a 2 км/ч
 * cone touch grades — and a student mid-manoeuvre in a bay, who is looking down
 * the side of his own car, must not be thrown into chase for a kerb kiss. The
 * cut answers a CRASH, so it uses the street's crash threshold; the flash obeys
...
/** A contact under this is a nudge, not a crash — the street tolerance. */
...
/**
 * Does this contact get a flash? Pure — a nudge under the street tolerance is
 * not a crash, and the parking family's `collisionMinKmh: 0` must not turn a
 * cone kiss into one (see the GATE note in the header).
 */
export function impactFlashes(impactKmh: number): boolean {
  return Number.isFinite(impactKmh) && Math.abs(impactKmh) >= IMPACT_MIN_KMH;
}

/**
 * The view this contact should cut to, or null for „leave the view alone".
 *
 * Null in three cases, each of them a refusal on purpose: the contact is a
 * nudge; a cut is already held (a second bang must not overwrite the seat the
 * student is owed back); or he is not in the cockpit, where chase and top-down
 * already show the car and the blank-wall frame cannot occur.
 */
...
export function ImpactCut({
  ...
  const impact = useCallback(
    (impactKmh: number) => {
      ...
      const next = impactCutView(from, impactKmh, restoreToRef.current !== null);
      if (next === null) return;
      restoreToRef.current = from;
      applyCameraMode(next);
    },
    [applyCameraMode, cameraModeRef],
  );
