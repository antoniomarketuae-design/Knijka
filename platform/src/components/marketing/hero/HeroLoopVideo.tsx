...
import { HERO_LOOP_GRADE_WIDE } from "./heroContrast";
...
/**
 * The loop's own reading grade — the ONE thing on this rung that is not also
 * true of the plate and the live scene.
 *
 * A desktop that fails the WebGL probe used to get the still plate and now
 * gets this video, and the two crop the same box but do not hold the same
 * picture: the drawing keeps its car hard against the right edge, the render
 * sweeps it further left, and its light bar — a near-white specular — passes
 * behind the last line of the subhead. Measured across the whole loop at
 * 1440 × 900, `--muted` over that pixel is 1.18 : 1 where WCAG AA wants 4.5.
 *
 * It lives HERE rather than in LiveHero's scrim for two reasons, and both are
 * the point of the fix:
 *   - LiveHero's scrim is painted over all three rungs, and only this one is
 *     too bright. Raising it would move a desktop composition the founder has
 *     signed off, to fix a frame it never shows.
 *   - inside the band it inherits the crossfade. If autoplay is refused, the
 *     file 404s or the codec is unsupported, the video never reports `playing`,
 *     the wrapper stays at opacity 0, and this goes with it — so a dark band
 *     can never appear over the plate that is standing in for a loop that
 *     failed.
 *
 * `lg:block` because it is answering a question only the desktop composition
 * asks. Below `lg` the copy runs full width, the scrim is already vertical and
 * flat at 0.78 across the whole copy block (SCRIM_NARROW_VERTICAL), and this
 * band would land on the 62–74 % release — re-crushing the lane dashes that
 * release exists to protect.
 *
 * No hooks, so heroContrast.test.ts can call it as a plain function and check
 * that the numbers next door are the numbers that get painted — the same trick
 * the suite already uses on LiveHero.
 */
export function HeroLoopGrade() {
  return (
    <div
      aria-hidden
      className="pointer-events-none absolute inset-0 hidden lg:block"
      style={{ background: HERO_LOOP_GRADE_WIDE }}
    />
  );
}
...
export function HeroLoopVideo({ onPlaying, paused = false, visible = false }: HeroLoopVideoProps) {
  ...
  return (
    ...
    >
      ...
      {/* After the <video>, so it paints over it. The band is already
          `absolute` (HERO_BAND_CLASS), i.e. already a containing block — this
          needs no `relative` of its own, and adding one would take the band out
          of the plate's rectangle entirely. */}
      <HeroLoopGrade />
    </div>
  );
}
