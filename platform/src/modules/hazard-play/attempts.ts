...
function cardFor(run: HazardRunRecord, dealt: HazardDealtItem[], index: number): HazardItemCard {
  return { ...dealt[index].card, index: index + 1, total: run.items.length };
}

/**
 * Ask the engine for one item's card again. Deals a run of length 1 seeded on
 * nothing in particular is NOT what happens here — `deal` is the only way to
 * obtain a card, so a single-item re-deal would be a different item. Instead
 * the caller keeps the dealt list for the length of the request; across
 * requests the card is rebuilt from the ORIGINAL deal, which the store holds
 * enough of (id, title, duration) plus the engine's clip URLs.
 *
 * Kept as one function so the day the engine grows a `cardFor(itemId)` method
 * there is exactly one call site to change.
 */
async function refreshCard(run: HazardRunRecord, index: number): Promise<HazardItemCard> {
  const record = run.items[index];
  const dealt = await getHazardEngine().deal({
    userId: run.userId,
    door: run.door,
    length: run.items.length,
    seed: undefined,
  });
  const match = dealt.find((d) => d.itemId === record.itemId);
  if (match !== undefined) {
    return { ...match.card, index: index + 1, total: run.items.length };
  }
  // The engine no longer offers this item (content edited mid-run). Rather
  // than strand the student, serve what the run row remembers: the clip URL is
  // derivable from the item id by convention and the brief is not worth a
  // broken run. The engine's own judge() still owns the scoring.
  return {
    itemId: record.itemId,
    clipSrc: "",
    posterSrc: null,
    durationSec: record.durationSec,
    titleBg: record.titleBg,
    briefBg: "",
    index: index + 1,
