...
export const SC_FO_BRAKELIGHT_CHAIN: ScenarioSpec = {
  ...
  mistakes: [
    ...
    },
    {
      ...
      // GEOMETRY 2026-08-10 — the card said „удар отзад" and now the engine can
      // prove it on bodies instead of centres. Measured through the production
      // ContactProbe on recordScFoBrakelightChainDrive: the hero box and
      // `sc-fbc-mid` overlap by 0.0123 m (player (4.06, 249.83) at 22.2 km/h,
      // the middle car at (4.06, 253.89) — 4.06 m of centres against a
      // 2.02 + 2.05 = 4.07 m nose-to-tail touching distance). The impact speed
      // is what the copy may claim; the 12 mm of interpenetration is NOT a
      // teaching number and is deliberately not printed at the student.
      whatWentWrongBg:
        "Дистанцията беше приемлива, но погледът не стигаше по-далеч от предната кола. Първата кола в колоната спря, средната се закова, а водачът разбра за това едва от нейните стопове — секунда по-късно и петнайсет метра по-близо. Пълната спирачка дойде, когато вече нямаше метри: удар отзад при 22 км/ч — не „почти“, а ПТП. Нарушени са две правила: съобразена скорост (ЗДвП чл. 20, ал. 2) и достатъчна дистанция (чл. 23, ал. 1). На изпита сблъсъкът е опасна грешка — 10 наказателни точки от изпитния лист по Наредба № 38 (приложение № 5, т. 10, б. „в“) при таван 9 за целия изпит, и единствената грешка, която го прекратява (същата наредба, чл. 48, ал. 3). Не са контролни точки по книжката. Правилно: гледаш ПРЕЗ колоната, вдигаш газта на чуждите стопове две коли напред и държиш поне 2 секунди. Гледаш ли в бронята, спираш с бронята.",
      codeRefs: ["COLLISION"],
    },
  ],
  teach: {
