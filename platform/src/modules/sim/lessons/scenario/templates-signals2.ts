...
export const SC_SIG_FLASH_AMBER_PED: ScenarioSpec = {
  ...
  teach: {
    ...
    lawRef: "ППЗДвП светлинни сигнали за регулиране на движението; ЗДвП чл. 119",
    examinerBg:
 ...
 * not any lamp — govern the south approach (ППЗДвП чл. 29, ал. 3; ЗДвП чл. 7). This
...
export const SC_SIG_CONTROLLER_POSTURES: ScenarioSpec = {
  ...
  objectiveBg:
    // „с отпуснати ръце" struck — sc-signal-controller:2b255403's rule applied
    // to its sibling: the officer mesh never lowers its arms
    // (`officerArmTarget`), and the discriminator the law gives is the shoulder.
    "Разчети позата: страничен профил, ти си срещу рамото му = премини; гърди или гръб към теб = спри; вдигната ръка = „внимание“ за всички.",
  archetypeIds: ["JU-18"],
  ...
  instructionsBg: [
    ...
    },
    {
      n: 2,
      textBg:
        // „с отпуснати ръце" struck — the same claim, on the same mesh, as
        // sc-signal-controller:2b255403. `officerArmTarget` holds both arms out
        // in every posture but „внимание", so the officer never lowers them and
        // the cue the law actually gives is the SHOULDER, not the limb (see
        // `traffic/controllerGestures.ts`'s sideProfile bubble for the frame and
        // the full reasoning). Naming the arms here is also the more dangerous
        // half of the error: the SAME arms out mean „спри" to the driver at his
        // chest, which is what `mistake-barge-chest` demonstrates.
        "Езикът му е от три думи и всяка отговаря на три въпроса — кой минава, кой спира, чие е предимството. СТРАНИЧЕН ПРОФИЛ, ти си срещу рамото му: минаваш ти и цялата твоя посока, напречното спира, предимството е твое. ГЪРДИ ИЛИ ГРЪБ: минава напречното, ти спираш, предимството не е твое. РЪКА НАГОРЕ: не минава никой — той сменя фазите.",
    },
    {
      n: 3,
      textBg:
    ...
    },
    {
      n: 5,
      textBg:
        "Щом се обърне със СТРАНИЧЕН ПРОФИЛ и застанеш срещу рамото му, посоката ти е отворена, напречната е спряна и предимството е твое — премини решително и спокойно на север (ППЗДвП — сигналите на регулировчика).",
    },
  ],
  success: [
    {
