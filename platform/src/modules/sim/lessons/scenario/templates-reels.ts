  ...
  mistakes: [
    {
      ...
      // The clip depicts a run-off onto the verge, not an impact with a named
      // object: ac-ice-v1 carries no roadside furniture at the departure point
      // (the graded beat is COLLISION „staticObject"), so the copy must not
      // promise a pole the picture cannot show — doc 66 R0.
      whatWentWrongBg:
        "Водачът не намали пред знака А15 и влезе в хлъзгавия участък с пълна скорост. В дъжда сцеплението не стига — колата поднася и излиза от платното вдясно, извън банкета.",
...
export const SC_ANIMAL_HAZARD: ScenarioSpec = {
  ...
  teach: {
    ...
    whyBg:
      "Отбиването в насрещното сменя дребен удар с челен сблъсък. Непрекъснатата осева линия М1 пази точно този сценарий: „На пътните превозни средства е забранено да я застъпват и пресичат“ (ППЗДвП — надлъжната пътна маркировка) — а при възникнала опасност чл. 20, ал. 2 иска незабавно намаляване и при нужда аварийно спиране, „като не извършват маневри, с които могат да застрашат своя живот или здраве, живота или здравето на други лица“. Спира се право.",
    // CITATION CORRECTED 2026-08-03: this cited „ЗДвП чл. 63", which is the
    // TUNNEL article („При движение в тунел, началото на който е обозначено с
    // пътен знак…"). The М1 ban is ППЗДвП чл. 63 — the same number in the other
    // act. The rule catalogue already cited it correctly on CROSSED_SOLID_LINE.
    lawRef: "ППЗДвП надлъжна пътна маркировка; ЗДвП чл. 20, ал. 2",
