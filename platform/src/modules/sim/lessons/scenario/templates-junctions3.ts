...
export const SC_JX_EQUAL_RIGHT_CONFLICT: PriorityFromRightSpec = {
  ...
  armDistM: 70,
  leadSec: -3.5,
  // jx-equal-v1 is all-`residential`: mouth 17.125 m, no paint. 18 m is this
  // map's line (contrast T7's sx-v1, where 18 was 9.7 m short of the paint).
  lineDistM: 18,
  clearSpeedMps: 11.5,
  // L7 (ledger §4): authored, not inherited. Without it the car committed on a
...
export const SC_JX_PRIORITY_CONFIDENCE: ScenarioSpec = {
  ...
  instructionsBg: [
    {
      n: 1,
      textBg:
        "Тръгни на изток по главната улица. Ти си по пътя с предимство — на кръстовището напред страничната улица излиза на знак Б2 „Спри!“. Вали ли, включи късите светлини преди това (чл. 70): предимството ти струва точно толкова, колкото те вижда чакащият на Б2 — в дъжд без светлини той просто те няма и тръгва.",
    },
    {
      n: 2,
      textBg:
  ...
  teach: {
    whenBg:
      "На всяко кръстовище, където предимството е твое по знак. Тук го виждаш по обратния начин — от Б2 „Спри!“, поставен на страничната улица: щом тя има „Спри!“, главната си ти. По големите артерии същото се обявява и отпред, с жълтия ромб Б3 „Път с предимство“, който важи за целия път до знака за край. Двете обозначения казват едно и също нещо от двете страни на кръстовището; на този учебен участък носителят е Б2 на пресечката.",
    whyBg:
