...
export const SC_PE_ZONE_LIVING: ScenarioSpec = {
  ...
  instructionsBg: [
    { n: 1, textBg: "Потегли и карай спокойно по обикновената улица — отпред е входът на жилищна зона." },
    {
      n: 2,
      textBg:
        "На синия знак Д15 ограничението пада на 20 км/ч. Свали скоростта ПРЕДИ знака: 20 е пешеходно темпо, не „бавно шофиране“.",
    },
    {
      n: 3,
      textBg:
        "Вътре няма тротоар и няма пътеки — цялото платно е на хората. Хора върху платното тук не нарушават нищо: ти си гостът.",
    },
    {
      n: 4,
      textBg:
    ...
    },
    {
      n: 6,
      textBg:
        "На изхода (знак Д16) излизането от зоната е включване в движението: пълзи до устието, огледай наляво и надясно и пропусни всичко по улицата — ти нямаш никакво предимство.",
    },
  ...
  mistakes: [
    {
      ...
      whatWentWrongBg:
        "„Това е просто квартална уличка“ — и колата влезе в зоната с почти 50 там, където знакът казва 20. Превишението е над 10 км/ч, тоест ОПАСНА грешка, и то не е формално: в жилищна зона детето излиза иззад кола на два метра пред теб. От 20 км/ч спираш за около 7 метра; от 50 — за около 27. Разликата е точно между спирачка и удар. Водачът намали и после пропусна хората коректно — но безопасността в зоната се решава на входа ѝ, не пред пешеходеца.",
