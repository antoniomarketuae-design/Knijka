 ...
 * mistake demos for „Лед по моста" (doc 72 AC-08 ice band — the icePatch
 ...
 * span [210, 300] (the exposed „bridge") and a STALLED CAR standing on it at
...
export function scAcIceShadowScript(): DriveScript {
  return {
    steps: [
      { kind: "annotation", textBg: "Ясна студена сутрин след влажна нощ — и знак А15 „Опасност от хлъзгане“ преди моста. Мостът замръзва пръв: няма топла земя под платното." },
      ...
      { kind: "annotation", textBg: "Върху леда: равна газ, прав волан, никакви резки движения. Напред е закъсал автомобил — подхлъзнал се е точно тук." },
      ...
      { kind: "annotation", textBg: "Спряхме свръхплавно на позицията, с дистанция до закъсалия — ледът получи нула резки команди." },
      { kind: "annotation", textBg: "Правилото: открит участък в студена сутрин = лед, докато не се докаже обратното. Скоростта пада ПРЕДИ него." },
    ],
  };
}
...
export function scAcIceMistakeBrakeOnIceScript(): DriveScript {
  return {
    steps: [
      { kind: "annotation", textBg: "Грешката: улицата е суха, колата носи 50 — а мостът е заледен и никой не намали преди него." },
      { kind: "glance", mirror: "rear" },
      ...
      { kind: "annotation", textBg: "При 15% сцепление спирачката не спира — тя се моли. Намаляването е трябвало да стане преди моста, на чист асфалт (чл. 20, ал. 2)." },
    ],
  };
}
...
export function scAcIceMistakeHarshSteerScript(): DriveScript {
  return {
    steps: [
      ...
      { kind: "pause", sec: 1.5, brake: true },
      { kind: "annotation", textBg: "Оцеляването беше късмет. На лед всяко движение е малко и плавно — а истинската корекция е скоростта, паднала преди моста." },
    ],
  };
}
