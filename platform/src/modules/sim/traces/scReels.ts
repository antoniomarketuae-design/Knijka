...
function signWarningMistake(): DriveScript {
  // The fault must land ON the ice so the clip shows the А15 sign (rendered at
  // y=210, the ice-zone start) on the APPROACH and the skid/run-off AT the
  // fault — not "50 in the rain" on a plain wet straight (founder R0). Grading
  // forces the shape: SPEED_TOO_FAST_FOR_CONDITIONS fires after 3 s over the
  // rain conditionLimit (50 × 0.85 = 42.5). Approaching UNDER 42.5 keeps the
  // episode from arming early on the dry-ish straight (the old y≈110 fault);
  // only when the driver holds ~50 THROUGH the warned ice does it convict —
  // ≈ 3 s past y=210 ⇒ the fault lands at y≈250, in the ice, just past the sign,
  // with the run-off immediately after (still on the ice). Ends ≤ 55 so no
  // SPEEDING_OVER_LIMIT leaks in.
  return {
    steps: [
      ...
      // Rain-prudent approach UNDER the conditions limit (42.5) — the А15 sign
      // (y=210) stays AHEAD in frame on this leg, and no fault arms early.
      { kind: "drive", points: [[LANE, 15], [LANE, 206]], targetKmh: 42, stopAtEnd: false },
      { kind: "annotation", textBg: "Пред знака трябва да свали до 25–30 за хлъзгавия участък — вместо това задържа висока скорост." },
      // ~50 km/h held THROUGH the ice (y 210–300) for > 3 s → the conditions
      // fault lands ON the slippery patch (50 > 42.5, ≤ 55 so no SPEEDING).
      { kind: "drive", points: [[LANE, 206], [LANE, 266]], targetKmh: 50, stopAtEnd: false },
      { kind: "annotation", textBg: "На хлъзгавия участък при 50 сцеплението не стига — задницата поднася и колата излиза от платното." },
      // The run-off happens ON the ice (still inside y 210–300) → COLLISION.
      { kind: "drive", points: [[LANE, 266], [9.5, 290]], targetKmh: 46, stopAtEnd: false },
...
function signWarningMistakeNoSlow(): DriveScript {
  return {
    steps: [
      ...
      { kind: "drive", points: [[LANE, 15], [LANE, 208]], targetKmh: 50, stopAtEnd: false },
      // 50 km/h held through the ice in rain (> 3 s) → SPEED_TOO_FAST_FOR_CONDITIONS.
      { kind: "drive", points: [[LANE, 208], [LANE, 300]], targetKmh: 50, stopAtEnd: false },
