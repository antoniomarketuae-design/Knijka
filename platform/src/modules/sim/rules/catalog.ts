...
export interface ViolationSpec {
   ...
   *
   * ⚠ IT WAS OPTIONAL UNTIL 2026-09-11 AND THAT IS WHY `sc-roundabout-entry:
   *   fe081cf1` KEPT BEING FILED. The paragraph that stood here said „absent
   *   means print the authored explanation, i.e. exactly today's behaviour. A
   *   row without one is not a defect the moment this field exists." Counted
   *   against the catalogue that sentence covered 55 of the 58 codes, and
   *   „today's behaviour" is the cut card the row is about: measured through
   *   `violationPeekBg` at this file's own budget, every one of the 55
   *   overflowed — the mildest by 4 lines into a window holding 1
   *   (PREDRIVE_SEATBELT_SKIPPED), the worst by 31 (STOPPED_WITHOUT_CAUSE,
   *   DRIVING_TOO_SLOW_IN_TOWN at 30, RAIL_CROSSING_VIOLATION at 23), and
   *   12 of them опасни −10 that also terminate the exam. The row was filed on
   *   FAILED_TO_YIELD, which is one of the three that HAD a summary, so it read
   *   as answered on its own frame while the class stayed open everywhere else.
   *
   * REQUIRED, therefore, exactly as `correctiveBg` above is: a new offence
   * cannot ship a phone card that fades out mid-clause, because `tsc` refuses
   * the row before a sweep can photograph it. ADR-002: this slot carries NO
   * citation and no figure; it is plain Bulgarian compressed out of the row's
   * own `explanationBg`, and every article stays in `explanationBg`/`lawRef`.
   * THEO-4: it names the CAUSE, never the verdict — the whole point is that the
   * one line that does reach the glass is a reason.
   */
  peekBg: string;
  /**
...
export const VIOLATIONS: Record<ViolationCode, ViolationSpec> = {
  SPEEDING_OVER_LIMIT: {
    ...
    peekBg: "Ограничението е таван, не цел.",
    correctiveBg:
  ...
  RED_LIGHT_CROSSED: {
    ...
    peekBg: "Напречните вече тръгват.",
    correctiveBg:
  ...
  TURN_WITHOUT_INDICATOR: {
    ...
    peekBg: "Мигачът е преди маневрата, не в нея.",
    correctiveBg:
  ...
  WRONG_LANE_FOR_DIRECTION: {
    ...
    peekBg: "Режеш пътя на правилния.",
    correctiveBg:
  ...
  LANE_CHANGE_WITHOUT_INDICATOR: {
    ...
    peekBg: "Задният няма как да те предвиди.",
    correctiveBg:
  ...
  LANE_CHANGE_WITHOUT_MIRROR_CHECK: {
    ...
    peekBg: "В мъртвата зона има кола.",
    correctiveBg:
  ...
  SEATBELT_OFF_WHILE_MOVING: {
    ...
    peekBg: "Ударът е като трети етаж.",
    correctiveBg:
  ...
  FOLLOWING_TOO_CLOSE: {
    ...
    peekBg: "Двете секунди са спирачният ти път.",
    correctiveBg:
  ...
  EMERGENCY_NOT_YIELDED: {
    ...
    peekBg: "Коридорът му остава чист.",
    correctiveBg:
  ...
  PEDESTRIAN_CROSSING_TOO_FAST: {
    ...
    peekBg: "Няма да успееш да спреш.",
    correctiveBg:
  ...
  PEDESTRIAN_NOT_YIELDED: {
    ...
    peekBg: "Стъпилият на пътеката минава пръв.",
    correctiveBg:
  ...
  ENGINE_STALLED: {
    ...
    peekBg: "Проблем е със съединителя и газта.",
    correctiveBg:
  ...
  MOVE_OFF_WITHOUT_OBSERVATION: {
    ...
    peekBg: "Лявото рамо не се вижда в огледало.",
    correctiveBg:
  ...
  STOP_LINE_OVERSHOOT: {
    ...
    peekBg: "Отвъд линията е мястото на пешеходеца.",
    correctiveBg:
  ...
  HARSH_BRAKING_NO_CAUSE: {
    ...
    peekBg: "Задният не очаква спирачка без причина.",
    correctiveBg:
  ...
  HESITATION_AT_GREEN: {
    ...
    peekBg: "Колоната зад теб чака теб.",
    correctiveBg:
  ...
  RED_YELLOW_CROSSED: {
    ...
    peekBg: "Напречните още изчистват.",
    correctiveBg:
  ...
  OVERTAKING_AT_CROSSING: {
    ...
    peekBg: "Предният пропуска човек.",
    correctiveBg:
  ...
  CLOSING_ON_LEAD_TOO_FAST: {
    ...
    peekBg: "Влизаш в чуждата спирачка със своята скорост.",
    correctiveBg:
  ...
  OVERTAKING_IN_BAN_ZONE: {
    ...
    peekBg: "Там видимостта не стига.",
    correctiveBg:
  ...
  DRIVING_IN_BUS_LANE: {
    ...
    peekBg: "Тя пази разписанието на градския транспорт.",
    correctiveBg:
  ...
  SPEED_TOO_FAST_FOR_CURVE: {
    ...
    peekBg: "Скоростта се сваля преди.",
    correctiveBg:
  ...
  WARNING_LAMP_IGNORED: {
    ...
    peekBg: "Червено значи спри сега.",
    correctiveBg:
...
> = {
  ...
  "entered-barred": {
    ...
    lawRef: "ЗДвП чл. 52",
    // „тръгнал." would trip the ADR-002 no-citation guard in
    // `lessons/__tests__/violation-peek-summary.test.tsx`: its regex carries no
    // word boundary, so ANY word ending in „ал." reads to it as the abbreviation
    // „ал.". The wording moved rather than the gate — a guard that over-refuses
    // is the safe direction and is not worth loosening for one adjective.
    peekBg: "Влакът вече идва насам.",
  },
  "stopped-on-track": {
