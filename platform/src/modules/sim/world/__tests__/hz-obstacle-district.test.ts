/**
 * HZ obstacle-street micro-map contract battery (Scenario Studio doc 76 §3; the
 * nm-district.test.ts pattern for a plain two-way 1+1 street).
 *
 * content/world/hz-obstacle-v1.json is the obstacle-swerve generated micro-map
 * (tools/maps/gen_hazard_obstacle.mjs — one straight two-way street, ONE lane
 * per direction, a posted limit and NOTHING else). The stalled obstacle is
 * STAGED lesson data (sc-hazard-obstacle); the map only hosts the street. The
 * battery proves the file satisfies the FULL engine contract.
 */
...
function loadRaw(id: string): unknown {
  ...
  throw new Error(`${id}.json not found (run: node tools/maps/gen_hazard_obstacle.mjs)`);
}
...
describe(`${ID} through the world runtime — the speed-limit surface`, () => {
  ...
  it("resolves the authored limit and the single northbound lane", () => {
    ...
    expect(tick.edgeId).toBe("hz-e-street");
