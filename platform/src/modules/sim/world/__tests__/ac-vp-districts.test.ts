/**
 * AC/VP cockpit micro-map contract battery (Scenario Studio doc 76 §3; the
 * sp-districts.test.ts pattern, parametrized over the cockpit-channel batch).
 *
 * content/world/{vp-ready,ac-night,ac-rain}-v1.json are the adverse-conditions
 * + cockpit-procedure generated micro-maps (tools/maps/gen_ac_vp_streets.mjs —
 * one straight two-way street, ONE lane per direction, a posted limit and
 * NOTHING else: no zebra, no lights, no junctions). The battery proves every
 * file satisfies the FULL engine contract each district drives through, so the
 * ONLY thing the rule engine can grade on it is the driver's cockpit state
 * (belt / handbrake / lights) against the recorded condition:
 *
 ...
 *   3. traffic — buildLaneGraph maps the street onto two directed lanes and an
 *                empty district is a legal config (leadGap = ∞, no ambient).
 */
...
const CASES: AcVpMapCase[] = [
  { id: "vp-ready-v1", edgeId: "vp-e-street", spawns: ["vp-spawn-approach", "vp-spawn-finish"], limitKmh: 50, lengthM: 360 },
  {
    id: "ac-night-v1",
    edgeId: "ac-night-e-street",
    spawns: ["ac-night-spawn-approach", "ac-night-spawn-finish"],
    limitKmh: 50,
    lengthM: 360,
  },
  {
    id: "ac-rain-v1",
    edgeId: "ac-rain-e-street",
    spawns: ["ac-rain-spawn-approach", "ac-rain-spawn-finish"],
    limitKmh: 50,
    lengthM: 360,
  },
];
...
function loadRaw(id: string): unknown {
  ...
  throw new Error(`${id}.json not found (run: node tools/maps/gen_ac_vp_streets.mjs) in: ${candidates.join(", ")}`);
}
...
for (const c of CASES) {
  describe(`${c.id} through the world builder`, () => {
    ...
    it("is a structurally valid district-v1 document (plain two-way street)", () => {
      ...
      expect(district.crossings.length).toBe(0); // a pure cockpit street: no zebra
      expect(district.roundabouts.length).toBe(0);
      expect(district.spawnPoints.map((s) => s.id).sort()).toEqual([...c.spawns].sort());
    });
  ...
  describe(`${c.id} through the world runtime — the speed-limit surface`, () => {
    ...
    it("surfaces the single northbound lane (laneId 0, laneCount 1) with the posted maxSpeedKmh", () => {
      ...
      expect(tick.edgeId).toBe(c.edgeId);
