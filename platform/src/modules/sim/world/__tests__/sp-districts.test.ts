/**
 * SP-family speed micro-map contract battery (Scenario Studio doc 76 §3; the
 * pe-districts.test.ts pattern, parametrized over the S3 batch-2 maps).
 *
 * content/world/{sp-creep,sp-danger,sp-rain}-v1.json are the speed-management
 * generated micro-maps (tools/maps/gen_sp_speed.mjs — one straight two-way
 ...
 *                the exact maxSpeedKmh the SPEEDING_* / SPEED_TOO_FAST detectors
 *                grade against;
...
function loadRaw(id: string): unknown {
  ...
  throw new Error(`${id}.json not found (run: node tools/maps/gen_sp_speed.mjs) in: ${candidates.join(", ")}`);
}
...
for (const c of CASES) {
  describe(`${c.id} through the world builder`, () => {
    ...
    it("is a structurally valid district-v1 document (plain two-way street)", () => {
      ...
      expect(district.crossings.length).toBe(0); // a pure speed street: no zebra
  ...
  describe(`${c.id} through the world runtime — the speed-limit surface`, () => {
    ...
    it("surfaces the single northbound lane (laneId 0, laneCount 1) with the posted maxSpeedKmh", () => {
      ...
      expect(tick.edgeId).toBe("sp-e-street");
