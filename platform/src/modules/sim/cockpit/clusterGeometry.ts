...
export function buildClusterFaceMesh(): ClusterFaceMesh {
  ...
  //     no second texture — the cell IS the digit).
  const digitQuad: number[] = [];
  const digitsSpan = DIGIT_COUNT * DIGIT_W + (DIGIT_COUNT - 1) * DIGIT_GAP;
  for (let i = 0; i < DIGIT_COUNT; i++) {
    const cx = DIAL_CX - digitsSpan / 2 + DIGIT_W / 2 + i * (DIGIT_W + DIGIT_GAP);
    digitQuad.push(
  ...
  pushRect(b, DIAL_CX, UNIT_CY, UNIT_W, UNIT_H, Z_TEXT, cellUv(UNIT_CELL), INK_MUTED);
...
export function buildClusterNeedleMesh(): ClusterNeedleMesh {
  ...
  // not visibly spin with the needle.
  pushRect(b, 0, 0, 34, 34, 0, halo, hexRgba("#48a9ff", 0.5));
  pushQuad(
    b,
    [
