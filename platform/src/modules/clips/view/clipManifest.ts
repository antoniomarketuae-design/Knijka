 ...
 *  — writer twin: ../capture/manifest.ClipActorCheck). `present` is the HONEST
...
export interface MistakeClip {
   ...
   * vision-inspection strip), `/clips/<id>.k0..k4.webp`. OPTIONAL and lenient:
   * absent/malformed reads as "no keyframes" so pre-R0 manifests keep parsing;
   * the gallery degrades to video-only. The extension is deliberately NOT
   * validated — leniency is the law here, and a stale `.png` entry still
   * renders, just heavily (see posterFrameFor).
   */
  keyframes?: string[];
  /**
 ...
 *
 * WEIGHT (audit 80 H-10): whatever this returns is downloaded EAGERLY by the
 * browser even though the <video> is preload="none" — the IntersectionObserver
 * lazy-load in MistakeMedia protects the .webm, never the poster. So the rig
 * writes the stills as ~18 KB WebP (tools/clips/headless/webp.mjs) instead of
 * the 1.1 MB PNGs it used to; this function just hands back the manifest URL,
 * and clipManifestAssets.test.ts pins the manifest to the light format.
 */
