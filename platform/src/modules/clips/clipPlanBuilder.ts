 ...
 * imported by the freshness gate (clipPlan.test.ts), the generator wrapper
 * (tools/clips/gen_clip_plan.mjs) and the node-only R0 forensic tests under
 * ./capture (see buildClipPlan's `subjects` parameter). Never import from
 * app, component or RIG code — those read clipPlan.generated.ts, and this
 * file drags node:fs + the whole recorder stack in with it.
 *
...
import { serializeScenarioTrace } from "@/modules/sim/traces";
// Deep import ON PURPOSE (audit M-26): the replay registry statically pulls in
// all ~40 scripted-drive recorders + the scenario catalogue, so it is NOT on
// the sim/traces barrel — that barrel is reached from the theory pages. This
// generator is node-only (`readFileSync` above), so the weight is free here.
import { replayPilotMistake } from "@/modules/sim/traces/clipReplay";
...
/**
 * The minimum a clip needs to be planned. `ClipPilotEntry` satisfies it
 * structurally, so the default path is unchanged — but a caller may also name
 * a clip the pilot list does NOT currently contain.
 *
 * Why that matters: the pilot list is DERIVED from the why-panel's current
 * representative pick per event, so re-pointing one event silently deletes a
 * clip's card. The R0 forensics under ./capture are evidence about
 * specific RECORDINGS (what the ghost feed staged, what the lamp clock did),
 * not about who represents an event this week — they name their own subjects
 * through this parameter instead of inheriting the theory mapping's churn.
 */
export interface ClipPlanSubject {
  /** `<templateId>__m<mistakeIndex>` — the shared manifest id. */
  id: string;
  templateId: string;
  mistakeIndex: number;
...
export function buildClipPlan(
  subjects: readonly ClipPlanSubject[] = clipPilotList(),
): ClipPlanEntry[] {
  const entries: ClipPlanEntry[] = [];
  for (const pilot of subjects) {
