...
// Node-only test seam (clipPlanBuilder's header): the В1 content-gap case
// below is evidence about a specific RECORDING, so it must survive the
// why-panel re-pointing the event that put that clip in the derived pilot
// list. Same builder, same committed inputs — never a fixture.
...
/** The clip's card: its committed CLIP_PLAN row, or — for a clip the derived
 *  pilot list no longer names — the same builder run over the same committed
 *  trace/template/district. */
function planFor(clipId: string): (typeof CLIP_PLAN)[number] {
  const committed = CLIP_PLAN.find((p) => p.id === clipId);
  if (committed) return committed;
  const parsed = /^(.+)__m(\d+)$/.exec(clipId);
  expect(parsed, `malformed clip id ${clipId}`).not.toBeNull();
  const [, templateId, indexRaw] = parsed!;
  const mistakeIndex = Number(indexRaw);
  const mistake = scenarioById(templateId)?.mistakes[mistakeIndex];
  expect(mistake, `no mistake ${mistakeIndex} on ${templateId}`).toBeDefined();
