 ...
 * or directly:
 *   GEN_CLIP_PLAN=1 npx vitest run src/modules/clips/clipPlan.test.ts
 */
...
describe("card derivation pins — known clips (R1/R2/R4)", () => {
  ...
  // Was pinned on sc-ed-d2-city-run__m0 („Преминаване на червено"). That clip
  // left the pilot when f7cca4e wired ev-junction-signalized straight to
  // sc-junction-stop, and the pilot list is DERIVED from the why-panel index —
  // so the pin died with the wiring, not with the derivation it guards. The
  // signal branch of controlFor() is still exercised, by the one clip that
  // reaches it today. (The wiring itself is a separate defect: a red-light
  // question now shows a STOP-sign reel. Reported, not fixed here.)
  it("sc-signal-controller__m0 is governed by the junction signal, positioned", () => {
    const control = builtById("sc-signal-controller__m0").governingControl;
  ...
  // Was pinned on sc-park-perp-rev__m0; ev-parking-maneuver now resolves to
  // sc-park-parallel (f7cca4e), an equally representative parking demo that
  // reaches the same parkedVehicle branch of requiredActorsFor().
  it("sc-park-parallel__m0 requires the parked cars it maneuvers between", () => {
    const entry = builtById("sc-park-parallel__m0");
    expect(entry.requiredActors.some((a) => a.kind === "parkedVehicle")).toBe(true);
  });
});

/**
 * The pins above name individual clips, and clip membership is DERIVED (the
 * why-panel index → clipPilot → plan). Rewiring one event therefore silently
 * deletes a pin's subject — which is exactly how the "signal" and
 * "parkedVehicle" derivations lost their only guard at f7cca4e and stayed lost
 * for four commits. These assert on the BRANCH, not on a clip id, so a future
 * rewire that drops the last such clip fails loudly instead of quietly
 * retiring the coverage.
 */
describe("derivation coverage — no branch may silently lose its last clip", () => {
  const controlKinds = new Set(built.map((e) => e.governingControl.kind));
  const actorKinds = new Set(built.flatMap((e) => e.requiredActors.map((a) => a.kind)));

  for (const kind of ["sign", "signal", "marking", "none"] as const) {
    it(`at least one clip is governed by a ${kind} control (R2)`, () => {
      expect(controlKinds.has(kind)).toBe(true);
    });
  }

  for (const kind of ["vehicle", "pedestrian", "parkedVehicle", "emergency"]) {
    it(`at least one clip requires a ${kind} actor (R1)`, () => {
      expect(actorKinds.has(kind)).toBe(true);
    });
  }

  for (const view of ["exterior", "cockpit", "exterior+dashboard"] as const) {
    it(`at least one clip is a ${view} clip (R4)`, () => {
      expect(built.some((e) => e.view === view)).toBe(true);
    });
  }
});
